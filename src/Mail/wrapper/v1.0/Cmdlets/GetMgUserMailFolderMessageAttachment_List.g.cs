...
namespace Microsoft.Graph.PowerShell.Mail
{
    ...
    [Cmdlet(VerbsCommon.Get, "MgUserMailFolderMessageAttachment_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Mail.Client.Models.Attachment))]
    public class GetMgUserMailFolderMessageAttachment_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].MailFolders[MailFolderId].Messages[MessageId].Attachments.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Users[UserId].MailFolders[MailFolderId].Messages[MessageId].Attachments.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
