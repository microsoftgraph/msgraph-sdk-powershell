...
{
    [GraphRoute("PATCH", "/users/{user-id}/mailFolders/{mailFolder-id}/childFolders/{mailFolder-id1}")]
    [Cmdlet(VerbsData.Update, "MgUserMailFolderChildFolder", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Mail.Client.Models.MailFolder))]
    public class UpdateMgUserMailFolderChildFolderCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(MailFolderId1, "Update"))
                return;
    ...
    if (this.IsParameterBound(nameof(UnreadItemCount)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].MailFolders[MailFolderId].ChildFolders[MailFolderId1].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Users[UserId].MailFolders[MailFolderId].ChildFolders[MailFolderId1].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException)
                {
                    ThrowGraphRequestFailed(ex, MailFolderId1);
