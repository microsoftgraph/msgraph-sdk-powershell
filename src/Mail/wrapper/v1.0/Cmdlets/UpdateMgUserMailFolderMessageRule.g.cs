...
{
    [GraphRoute("PATCH", "/users/{user-id}/mailFolders/{mailFolder-id}/messageRules/{messageRule-id}")]
    [Cmdlet(VerbsData.Update, "MgUserMailFolderMessageRule", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Mail.Client.Models.MessageRule))]
    public class UpdateMgUserMailFolderMessageRuleCommand : PSCmdlet
    {
        ...
        {
            if (!ShouldProcess(MessageRuleId, "Update"))
                return;
    ...
    if (this.IsParameterBound(nameof(Exceptions)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].MailFolders[MailFolderId].MessageRules[MessageRuleId].PatchAsync(body, requestConfiguration =>
                {
            ...
            {
                ThrowTerminatingError(new ErrorRecord(ex, "GraphRequestFailed", ErrorCategory.InvalidOperation, MessageRuleId));
                return;
            }
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Users[UserId].MailFolders[MailFolderId].MessageRules[MessageRuleId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex)
                {
                    ThrowTerminatingError(new ErrorRecord(ex, "GraphRequestFailed", ErrorCategory.InvalidOperation, MessageRuleId));
