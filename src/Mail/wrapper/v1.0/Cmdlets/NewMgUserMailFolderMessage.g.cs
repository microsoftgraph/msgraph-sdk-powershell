...
{
    [GraphRoute("POST", "/users/{user-id}/mailFolders/{mailFolder-id}/messages")]
    [Cmdlet(VerbsCommon.New, "MgUserMailFolderMessage", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Mail.Client.Models.Message))]
    public class NewMgUserMailFolderMessageCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(UniqueBody)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].MailFolders[MailFolderId].Messages.PostAsync(body, requestConfiguration =>
                {
