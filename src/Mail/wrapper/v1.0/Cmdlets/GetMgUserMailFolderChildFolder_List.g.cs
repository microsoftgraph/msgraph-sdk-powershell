...
{
    [GraphRoute("GET", "/users/{user-id}/mailFolders/{mailFolder-id}/childFolders")]
    [Cmdlet(VerbsCommon.Get, "MgUserMailFolderChildFolder_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Mail.Client.Models.MailFolder))]
    public class GetMgUserMailFolderChildFolder_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Mail.Client.Models.MailFolderCollectionResponse? result;
            try
            {
                result = client.Users[UserId].MailFolders[MailFolderId].ChildFolders.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Users[UserId].MailFolders[MailFolderId].ChildFolders.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
