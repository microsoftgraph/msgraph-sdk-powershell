...
namespace Microsoft.Graph.PowerShell.Mail
{
    ...
    [Cmdlet(VerbsCommon.Get, "MgUserMailFolderMessageRule_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Mail.Client.Models.MessageRule))]
    public class GetMgUserMailFolderMessageRule_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Mail.Client.Models.MessageRuleCollectionResponse? result;
            try
            {
                result = client.Users[UserId].MailFolders[MailFolderId].MessageRules.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Users[UserId].MailFolders[MailFolderId].MessageRules.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
