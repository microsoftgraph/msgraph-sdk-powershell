...
{
    [GraphRoute("PATCH", "/users/{user-id}/mailFolders/{mailFolder-id}/messages/{message-id}")]
    [Cmdlet(VerbsData.Update, "MgUserMailFolderMessage", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Mail.Client.Models.Message))]
    public class UpdateMgUserMailFolderMessageCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(UniqueBody)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].MailFolders[MailFolderId].Messages[MessageId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Users[UserId].MailFolders[MailFolderId].Messages[MessageId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex)
                {
                    ThrowGraphRequestFailed(ex, MessageId);
