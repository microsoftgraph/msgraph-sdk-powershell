...
namespace Microsoft.Graph.PowerShell.Mail
{
    ...
    [Cmdlet(VerbsCommon.Get, "MgUserMailFolderChildFolderMessageRule_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Mail.Client.Models.MessageRule))]
    public class GetMgUserMailFolderChildFolderMessageRule_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].MailFolders[MailFolderId].ChildFolders[MailFolderId1].MessageRules.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Users[UserId].MailFolders[MailFolderId].ChildFolders[MailFolderId1].MessageRules.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
