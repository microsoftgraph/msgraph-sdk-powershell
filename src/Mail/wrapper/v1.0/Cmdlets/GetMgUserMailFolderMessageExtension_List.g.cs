...
{
    [GraphRoute("GET", "/users/{user-id}/mailFolders/{mailFolder-id}/messages/{message-id}/extensions")]
    [Cmdlet(VerbsCommon.Get, "MgUserMailFolderMessageExtension_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Mail.Client.Models.Extension))]
    public class GetMgUserMailFolderMessageExtension_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].MailFolders[MailFolderId].Messages[MessageId].Extensions.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Users[UserId].MailFolders[MailFolderId].Messages[MessageId].Extensions.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
