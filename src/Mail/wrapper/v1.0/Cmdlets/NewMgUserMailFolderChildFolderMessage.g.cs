...
{
    [GraphRoute("POST", "/users/{user-id}/mailFolders/{mailFolder-id}/childFolders/{mailFolder-id1}/messages")]
    [Cmdlet(VerbsCommon.New, "MgUserMailFolderChildFolderMessage", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Mail.Client.Models.Message))]
    public class NewMgUserMailFolderChildFolderMessageCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(UniqueBody)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].MailFolders[MailFolderId].ChildFolders[MailFolderId1].Messages.PostAsync(body, requestConfiguration =>
                {
