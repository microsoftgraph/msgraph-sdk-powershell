...
{
    [GraphRoute("GET", "/users/{user-id}/mailFolders/{mailFolder-id}/messages/{message-id}")]
    [Cmdlet(VerbsCommon.Get, "MgUserMailFolderMessage_Get")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Mail.Client.Models.Message))]
    public class GetMgUserMailFolderMessage_GetCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].MailFolders[MailFolderId].Messages[MessageId].GetAsync(requestConfiguration =>
                {
