...
{
    [GraphRoute("POST", "/users/{user-id}/messages/{message-id}/attachments")]
    [Cmdlet(VerbsCommon.New, "MgUserMessageAttachment", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Mail.Client.Models.Attachment))]
    public class NewMgUserMessageAttachmentCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Size)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].Messages[MessageId].Attachments.PostAsync(body, requestConfiguration =>
                {
