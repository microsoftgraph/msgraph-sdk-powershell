...
{
    [GraphRoute("POST", "/users/{user-id}/mailFolders/{mailFolder-id}/childFolders")]
    [Cmdlet(VerbsCommon.New, "MgUserMailFolderChildFolder", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Mail.Client.Models.MailFolder))]
    public class NewMgUserMailFolderChildFolderCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(UnreadItemCount)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].MailFolders[MailFolderId].ChildFolders.PostAsync(body, requestConfiguration =>
                {
