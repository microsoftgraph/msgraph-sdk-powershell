...
{
    [GraphRoute("GET", "/users/{user-id}/messages/{message-id}/attachments")]
    [Cmdlet(VerbsCommon.Get, "MgUserMessageAttachment_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Mail.Client.Models.Attachment))]
    public class GetMgUserMessageAttachment_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].Messages[MessageId].Attachments.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Users[UserId].Messages[MessageId].Attachments.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
