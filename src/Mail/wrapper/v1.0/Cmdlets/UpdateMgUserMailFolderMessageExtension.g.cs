...
{
    [GraphRoute("PATCH", "/users/{user-id}/mailFolders/{mailFolder-id}/messages/{message-id}/extensions/{extension-id}")]
    [Cmdlet(VerbsData.Update, "MgUserMailFolderMessageExtension", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Mail.Client.Models.Extension))]
    public class UpdateMgUserMailFolderMessageExtensionCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].MailFolders[MailFolderId].Messages[MessageId].Extensions[ExtensionId].PatchAsync(body, requestConfiguration =>
                {
