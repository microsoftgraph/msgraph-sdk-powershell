...
{
    [GraphRoute("GET", "/users/{user-id}/mailFolders/{mailFolder-id}/childFolders/{mailFolder-id1}/messages/{message-id}")]
    [Cmdlet(VerbsCommon.Get, "MgUserMailFolderChildFolderMessage_Get")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Mail.Client.Models.Message))]
    public class GetMgUserMailFolderChildFolderMessage_GetCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].MailFolders[MailFolderId].ChildFolders[MailFolderId1].Messages[MessageId].GetAsync(requestConfiguration =>
                {
