...
{
    [GraphRoute("POST", "/users/{user-id}/mailFolders/{mailFolder-id}/messages/{message-id}/extensions")]
    [Cmdlet(VerbsCommon.New, "MgUserMailFolderMessageExtension", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Mail.Client.Models.Extension))]
    public class NewMgUserMailFolderMessageExtensionCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].MailFolders[MailFolderId].Messages[MessageId].Extensions.PostAsync(body, requestConfiguration =>
                {
