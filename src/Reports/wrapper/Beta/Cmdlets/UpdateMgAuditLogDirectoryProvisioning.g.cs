...
{
    [GraphRoute("PATCH", "/auditLogs/directoryProvisioning/{provisioningObjectSummary-id}")]
    [Cmdlet(VerbsData.Update, "MgAuditLogDirectoryProvisioning", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Reports.Client.Models.ProvisioningObjectSummary))]
    public class UpdateMgAuditLogDirectoryProvisioningCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(ProvisioningObjectSummaryId, "Update"))
                return;
    ...
    if (this.IsParameterBound(nameof(TargetSystem)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.AuditLogs.DirectoryProvisioning[ProvisioningObjectSummaryId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.AuditLogs.DirectoryProvisioning[ProvisioningObjectSummaryId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, ProvisioningObjectSummaryId);
