...
{
    [GraphRoute("POST", "/auditLogs/provisioning")]
    [Cmdlet(VerbsCommon.New, "MgAuditLogProvisioning", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Reports.Client.Models.ProvisioningObjectSummary))]
    public class NewMgAuditLogProvisioningCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(TargetSystem)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.AuditLogs.Provisioning.PostAsync(body, requestConfiguration =>
                {
