...
{
    [GraphRoute("PATCH", "/reports/monthlyPrintUsageSummariesByUser/{printUsageByUser-id}")]
    [Cmdlet(VerbsData.Update, "MgReportMonthlyPrintUsageSummaryByUser", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Reports.Client.Models.PrintUsageByUser))]
    public class UpdateMgReportMonthlyPrintUsageSummaryByUserCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(UserPrincipalName)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Reports.MonthlyPrintUsageSummariesByUser[PrintUsageByUserId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Reports.MonthlyPrintUsageSummariesByUser[PrintUsageByUserId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, PrintUsageByUserId);
