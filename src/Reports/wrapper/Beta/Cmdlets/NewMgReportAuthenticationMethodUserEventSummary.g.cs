...
{
    [GraphRoute("POST", "/reports/authenticationMethods/userEventsSummary")]
    [Cmdlet(VerbsCommon.New, "MgReportAuthenticationMethodUserEventSummary", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Reports.Client.Models.UserEventsSummary))]
    public class NewMgReportAuthenticationMethodUserEventSummaryCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Feature)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Reports.AuthenticationMethods.UserEventsSummary.PostAsync(body, requestConfiguration =>
                {
