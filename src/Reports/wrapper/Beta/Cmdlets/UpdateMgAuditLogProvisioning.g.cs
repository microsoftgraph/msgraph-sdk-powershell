...
{
    [GraphRoute("PATCH", "/auditLogs/provisioning/{provisioningObjectSummary-id}")]
    [Cmdlet(VerbsData.Update, "MgAuditLogProvisioning", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Reports.Client.Models.ProvisioningObjectSummary))]
    public class UpdateMgAuditLogProvisioningCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(TargetSystem)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.AuditLogs.Provisioning[ProvisioningObjectSummaryId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.AuditLogs.Provisioning[ProvisioningObjectSummaryId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, ProvisioningObjectSummaryId);
