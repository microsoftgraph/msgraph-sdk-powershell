...
{
    [GraphRoute("GET", "/reports/microsoftAppsFileStorageContainerUsageSummary/usageByDataLocation")]
    [Cmdlet(VerbsCommon.Get, "MgReportMicrosoftAppFileStorageContainerUsageSummaryUsageByDataLocation_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Reports.Client.Models.MicrosoftAppsFileStorageContainerGeoUsage))]
    public class GetMgReportMicrosoftAppFileStorageContainerUsageSummaryUsageByDataLocation_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Reports.Client.Models.MicrosoftAppsFileStorageContainerGeoUsageCollectionResponse? result;
            try
            {
                result = client.Reports.MicrosoftAppsFileStorageContainerUsageSummary.UsageByDataLocation.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Reports.MicrosoftAppsFileStorageContainerUsageSummary.UsageByDataLocation.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
