...
{
    [GraphRoute("PATCH", "/reports/monthlyPrintUsageByUser/{printUsageByUser-id}")]
    [Cmdlet(VerbsData.Update, "MgReportMonthlyPrintUsageByUser", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Reports.Client.Models.PrintUsageByUser))]
    public class UpdateMgReportMonthlyPrintUsageByUserCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(UserPrincipalName)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Reports.MonthlyPrintUsageByUser[PrintUsageByUserId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Reports.MonthlyPrintUsageByUser[PrintUsageByUserId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, PrintUsageByUserId);
