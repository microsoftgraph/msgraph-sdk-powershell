...
{
    [GraphRoute("PATCH", "/auditLogs/directoryAudits/{directoryAudit-id}")]
    [Cmdlet(VerbsData.Update, "MgAuditLogDirectoryAudit", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Reports.Client.Models.DirectoryAudit))]
    public class UpdateMgAuditLogDirectoryAuditCommand : GraphClientCmdlet
    {
        ...
        [Parameter(Mandatory = false)]
        public Microsoft.Graph.PowerShell.Reports.Client.Models.AuditActivityPerformer? PerformedBy { get; set; }
        ...
        {
            if (!ShouldProcess(DirectoryAuditId, "Update"))
                return;

            var body = new Microsoft.Graph.PowerShell.Reports.Client.Models.DirectoryAudit();
    ...
    if (this.IsParameterBound(nameof(TargetResources)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.AuditLogs.DirectoryAudits[DirectoryAuditId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.AuditLogs.DirectoryAudits[DirectoryAuditId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, DirectoryAuditId);
