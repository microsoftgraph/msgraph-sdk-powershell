...
{
    [GraphRoute("GET", "/reports/servicePrincipalSignInActivities")]
    [Cmdlet(VerbsCommon.Get, "MgReportServicePrincipalSignInActivity_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Reports.Client.Models.ServicePrincipalSignInActivity))]
    public class GetMgReportServicePrincipalSignInActivity_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Reports.Client.Models.ServicePrincipalSignInActivityCollectionResponse? result;
            try
            {
                result = client.Reports.ServicePrincipalSignInActivities.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Reports.ServicePrincipalSignInActivities.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
