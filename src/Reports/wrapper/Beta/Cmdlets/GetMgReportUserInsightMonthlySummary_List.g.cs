...
{
    [GraphRoute("GET", "/reports/userInsights/monthly/summary")]
    [Cmdlet(VerbsCommon.Get, "MgReportUserInsightMonthlySummary_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Reports.Client.Models.InsightSummary))]
    public class GetMgReportUserInsightMonthlySummary_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Reports.Client.Models.InsightSummaryCollectionResponse? result;
            try
            {
                result = client.Reports.UserInsights.Monthly.Summary.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Reports.UserInsights.Monthly.Summary.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
