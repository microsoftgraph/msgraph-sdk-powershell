...
{
    [GraphRoute("PATCH", "/reports/identityAnalytics/groups/{groupAnalytics-id}")]
    [Cmdlet(VerbsData.Update, "MgReportIdentityAnalyticGroup", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Reports.Client.Models.GroupAnalytics))]
    public class UpdateMgReportIdentityAnalyticGroupCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(GroupAnalyticsId, "Update"))
                return;
    ...
    if (this.IsParameterBound(nameof(GroupType)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Reports.IdentityAnalytics.Groups[GroupAnalyticsId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Reports.IdentityAnalytics.Groups[GroupAnalyticsId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, GroupAnalyticsId);
