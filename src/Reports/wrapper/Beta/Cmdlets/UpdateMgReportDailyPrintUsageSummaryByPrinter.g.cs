...
{
    [GraphRoute("PATCH", "/reports/dailyPrintUsageSummariesByPrinter/{printUsageByPrinter-id}")]
    [Cmdlet(VerbsData.Update, "MgReportDailyPrintUsageSummaryByPrinter", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Reports.Client.Models.PrintUsageByPrinter))]
    public class UpdateMgReportDailyPrintUsageSummaryByPrinterCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(PrinterName)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Reports.DailyPrintUsageSummariesByPrinter[PrintUsageByPrinterId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Reports.DailyPrintUsageSummariesByPrinter[PrintUsageByPrinterId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, PrintUsageByPrinterId);
