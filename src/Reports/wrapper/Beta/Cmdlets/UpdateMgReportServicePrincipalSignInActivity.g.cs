...
{
    [GraphRoute("PATCH", "/reports/servicePrincipalSignInActivities/{servicePrincipalSignInActivity-id}")]
    [Cmdlet(VerbsData.Update, "MgReportServicePrincipalSignInActivity", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Reports.Client.Models.ServicePrincipalSignInActivity))]
    public class UpdateMgReportServicePrincipalSignInActivityCommand : GraphClientCmdlet
    {
        [Parameter(Mandatory = true, Position = 0)]
        public string ServicePrincipalSignInActivityId { get; set; } = string.Empty;
        ...
        {
            if (!ShouldProcess(ServicePrincipalSignInActivityId, "Update"))
                return;
    ...
    if (this.IsParameterBound(nameof(LastSignInActivity)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Reports.ServicePrincipalSignInActivities[ServicePrincipalSignInActivityId].PatchAsync(body, requestConfiguration =>
                {
            ...
            {
                ThrowGraphRequestFailed(ex, ServicePrincipalSignInActivityId);
                return;
            }
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Reports.ServicePrincipalSignInActivities[ServicePrincipalSignInActivityId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, ServicePrincipalSignInActivityId);
