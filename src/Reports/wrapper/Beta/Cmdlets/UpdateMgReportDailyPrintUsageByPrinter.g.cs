...
{
    [GraphRoute("PATCH", "/reports/dailyPrintUsageByPrinter/{printUsageByPrinter-id}")]
    [Cmdlet(VerbsData.Update, "MgReportDailyPrintUsageByPrinter", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Reports.Client.Models.PrintUsageByPrinter))]
    public class UpdateMgReportDailyPrintUsageByPrinterCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(PrinterName)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Reports.DailyPrintUsageByPrinter[PrintUsageByPrinterId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Reports.DailyPrintUsageByPrinter[PrintUsageByPrinterId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, PrintUsageByPrinterId);
