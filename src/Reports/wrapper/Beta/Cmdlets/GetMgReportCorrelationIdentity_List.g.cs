...
{
    [GraphRoute("GET", "/reports/correlations/{identityCorrelation-id}/identities")]
    [Cmdlet(VerbsCommon.Get, "MgReportCorrelationIdentity_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Reports.Client.Models.CorrelatedIdentity))]
    public class GetMgReportCorrelationIdentity_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Reports.Client.Models.CorrelatedIdentityCollectionResponse? result;
            try
            {
                result = client.Reports.Correlations[IdentityCorrelationId].Identities.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Reports.Correlations[IdentityCorrelationId].Identities.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
