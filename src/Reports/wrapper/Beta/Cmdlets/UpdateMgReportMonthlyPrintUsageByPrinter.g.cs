...
{
    [GraphRoute("PATCH", "/reports/monthlyPrintUsageByPrinter/{printUsageByPrinter-id}")]
    [Cmdlet(VerbsData.Update, "MgReportMonthlyPrintUsageByPrinter", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Reports.Client.Models.PrintUsageByPrinter))]
    public class UpdateMgReportMonthlyPrintUsageByPrinterCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(PrintUsageByPrinterId, "Update"))
                return;
    ...
    if (this.IsParameterBound(nameof(PrinterName)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Reports.MonthlyPrintUsageByPrinter[PrintUsageByPrinterId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Reports.MonthlyPrintUsageByPrinter[PrintUsageByPrinterId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, PrintUsageByPrinterId);
