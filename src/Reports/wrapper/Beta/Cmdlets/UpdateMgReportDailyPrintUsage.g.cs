...
{
    [GraphRoute("PATCH", "/reports/dailyPrintUsage/{printUsage-id}")]
    [Cmdlet(VerbsData.Update, "MgReportDailyPrintUsage", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Reports.Client.Models.PrintUsage))]
    public class UpdateMgReportDailyPrintUsageCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(PrintUsageId, "Update"))
                return;

            var body = new Microsoft.Graph.PowerShell.Reports.Client.Models.PrintUsage();
    ...
    if (this.IsParameterBound(nameof(UsageDate)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Reports.DailyPrintUsage[PrintUsageId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Reports.DailyPrintUsage[PrintUsageId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, PrintUsageId);
