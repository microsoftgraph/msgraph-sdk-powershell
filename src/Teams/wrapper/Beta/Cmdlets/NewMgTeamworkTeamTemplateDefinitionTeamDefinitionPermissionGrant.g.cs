...
{
    [GraphRoute("POST", "/teamwork/teamTemplates/{teamTemplate-id}/definitions/{teamTemplateDefinition-id}/teamDefinition/permissionGrants")]
    [Cmdlet(VerbsCommon.New, "MgTeamworkTeamTemplateDefinitionTeamDefinitionPermissionGrant", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.ResourceSpecificPermissionGrant))]
    public class NewMgTeamworkTeamTemplateDefinitionTeamDefinitionPermissionGrantCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(ResourceAppId)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Teamwork.TeamTemplates[TeamTemplateId].Definitions[TeamTemplateDefinitionId].TeamDefinition.PermissionGrants.PostAsync(body, requestConfiguration =>
                {
