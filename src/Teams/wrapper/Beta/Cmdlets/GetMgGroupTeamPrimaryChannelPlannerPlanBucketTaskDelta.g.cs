...
{
    [GraphRoute("GET", "/groups/{group-id}/team/primaryChannel/planner/plans/{plannerPlan-id}/buckets/{plannerBucket-id}/tasks/delta()")]
    [Cmdlet(VerbsCommon.Get, "MgGroupTeamPrimaryChannelPlannerPlanBucketTaskDelta", DefaultParameterSetName = "DeltaSync")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.PlannerTask))]
    public class GetMgGroupTeamPrimaryChannelPlannerPlanBucketTaskDeltaCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            global::Microsoft.Graph.PowerShell.Teams.Client.Groups.Item.Team.PrimaryChannel.Planner.Plans.Item.Buckets.Item.Tasks.Delta.DeltaGetResponse? result;
            try
            {
                result = ParameterSetName == "Resume"
                    ? client.Groups[GroupId].Team.PrimaryChannel.Planner.Plans[PlannerPlanId].Buckets[PlannerBucketId].Tasks.Delta.WithUrl(ValidateContinuationUrl(DeltaLink!, requestAdapter, nameof(DeltaLink))).GetAsDeltaGetResponseAsync(requestConfiguration =>
                        {

                AddRequestHeaders(requestConfiguration.Headers);
                        }).GetAwaiter().GetResult()
                    : client.Groups[GroupId].Team.PrimaryChannel.Planner.Plans[PlannerPlanId].Buckets[PlannerBucketId].Tasks.Delta.GetAsDeltaGetResponseAsync(requestConfiguration =>
                {
                ...
                while (true)
                {
                    ...
                    result = client.Groups[GroupId].Team.PrimaryChannel.Planner.Plans[PlannerPlanId].Buckets[PlannerBucketId].Tasks.Delta.WithUrl(nextLink).GetAsDeltaGetResponseAsync(requestConfiguration =>
                    {
