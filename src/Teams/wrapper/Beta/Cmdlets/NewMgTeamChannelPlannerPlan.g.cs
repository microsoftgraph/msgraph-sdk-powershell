...
{
    [GraphRoute("POST", "/teams/{team-id}/channels/{channel-id}/planner/plans")]
    [Cmdlet(VerbsCommon.New, "MgTeamChannelPlannerPlan", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.PlannerPlan))]
    public class NewMgTeamChannelPlannerPlanCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(SharedWithContainers)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Teams[TeamId].Channels[ChannelId].Planner.Plans.PostAsync(body, requestConfiguration =>
                {
