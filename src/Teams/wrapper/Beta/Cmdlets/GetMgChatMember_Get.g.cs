...
{
    [GraphRoute("GET", "/chats/{chat-id}/members/{conversationMember-id}")]
    [Cmdlet(VerbsCommon.Get, "MgChatMember_Get")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.ConversationMember))]
    public class GetMgChatMember_GetCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Chats[ChatId].Members[ConversationMemberId].GetAsync(requestConfiguration =>
                {
