...
{
    [GraphRoute("POST", "/teams/{team-id}/channels/{channel-id}/messages/{chatMessage-id}/hostedContents")]
    [Cmdlet(VerbsCommon.New, "MgTeamChannelMessageHostedContent", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.ChatMessageHostedContent))]
    public class NewMgTeamChannelMessageHostedContentCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(ContentType)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Teams[TeamId].Channels[ChannelId].Messages[ChatMessageId].HostedContents.PostAsync(body, requestConfiguration =>
                {
