...
{
    [GraphRoute("POST", "/teams/{team-id}/primaryChannel/planner/plans")]
    [Cmdlet(VerbsCommon.New, "MgTeamPrimaryChannelPlannerPlan", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.PlannerPlan))]
    public class NewMgTeamPrimaryChannelPlannerPlanCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(SharedWithContainers)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Teams[TeamId].PrimaryChannel.Planner.Plans.PostAsync(body, requestConfiguration =>
                {
