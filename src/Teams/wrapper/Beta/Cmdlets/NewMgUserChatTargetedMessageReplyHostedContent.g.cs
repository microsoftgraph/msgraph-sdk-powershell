...
{
    [GraphRoute("POST", "/users/{user-id}/chats/{chat-id}/targetedMessages/{targetedChatMessage-id}/replies/{chatMessage-id}/hostedContents")]
    [Cmdlet(VerbsCommon.New, "MgUserChatTargetedMessageReplyHostedContent", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.ChatMessageHostedContent))]
    public class NewMgUserChatTargetedMessageReplyHostedContentCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(ContentType)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].Chats[ChatId].TargetedMessages[TargetedChatMessageId].Replies[ChatMessageId].HostedContents.PostAsync(body, requestConfiguration =>
                {
