...
{
    [GraphRoute("POST", "/chats/{chat-id}/targetedMessages")]
    [Cmdlet(VerbsCommon.New, "MgChatTargetedMessage", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.TargetedChatMessage))]
    public class NewMgChatTargetedMessageCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Recipient)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Chats[ChatId].TargetedMessages.PostAsync(body, requestConfiguration =>
                {
