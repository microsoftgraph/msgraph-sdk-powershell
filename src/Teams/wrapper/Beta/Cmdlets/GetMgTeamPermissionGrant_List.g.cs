...
{
    [GraphRoute("GET", "/teams/{team-id}/permissionGrants")]
    [Cmdlet(VerbsCommon.Get, "MgTeamPermissionGrant_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.ResourceSpecificPermissionGrant))]
    public class GetMgTeamPermissionGrant_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Teams.Client.Models.ResourceSpecificPermissionGrantCollectionResponse? result;
            try
            {
                result = client.Teams[TeamId].PermissionGrants.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Teams[TeamId].PermissionGrants.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
