...
{
    [GraphRoute("PATCH", "/groups/{group-id}/team/schedule/offerShiftRequests/{offerShiftRequest-id}")]
    [Cmdlet(VerbsData.Update, "MgGroupTeamScheduleOfferShiftRequest", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.OfferShiftRequest))]
    public class UpdateMgGroupTeamScheduleOfferShiftRequestCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(State)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Team.Schedule.OfferShiftRequests[OfferShiftRequestId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Groups[GroupId].Team.Schedule.OfferShiftRequests[OfferShiftRequestId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, OfferShiftRequestId);
