...
{
    [GraphRoute("PATCH", "/users/{user-id}/chats/{chat-id}/installedApps/{teamsAppInstallation-id}")]
    ...
    if (this.IsParameterBound(nameof(ScopeInfo)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].Chats[ChatId].InstalledApps[TeamsAppInstallationId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Users[UserId].Chats[ChatId].InstalledApps[TeamsAppInstallationId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, TeamsAppInstallationId);
