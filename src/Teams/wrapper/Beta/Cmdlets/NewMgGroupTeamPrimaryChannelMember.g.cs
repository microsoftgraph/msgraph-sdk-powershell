...
{
    [GraphRoute("POST", "/groups/{group-id}/team/primaryChannel/members")]
    [Cmdlet(VerbsCommon.New, "MgGroupTeamPrimaryChannelMember", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.ConversationMember))]
    public class NewMgGroupTeamPrimaryChannelMemberCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(VisibleHistoryStartDateTime)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Team.PrimaryChannel.Members.PostAsync(body, requestConfiguration =>
                {
