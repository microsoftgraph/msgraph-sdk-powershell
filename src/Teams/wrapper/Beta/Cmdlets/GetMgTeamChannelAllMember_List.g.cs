...
{
    [GraphRoute("GET", "/teams/{team-id}/channels/{channel-id}/allMembers")]
    [Cmdlet(VerbsCommon.Get, "MgTeamChannelAllMember_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.ConversationMember))]
    public class GetMgTeamChannelAllMember_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Teams[TeamId].Channels[ChannelId].AllMembers.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Teams[TeamId].Channels[ChannelId].AllMembers.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
