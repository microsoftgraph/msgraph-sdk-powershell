...
{
    [GraphRoute("PATCH", "/users/{user-id}/chats/{chat-id}/operations/{teamsAsyncOperation-id}")]
    [Cmdlet(VerbsData.Update, "MgUserChatOperation", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.TeamsAsyncOperation))]
    public class UpdateMgUserChatOperationCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Status)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].Chats[ChatId].Operations[TeamsAsyncOperationId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Users[UserId].Chats[ChatId].Operations[TeamsAsyncOperationId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, TeamsAsyncOperationId);
