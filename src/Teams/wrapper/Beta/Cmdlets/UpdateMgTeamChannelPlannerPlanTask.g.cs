...
{
    [GraphRoute("PATCH", "/teams/{team-id}/channels/{channel-id}/planner/plans/{plannerPlan-id}/tasks/{plannerTask-id}")]
    [Cmdlet(VerbsData.Update, "MgTeamChannelPlannerPlanTask", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.PlannerTask))]
    public class UpdateMgTeamChannelPlannerPlanTaskCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(SpecifiedCompletionRequirements)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Teams[TeamId].Channels[ChannelId].Planner.Plans[PlannerPlanId].Tasks[PlannerTaskId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Teams[TeamId].Channels[ChannelId].Planner.Plans[PlannerPlanId].Tasks[PlannerTaskId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, PlannerTaskId);
