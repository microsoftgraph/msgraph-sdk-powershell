...
{
    [GraphRoute("GET", "/groups/{group-id}/team/channels/{channel-id}/planner/plans/delta()")]
    [Cmdlet(VerbsCommon.Get, "MgGroupTeamChannelPlannerPlanDelta", DefaultParameterSetName = "DeltaSync")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.PlannerPlan))]
    public class GetMgGroupTeamChannelPlannerPlanDeltaCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            global::Microsoft.Graph.PowerShell.Teams.Client.Groups.Item.Team.Channels.Item.Planner.Plans.Delta.DeltaGetResponse? result;
            try
            {
                result = ParameterSetName == "Resume"
                    ? client.Groups[GroupId].Team.Channels[ChannelId].Planner.Plans.Delta.WithUrl(ValidateContinuationUrl(DeltaLink!, requestAdapter, nameof(DeltaLink))).GetAsDeltaGetResponseAsync(requestConfiguration =>
                        {

                AddRequestHeaders(requestConfiguration.Headers);
                        }).GetAwaiter().GetResult()
                    : client.Groups[GroupId].Team.Channels[ChannelId].Planner.Plans.Delta.GetAsDeltaGetResponseAsync(requestConfiguration =>
                {
                ...
                while (true)
                {
                    ...
                    result = client.Groups[GroupId].Team.Channels[ChannelId].Planner.Plans.Delta.WithUrl(nextLink).GetAsDeltaGetResponseAsync(requestConfiguration =>
                    {
