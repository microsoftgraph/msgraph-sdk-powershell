...
{
    [GraphRoute("POST", "/groups/{group-id}/team/channels/{channel-id}/joinedUsers")]
    [Cmdlet(VerbsCommon.New, "MgGroupTeamChannelJoinedUser", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.ConversationMember))]
    public class NewMgGroupTeamChannelJoinedUserCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(VisibleHistoryStartDateTime)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Team.Channels[ChannelId].JoinedUsers.PostAsync(body, requestConfiguration =>
                {
