...
{
    [GraphRoute("GET", "/groups/{group-id}/team/owners/{user-id}/serviceProvisioningErrors")]
    [Cmdlet(VerbsCommon.Get, "MgGroupTeamOwnerServiceProvisioningError")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.ServiceProvisioningError))]
    public class GetMgGroupTeamOwnerServiceProvisioningErrorCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Team.Owners[UserId].ServiceProvisioningErrors.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Groups[GroupId].Team.Owners[UserId].ServiceProvisioningErrors.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
