...
{
    [GraphRoute("POST", "/teams/{team-id}/channels/{channel-id}/tabs")]
    [Cmdlet(VerbsCommon.New, "MgTeamChannelTab", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.TeamsTab))]
    public class NewMgTeamChannelTabCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Configuration)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Teams[TeamId].Channels[ChannelId].Tabs.PostAsync(body, requestConfiguration =>
                {
