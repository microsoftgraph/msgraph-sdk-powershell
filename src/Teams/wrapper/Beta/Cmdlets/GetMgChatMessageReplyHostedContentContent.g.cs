...
{
    [GraphRoute("GET", "/chats/{chat-id}/messages/{chatMessage-id}/replies/{chatMessage-id1}/hostedContents/{chatMessageHostedContent-id}/$value")]
    [Cmdlet(VerbsCommon.Get, "MgChatMessageReplyHostedContentContent")]
    [OutputType(typeof(byte[]))]
    public class GetMgChatMessageReplyHostedContentContentCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Chats[ChatId].Messages[ChatMessageId].Replies[ChatMessageId1].HostedContents[ChatMessageHostedContentId].Content.GetAsync(requestConfiguration =>
                {
