...
{
    [GraphRoute("GET", "/groups/{group-id}/team/primaryChannel/planner/plans/{plannerPlan-id}/historyItems")]
    [Cmdlet(VerbsCommon.Get, "MgGroupTeamPrimaryChannelPlannerPlanHistoryItem_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.PlannerHistoryItem))]
    public class GetMgGroupTeamPrimaryChannelPlannerPlanHistoryItem_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Team.PrimaryChannel.Planner.Plans[PlannerPlanId].HistoryItems.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Groups[GroupId].Team.PrimaryChannel.Planner.Plans[PlannerPlanId].HistoryItems.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
