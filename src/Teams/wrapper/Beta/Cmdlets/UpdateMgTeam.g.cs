...
{
    [GraphRoute("PATCH", "/teams/{team-id}")]
    [Cmdlet(VerbsData.Update, "MgTeam", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.Team))]
    public class UpdateMgTeamCommand : GraphClientCmdlet
    {
        ...
        [Parameter(Mandatory = false)]
        public Microsoft.Graph.PowerShell.Teams.Client.Models.TeamDiscoverySettings? DiscoverySettings { get; set; }
    ...
    if (this.IsParameterBound(nameof(IsMembershipLimitedToOwners)))
        body.IsMembershipLimitedToOwners = IsMembershipLimitedToOwners;
    ...
    if (this.IsParameterBound(nameof(DiscoverySettings)))
        body.DiscoverySettings = DiscoverySettings;
    ...
    if (this.IsParameterBound(nameof(Visibility)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Teams[TeamId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Teams[TeamId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, TeamId);
