...
namespace Microsoft.Graph.PowerShell.Teams
{
    ...
    [Cmdlet(VerbsCommon.Get, "MgChatTargetedMessageReplyHostedContent_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.ChatMessageHostedContent))]
    public class GetMgChatTargetedMessageReplyHostedContent_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Chats[ChatId].TargetedMessages[TargetedChatMessageId].Replies[ChatMessageId].HostedContents.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Chats[ChatId].TargetedMessages[TargetedChatMessageId].Replies[ChatMessageId].HostedContents.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
