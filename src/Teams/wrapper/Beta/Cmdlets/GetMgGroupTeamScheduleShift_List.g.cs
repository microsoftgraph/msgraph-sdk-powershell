...
{
    [GraphRoute("GET", "/groups/{group-id}/team/schedule/shifts")]
    [Cmdlet(VerbsCommon.Get, "MgGroupTeamScheduleShift_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.Shift))]
    public class GetMgGroupTeamScheduleShift_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Team.Schedule.Shifts.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Groups[GroupId].Team.Schedule.Shifts.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
