...
{
    [GraphRoute("GET", "/chats/{chat-id}/pinnedMessages")]
    [Cmdlet(VerbsCommon.Get, "MgChatPinnedMessage_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.PinnedChatMessageInfo))]
    public class GetMgChatPinnedMessage_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Chats[ChatId].PinnedMessages.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Chats[ChatId].PinnedMessages.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
