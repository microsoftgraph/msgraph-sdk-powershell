...
{
    [GraphRoute("GET", "/users/{user-id}/chats/{chat-id}/pinnedMessages")]
    [Cmdlet(VerbsCommon.Get, "MgUserChatPinnedMessage_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.PinnedChatMessageInfo))]
    public class GetMgUserChatPinnedMessage_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Teams.Client.Models.PinnedChatMessageInfoCollectionResponse? result;
            try
            {
                result = client.Users[UserId].Chats[ChatId].PinnedMessages.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Users[UserId].Chats[ChatId].PinnedMessages.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
