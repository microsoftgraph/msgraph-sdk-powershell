...
{
    [GraphRoute("POST", "/users/{user-id}/chats/{chat-id}/targetedMessages")]
    [Cmdlet(VerbsCommon.New, "MgUserChatTargetedMessage", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.TargetedChatMessage))]
    public class NewMgUserChatTargetedMessageCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Recipient)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].Chats[ChatId].TargetedMessages.PostAsync(body, requestConfiguration =>
                {
