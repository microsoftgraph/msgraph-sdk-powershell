...
{
    [GraphRoute("GET", "/groups/{group-id}/team/allChannels")]
    [Cmdlet(VerbsCommon.Get, "MgGroupTeamAllChannel_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.Channel))]
    public class GetMgGroupTeamAllChannel_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Team.AllChannels.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Groups[GroupId].Team.AllChannels.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
