...
{
    [GraphRoute("POST", "/groups/{group-id}/team/channels/{channel-id}/planner/plans")]
    [Cmdlet(VerbsCommon.New, "MgGroupTeamChannelPlannerPlan", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.PlannerPlan))]
    public class NewMgGroupTeamChannelPlannerPlanCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(SharedWithContainers)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Team.Channels[ChannelId].Planner.Plans.PostAsync(body, requestConfiguration =>
                {
