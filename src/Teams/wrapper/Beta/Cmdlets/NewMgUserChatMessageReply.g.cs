...
{
    [GraphRoute("POST", "/users/{user-id}/chats/{chat-id}/messages/{chatMessage-id}/replies")]
    [Cmdlet(VerbsCommon.New, "MgUserChatMessageReply", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.ChatMessage))]
    public class NewMgUserChatMessageReplyCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Reactions)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].Chats[ChatId].Messages[ChatMessageId].Replies.PostAsync(body, requestConfiguration =>
                {
