...
{
    [GraphRoute("POST", "/groups/{group-id}/team/primaryChannel/planner/plans/{plannerPlan-id}/tasks")]
    [Cmdlet(VerbsCommon.New, "MgGroupTeamPrimaryChannelPlannerPlanTask", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.PlannerTask))]
    public class NewMgGroupTeamPrimaryChannelPlannerPlanTaskCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(SpecifiedCompletionRequirements)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Team.PrimaryChannel.Planner.Plans[PlannerPlanId].Tasks.PostAsync(body, requestConfiguration =>
                {
