...
{
    [GraphRoute("POST", "/teamwork/devices/{teamworkDevice-id}/operations")]
    [Cmdlet(VerbsCommon.New, "MgTeamworkDeviceOperation", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.TeamworkDeviceOperation))]
    public class NewMgTeamworkDeviceOperationCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(TeamworkDeviceId, "New"))
                return;
    ...
    if (this.IsParameterBound(nameof(OperationType)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Teamwork.Devices[TeamworkDeviceId].Operations.PostAsync(body, requestConfiguration =>
                {
