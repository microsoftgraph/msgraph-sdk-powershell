...
{
    [GraphRoute("POST", "/teams/{team-id}/primaryChannel/messages")]
    [Cmdlet(VerbsCommon.New, "MgTeamPrimaryChannelMessage", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.ChatMessage))]
    public class NewMgTeamPrimaryChannelMessageCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Reactions)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Teams[TeamId].PrimaryChannel.Messages.PostAsync(body, requestConfiguration =>
                {
