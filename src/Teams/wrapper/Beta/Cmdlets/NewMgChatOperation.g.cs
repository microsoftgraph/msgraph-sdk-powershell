...
{
    [GraphRoute("POST", "/chats/{chat-id}/operations")]
    [Cmdlet(VerbsCommon.New, "MgChatOperation", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.TeamsAsyncOperation))]
    public class NewMgChatOperationCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Status)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Chats[ChatId].Operations.PostAsync(body, requestConfiguration =>
                {
