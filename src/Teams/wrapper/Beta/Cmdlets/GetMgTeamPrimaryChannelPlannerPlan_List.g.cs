...
{
    [GraphRoute("GET", "/teams/{team-id}/primaryChannel/planner/plans")]
    [Cmdlet(VerbsCommon.Get, "MgTeamPrimaryChannelPlannerPlan_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.PlannerPlan))]
    public class GetMgTeamPrimaryChannelPlannerPlan_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Teams[TeamId].PrimaryChannel.Planner.Plans.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Teams[TeamId].PrimaryChannel.Planner.Plans.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
