...
{
    [GraphRoute("GET", "/teams/{team-id}/channels/{channel-id}/planner/plans/{plannerPlan-id}/buckets")]
    [Cmdlet(VerbsCommon.Get, "MgTeamChannelPlannerPlanBucket_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.PlannerBucket))]
    public class GetMgTeamChannelPlannerPlanBucket_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Teams[TeamId].Channels[ChannelId].Planner.Plans[PlannerPlanId].Buckets.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Teams[TeamId].Channels[ChannelId].Planner.Plans[PlannerPlanId].Buckets.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
