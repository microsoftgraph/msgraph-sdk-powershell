...
{
    [GraphRoute("POST", "/users/{user-id}/teamwork/sections")]
    [Cmdlet(VerbsCommon.New, "MgUserTeamworkSection", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.TeamworkSection))]
    public class NewMgUserTeamworkSectionCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(SortType)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].Teamwork.Sections.PostAsync(body, requestConfiguration =>
                {
