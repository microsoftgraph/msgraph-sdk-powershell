...
{
    [GraphRoute("PATCH", "/teamwork/deletedTeams/{deletedTeam-id}/channels/{channel-id}/joinedUsers/{conversationMember-id}")]
    [Cmdlet(VerbsData.Update, "MgTeamworkDeletedTeamChannelJoinedUser", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.ConversationMember))]
    public class UpdateMgTeamworkDeletedTeamChannelJoinedUserCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(VisibleHistoryStartDateTime)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Teamwork.DeletedTeams[DeletedTeamId].Channels[ChannelId].JoinedUsers[ConversationMemberId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Teamwork.DeletedTeams[DeletedTeamId].Channels[ChannelId].JoinedUsers[ConversationMemberId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, ConversationMemberId);
