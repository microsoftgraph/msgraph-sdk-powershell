...
{
    [GraphRoute("POST", "/groups/{group-id}/team/primaryChannel/planner/plans/{plannerPlan-id}/historyItems")]
    [Cmdlet(VerbsCommon.New, "MgGroupTeamPrimaryChannelPlannerPlanHistoryItem", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.PlannerHistoryItem))]
    public class NewMgGroupTeamPrimaryChannelPlannerPlanHistoryItemCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(EventType)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Team.PrimaryChannel.Planner.Plans[PlannerPlanId].HistoryItems.PostAsync(body, requestConfiguration =>
                {
