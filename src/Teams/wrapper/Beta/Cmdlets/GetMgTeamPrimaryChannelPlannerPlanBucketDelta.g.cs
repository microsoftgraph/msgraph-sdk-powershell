...
{
    [GraphRoute("GET", "/teams/{team-id}/primaryChannel/planner/plans/{plannerPlan-id}/buckets/delta()")]
    [Cmdlet(VerbsCommon.Get, "MgTeamPrimaryChannelPlannerPlanBucketDelta", DefaultParameterSetName = "DeltaSync")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.PlannerBucket))]
    public class GetMgTeamPrimaryChannelPlannerPlanBucketDeltaCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            global::Microsoft.Graph.PowerShell.Teams.Client.Teams.Item.PrimaryChannel.Planner.Plans.Item.Buckets.Delta.DeltaGetResponse? result;
            try
            {
                result = ParameterSetName == "Resume"
                    ? client.Teams[TeamId].PrimaryChannel.Planner.Plans[PlannerPlanId].Buckets.Delta.WithUrl(ValidateContinuationUrl(DeltaLink!, requestAdapter, nameof(DeltaLink))).GetAsDeltaGetResponseAsync(requestConfiguration =>
                        {

                AddRequestHeaders(requestConfiguration.Headers);
                        }).GetAwaiter().GetResult()
                    : client.Teams[TeamId].PrimaryChannel.Planner.Plans[PlannerPlanId].Buckets.Delta.GetAsDeltaGetResponseAsync(requestConfiguration =>
                {
                ...
                while (true)
                {
                    ...
                    result = client.Teams[TeamId].PrimaryChannel.Planner.Plans[PlannerPlanId].Buckets.Delta.WithUrl(nextLink).GetAsDeltaGetResponseAsync(requestConfiguration =>
                    {
