...
{
    [GraphRoute("GET", "/teams/{team-id}/primaryChannel/joinedUsers")]
    [Cmdlet(VerbsCommon.Get, "MgTeamPrimaryChannelJoinedUser_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.ConversationMember))]
    public class GetMgTeamPrimaryChannelJoinedUser_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Teams[TeamId].PrimaryChannel.JoinedUsers.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Teams[TeamId].PrimaryChannel.JoinedUsers.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
