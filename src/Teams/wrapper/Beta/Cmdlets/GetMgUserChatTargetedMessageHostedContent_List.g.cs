...
{
    [GraphRoute("GET", "/users/{user-id}/chats/{chat-id}/targetedMessages/{targetedChatMessage-id}/hostedContents")]
    [Cmdlet(VerbsCommon.Get, "MgUserChatTargetedMessageHostedContent_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.ChatMessageHostedContent))]
    public class GetMgUserChatTargetedMessageHostedContent_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].Chats[ChatId].TargetedMessages[TargetedChatMessageId].HostedContents.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Users[UserId].Chats[ChatId].TargetedMessages[TargetedChatMessageId].HostedContents.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
