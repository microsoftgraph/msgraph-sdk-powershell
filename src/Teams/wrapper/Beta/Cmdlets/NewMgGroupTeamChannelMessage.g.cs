...
{
    [GraphRoute("POST", "/groups/{group-id}/team/channels/{channel-id}/messages")]
    [Cmdlet(VerbsCommon.New, "MgGroupTeamChannelMessage", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.ChatMessage))]
    public class NewMgGroupTeamChannelMessageCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Reactions)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Team.Channels[ChannelId].Messages.PostAsync(body, requestConfiguration =>
                {
