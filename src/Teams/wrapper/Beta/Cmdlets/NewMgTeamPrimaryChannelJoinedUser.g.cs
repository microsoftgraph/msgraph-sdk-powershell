...
{
    [GraphRoute("POST", "/teams/{team-id}/primaryChannel/joinedUsers")]
    [Cmdlet(VerbsCommon.New, "MgTeamPrimaryChannelJoinedUser", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.ConversationMember))]
    public class NewMgTeamPrimaryChannelJoinedUserCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(VisibleHistoryStartDateTime)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Teams[TeamId].PrimaryChannel.JoinedUsers.PostAsync(body, requestConfiguration =>
                {
