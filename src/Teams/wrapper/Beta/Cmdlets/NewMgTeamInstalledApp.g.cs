...
{
    [GraphRoute("POST", "/teams/{team-id}/installedApps")]
    [Cmdlet(VerbsCommon.New, "MgTeamInstalledApp", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.TeamsAppInstallation))]
    public class NewMgTeamInstalledAppCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(ScopeInfo)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Teams[TeamId].InstalledApps.PostAsync(body, requestConfiguration =>
                {
