...
{
    [GraphRoute("POST", "/teams/{team-id}/channels/{channel-id}/planner/plans/{plannerPlan-id}/historyItems")]
    [Cmdlet(VerbsCommon.New, "MgTeamChannelPlannerPlanHistoryItem", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.PlannerHistoryItem))]
    public class NewMgTeamChannelPlannerPlanHistoryItemCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(EventType)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Teams[TeamId].Channels[ChannelId].Planner.Plans[PlannerPlanId].HistoryItems.PostAsync(body, requestConfiguration =>
                {
