...
{
    [GraphRoute("POST", "/groups/{group-id}/team/schedule/openShifts")]
    [Cmdlet(VerbsCommon.New, "MgGroupTeamScheduleOpenShift", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.OpenShift))]
    public class NewMgGroupTeamScheduleOpenShiftCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(SharedOpenShift)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Team.Schedule.OpenShifts.PostAsync(body, requestConfiguration =>
                {
