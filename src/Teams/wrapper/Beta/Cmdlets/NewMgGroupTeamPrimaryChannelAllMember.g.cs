...
{
    [GraphRoute("POST", "/groups/{group-id}/team/primaryChannel/allMembers")]
    [Cmdlet(VerbsCommon.New, "MgGroupTeamPrimaryChannelAllMember", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.ConversationMember))]
    public class NewMgGroupTeamPrimaryChannelAllMemberCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(VisibleHistoryStartDateTime)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Team.PrimaryChannel.AllMembers.PostAsync(body, requestConfiguration =>
                {
