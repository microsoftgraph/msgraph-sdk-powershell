...
{
    [GraphRoute("GET", "/teams/{team-id}/primaryChannel/planner/plans/{plannerPlan-id}/tasks/delta()")]
    [Cmdlet(VerbsCommon.Get, "MgTeamPrimaryChannelPlannerPlanTaskDelta", DefaultParameterSetName = "DeltaSync")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.PlannerTask))]
    public class GetMgTeamPrimaryChannelPlannerPlanTaskDeltaCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            global::Microsoft.Graph.PowerShell.Teams.Client.Teams.Item.PrimaryChannel.Planner.Plans.Item.Tasks.Delta.DeltaGetResponse? result;
            try
            {
                result = ParameterSetName == "Resume"
                    ? client.Teams[TeamId].PrimaryChannel.Planner.Plans[PlannerPlanId].Tasks.Delta.WithUrl(ValidateContinuationUrl(DeltaLink!, requestAdapter, nameof(DeltaLink))).GetAsDeltaGetResponseAsync(requestConfiguration =>
                        {

                AddRequestHeaders(requestConfiguration.Headers);
                        }).GetAwaiter().GetResult()
                    : client.Teams[TeamId].PrimaryChannel.Planner.Plans[PlannerPlanId].Tasks.Delta.GetAsDeltaGetResponseAsync(requestConfiguration =>
                {
                ...
                while (true)
                {
                    ...
                    result = client.Teams[TeamId].PrimaryChannel.Planner.Plans[PlannerPlanId].Tasks.Delta.WithUrl(nextLink).GetAsDeltaGetResponseAsync(requestConfiguration =>
                    {
