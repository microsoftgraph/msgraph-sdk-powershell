...
{
    [GraphRoute("GET", "/groups/{group-id}/team/primaryChannel/joinedUsers")]
    [Cmdlet(VerbsCommon.Get, "MgGroupTeamPrimaryChannelJoinedUser_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.ConversationMember))]
    public class GetMgGroupTeamPrimaryChannelJoinedUser_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Team.PrimaryChannel.JoinedUsers.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Groups[GroupId].Team.PrimaryChannel.JoinedUsers.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
