...
{
    [GraphRoute("GET", "/teams/{team-id}/primaryChannel/planner/plans/{plannerPlan-id}/buckets/{plannerBucket-id}/tasks")]
    [Cmdlet(VerbsCommon.Get, "MgTeamPrimaryChannelPlannerPlanBucketTask_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.PlannerTask))]
    public class GetMgTeamPrimaryChannelPlannerPlanBucketTask_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Teams[TeamId].PrimaryChannel.Planner.Plans[PlannerPlanId].Buckets[PlannerBucketId].Tasks.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Teams[TeamId].PrimaryChannel.Planner.Plans[PlannerPlanId].Buckets[PlannerBucketId].Tasks.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
