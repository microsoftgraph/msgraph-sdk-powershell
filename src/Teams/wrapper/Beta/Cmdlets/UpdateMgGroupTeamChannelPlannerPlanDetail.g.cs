...
{
    [GraphRoute("PATCH", "/groups/{group-id}/team/channels/{channel-id}/planner/plans/{plannerPlan-id}/details")]
    [Cmdlet(VerbsData.Update, "MgGroupTeamChannelPlannerPlanDetail", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.PlannerPlanDetails))]
    public class UpdateMgGroupTeamChannelPlannerPlanDetailCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(SharedWith)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Team.Channels[ChannelId].Planner.Plans[PlannerPlanId].Details.PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Groups[GroupId].Team.Channels[ChannelId].Planner.Plans[PlannerPlanId].Details.GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, PlannerPlanId);
