...
{
    [GraphRoute("PATCH", "/groups/{group-id}/team/primaryChannel/planner/plans/{plannerPlan-id}/historyItems/{plannerHistoryItem-id}")]
    [Cmdlet(VerbsData.Update, "MgGroupTeamPrimaryChannelPlannerPlanHistoryItem", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.PlannerHistoryItem))]
    public class UpdateMgGroupTeamPrimaryChannelPlannerPlanHistoryItemCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(EventType)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Team.PrimaryChannel.Planner.Plans[PlannerPlanId].HistoryItems[PlannerHistoryItemId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Groups[GroupId].Team.PrimaryChannel.Planner.Plans[PlannerPlanId].HistoryItems[PlannerHistoryItemId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, PlannerHistoryItemId);
