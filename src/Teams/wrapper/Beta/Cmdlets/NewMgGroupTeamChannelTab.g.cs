...
{
    [GraphRoute("POST", "/groups/{group-id}/team/channels/{channel-id}/tabs")]
    [Cmdlet(VerbsCommon.New, "MgGroupTeamChannelTab", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.TeamsTab))]
    public class NewMgGroupTeamChannelTabCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Configuration)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Team.Channels[ChannelId].Tabs.PostAsync(body, requestConfiguration =>
                {
