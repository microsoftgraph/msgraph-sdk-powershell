...
{
    [GraphRoute("POST", "/groups/{group-id}/team/primaryChannel/joinedUsers")]
    [Cmdlet(VerbsCommon.New, "MgGroupTeamPrimaryChannelJoinedUser", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.ConversationMember))]
    public class NewMgGroupTeamPrimaryChannelJoinedUserCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(VisibleHistoryStartDateTime)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Team.PrimaryChannel.JoinedUsers.PostAsync(body, requestConfiguration =>
                {
