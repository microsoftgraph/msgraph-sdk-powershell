...
{
    [GraphRoute("POST", "/teamwork/teamTemplates/{teamTemplate-id}/definitions/{teamTemplateDefinition-id}/teamDefinition/channels/{channel-id}/planner/plans/{plannerPlan-id}/tasks")]
    [Cmdlet(VerbsCommon.New, "MgTeamworkTeamTemplateDefinitionTeamDefinitionChannelPlannerPlanTask", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.PlannerTask))]
    public class NewMgTeamworkTeamTemplateDefinitionTeamDefinitionChannelPlannerPlanTaskCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(SpecifiedCompletionRequirements)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Teamwork.TeamTemplates[TeamTemplateId].Definitions[TeamTemplateDefinitionId].TeamDefinition.Channels[ChannelId].Planner.Plans[PlannerPlanId].Tasks.PostAsync(body, requestConfiguration =>
                {
