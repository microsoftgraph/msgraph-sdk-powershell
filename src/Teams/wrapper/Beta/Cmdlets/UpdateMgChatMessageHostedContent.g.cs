...
{
    [GraphRoute("PATCH", "/chats/{chat-id}/messages/{chatMessage-id}/hostedContents/{chatMessageHostedContent-id}")]
    [Cmdlet(VerbsData.Update, "MgChatMessageHostedContent", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.ChatMessageHostedContent))]
    public class UpdateMgChatMessageHostedContentCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(ContentType)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Chats[ChatId].Messages[ChatMessageId].HostedContents[ChatMessageHostedContentId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Chats[ChatId].Messages[ChatMessageId].HostedContents[ChatMessageHostedContentId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, ChatMessageHostedContentId);
