...
{
    [GraphRoute("PATCH", "/groups/{group-id}/team/channels/{channel-id}/planner/plans/{plannerPlan-id}")]
    [Cmdlet(VerbsData.Update, "MgGroupTeamChannelPlannerPlan", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.PlannerPlan))]
    public class UpdateMgGroupTeamChannelPlannerPlanCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(SharedWithContainers)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Team.Channels[ChannelId].Planner.Plans[PlannerPlanId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Groups[GroupId].Team.Channels[ChannelId].Planner.Plans[PlannerPlanId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, PlannerPlanId);
