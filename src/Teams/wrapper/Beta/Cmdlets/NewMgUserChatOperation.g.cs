...
{
    [GraphRoute("POST", "/users/{user-id}/chats/{chat-id}/operations")]
    [Cmdlet(VerbsCommon.New, "MgUserChatOperation", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.TeamsAsyncOperation))]
    public class NewMgUserChatOperationCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Status)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].Chats[ChatId].Operations.PostAsync(body, requestConfiguration =>
                {
