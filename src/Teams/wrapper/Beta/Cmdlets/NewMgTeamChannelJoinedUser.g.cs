...
{
    [GraphRoute("POST", "/teams/{team-id}/channels/{channel-id}/joinedUsers")]
    [Cmdlet(VerbsCommon.New, "MgTeamChannelJoinedUser", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.ConversationMember))]
    public class NewMgTeamChannelJoinedUserCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(VisibleHistoryStartDateTime)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Teams[TeamId].Channels[ChannelId].JoinedUsers.PostAsync(body, requestConfiguration =>
                {
