...
{
    [GraphRoute("GET", "/chats/{chat-id}/operations/{teamsAsyncOperation-id}")]
    [Cmdlet(VerbsCommon.Get, "MgChatOperation_Get")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.TeamsAsyncOperation))]
    public class GetMgChatOperation_GetCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Chats[ChatId].Operations[TeamsAsyncOperationId].GetAsync(requestConfiguration =>
                {
