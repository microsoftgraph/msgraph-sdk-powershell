...
{
    [GraphRoute("GET", "/groups/{group-id}/team/primaryChannel/planner/plans/delta()")]
    [Cmdlet(VerbsCommon.Get, "MgGroupTeamPrimaryChannelPlannerPlanDelta", DefaultParameterSetName = "DeltaSync")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.PlannerPlan))]
    public class GetMgGroupTeamPrimaryChannelPlannerPlanDeltaCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            global::Microsoft.Graph.PowerShell.Teams.Client.Groups.Item.Team.PrimaryChannel.Planner.Plans.Delta.DeltaGetResponse? result;
            try
            {
                result = ParameterSetName == "Resume"
                    ? client.Groups[GroupId].Team.PrimaryChannel.Planner.Plans.Delta.WithUrl(ValidateContinuationUrl(DeltaLink!, requestAdapter, nameof(DeltaLink))).GetAsDeltaGetResponseAsync(requestConfiguration =>
                        {

                AddRequestHeaders(requestConfiguration.Headers);
                        }).GetAwaiter().GetResult()
                    : client.Groups[GroupId].Team.PrimaryChannel.Planner.Plans.Delta.GetAsDeltaGetResponseAsync(requestConfiguration =>
                {
                ...
                while (true)
                {
                    ...
                    result = client.Groups[GroupId].Team.PrimaryChannel.Planner.Plans.Delta.WithUrl(nextLink).GetAsDeltaGetResponseAsync(requestConfiguration =>
                    {
