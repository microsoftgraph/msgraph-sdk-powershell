...
{
    [GraphRoute("PATCH", "/teams/{team-id}/primaryChannel")]
    [Cmdlet(VerbsData.Update, "MgTeamPrimaryChannel", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.Channel))]
    public class UpdateMgTeamPrimaryChannelCommand : GraphClientCmdlet
    {
        ...
        [Parameter(Mandatory = false)]
        public Microsoft.Graph.PowerShell.Teams.Client.Models.ChannelModerationSettings? ModerationSettings { get; set; }
    ...
    if (this.IsParameterBound(nameof(ModerationSettings)))
        body.ModerationSettings = ModerationSettings;
    ...
    if (this.IsParameterBound(nameof(Summary)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Teams[TeamId].PrimaryChannel.PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Teams[TeamId].PrimaryChannel.GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, TeamId);
