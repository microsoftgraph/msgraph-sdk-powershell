...
{
    [GraphRoute("PATCH", "/teams/{team-id}/channels/{channel-id}/messages/{chatMessage-id}")]
    [Cmdlet(VerbsData.Update, "MgTeamChannelMessage", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.ChatMessage))]
    public class UpdateMgTeamChannelMessageCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Reactions)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Teams[TeamId].Channels[ChannelId].Messages[ChatMessageId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Teams[TeamId].Channels[ChannelId].Messages[ChatMessageId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, ChatMessageId);
