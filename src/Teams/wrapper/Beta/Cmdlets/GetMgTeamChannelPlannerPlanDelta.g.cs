...
{
    [GraphRoute("GET", "/teams/{team-id}/channels/{channel-id}/planner/plans/delta()")]
    [Cmdlet(VerbsCommon.Get, "MgTeamChannelPlannerPlanDelta", DefaultParameterSetName = "DeltaSync")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.PlannerPlan))]
    public class GetMgTeamChannelPlannerPlanDeltaCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            global::Microsoft.Graph.PowerShell.Teams.Client.Teams.Item.Channels.Item.Planner.Plans.Delta.DeltaGetResponse? result;
            try
            {
                result = ParameterSetName == "Resume"
                    ? client.Teams[TeamId].Channels[ChannelId].Planner.Plans.Delta.WithUrl(ValidateContinuationUrl(DeltaLink!, requestAdapter, nameof(DeltaLink))).GetAsDeltaGetResponseAsync(requestConfiguration =>
                        {

                AddRequestHeaders(requestConfiguration.Headers);
                        }).GetAwaiter().GetResult()
                    : client.Teams[TeamId].Channels[ChannelId].Planner.Plans.Delta.GetAsDeltaGetResponseAsync(requestConfiguration =>
                {
                ...
                while (true)
                {
                    ...
                    result = client.Teams[TeamId].Channels[ChannelId].Planner.Plans.Delta.WithUrl(nextLink).GetAsDeltaGetResponseAsync(requestConfiguration =>
                    {
