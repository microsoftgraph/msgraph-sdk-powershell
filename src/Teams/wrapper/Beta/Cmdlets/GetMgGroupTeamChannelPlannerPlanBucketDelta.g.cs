...
{
    [GraphRoute("GET", "/groups/{group-id}/team/channels/{channel-id}/planner/plans/{plannerPlan-id}/buckets/delta()")]
    [Cmdlet(VerbsCommon.Get, "MgGroupTeamChannelPlannerPlanBucketDelta", DefaultParameterSetName = "DeltaSync")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.PlannerBucket))]
    public class GetMgGroupTeamChannelPlannerPlanBucketDeltaCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            global::Microsoft.Graph.PowerShell.Teams.Client.Groups.Item.Team.Channels.Item.Planner.Plans.Item.Buckets.Delta.DeltaGetResponse? result;
            try
            {
                result = ParameterSetName == "Resume"
                    ? client.Groups[GroupId].Team.Channels[ChannelId].Planner.Plans[PlannerPlanId].Buckets.Delta.WithUrl(ValidateContinuationUrl(DeltaLink!, requestAdapter, nameof(DeltaLink))).GetAsDeltaGetResponseAsync(requestConfiguration =>
                        {

                AddRequestHeaders(requestConfiguration.Headers);
                        }).GetAwaiter().GetResult()
                    : client.Groups[GroupId].Team.Channels[ChannelId].Planner.Plans[PlannerPlanId].Buckets.Delta.GetAsDeltaGetResponseAsync(requestConfiguration =>
                {
                ...
                while (true)
                {
                    ...
                    result = client.Groups[GroupId].Team.Channels[ChannelId].Planner.Plans[PlannerPlanId].Buckets.Delta.WithUrl(nextLink).GetAsDeltaGetResponseAsync(requestConfiguration =>
                    {
