...
{
    [GraphRoute("GET", "/teams/{team-id}/primaryChannel/planner/plans/{plannerPlan-id}/tasks/{plannerTask-id}/messages")]
    [Cmdlet(VerbsCommon.Get, "MgTeamPrimaryChannelPlannerPlanTaskMessage_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.PlannerTaskChatMessage))]
    public class GetMgTeamPrimaryChannelPlannerPlanTaskMessage_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Teams[TeamId].PrimaryChannel.Planner.Plans[PlannerPlanId].Tasks[PlannerTaskId].Messages.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Teams[TeamId].PrimaryChannel.Planner.Plans[PlannerPlanId].Tasks[PlannerTaskId].Messages.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
