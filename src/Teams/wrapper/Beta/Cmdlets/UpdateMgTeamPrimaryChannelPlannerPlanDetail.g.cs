...
{
    [GraphRoute("PATCH", "/teams/{team-id}/primaryChannel/planner/plans/{plannerPlan-id}/details")]
    [Cmdlet(VerbsData.Update, "MgTeamPrimaryChannelPlannerPlanDetail", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.PlannerPlanDetails))]
    public class UpdateMgTeamPrimaryChannelPlannerPlanDetailCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(SharedWith)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Teams[TeamId].PrimaryChannel.Planner.Plans[PlannerPlanId].Details.PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Teams[TeamId].PrimaryChannel.Planner.Plans[PlannerPlanId].Details.GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, PlannerPlanId);
