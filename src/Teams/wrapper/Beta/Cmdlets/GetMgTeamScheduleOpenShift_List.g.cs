...
{
    [GraphRoute("GET", "/teams/{team-id}/schedule/openShifts")]
    [Cmdlet(VerbsCommon.Get, "MgTeamScheduleOpenShift_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.OpenShift))]
    public class GetMgTeamScheduleOpenShift_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Teams[TeamId].Schedule.OpenShifts.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Teams[TeamId].Schedule.OpenShifts.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
