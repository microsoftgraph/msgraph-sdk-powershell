...
{
    [GraphRoute("POST", "/teams/{team-id}/schedule/timeCards")]
    [Cmdlet(VerbsCommon.New, "MgTeamScheduleTimeCard", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.TimeCard))]
    public class NewMgTeamScheduleTimeCardCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(State)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Teams[TeamId].Schedule.TimeCards.PostAsync(body, requestConfiguration =>
                {
