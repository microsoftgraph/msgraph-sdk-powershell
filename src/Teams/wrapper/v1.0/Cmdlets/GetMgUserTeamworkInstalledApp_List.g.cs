...
{
    [GraphRoute("GET", "/users/{user-id}/teamwork/installedApps")]
    [Cmdlet(VerbsCommon.Get, "MgUserTeamworkInstalledApp_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.UserScopeTeamsAppInstallation))]
    public class GetMgUserTeamworkInstalledApp_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Teams.Client.Models.UserScopeTeamsAppInstallationCollectionResponse? result;
            try
            {
                result = client.Users[UserId].Teamwork.InstalledApps.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Users[UserId].Teamwork.InstalledApps.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
