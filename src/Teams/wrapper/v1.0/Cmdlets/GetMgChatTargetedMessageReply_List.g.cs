...
{
    [GraphRoute("GET", "/chats/{chat-id}/targetedMessages/{targetedChatMessage-id}/replies")]
    [Cmdlet(VerbsCommon.Get, "MgChatTargetedMessageReply_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.ChatMessage))]
    public class GetMgChatTargetedMessageReply_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Chats[ChatId].TargetedMessages[TargetedChatMessageId].Replies.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Chats[ChatId].TargetedMessages[TargetedChatMessageId].Replies.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
