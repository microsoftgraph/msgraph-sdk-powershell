...
{
    [GraphRoute("POST", "/users/{user-id}/chats/{chat-id}/permissionGrants")]
    [Cmdlet(VerbsCommon.New, "MgUserChatPermissionGrant", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.ResourceSpecificPermissionGrant))]
    public class NewMgUserChatPermissionGrantCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(ResourceAppId)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].Chats[ChatId].PermissionGrants.PostAsync(body, requestConfiguration =>
                {
