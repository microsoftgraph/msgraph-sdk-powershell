...
namespace Microsoft.Graph.PowerShell.Teams
{
    ...
    [Cmdlet(VerbsCommon.Get, "MgTeamPrimaryChannelMessageReplyHostedContent_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.ChatMessageHostedContent))]
    public class GetMgTeamPrimaryChannelMessageReplyHostedContent_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Teams[TeamId].PrimaryChannel.Messages[ChatMessageId].Replies[ChatMessageId1].HostedContents.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Teams[TeamId].PrimaryChannel.Messages[ChatMessageId].Replies[ChatMessageId1].HostedContents.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
