...
{
    [GraphRoute("GET", "/teams/{team-id}/primaryChannel/tabs")]
    [Cmdlet(VerbsCommon.Get, "MgTeamPrimaryChannelTab_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.TeamsTab))]
    public class GetMgTeamPrimaryChannelTab_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Teams[TeamId].PrimaryChannel.Tabs.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Teams[TeamId].PrimaryChannel.Tabs.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
