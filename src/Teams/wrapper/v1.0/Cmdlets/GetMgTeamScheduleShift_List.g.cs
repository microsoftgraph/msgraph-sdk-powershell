...
{
    [GraphRoute("GET", "/teams/{team-id}/schedule/shifts")]
    [Cmdlet(VerbsCommon.Get, "MgTeamScheduleShift_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.Shift))]
    public class GetMgTeamScheduleShift_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Teams[TeamId].Schedule.Shifts.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Teams[TeamId].Schedule.Shifts.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
