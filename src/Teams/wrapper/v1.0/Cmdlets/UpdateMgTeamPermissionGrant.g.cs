...
{
    [GraphRoute("PATCH", "/teams/{team-id}/permissionGrants/{resourceSpecificPermissionGrant-id}")]
    [Cmdlet(VerbsData.Update, "MgTeamPermissionGrant", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.ResourceSpecificPermissionGrant))]
    public class UpdateMgTeamPermissionGrantCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(ResourceAppId)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Teams[TeamId].PermissionGrants[ResourceSpecificPermissionGrantId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Teams[TeamId].PermissionGrants[ResourceSpecificPermissionGrantId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException)
                {
                    ThrowGraphRequestFailed(ex, ResourceSpecificPermissionGrantId);
