...
{
    [GraphRoute("PATCH", "/groups/{group-id}/team/schedule/timesOff/{timeOff-id}")]
    [Cmdlet(VerbsData.Update, "MgGroupTeamScheduleTimeOff", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.TimeOff))]
    public class UpdateMgGroupTeamScheduleTimeOffCommand : PSCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(SharedTimeOff)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Team.Schedule.TimesOff[TimeOffId].PatchAsync(body, requestConfiguration =>
                {
            ...
            {
                ThrowTerminatingError(new ErrorRecord(ex, "GraphRequestFailed", ErrorCategory.InvalidOperation, TimeOffId));
                return;
            }
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Groups[GroupId].Team.Schedule.TimesOff[TimeOffId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex)
                {
                    ThrowTerminatingError(new ErrorRecord(ex, "GraphRequestFailed", ErrorCategory.InvalidOperation, TimeOffId));
