...
{
    [GraphRoute("GET", "/groups/{group-id}/team/group/serviceProvisioningErrors")]
    [Cmdlet(VerbsCommon.Get, "MgGroupTeamGroupServiceProvisioningError")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.ServiceProvisioningError))]
    public class GetMgGroupTeamGroupServiceProvisioningErrorCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Team.Group.ServiceProvisioningErrors.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Groups[GroupId].Team.Group.ServiceProvisioningErrors.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
