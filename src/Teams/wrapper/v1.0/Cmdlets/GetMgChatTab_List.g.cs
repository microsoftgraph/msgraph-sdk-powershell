...
{
    [GraphRoute("GET", "/chats/{chat-id}/tabs")]
    [Cmdlet(VerbsCommon.Get, "MgChatTab_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.TeamsTab))]
    public class GetMgChatTab_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Chats[ChatId].Tabs.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Chats[ChatId].Tabs.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
