...
{
    [GraphRoute("GET", "/teams/{team-id}/primaryChannel/messages/{chatMessage-id}/replies/delta()")]
    [Cmdlet(VerbsCommon.Get, "MgTeamPrimaryChannelMessageReplyDelta", DefaultParameterSetName = "DeltaSync")]
    [OutputType(typeof(global::Microsoft.Graph.PowerShell.Teams.Client.Teams.Item.PrimaryChannel.Messages.Item.Replies.Delta.DeltaGetResponse))]
    public class GetMgTeamPrimaryChannelMessageReplyDeltaCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            global::Microsoft.Graph.PowerShell.Teams.Client.Teams.Item.PrimaryChannel.Messages.Item.Replies.Delta.DeltaGetResponse? result;
            try
            {
                result = ParameterSetName == "Resume"
                    ? client.Teams[TeamId].PrimaryChannel.Messages[ChatMessageId].Replies.Delta.WithUrl(ValidateContinuationUrl(DeltaLink!, requestAdapter, nameof(DeltaLink))).GetAsDeltaGetResponseAsync(requestConfiguration =>
                        {

                AddRequestHeaders(requestConfiguration.Headers);
                        }).GetAwaiter().GetResult()
                    : client.Teams[TeamId].PrimaryChannel.Messages[ChatMessageId].Replies.Delta.GetAsDeltaGetResponseAsync(requestConfiguration =>
                {
                ...
                while (true)
                {
                    ...
                    result = client.Teams[TeamId].PrimaryChannel.Messages[ChatMessageId].Replies.Delta.WithUrl(nextLink).GetAsDeltaGetResponseAsync(requestConfiguration =>
                    {
