...
{
    [GraphRoute("GET", "/groups/{group-id}/team/channels/{channel-id}/sharedWithTeams/{sharedWithChannelTeamInfo-id}/allowedMembers")]
    [Cmdlet(VerbsCommon.Get, "MgGroupTeamChannelSharedWithTeamAllowedMember_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.ConversationMember))]
    public class GetMgGroupTeamChannelSharedWithTeamAllowedMember_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Team.Channels[ChannelId].SharedWithTeams[SharedWithChannelTeamInfoId].AllowedMembers.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Groups[GroupId].Team.Channels[ChannelId].SharedWithTeams[SharedWithChannelTeamInfoId].AllowedMembers.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
