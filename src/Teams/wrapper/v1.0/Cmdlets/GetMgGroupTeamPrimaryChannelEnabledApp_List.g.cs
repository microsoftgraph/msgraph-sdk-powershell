...
{
    [GraphRoute("GET", "/groups/{group-id}/team/primaryChannel/enabledApps")]
    [Cmdlet(VerbsCommon.Get, "MgGroupTeamPrimaryChannelEnabledApp_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.TeamsApp))]
    public class GetMgGroupTeamPrimaryChannelEnabledApp_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Teams.Client.Models.TeamsAppCollectionResponse? result;
            try
            {
                result = client.Groups[GroupId].Team.PrimaryChannel.EnabledApps.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Groups[GroupId].Team.PrimaryChannel.EnabledApps.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
