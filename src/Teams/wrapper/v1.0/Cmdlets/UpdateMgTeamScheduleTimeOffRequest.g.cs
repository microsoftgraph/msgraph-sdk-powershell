...
{
    [GraphRoute("PATCH", "/teams/{team-id}/schedule/timeOffRequests/{timeOffRequest-id}")]
    [Cmdlet(VerbsData.Update, "MgTeamScheduleTimeOffRequest", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.TimeOffRequest))]
    public class UpdateMgTeamScheduleTimeOffRequestCommand : PSCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(State)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Teams[TeamId].Schedule.TimeOffRequests[TimeOffRequestId].PatchAsync(body, requestConfiguration =>
                {
            ...
            {
                ThrowTerminatingError(new ErrorRecord(ex, "GraphRequestFailed", ErrorCategory.InvalidOperation, TimeOffRequestId));
                return;
            }
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Teams[TeamId].Schedule.TimeOffRequests[TimeOffRequestId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex)
                {
                    ThrowTerminatingError(new ErrorRecord(ex, "GraphRequestFailed", ErrorCategory.InvalidOperation, TimeOffRequestId));
