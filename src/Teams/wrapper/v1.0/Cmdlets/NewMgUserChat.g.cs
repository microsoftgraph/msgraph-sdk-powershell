...
{
    [GraphRoute("POST", "/users/{user-id}/chats")]
    [Cmdlet(VerbsCommon.New, "MgUserChat", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.Chat))]
    public class NewMgUserChatCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Viewpoint)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].Chats.PostAsync(body, requestConfiguration =>
                {
