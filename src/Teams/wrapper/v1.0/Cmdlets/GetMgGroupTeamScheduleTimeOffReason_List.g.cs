...
namespace Microsoft.Graph.PowerShell.Teams
{
    ...
    [Cmdlet(VerbsCommon.Get, "MgGroupTeamScheduleTimeOffReason_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.TimeOffReason))]
    public class GetMgGroupTeamScheduleTimeOffReason_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Teams.Client.Models.TimeOffReasonCollectionResponse? result;
            try
            {
                result = client.Groups[GroupId].Team.Schedule.TimeOffReasons.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Groups[GroupId].Team.Schedule.TimeOffReasons.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
