...
{
    [GraphRoute("POST", "/users/{user-id}/chats/{chat-id}/members")]
    [Cmdlet(VerbsCommon.New, "MgUserChatMember", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.ConversationMember))]
    public class NewMgUserChatMemberCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(VisibleHistoryStartDateTime)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].Chats[ChatId].Members.PostAsync(body, requestConfiguration =>
                {
