...
{
    [GraphRoute("POST", "/groups/{group-id}/team/channels/{channel-id}/messages/{chatMessage-id}/hostedContents")]
    [Cmdlet(VerbsCommon.New, "MgGroupTeamChannelMessageHostedContent", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.ChatMessageHostedContent))]
    public class NewMgGroupTeamChannelMessageHostedContentCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(ContentType)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Team.Channels[ChannelId].Messages[ChatMessageId].HostedContents.PostAsync(body, requestConfiguration =>
                {
