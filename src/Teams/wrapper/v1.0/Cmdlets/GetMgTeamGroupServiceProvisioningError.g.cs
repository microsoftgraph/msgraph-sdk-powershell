...
{
    [GraphRoute("GET", "/teams/{team-id}/group/serviceProvisioningErrors")]
    [Cmdlet(VerbsCommon.Get, "MgTeamGroupServiceProvisioningError")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.ServiceProvisioningError))]
    public class GetMgTeamGroupServiceProvisioningErrorCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Teams[TeamId].Group.ServiceProvisioningErrors.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Teams[TeamId].Group.ServiceProvisioningErrors.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
