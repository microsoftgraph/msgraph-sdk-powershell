...
{
    [GraphRoute("POST", "/groups/{group-id}/team/sendActivityNotification")]
    [Cmdlet(VerbsCommunications.Send, "MgGroupTeamActivityNotification", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]

    public class SendMgGroupTeamActivityNotificationCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(GroupId, "Send"))
                return;

            var body = new global::Microsoft.Graph.PowerShell.Teams.Client.Groups.Item.Team.SendActivityNotification.SendActivityNotificationPostRequestBody();
    ...
    if (this.IsParameterBound(nameof(Recipient)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                client.Groups[GroupId].Team.SendActivityNotification.PostAsync(body, requestConfiguration =>
                {
