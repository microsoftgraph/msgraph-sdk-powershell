...
{
    [GraphRoute("POST", "/teams/{team-id}/primaryChannel/tabs")]
    [Cmdlet(VerbsCommon.New, "MgTeamPrimaryChannelTab", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.TeamsTab))]
    public class NewMgTeamPrimaryChannelTabCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Configuration)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Teams[TeamId].PrimaryChannel.Tabs.PostAsync(body, requestConfiguration =>
                {
