...
{
    [GraphRoute("GET", "/teams/{team-id}/primaryChannel/sharedWithTeams")]
    [Cmdlet(VerbsCommon.Get, "MgTeamPrimaryChannelSharedWithTeam_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.SharedWithChannelTeamInfo))]
    public class GetMgTeamPrimaryChannelSharedWithTeam_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Teams.Client.Models.SharedWithChannelTeamInfoCollectionResponse? result;
            try
            {
                result = client.Teams[TeamId].PrimaryChannel.SharedWithTeams.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Teams[TeamId].PrimaryChannel.SharedWithTeams.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
