...
{
    [GraphRoute("GET", "/teams/{team-id}/primaryChannel/messages/{chatMessage-id}/replies")]
    [Cmdlet(VerbsCommon.Get, "MgTeamPrimaryChannelMessageReply_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.ChatMessage))]
    public class GetMgTeamPrimaryChannelMessageReply_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Teams[TeamId].PrimaryChannel.Messages[ChatMessageId].Replies.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Teams[TeamId].PrimaryChannel.Messages[ChatMessageId].Replies.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
