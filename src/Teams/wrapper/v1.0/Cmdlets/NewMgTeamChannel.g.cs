...
{
    [GraphRoute("POST", "/teams/{team-id}/channels")]
    [Cmdlet(VerbsCommon.New, "MgTeamChannel", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.Channel))]
    public class NewMgTeamChannelCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Summary)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Teams[TeamId].Channels.PostAsync(body, requestConfiguration =>
                {
