...
{
    [GraphRoute("PATCH", "/chats/{chat-id}/permissionGrants/{resourceSpecificPermissionGrant-id}")]
    [Cmdlet(VerbsData.Update, "MgChatPermissionGrant", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.ResourceSpecificPermissionGrant))]
    public class UpdateMgChatPermissionGrantCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(ResourceAppId)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Chats[ChatId].PermissionGrants[ResourceSpecificPermissionGrantId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Chats[ChatId].PermissionGrants[ResourceSpecificPermissionGrantId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException)
                {
                    ThrowGraphRequestFailed(ex, ResourceSpecificPermissionGrantId);
