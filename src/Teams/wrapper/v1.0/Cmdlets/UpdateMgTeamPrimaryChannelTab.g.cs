...
{
    [GraphRoute("PATCH", "/teams/{team-id}/primaryChannel/tabs/{teamsTab-id}")]
    [Cmdlet(VerbsData.Update, "MgTeamPrimaryChannelTab", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.TeamsTab))]
    public class UpdateMgTeamPrimaryChannelTabCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Configuration)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Teams[TeamId].PrimaryChannel.Tabs[TeamsTabId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Teams[TeamId].PrimaryChannel.Tabs[TeamsTabId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, TeamsTabId);
