...
{
    [GraphRoute("POST", "/teams/{team-id}/channels/{channel-id}/allMembers")]
    [Cmdlet(VerbsCommon.New, "MgTeamChannelMember", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.ConversationMember))]
    public class NewMgTeamChannelMemberCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(VisibleHistoryStartDateTime)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Teams[TeamId].Channels[ChannelId].AllMembers.PostAsync(body, requestConfiguration =>
                {
