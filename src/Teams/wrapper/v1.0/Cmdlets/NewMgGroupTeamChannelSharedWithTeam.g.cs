...
{
    [GraphRoute("POST", "/groups/{group-id}/team/channels/{channel-id}/sharedWithTeams")]
    [Cmdlet(VerbsCommon.New, "MgGroupTeamChannelSharedWithTeam", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.SharedWithChannelTeamInfo))]
    public class NewMgGroupTeamChannelSharedWithTeamCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(IsHostTeam)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Team.Channels[ChannelId].SharedWithTeams.PostAsync(body, requestConfiguration =>
                {
