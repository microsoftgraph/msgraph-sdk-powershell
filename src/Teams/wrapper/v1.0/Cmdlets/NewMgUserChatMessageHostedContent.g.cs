...
{
    [GraphRoute("POST", "/users/{user-id}/chats/{chat-id}/messages/{chatMessage-id}/hostedContents")]
    [Cmdlet(VerbsCommon.New, "MgUserChatMessageHostedContent", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.ChatMessageHostedContent))]
    public class NewMgUserChatMessageHostedContentCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(ContentType)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].Chats[ChatId].Messages[ChatMessageId].HostedContents.PostAsync(body, requestConfiguration =>
                {
