...
{
    [GraphRoute("GET", "/teams/{team-id}/schedule/timeOffReasons")]
    [Cmdlet(VerbsCommon.Get, "MgTeamScheduleTimeOffReason_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.TimeOffReason))]
    public class GetMgTeamScheduleTimeOffReason_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Teams[TeamId].Schedule.TimeOffReasons.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Teams[TeamId].Schedule.TimeOffReasons.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
