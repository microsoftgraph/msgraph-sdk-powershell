...
{
    [GraphRoute("POST", "/groups/{group-id}/team/permissionGrants")]
    [Cmdlet(VerbsCommon.New, "MgGroupTeamPermissionGrant", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.ResourceSpecificPermissionGrant))]
    public class NewMgGroupTeamPermissionGrantCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(ResourceAppId)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Team.PermissionGrants.PostAsync(body, requestConfiguration =>
                {
