...
{
    [GraphRoute("PATCH", "/chats/{chat-id}/tabs/{teamsTab-id}")]
    [Cmdlet(VerbsData.Update, "MgChatTab", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.TeamsTab))]
    public class UpdateMgChatTabCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Configuration)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Chats[ChatId].Tabs[TeamsTabId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Chats[ChatId].Tabs[TeamsTabId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex)
                {
                    ThrowGraphRequestFailed(ex, TeamsTabId);
