...
{
    [GraphRoute("POST", "/chats/{chat-id}/messages/{chatMessage-id}/hostedContents")]
    [Cmdlet(VerbsCommon.New, "MgChatMessageHostedContent", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.ChatMessageHostedContent))]
    public class NewMgChatMessageHostedContentCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(ContentType)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Chats[ChatId].Messages[ChatMessageId].HostedContents.PostAsync(body, requestConfiguration =>
                {
