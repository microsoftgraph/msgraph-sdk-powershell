...
{
    [GraphRoute("POST", "/groups/{group-id}/team/schedule/timeOffRequests")]
    [Cmdlet(VerbsCommon.New, "MgGroupTeamScheduleTimeOffRequest", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.TimeOffRequest))]
    public class NewMgGroupTeamScheduleTimeOffRequestCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(State)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Team.Schedule.TimeOffRequests.PostAsync(body, requestConfiguration =>
                {
