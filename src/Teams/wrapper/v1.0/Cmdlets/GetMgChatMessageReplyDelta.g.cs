...
{
    [GraphRoute("GET", "/chats/{chat-id}/messages/{chatMessage-id}/replies/delta()")]
    [Cmdlet(VerbsCommon.Get, "MgChatMessageReplyDelta", DefaultParameterSetName = "DeltaSync")]
    [OutputType(typeof(global::Microsoft.Graph.PowerShell.Teams.Client.Chats.Item.Messages.Item.Replies.Delta.DeltaGetResponse))]
    public class GetMgChatMessageReplyDeltaCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            global::Microsoft.Graph.PowerShell.Teams.Client.Chats.Item.Messages.Item.Replies.Delta.DeltaGetResponse? result;
            try
            {
                result = ParameterSetName == "Resume"
                    ? client.Chats[ChatId].Messages[ChatMessageId].Replies.Delta.WithUrl(ValidateContinuationUrl(DeltaLink!, requestAdapter, nameof(DeltaLink))).GetAsDeltaGetResponseAsync(requestConfiguration =>
                        {

                AddRequestHeaders(requestConfiguration.Headers);
                        }).GetAwaiter().GetResult()
                    : client.Chats[ChatId].Messages[ChatMessageId].Replies.Delta.GetAsDeltaGetResponseAsync(requestConfiguration =>
                {
                ...
                while (true)
                {
                    ...
                    result = client.Chats[ChatId].Messages[ChatMessageId].Replies.Delta.WithUrl(nextLink).GetAsDeltaGetResponseAsync(requestConfiguration =>
                    {
