...
{
    [GraphRoute("GET", "/chats/{chat-id}/messages/{chatMessage-id}/replies/{chatMessage-id1}/hostedContents")]
    [Cmdlet(VerbsCommon.Get, "MgChatMessageReplyHostedContent_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.ChatMessageHostedContent))]
    public class GetMgChatMessageReplyHostedContent_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Chats[ChatId].Messages[ChatMessageId].Replies[ChatMessageId1].HostedContents.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Chats[ChatId].Messages[ChatMessageId].Replies[ChatMessageId1].HostedContents.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
