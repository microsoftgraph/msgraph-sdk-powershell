...
{
    [GraphRoute("PATCH", "/groups/{group-id}/team/schedule/dayNotes/{dayNote-id}")]
    [Cmdlet(VerbsData.Update, "MgGroupTeamScheduleDayNote", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.DayNote))]
    public class UpdateMgGroupTeamScheduleDayNoteCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(SharedDayNote)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Team.Schedule.DayNotes[DayNoteId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Groups[GroupId].Team.Schedule.DayNotes[DayNoteId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, DayNoteId);
