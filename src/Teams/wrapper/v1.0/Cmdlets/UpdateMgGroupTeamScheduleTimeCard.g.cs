...
{
    [GraphRoute("PATCH", "/groups/{group-id}/team/schedule/timeCards/{timeCard-id}")]
    [Cmdlet(VerbsData.Update, "MgGroupTeamScheduleTimeCard", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.TimeCard))]
    public class UpdateMgGroupTeamScheduleTimeCardCommand : PSCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(State)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Team.Schedule.TimeCards[TimeCardId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Groups[GroupId].Team.Schedule.TimeCards[TimeCardId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex)
                {
                    ThrowTerminatingError(new ErrorRecord(ex, "GraphRequestFailed", ErrorCategory.InvalidOperation, TimeCardId));
