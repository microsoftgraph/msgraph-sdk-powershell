...
{
    [GraphRoute("POST", "/groups/{group-id}/team/channels/{channel-id}/messages/{chatMessage-id}/replies")]
    [Cmdlet(VerbsCommon.New, "MgGroupTeamChannelMessageReply", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.ChatMessage))]
    public class NewMgGroupTeamChannelMessageReplyCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Reactions)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Team.Channels[ChannelId].Messages[ChatMessageId].Replies.PostAsync(body, requestConfiguration =>
                {
