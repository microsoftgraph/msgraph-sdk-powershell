...
{
    [GraphRoute("POST", "/users/{user-id}/chats/{chat-id}/targetedMessages/{targetedChatMessage-id}/replies")]
    [Cmdlet(VerbsCommon.New, "MgUserChatTargetedMessageReply", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.ChatMessage))]
    public class NewMgUserChatTargetedMessageReplyCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Reactions)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].Chats[ChatId].TargetedMessages[TargetedChatMessageId].Replies.PostAsync(body, requestConfiguration =>
                {
