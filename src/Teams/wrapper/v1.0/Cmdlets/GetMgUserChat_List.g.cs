...
{
    [GraphRoute("GET", "/users/{user-id}/chats")]
    [Cmdlet(VerbsCommon.Get, "MgUserChat_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.Chat))]
    public class GetMgUserChat_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Teams.Client.Models.ChatCollectionResponse? result;
            try
            {
                result = client.Users[UserId].Chats.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Users[UserId].Chats.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
