...
{
    [GraphRoute("POST", "/chats/{chat-id}/tabs")]
    [Cmdlet(VerbsCommon.New, "MgChatTab", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.TeamsTab))]
    public class NewMgChatTabCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Configuration)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Chats[ChatId].Tabs.PostAsync(body, requestConfiguration =>
                {
