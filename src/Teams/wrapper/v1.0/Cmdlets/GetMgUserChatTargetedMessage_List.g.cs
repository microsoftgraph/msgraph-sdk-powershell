...
{
    [GraphRoute("GET", "/users/{user-id}/chats/{chat-id}/targetedMessages")]
    [Cmdlet(VerbsCommon.Get, "MgUserChatTargetedMessage_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.TargetedChatMessage))]
    public class GetMgUserChatTargetedMessage_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Teams.Client.Models.TargetedChatMessageCollectionResponse? result;
            try
            {
                result = client.Users[UserId].Chats[ChatId].TargetedMessages.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Users[UserId].Chats[ChatId].TargetedMessages.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
