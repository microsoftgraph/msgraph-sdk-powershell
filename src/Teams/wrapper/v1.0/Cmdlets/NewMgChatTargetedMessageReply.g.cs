...
{
    [GraphRoute("POST", "/chats/{chat-id}/targetedMessages/{targetedChatMessage-id}/replies")]
    [Cmdlet(VerbsCommon.New, "MgChatTargetedMessageReply", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.ChatMessage))]
    public class NewMgChatTargetedMessageReplyCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(TargetedChatMessageId, "New"))
                return;
    ...
    if (this.IsParameterBound(nameof(Reactions)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Chats[ChatId].TargetedMessages[TargetedChatMessageId].Replies.PostAsync(body, requestConfiguration =>
                {
