...
{
    [GraphRoute("PATCH", "/users/{user-id}/chats/{chat-id}/permissionGrants/{resourceSpecificPermissionGrant-id}")]
    [Cmdlet(VerbsData.Update, "MgUserChatPermissionGrant", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.ResourceSpecificPermissionGrant))]
    public class UpdateMgUserChatPermissionGrantCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(ResourceAppId)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].Chats[ChatId].PermissionGrants[ResourceSpecificPermissionGrantId].PatchAsync(body, requestConfiguration =>
                {
