...
{
    [GraphRoute("PATCH", "/teams/{team-id}/channels/{channel-id}/sharedWithTeams/{sharedWithChannelTeamInfo-id}")]
    [Cmdlet(VerbsData.Update, "MgTeamChannelSharedWithTeam", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.SharedWithChannelTeamInfo))]
    public class UpdateMgTeamChannelSharedWithTeamCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(IsHostTeam)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Teams[TeamId].Channels[ChannelId].SharedWithTeams[SharedWithChannelTeamInfoId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Teams[TeamId].Channels[ChannelId].SharedWithTeams[SharedWithChannelTeamInfoId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex)
                {
                    ThrowGraphRequestFailed(ex, SharedWithChannelTeamInfoId);
