...
{
    [GraphRoute("GET", "/teams/{team-id}/channels/{channel-id}/messages/{chatMessage-id}/replies/delta()")]
    [Cmdlet(VerbsCommon.Get, "MgTeamChannelMessageReplyDelta", DefaultParameterSetName = "DeltaSync")]
    [OutputType(typeof(global::Microsoft.Graph.PowerShell.Teams.Client.Teams.Item.Channels.Item.Messages.Item.Replies.Delta.DeltaGetResponse))]
    public class GetMgTeamChannelMessageReplyDeltaCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            global::Microsoft.Graph.PowerShell.Teams.Client.Teams.Item.Channels.Item.Messages.Item.Replies.Delta.DeltaGetResponse? result;
            try
            {
                result = ParameterSetName == "Resume"
                    ? client.Teams[TeamId].Channels[ChannelId].Messages[ChatMessageId].Replies.Delta.WithUrl(ValidateContinuationUrl(DeltaLink!, requestAdapter, nameof(DeltaLink))).GetAsDeltaGetResponseAsync(requestConfiguration =>
                        {

                AddRequestHeaders(requestConfiguration.Headers);
                        }).GetAwaiter().GetResult()
                    : client.Teams[TeamId].Channels[ChannelId].Messages[ChatMessageId].Replies.Delta.GetAsDeltaGetResponseAsync(requestConfiguration =>
                {
                ...
                while (true)
                {
                    ...
                    result = client.Teams[TeamId].Channels[ChannelId].Messages[ChatMessageId].Replies.Delta.WithUrl(nextLink).GetAsDeltaGetResponseAsync(requestConfiguration =>
                    {
