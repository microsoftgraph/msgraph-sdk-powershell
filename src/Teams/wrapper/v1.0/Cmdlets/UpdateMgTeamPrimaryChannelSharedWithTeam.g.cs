...
{
    [GraphRoute("PATCH", "/teams/{team-id}/primaryChannel/sharedWithTeams/{sharedWithChannelTeamInfo-id}")]
    [Cmdlet(VerbsData.Update, "MgTeamPrimaryChannelSharedWithTeam", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.SharedWithChannelTeamInfo))]
    public class UpdateMgTeamPrimaryChannelSharedWithTeamCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(IsHostTeam)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Teams[TeamId].PrimaryChannel.SharedWithTeams[SharedWithChannelTeamInfoId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Teams[TeamId].PrimaryChannel.SharedWithTeams[SharedWithChannelTeamInfoId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, SharedWithChannelTeamInfoId);
