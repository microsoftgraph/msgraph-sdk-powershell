...
{
    [GraphRoute("POST", "/teams/{team-id}/channels/{channel-id}/messages")]
    [Cmdlet(VerbsCommon.New, "MgTeamChannelMessage", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.ChatMessage))]
    public class NewMgTeamChannelMessageCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Reactions)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Teams[TeamId].Channels[ChannelId].Messages.PostAsync(body, requestConfiguration =>
                {
