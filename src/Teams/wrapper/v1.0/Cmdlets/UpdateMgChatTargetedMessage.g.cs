...
{
    [GraphRoute("PATCH", "/chats/{chat-id}/targetedMessages/{targetedChatMessage-id}")]
    [Cmdlet(VerbsData.Update, "MgChatTargetedMessage", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.TargetedChatMessage))]
    public class UpdateMgChatTargetedMessageCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Recipient)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Chats[ChatId].TargetedMessages[TargetedChatMessageId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Chats[ChatId].TargetedMessages[TargetedChatMessageId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, TargetedChatMessageId);
