...
{
    [GraphRoute("PUT", "/groups/{group-id}/team")]
    [Cmdlet(VerbsCommon.Set, "MgGroupTeam", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.Team))]
    public class SetMgGroupTeamCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Visibility)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Team.PutAsync(body, requestConfiguration =>
                {
