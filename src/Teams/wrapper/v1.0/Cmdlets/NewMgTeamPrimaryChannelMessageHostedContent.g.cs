...
{
    [GraphRoute("POST", "/teams/{team-id}/primaryChannel/messages/{chatMessage-id}/hostedContents")]
    [Cmdlet(VerbsCommon.New, "MgTeamPrimaryChannelMessageHostedContent", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Teams.Client.Models.ChatMessageHostedContent))]
    public class NewMgTeamPrimaryChannelMessageHostedContentCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(ContentType)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Teams[TeamId].PrimaryChannel.Messages[ChatMessageId].HostedContents.PostAsync(body, requestConfiguration =>
                {
