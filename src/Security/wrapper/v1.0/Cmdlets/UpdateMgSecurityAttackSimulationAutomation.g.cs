...
{
    [GraphRoute("PATCH", "/security/attackSimulation/simulationAutomations/{simulationAutomation-id}")]
    [Cmdlet(VerbsData.Update, "MgSecurityAttackSimulationAutomation", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Security.Client.Models.SimulationAutomation))]
    public class UpdateMgSecurityAttackSimulationAutomationCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(SimulationAutomationId, "Update"))
                return;
    ...
    if (this.IsParameterBound(nameof(Status)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Security.AttackSimulation.SimulationAutomations[SimulationAutomationId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Security.AttackSimulation.SimulationAutomations[SimulationAutomationId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex)
                {
                    ThrowGraphRequestFailed(ex, SimulationAutomationId);
