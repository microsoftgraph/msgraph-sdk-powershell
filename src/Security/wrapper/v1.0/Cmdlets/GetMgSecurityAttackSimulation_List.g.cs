...
{
    [GraphRoute("GET", "/security/attackSimulation/simulations")]
    [Cmdlet(VerbsCommon.Get, "MgSecurityAttackSimulation_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Security.Client.Models.Simulation))]
    public class GetMgSecurityAttackSimulation_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Security.Client.Models.SimulationCollectionResponse? result;
            try
            {
                result = client.Security.AttackSimulation.Simulations.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Security.AttackSimulation.Simulations.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
