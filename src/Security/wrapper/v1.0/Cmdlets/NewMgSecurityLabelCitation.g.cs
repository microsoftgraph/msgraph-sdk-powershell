...
{
    [GraphRoute("POST", "/security/labels/citations")]
    [Cmdlet(VerbsCommon.New, "MgSecurityLabelCitation", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Security.Client.Models.Security.CitationTemplate))]
    public class NewMgSecurityLabelCitationCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(CreatedBy)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Security.Labels.Citations.PostAsync(body, requestConfiguration =>
                {
