...
{
    [GraphRoute("GET", "/security/subjectRightsRequests/{subjectRightsRequest-id}/collaborators")]
    [Cmdlet(VerbsCommon.Get, "MgSecuritySubjectRightsRequestCollaborator_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Security.Client.Models.User))]
    public class GetMgSecuritySubjectRightsRequestCollaborator_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Security.Client.Models.UserCollectionResponse? result;
            try
            {
                result = client.Security.SubjectRightsRequests[SubjectRightsRequestId].Collaborators.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Security.SubjectRightsRequests[SubjectRightsRequestId].Collaborators.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
