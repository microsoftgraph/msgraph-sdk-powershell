...
{
    [GraphRoute("GET", "/security/subjectRightsRequests/{subjectRightsRequest-id}/approvers/{user-id}/serviceProvisioningErrors")]
    [Cmdlet(VerbsCommon.Get, "MgSecuritySubjectRightsRequestApproverServiceProvisioningError")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Security.Client.Models.ServiceProvisioningError))]
    public class GetMgSecuritySubjectRightsRequestApproverServiceProvisioningErrorCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Security.SubjectRightsRequests[SubjectRightsRequestId].Approvers[UserId].ServiceProvisioningErrors.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Security.SubjectRightsRequests[SubjectRightsRequestId].Approvers[UserId].ServiceProvisioningErrors.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
