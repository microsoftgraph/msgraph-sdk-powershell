...
{
    [GraphRoute("GET", "/security/collaboration/analyzedEmails")]
    [Cmdlet(VerbsCommon.Get, "MgSecurityCollaborationAnalyzedEmail_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Security.Client.Models.Security.AnalyzedEmail))]
    public class GetMgSecurityCollaborationAnalyzedEmail_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Security.Client.Models.Security.AnalyzedEmailCollectionResponse? result;
            try
            {
                result = client.Security.Collaboration.AnalyzedEmails.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Security.Collaboration.AnalyzedEmails.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
