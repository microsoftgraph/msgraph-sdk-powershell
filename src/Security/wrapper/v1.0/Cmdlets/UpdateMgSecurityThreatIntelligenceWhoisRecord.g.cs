...
{
    [GraphRoute("PATCH", "/security/threatIntelligence/whoisRecords/{whoisRecord-id}")]
    [Cmdlet(VerbsData.Update, "MgSecurityThreatIntelligenceWhoisRecord", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Security.Client.Models.Security.WhoisRecord))]
    public class UpdateMgSecurityThreatIntelligenceWhoisRecordCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(WhoisRecordId, "Update"))
                return;
    ...
    if (this.IsParameterBound(nameof(Host)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Security.ThreatIntelligence.WhoisRecords[WhoisRecordId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Security.ThreatIntelligence.WhoisRecords[WhoisRecordId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, WhoisRecordId);
