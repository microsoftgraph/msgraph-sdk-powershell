...
{
    [GraphRoute("POST", "/security/labels/retentionLabels")]
    [Cmdlet(VerbsCommon.New, "MgSecurityLabelRetentionLabel", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Security.Client.Models.Security.RetentionLabel))]
    public class NewMgSecurityLabelRetentionLabelCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(RetentionTrigger)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Security.Labels.RetentionLabels.PostAsync(body, requestConfiguration =>
                {
