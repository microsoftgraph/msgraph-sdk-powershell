...
{
    [GraphRoute("GET", "/security/threatIntelligence/whoisRecords/{whoisRecord-id}/history")]
    [Cmdlet(VerbsCommon.Get, "MgSecurityThreatIntelligenceWhoisRecordHistory_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Security.Client.Models.Security.WhoisHistoryRecord))]
    public class GetMgSecurityThreatIntelligenceWhoisRecordHistory_ListCommand : GraphClientCmdlet
    {
        [Parameter(Mandatory = true, Position = 0)]
        public string WhoisRecordId { get; set; } = string.Empty;
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Security.Client.Models.Security.WhoisHistoryRecordCollectionResponse? result;
            try
            {
                result = client.Security.ThreatIntelligence.WhoisRecords[WhoisRecordId].History.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Security.ThreatIntelligence.WhoisRecords[WhoisRecordId].History.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
            ...
            {
                ThrowGraphRequestFailed(ex, WhoisRecordId);
                return;
            }
        }
    }
