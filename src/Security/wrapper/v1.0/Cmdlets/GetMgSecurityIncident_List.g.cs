...
{
    [GraphRoute("GET", "/security/incidents")]
    [Cmdlet(VerbsCommon.Get, "MgSecurityIncident_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Security.Client.Models.Security.Incident))]
    public class GetMgSecurityIncident_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Security.Client.Models.Security.IncidentCollectionResponse? result;
            try
            {
                result = client.Security.Incidents.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Security.Incidents.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
