...
{
    [GraphRoute("POST", "/security/collaboration/analyzedEmails")]
    [Cmdlet(VerbsCommon.New, "MgSecurityCollaborationAnalyzedEmail", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Security.Client.Models.Security.AnalyzedEmail))]
    public class NewMgSecurityCollaborationAnalyzedEmailCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Urls)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Security.Collaboration.AnalyzedEmails.PostAsync(body, requestConfiguration =>
                {
