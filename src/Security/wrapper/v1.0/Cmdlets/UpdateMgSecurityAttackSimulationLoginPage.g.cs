...
{
    [GraphRoute("PATCH", "/security/attackSimulation/loginPages/{loginPage-id}")]
    [Cmdlet(VerbsData.Update, "MgSecurityAttackSimulationLoginPage", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Security.Client.Models.LoginPage))]
    public class UpdateMgSecurityAttackSimulationLoginPageCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(LoginPageId, "Update"))
                return;
    ...
    if (this.IsParameterBound(nameof(Status)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Security.AttackSimulation.LoginPages[LoginPageId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Security.AttackSimulation.LoginPages[LoginPageId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException)
                {
                    ThrowGraphRequestFailed(ex, LoginPageId);
