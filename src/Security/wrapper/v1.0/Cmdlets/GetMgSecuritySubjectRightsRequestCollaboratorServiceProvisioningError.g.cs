...
{
    [GraphRoute("GET", "/security/subjectRightsRequests/{subjectRightsRequest-id}/collaborators/{user-id}/serviceProvisioningErrors")]
    [Cmdlet(VerbsCommon.Get, "MgSecuritySubjectRightsRequestCollaboratorServiceProvisioningError")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Security.Client.Models.ServiceProvisioningError))]
    public class GetMgSecuritySubjectRightsRequestCollaboratorServiceProvisioningErrorCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Security.SubjectRightsRequests[SubjectRightsRequestId].Collaborators[UserId].ServiceProvisioningErrors.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Security.SubjectRightsRequests[SubjectRightsRequestId].Collaborators[UserId].ServiceProvisioningErrors.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
