...
{
    [GraphRoute("PATCH", "/security/threatIntelligence/hosts/{host-id}")]
    [Cmdlet(VerbsData.Update, "MgSecurityThreatIntelligenceHost", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Security.Client.Models.Security.Host))]
    public class UpdateMgSecurityThreatIntelligenceHostCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(LastSeenDateTime)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Security.ThreatIntelligence.Hosts[HostId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Security.ThreatIntelligence.Hosts[HostId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, HostId);
