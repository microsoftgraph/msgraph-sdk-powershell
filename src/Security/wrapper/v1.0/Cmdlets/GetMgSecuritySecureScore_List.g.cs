...
{
    [GraphRoute("GET", "/security/secureScores")]
    [Cmdlet(VerbsCommon.Get, "MgSecuritySecureScore_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Security.Client.Models.SecureScore))]
    public class GetMgSecuritySecureScore_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Security.Client.Models.SecureScoreCollectionResponse? result;
            try
            {
                result = client.Security.SecureScores.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Security.SecureScores.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
