...
{
    [GraphRoute("PATCH", "/security/cases/ediscoveryCases/{ediscoveryCase-id}/reviewSets/{ediscoveryReviewSet-id}/queries/{ediscoveryReviewSetQuery-id}")]
    [Cmdlet(VerbsData.Update, "MgSecurityCaseEdiscoveryCaseReviewSetQuery", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Security.Client.Models.Security.EdiscoveryReviewSetQuery))]
    public class UpdateMgSecurityCaseEdiscoveryCaseReviewSetQueryCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(EdiscoveryReviewSetQueryId, "Update"))
                return;
    ...
    if (this.IsParameterBound(nameof(LastModifiedBy)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Security.Cases.EdiscoveryCases[EdiscoveryCaseId].ReviewSets[EdiscoveryReviewSetId].Queries[EdiscoveryReviewSetQueryId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Security.Cases.EdiscoveryCases[EdiscoveryCaseId].ReviewSets[EdiscoveryReviewSetId].Queries[EdiscoveryReviewSetQueryId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex)
                {
                    ThrowGraphRequestFailed(ex, EdiscoveryReviewSetQueryId);
