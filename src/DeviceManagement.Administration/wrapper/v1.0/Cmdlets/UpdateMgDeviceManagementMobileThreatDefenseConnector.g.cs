...
{
    [GraphRoute("PATCH", "/deviceManagement/mobileThreatDefenseConnectors/{mobileThreatDefenseConnector-id}")]
    [Cmdlet(VerbsData.Update, "MgDeviceManagementMobileThreatDefenseConnector", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.DeviceManagement.Administration.Client.Models.MobileThreatDefenseConnector))]
    public class UpdateMgDeviceManagementMobileThreatDefenseConnectorCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(MobileThreatDefenseConnectorId, "Update"))
                return;
    ...
    if (this.IsParameterBound(nameof(PartnerState)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.DeviceManagement.MobileThreatDefenseConnectors[MobileThreatDefenseConnectorId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.DeviceManagement.MobileThreatDefenseConnectors[MobileThreatDefenseConnectorId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex)
                {
                    ThrowGraphRequestFailed(ex, MobileThreatDefenseConnectorId);
