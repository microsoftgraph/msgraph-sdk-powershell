...
{
    [GraphRoute("PATCH", "/deviceManagement/termsAndConditions/{termsAndConditions-id}")]
    [Cmdlet(VerbsData.Update, "MgDeviceManagementTermAndCondition", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.DeviceManagement.Administration.Client.Models.TermsAndConditions))]
    public class UpdateMgDeviceManagementTermAndConditionCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(TermsAndConditionsId, "Update"))
                return;
    ...
    if (this.IsParameterBound(nameof(Version)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.DeviceManagement.TermsAndConditions[TermsAndConditionsId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.DeviceManagement.TermsAndConditions[TermsAndConditionsId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, TermsAndConditionsId);
