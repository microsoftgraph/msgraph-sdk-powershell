...
{
    [GraphRoute("PATCH", "/deviceManagement/virtualEndpoint/provisioningPolicies/{cloudPcProvisioningPolicy-id}")]
    [Cmdlet(VerbsData.Update, "MgDeviceManagementVirtualEndpointProvisioningPolicy", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.DeviceManagement.Administration.Client.Models.CloudPcProvisioningPolicy))]
    public class UpdateMgDeviceManagementVirtualEndpointProvisioningPolicyCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(CloudPcProvisioningPolicyId, "Update"))
                return;
    ...
    if (this.IsParameterBound(nameof(WindowsSetting)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.DeviceManagement.VirtualEndpoint.ProvisioningPolicies[CloudPcProvisioningPolicyId].PatchAsync(body, requestConfiguration =>
                {
