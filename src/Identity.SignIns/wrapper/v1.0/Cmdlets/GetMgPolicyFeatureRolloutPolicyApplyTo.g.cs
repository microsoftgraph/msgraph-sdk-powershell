...
{
    [GraphRoute("GET", "/policies/featureRolloutPolicies/{featureRolloutPolicy-id}/appliesTo")]
    [Cmdlet(VerbsCommon.Get, "MgPolicyFeatureRolloutPolicyApplyTo")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.SignIns.Client.Models.DirectoryObject))]
    public class GetMgPolicyFeatureRolloutPolicyApplyToCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Policies.FeatureRolloutPolicies[FeatureRolloutPolicyId].AppliesTo.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Policies.FeatureRolloutPolicies[FeatureRolloutPolicyId].AppliesTo.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
