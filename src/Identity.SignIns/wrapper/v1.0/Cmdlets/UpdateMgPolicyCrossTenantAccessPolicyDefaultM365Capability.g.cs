...
{
    [GraphRoute("PATCH", "/policies/crossTenantAccessPolicy/default/m365Capabilities/{m365CapabilityBase-name}")]
    [Cmdlet(VerbsData.Update, "MgPolicyCrossTenantAccessPolicyDefaultM365Capability", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.SignIns.Client.Models.M365CapabilityBase))]
    public class UpdateMgPolicyCrossTenantAccessPolicyDefaultM365CapabilityCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(InboundAccess)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Policies.CrossTenantAccessPolicy.Default.M365Capabilities[M365CapabilityBaseName].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Policies.CrossTenantAccessPolicy.Default.M365Capabilities[M365CapabilityBaseName].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, M365CapabilityBaseName);
