...
{
    [GraphRoute("POST", "/identityProtection/servicePrincipalRiskDetections")]
    [Cmdlet(VerbsCommon.New, "MgServicePrincipalRiskDetection", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.SignIns.Client.Models.ServicePrincipalRiskDetection))]
    public class NewMgServicePrincipalRiskDetectionCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(TokenIssuerType)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.IdentityProtection.ServicePrincipalRiskDetections.PostAsync(body, requestConfiguration =>
                {
