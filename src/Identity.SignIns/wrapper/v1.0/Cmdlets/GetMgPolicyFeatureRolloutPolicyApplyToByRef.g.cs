...
{
    [GraphRoute("GET", "/policies/featureRolloutPolicies/{featureRolloutPolicy-id}/appliesTo/$ref")]
    [Cmdlet(VerbsCommon.Get, "MgPolicyFeatureRolloutPolicyApplyToByRef")]
    [OutputType(typeof(string))]
    public class GetMgPolicyFeatureRolloutPolicyApplyToByRefCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Policies.FeatureRolloutPolicies[FeatureRolloutPolicyId].AppliesTo.Ref.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Policies.FeatureRolloutPolicies[FeatureRolloutPolicyId].AppliesTo.Ref.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
