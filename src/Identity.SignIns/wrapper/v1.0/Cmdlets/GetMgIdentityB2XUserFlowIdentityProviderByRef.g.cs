...
{
    [GraphRoute("GET", "/identity/b2xUserFlows/{b2xIdentityUserFlow-id}/userFlowIdentityProviders/$ref")]
    [Cmdlet(VerbsCommon.Get, "MgIdentityB2XUserFlowIdentityProviderByRef")]
    [OutputType(typeof(string))]
    public class GetMgIdentityB2XUserFlowIdentityProviderByRefCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Identity.B2xUserFlows[B2xIdentityUserFlowId].UserFlowIdentityProviders.Ref.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Identity.B2xUserFlows[B2xIdentityUserFlowId].UserFlowIdentityProviders.Ref.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
