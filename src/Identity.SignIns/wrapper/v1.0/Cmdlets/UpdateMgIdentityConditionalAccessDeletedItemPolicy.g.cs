...
{
    [GraphRoute("PATCH", "/identity/conditionalAccess/deletedItems/policies/{conditionalAccessPolicy-id}")]
    [Cmdlet(VerbsData.Update, "MgIdentityConditionalAccessDeletedItemPolicy", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.SignIns.Client.Models.ConditionalAccessPolicy))]
    public class UpdateMgIdentityConditionalAccessDeletedItemPolicyCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(State)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Identity.ConditionalAccess.DeletedItems.Policies[ConditionalAccessPolicyId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Identity.ConditionalAccess.DeletedItems.Policies[ConditionalAccessPolicyId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException)
                {
                    ThrowGraphRequestFailed(ex, ConditionalAccessPolicyId);
