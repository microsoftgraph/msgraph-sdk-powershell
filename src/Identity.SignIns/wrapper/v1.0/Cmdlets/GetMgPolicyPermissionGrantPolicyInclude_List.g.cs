...
{
    [GraphRoute("GET", "/policies/permissionGrantPolicies/{permissionGrantPolicy-id}/includes")]
    [Cmdlet(VerbsCommon.Get, "MgPolicyPermissionGrantPolicyInclude_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.SignIns.Client.Models.PermissionGrantConditionSet))]
    public class GetMgPolicyPermissionGrantPolicyInclude_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Identity.SignIns.Client.Models.PermissionGrantConditionSetCollectionResponse? result;
            try
            {
                result = client.Policies.PermissionGrantPolicies[PermissionGrantPolicyId].Includes.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Policies.PermissionGrantPolicies[PermissionGrantPolicyId].Includes.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
