...
{
    [GraphRoute("PATCH", "/identity/b2xUserFlows/{b2xIdentityUserFlow-id}/apiConnectorConfiguration/postFederationSignup")]
    [Cmdlet(VerbsData.Update, "MgIdentityB2XUserFlowPostFederationSignup", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.SignIns.Client.Models.IdentityApiConnector))]
    public class UpdateMgIdentityB2XUserFlowPostFederationSignupCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(AuthenticationConfiguration)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Identity.B2xUserFlows[B2xIdentityUserFlowId].ApiConnectorConfiguration.PostFederationSignup.PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Identity.B2xUserFlows[B2xIdentityUserFlowId].ApiConnectorConfiguration.PostFederationSignup.GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex)
                {
                    ThrowGraphRequestFailed(ex, B2xIdentityUserFlowId);
