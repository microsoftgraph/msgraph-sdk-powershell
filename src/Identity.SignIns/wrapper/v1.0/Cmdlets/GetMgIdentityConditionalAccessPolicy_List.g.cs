...
{
    [GraphRoute("GET", "/identity/conditionalAccess/policies")]
    [Cmdlet(VerbsCommon.Get, "MgIdentityConditionalAccessPolicy_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.SignIns.Client.Models.ConditionalAccessPolicy))]
    public class GetMgIdentityConditionalAccessPolicy_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Identity.ConditionalAccess.Policies.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Identity.ConditionalAccess.Policies.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
