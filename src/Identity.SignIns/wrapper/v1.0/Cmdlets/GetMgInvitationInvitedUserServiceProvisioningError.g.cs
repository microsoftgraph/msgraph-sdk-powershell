...
{
    [GraphRoute("GET", "/invitations/invitedUser/serviceProvisioningErrors")]
    [Cmdlet(VerbsCommon.Get, "MgInvitationInvitedUserServiceProvisioningError")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.SignIns.Client.Models.ServiceProvisioningError))]
    public class GetMgInvitationInvitedUserServiceProvisioningErrorCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Identity.SignIns.Client.Models.ServiceProvisioningErrorCollectionResponse? result;
            try
            {
                result = client.Invitations.InvitedUser.ServiceProvisioningErrors.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Invitations.InvitedUser.ServiceProvisioningErrors.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
