...
{
    [GraphRoute("GET", "/policies/authenticationStrengthPolicies/{authenticationStrengthPolicy-id}/combinationConfigurations")]
    [Cmdlet(VerbsCommon.Get, "MgPolicyAuthenticationStrengthPolicyCombinationConfiguration_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.SignIns.Client.Models.AuthenticationCombinationConfiguration))]
    public class GetMgPolicyAuthenticationStrengthPolicyCombinationConfiguration_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Identity.SignIns.Client.Models.AuthenticationCombinationConfigurationCollectionResponse? result;
            try
            {
                result = client.Policies.AuthenticationStrengthPolicies[AuthenticationStrengthPolicyId].CombinationConfigurations.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Policies.AuthenticationStrengthPolicies[AuthenticationStrengthPolicyId].CombinationConfigurations.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
