...
{
    [GraphRoute("GET", "/oauth2PermissionGrants")]
    [Cmdlet(VerbsCommon.Get, "MgOauth2PermissionGrant_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.SignIns.Client.Models.OAuth2PermissionGrant))]
    public class GetMgOauth2PermissionGrant_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Identity.SignIns.Client.Models.OAuth2PermissionGrantCollectionResponse? result;
            try
            {
                result = client.Oauth2PermissionGrants.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Oauth2PermissionGrants.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
