...
{
    [GraphRoute("GET", "/identityProtection/riskyServicePrincipals/{riskyServicePrincipal-id}/history")]
    [Cmdlet(VerbsCommon.Get, "MgRiskyServicePrincipalHistory_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.SignIns.Client.Models.RiskyServicePrincipalHistoryItem))]
    public class GetMgRiskyServicePrincipalHistory_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Identity.SignIns.Client.Models.RiskyServicePrincipalHistoryItemCollectionResponse? result;
            try
            {
                result = client.IdentityProtection.RiskyServicePrincipals[RiskyServicePrincipalId].History.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.IdentityProtection.RiskyServicePrincipals[RiskyServicePrincipalId].History.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
