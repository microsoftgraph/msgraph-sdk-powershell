...
{
    [GraphRoute("POST", "/policies/tokenLifetimePolicies")]
    [Cmdlet(VerbsCommon.New, "MgPolicyTokenLifetimePolicy", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.SignIns.Client.Models.TokenLifetimePolicy))]
    public class NewMgPolicyTokenLifetimePolicyCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(IsOrganizationDefault)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Policies.TokenLifetimePolicies.PostAsync(body, requestConfiguration =>
                {
