...
{
    [GraphRoute("POST", "/policies/authenticationStrengthPolicies")]
    [Cmdlet(VerbsCommon.New, "MgPolicyAuthenticationStrengthPolicy", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.SignIns.Client.Models.AuthenticationStrengthPolicy))]
    public class NewMgPolicyAuthenticationStrengthPolicyCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(RequirementsSatisfied)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Policies.AuthenticationStrengthPolicies.PostAsync(body, requestConfiguration =>
                {
