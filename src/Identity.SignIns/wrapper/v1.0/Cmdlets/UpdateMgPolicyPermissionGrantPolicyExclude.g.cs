...
{
    [GraphRoute("PATCH", "/policies/permissionGrantPolicies/{permissionGrantPolicy-id}/excludes/{permissionGrantConditionSet-id}")]
    [Cmdlet(VerbsData.Update, "MgPolicyPermissionGrantPolicyExclude", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.SignIns.Client.Models.PermissionGrantConditionSet))]
    public class UpdateMgPolicyPermissionGrantPolicyExcludeCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(PermissionType)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Policies.PermissionGrantPolicies[PermissionGrantPolicyId].Excludes[PermissionGrantConditionSetId].PatchAsync(body, requestConfiguration =>
                {
