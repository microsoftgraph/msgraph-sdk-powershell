...
{
    [GraphRoute("GET", "/dataPolicyOperations")]
    [Cmdlet(VerbsCommon.Get, "MgDataPolicyOperation_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.SignIns.Client.Models.DataPolicyOperation))]
    public class GetMgDataPolicyOperation_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Identity.SignIns.Client.Models.DataPolicyOperationCollectionResponse? result;
            try
            {
                result = client.DataPolicyOperations.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.DataPolicyOperations.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
