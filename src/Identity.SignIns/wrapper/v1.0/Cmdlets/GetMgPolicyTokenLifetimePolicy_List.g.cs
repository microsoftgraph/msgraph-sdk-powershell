...
{
    [GraphRoute("GET", "/policies/tokenLifetimePolicies")]
    [Cmdlet(VerbsCommon.Get, "MgPolicyTokenLifetimePolicy_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.SignIns.Client.Models.TokenLifetimePolicy))]
    public class GetMgPolicyTokenLifetimePolicy_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Identity.SignIns.Client.Models.TokenLifetimePolicyCollectionResponse? result;
            try
            {
                result = client.Policies.TokenLifetimePolicies.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Policies.TokenLifetimePolicies.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
