...
{
    [GraphRoute("POST", "/identity/conditionalAccess/namedLocations")]
    [Cmdlet(VerbsCommon.New, "MgIdentityConditionalAccessNamedLocation", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.SignIns.Client.Models.NamedLocation))]
    public class NewMgIdentityConditionalAccessNamedLocationCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(ModifiedDateTime)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Identity.ConditionalAccess.NamedLocations.PostAsync(body, requestConfiguration =>
                {
