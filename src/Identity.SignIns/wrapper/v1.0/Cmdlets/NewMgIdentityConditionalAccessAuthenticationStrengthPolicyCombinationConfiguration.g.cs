...
{
    [GraphRoute("POST", "/identity/conditionalAccess/authenticationStrength/policies/{authenticationStrengthPolicy-id}/combinationConfigurations")]
    [Cmdlet(VerbsCommon.New, "MgIdentityConditionalAccessAuthenticationStrengthPolicyCombinationConfiguration", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.SignIns.Client.Models.AuthenticationCombinationConfiguration))]
    public class NewMgIdentityConditionalAccessAuthenticationStrengthPolicyCombinationConfigurationCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(AppliesToCombinations)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Identity.ConditionalAccess.AuthenticationStrength.Policies[AuthenticationStrengthPolicyId].CombinationConfigurations.PostAsync(body, requestConfiguration =>
                {
