...
{
    [GraphRoute("GET", "/invitations")]
    [Cmdlet(VerbsCommon.Get, "MgInvitation")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.SignIns.Client.Models.Invitation))]
    public class GetMgInvitationCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Identity.SignIns.Client.Models.InvitationCollectionResponse? result;
            try
            {
                result = client.Invitations.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Invitations.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
