...
{
    [GraphRoute("PATCH", "/policies/featureRolloutPolicies/{featureRolloutPolicy-id}")]
    [Cmdlet(VerbsData.Update, "MgPolicyFeatureRolloutPolicy", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.SignIns.Client.Models.FeatureRolloutPolicy))]
    public class UpdateMgPolicyFeatureRolloutPolicyCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(FeatureRolloutPolicyId, "Update"))
                return;
    ...
    if (this.IsParameterBound(nameof(Feature)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Policies.FeatureRolloutPolicies[FeatureRolloutPolicyId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Policies.FeatureRolloutPolicies[FeatureRolloutPolicyId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, FeatureRolloutPolicyId);
