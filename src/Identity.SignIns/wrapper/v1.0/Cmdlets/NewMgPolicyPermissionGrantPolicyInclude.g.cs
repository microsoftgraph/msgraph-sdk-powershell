...
{
    [GraphRoute("POST", "/policies/permissionGrantPolicies/{permissionGrantPolicy-id}/includes")]
    [Cmdlet(VerbsCommon.New, "MgPolicyPermissionGrantPolicyInclude", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.SignIns.Client.Models.PermissionGrantConditionSet))]
    public class NewMgPolicyPermissionGrantPolicyIncludeCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(PermissionType)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Policies.PermissionGrantPolicies[PermissionGrantPolicyId].Includes.PostAsync(body, requestConfiguration =>
                {
