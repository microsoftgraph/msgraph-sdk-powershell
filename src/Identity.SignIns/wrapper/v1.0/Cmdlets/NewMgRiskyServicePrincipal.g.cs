...
{
    [GraphRoute("POST", "/identityProtection/riskyServicePrincipals")]
    [Cmdlet(VerbsCommon.New, "MgRiskyServicePrincipal", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.SignIns.Client.Models.RiskyServicePrincipal))]
    public class NewMgRiskyServicePrincipalCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(RiskState)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.IdentityProtection.RiskyServicePrincipals.PostAsync(body, requestConfiguration =>
                {
