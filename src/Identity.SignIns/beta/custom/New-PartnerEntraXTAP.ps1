...
function New-PartnerEntraXTAP {
    ...
    param(
        [Parameter(Mandatory = $true, ParameterSetName = 'Expanded')]
        [ValidateScript({
    ...
    process {
        ...
        foreach ($key in @('PartnerTenantId', 'M365CollaborationInbound', 'M365CollaborationInboundTargetUsers', 'M365CollaborationOutbound', 'M365CollaborationOutboundTargetUsers', 'M365CollaborationOutboundTargetGroups', 'AppServiceConnectInbound', 'AppServiceConnectInboundTargetApplications')) { $null = $PSBoundParameters.Remove($key) }
        ...
        # Build M365 outbound (merge users + groups)
        $outboundConfig = $null
        ...
        if ($PSCmdlet.ShouldProcess($PartnerTenantId, 'New-PartnerEntraXTAP')) {
            $splat = @{ TenantId = $PartnerTenantId }
            ...
            $result = New-MgBetaPolicyCrossTenantAccessPolicyPartner @splat @PSBoundParameters
