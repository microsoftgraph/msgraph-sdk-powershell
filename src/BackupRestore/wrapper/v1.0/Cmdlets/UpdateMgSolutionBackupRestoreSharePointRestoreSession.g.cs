...
{
    [GraphRoute("PATCH", "/solutions/backupRestore/sharePointRestoreSessions/{sharePointRestoreSession-id}")]
    [Cmdlet(VerbsData.Update, "MgSolutionBackupRestoreSharePointRestoreSession", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.BackupRestore.Client.Models.SharePointRestoreSession))]
    public class UpdateMgSolutionBackupRestoreSharePointRestoreSessionCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(SharePointRestoreSessionId, "Update"))
                return;
    ...
    if (this.IsParameterBound(nameof(Status)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Solutions.BackupRestore.SharePointRestoreSessions[SharePointRestoreSessionId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Solutions.BackupRestore.SharePointRestoreSessions[SharePointRestoreSessionId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex)
                {
                    ThrowGraphRequestFailed(ex, SharePointRestoreSessionId);
