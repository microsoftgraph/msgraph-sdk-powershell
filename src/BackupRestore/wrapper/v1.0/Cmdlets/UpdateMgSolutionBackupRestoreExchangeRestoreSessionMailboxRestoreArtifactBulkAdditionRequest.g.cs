...
{
    [GraphRoute("PATCH", "/solutions/backupRestore/exchangeRestoreSessions/{exchangeRestoreSession-id}/mailboxRestoreArtifactsBulkAdditionRequests/{mailboxRestoreArtifactsBulkAdditionRequest-id}")]
    [Cmdlet(VerbsData.Update, "MgSolutionBackupRestoreExchangeRestoreSessionMailboxRestoreArtifactBulkAdditionRequest", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.BackupRestore.Client.Models.MailboxRestoreArtifactsBulkAdditionRequest))]
    public class UpdateMgSolutionBackupRestoreExchangeRestoreSessionMailboxRestoreArtifactBulkAdditionRequestCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(MailboxRestoreArtifactsBulkAdditionRequestId, "Update"))
                return;
    ...
    if (this.IsParameterBound(nameof(Tags)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Solutions.BackupRestore.ExchangeRestoreSessions[ExchangeRestoreSessionId].MailboxRestoreArtifactsBulkAdditionRequests[MailboxRestoreArtifactsBulkAdditionRequestId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Solutions.BackupRestore.ExchangeRestoreSessions[ExchangeRestoreSessionId].MailboxRestoreArtifactsBulkAdditionRequests[MailboxRestoreArtifactsBulkAdditionRequestId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex)
                {
                    ThrowGraphRequestFailed(ex, MailboxRestoreArtifactsBulkAdditionRequestId);
