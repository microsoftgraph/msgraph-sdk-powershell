...
{
    [GraphRoute("PATCH", "/solutions/backupRestore/sharePointRestoreSessions/{sharePointRestoreSession-id}/granularSiteRestoreArtifacts/{granularSiteRestoreArtifact-id}")]
    [Cmdlet(VerbsData.Update, "MgSolutionBackupRestoreSharePointRestoreSessionGranularSiteRestoreArtifact", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.BackupRestore.Client.Models.GranularSiteRestoreArtifact))]
    public class UpdateMgSolutionBackupRestoreSharePointRestoreSessionGranularSiteRestoreArtifactCommand : PSCmdlet
    {
        ...
        {
            if (!ShouldProcess(GranularSiteRestoreArtifactId, "Update"))
                return;
    ...
    if (this.IsParameterBound(nameof(Status)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Solutions.BackupRestore.SharePointRestoreSessions[SharePointRestoreSessionId].GranularSiteRestoreArtifacts[GranularSiteRestoreArtifactId].PatchAsync(body, requestConfiguration =>
                {
            ...
            {
                ThrowTerminatingError(new ErrorRecord(ex, "GraphRequestFailed", ErrorCategory.InvalidOperation, GranularSiteRestoreArtifactId));
                return;
            }
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Solutions.BackupRestore.SharePointRestoreSessions[SharePointRestoreSessionId].GranularSiteRestoreArtifacts[GranularSiteRestoreArtifactId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex)
                {
                    ThrowTerminatingError(new ErrorRecord(ex, "GraphRequestFailed", ErrorCategory.InvalidOperation, GranularSiteRestoreArtifactId));
