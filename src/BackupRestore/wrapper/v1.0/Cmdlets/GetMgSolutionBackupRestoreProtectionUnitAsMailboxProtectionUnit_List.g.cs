...
{
    [GraphRoute("GET", "/solutions/backupRestore/protectionUnits/graph.mailboxProtectionUnit")]
    [Cmdlet(VerbsCommon.Get, "MgSolutionBackupRestoreProtectionUnitAsMailboxProtectionUnit_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.BackupRestore.Client.Models.MailboxProtectionUnit))]
    public class GetMgSolutionBackupRestoreProtectionUnitAsMailboxProtectionUnit_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.BackupRestore.Client.Models.MailboxProtectionUnitCollectionResponse? result;
            try
            {
                result = client.Solutions.BackupRestore.ProtectionUnits.GraphMailboxProtectionUnit.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Solutions.BackupRestore.ProtectionUnits.GraphMailboxProtectionUnit.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
