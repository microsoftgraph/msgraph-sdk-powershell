...
{
    [GraphRoute("PATCH", "/solutions/backupRestore/oneDriveForBusinessRestoreSessions/{oneDriveForBusinessRestoreSession-id}/driveRestoreArtifacts/{driveRestoreArtifact-id}")]
    [Cmdlet(VerbsData.Update, "MgSolutionBackupRestoreOneDriveForBusinessRestoreSessionDriveRestoreArtifact", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.BackupRestore.Client.Models.DriveRestoreArtifact))]
    public class UpdateMgSolutionBackupRestoreOneDriveForBusinessRestoreSessionDriveRestoreArtifactCommand : PSCmdlet
    {
        ...
        {
            if (!ShouldProcess(DriveRestoreArtifactId, "Update"))
                return;
    ...
    if (this.IsParameterBound(nameof(Status)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Solutions.BackupRestore.OneDriveForBusinessRestoreSessions[OneDriveForBusinessRestoreSessionId].DriveRestoreArtifacts[DriveRestoreArtifactId].PatchAsync(body, requestConfiguration =>
                {
            ...
            {
                ThrowTerminatingError(new ErrorRecord(ex, "GraphRequestFailed", ErrorCategory.InvalidOperation, DriveRestoreArtifactId));
                return;
            }
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Solutions.BackupRestore.OneDriveForBusinessRestoreSessions[OneDriveForBusinessRestoreSessionId].DriveRestoreArtifacts[DriveRestoreArtifactId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex)
                {
                    ThrowTerminatingError(new ErrorRecord(ex, "GraphRequestFailed", ErrorCategory.InvalidOperation, DriveRestoreArtifactId));
