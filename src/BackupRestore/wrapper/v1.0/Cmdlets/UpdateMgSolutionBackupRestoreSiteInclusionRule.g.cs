...
{
    [GraphRoute("PATCH", "/solutions/backupRestore/siteInclusionRules/{siteProtectionRule-id}")]
    [Cmdlet(VerbsData.Update, "MgSolutionBackupRestoreSiteInclusionRule", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.BackupRestore.Client.Models.SiteProtectionRule))]
    public class UpdateMgSolutionBackupRestoreSiteInclusionRuleCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(SiteProtectionRuleId, "Update"))
                return;

            var body = new Microsoft.Graph.PowerShell.BackupRestore.Client.Models.SiteProtectionRule();
    ...
    if (this.IsParameterBound(nameof(SiteExpression)))
        body.SiteExpression = SiteExpression;
    ...
    if (this.IsParameterBound(nameof(Status)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Solutions.BackupRestore.SiteInclusionRules[SiteProtectionRuleId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Solutions.BackupRestore.SiteInclusionRules[SiteProtectionRuleId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException)
                {
                    ThrowGraphRequestFailed(ex, SiteProtectionRuleId);
