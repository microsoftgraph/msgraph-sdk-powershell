...
{
    [GraphRoute("PATCH", "/solutions/backupRestore/mailboxProtectionUnits/{mailboxProtectionUnit-id}")]
    [Cmdlet(VerbsData.Update, "MgSolutionBackupRestoreMailboxProtectionUnit", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.BackupRestore.Client.Models.MailboxProtectionUnit))]
    public class UpdateMgSolutionBackupRestoreMailboxProtectionUnitCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(MailboxProtectionUnitId, "Update"))
                return;
    ...
    if (this.IsParameterBound(nameof(MailboxType)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Solutions.BackupRestore.MailboxProtectionUnits[MailboxProtectionUnitId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Solutions.BackupRestore.MailboxProtectionUnits[MailboxProtectionUnitId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, MailboxProtectionUnitId);
