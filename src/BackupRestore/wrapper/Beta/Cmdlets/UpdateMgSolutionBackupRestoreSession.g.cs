...
{
    [GraphRoute("PATCH", "/solutions/backupRestore/restoreSessions/{restoreSessionBase-id}")]
    [Cmdlet(VerbsData.Update, "MgSolutionBackupRestoreSession", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.BackupRestore.Client.Models.RestoreSessionBase))]
    public class UpdateMgSolutionBackupRestoreSessionCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(RestoreSessionBaseId, "Update"))
                return;

            var body = new Microsoft.Graph.PowerShell.BackupRestore.Client.Models.RestoreSessionBase();
    ...
    if (this.IsParameterBound(nameof(Status)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Solutions.BackupRestore.RestoreSessions[RestoreSessionBaseId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Solutions.BackupRestore.RestoreSessions[RestoreSessionBaseId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, RestoreSessionBaseId);
