...
{
    [GraphRoute("GET", "/planner/plans/{plannerPlan-id}/tasks")]
    [Cmdlet(VerbsCommon.Get, "MgPlannerPlanTask_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Planner.Client.Models.PlannerTask))]
    public class GetMgPlannerPlanTask_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Planner.Client.Models.PlannerTaskCollectionResponse? result;
            try
            {
                result = client.Planner.Plans[PlannerPlanId].Tasks.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Planner.Plans[PlannerPlanId].Tasks.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
