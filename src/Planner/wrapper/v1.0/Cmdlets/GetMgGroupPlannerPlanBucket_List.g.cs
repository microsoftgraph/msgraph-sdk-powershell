...
{
    [GraphRoute("GET", "/groups/{group-id}/planner/plans/{plannerPlan-id}/buckets")]
    [Cmdlet(VerbsCommon.Get, "MgGroupPlannerPlanBucket_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Planner.Client.Models.PlannerBucket))]
    public class GetMgGroupPlannerPlanBucket_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Planner.Client.Models.PlannerBucketCollectionResponse? result;
            try
            {
                result = client.Groups[GroupId].Planner.Plans[PlannerPlanId].Buckets.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Groups[GroupId].Planner.Plans[PlannerPlanId].Buckets.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
