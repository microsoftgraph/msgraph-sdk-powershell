...
{
    [GraphRoute("GET", "/planner/plans/{plannerPlan-id}/tasks/{plannerTask-id}")]
    [Cmdlet(VerbsCommon.Get, "MgPlannerPlanTask_Get")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Planner.Client.Models.PlannerTask))]
    public class GetMgPlannerPlanTask_GetCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Planner.Plans[PlannerPlanId].Tasks[PlannerTaskId].GetAsync(requestConfiguration =>
                {
