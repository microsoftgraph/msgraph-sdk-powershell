...
{
    [GraphRoute("GET", "/planner/goals/{plannerGoal-id}/tasks/{plannerTask-id}")]
    [Cmdlet(VerbsCommon.Get, "MgPlannerGoalTask_Get")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Planner.Client.Models.PlannerTask))]
    public class GetMgPlannerGoalTask_GetCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Planner.Goals[PlannerGoalId].Tasks[PlannerTaskId].GetAsync(requestConfiguration =>
                {
