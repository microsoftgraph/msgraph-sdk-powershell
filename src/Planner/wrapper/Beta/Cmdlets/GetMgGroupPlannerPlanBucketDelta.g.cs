...
{
    [GraphRoute("GET", "/groups/{group-id}/planner/plans/{plannerPlan-id}/buckets/delta()")]
    [Cmdlet(VerbsCommon.Get, "MgGroupPlannerPlanBucketDelta", DefaultParameterSetName = "DeltaSync")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Planner.Client.Models.PlannerBucket))]
    public class GetMgGroupPlannerPlanBucketDeltaCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            global::Microsoft.Graph.PowerShell.Planner.Client.Groups.Item.Planner.Plans.Item.Buckets.Delta.DeltaGetResponse? result;
            try
            {
                result = ParameterSetName == "Resume"
                    ? client.Groups[GroupId].Planner.Plans[PlannerPlanId].Buckets.Delta.WithUrl(ValidateContinuationUrl(DeltaLink!, requestAdapter, nameof(DeltaLink))).GetAsDeltaGetResponseAsync(requestConfiguration =>
                        {

                AddRequestHeaders(requestConfiguration.Headers);
                        }).GetAwaiter().GetResult()
                    : client.Groups[GroupId].Planner.Plans[PlannerPlanId].Buckets.Delta.GetAsDeltaGetResponseAsync(requestConfiguration =>
                {
                ...
                while (true)
                {
                    ...
                    result = client.Groups[GroupId].Planner.Plans[PlannerPlanId].Buckets.Delta.WithUrl(nextLink).GetAsDeltaGetResponseAsync(requestConfiguration =>
                    {
