...
{
    [GraphRoute("GET", "/planner/plans/{plannerPlan-id}/buckets/{plannerBucket-id}/tasks")]
    [Cmdlet(VerbsCommon.Get, "MgPlannerPlanBucketTask_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Planner.Client.Models.PlannerTask))]
    public class GetMgPlannerPlanBucketTask_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Planner.Plans[PlannerPlanId].Buckets[PlannerBucketId].Tasks.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Planner.Plans[PlannerPlanId].Buckets[PlannerBucketId].Tasks.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
