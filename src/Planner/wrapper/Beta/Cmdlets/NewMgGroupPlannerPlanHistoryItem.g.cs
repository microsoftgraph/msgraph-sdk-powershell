...
{
    [GraphRoute("POST", "/groups/{group-id}/planner/plans/{plannerPlan-id}/historyItems")]
    [Cmdlet(VerbsCommon.New, "MgGroupPlannerPlanHistoryItem", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Planner.Client.Models.PlannerHistoryItem))]
    public class NewMgGroupPlannerPlanHistoryItemCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(EventType)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Planner.Plans[PlannerPlanId].HistoryItems.PostAsync(body, requestConfiguration =>
                {
