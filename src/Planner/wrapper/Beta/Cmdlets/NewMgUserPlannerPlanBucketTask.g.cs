...
{
    [GraphRoute("POST", "/users/{user-id}/planner/plans/{plannerPlan-id}/buckets/{plannerBucket-id}/tasks")]
    [Cmdlet(VerbsCommon.New, "MgUserPlannerPlanBucketTask", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Planner.Client.Models.PlannerTask))]
    public class NewMgUserPlannerPlanBucketTaskCommand : GraphClientCmdlet
    {
        ...
        [Parameter(Mandatory = false)]
        public Microsoft.Graph.PowerShell.Planner.Client.Models.PlannerTaskCreation? CreationSource { get; set; }

        [Parameter(Mandatory = false)]
        public Microsoft.Graph.PowerShell.Planner.Client.Models.IdentitySet? LastModifiedBy { get; set; }

        [Parameter(Mandatory = false)]
        public Microsoft.Graph.PowerShell.Planner.Client.Models.PlannerPreviewType? PreviewType { get; set; }

        [Parameter(Mandatory = false)]
        public Microsoft.Graph.PowerShell.Planner.Client.Models.PlannerTaskRecurrence? Recurrence { get; set; }

        [Parameter(Mandatory = false)]
        public Microsoft.Graph.PowerShell.Planner.Client.Models.PlannerTaskCompletionRequirements? SpecifiedCompletionRequirements { get; set; }
        ...
        {
            if (!ShouldProcess(PlannerBucketId, "New"))
                return;
    ...
    if (this.IsParameterBound(nameof(SpecifiedCompletionRequirements)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].Planner.Plans[PlannerPlanId].Buckets[PlannerBucketId].Tasks.PostAsync(body, requestConfiguration =>
                {
