...
{
    [GraphRoute("GET", "/users/{user-id}/planner/plans/{plannerPlan-id}/goals")]
    [Cmdlet(VerbsCommon.Get, "MgUserPlannerPlanGoal_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Planner.Client.Models.PlannerGoal))]
    public class GetMgUserPlannerPlanGoal_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Planner.Client.Models.PlannerGoalCollectionResponse? result;
            try
            {
                result = client.Users[UserId].Planner.Plans[PlannerPlanId].Goals.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Users[UserId].Planner.Plans[PlannerPlanId].Goals.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
