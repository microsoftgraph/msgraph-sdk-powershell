...
{
    [GraphRoute("GET", "/planner/rosters/{plannerRoster-id}/members")]
    [Cmdlet(VerbsCommon.Get, "MgPlannerRosterMember", DefaultParameterSetName = "List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Planner.Client.Models.PlannerRosterMemberCollectionResponse), ParameterSetName = new[] { "List" })]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Planner.Client.Models.PlannerRosterMember), ParameterSetName = new[] { "Get" })]
    public class GetMgPlannerRosterMemberCommand : GraphClientCmdlet
    {
        ...
        // Delegates to Get-MgPlannerRosterMember_Get or Get-MgPlannerRosterMember_List, the two cmdlets
        // that actually call Graph.
        protected override void ProcessRecord()
        {
            var internalCmdletName = ParameterSetName == "Get" ? "Get-MgPlannerRosterMember_Get" : "Get-MgPlannerRosterMember_List";
            try
            {
            ...
            {
                ThrowGraphRequestFailed(ex, ParameterSetName == "Get" ? PlannerRosterMemberId : PlannerRosterId);
                return;
            }
        }
    }
