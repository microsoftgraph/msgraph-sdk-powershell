...
{
    [GraphRoute("PATCH", "/planner/plans/{plannerPlan-id}/tasks/{plannerTask-id}/progressTaskBoardFormat")]
    [Cmdlet(VerbsData.Update, "MgPlannerPlanTaskProgressTaskBoardFormat", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Planner.Client.Models.PlannerProgressTaskBoardTaskFormat))]
    public class UpdateMgPlannerPlanTaskProgressTaskBoardFormatCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(OrderHint)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Planner.Plans[PlannerPlanId].Tasks[PlannerTaskId].ProgressTaskBoardFormat.PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Planner.Plans[PlannerPlanId].Tasks[PlannerTaskId].ProgressTaskBoardFormat.GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, PlannerTaskId);
