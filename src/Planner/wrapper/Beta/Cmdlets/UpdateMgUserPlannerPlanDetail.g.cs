...
{
    [GraphRoute("PATCH", "/users/{user-id}/planner/plans/{plannerPlan-id}/details")]
    [Cmdlet(VerbsData.Update, "MgUserPlannerPlanDetail", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Planner.Client.Models.PlannerPlanDetails))]
    public class UpdateMgUserPlannerPlanDetailCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(SharedWith)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].Planner.Plans[PlannerPlanId].Details.PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Users[UserId].Planner.Plans[PlannerPlanId].Details.GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, PlannerPlanId);
