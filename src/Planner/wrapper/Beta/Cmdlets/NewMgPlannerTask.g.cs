...
{
    [GraphRoute("POST", "/planner/tasks")]
    [Cmdlet(VerbsCommon.New, "MgPlannerTask", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Planner.Client.Models.PlannerTask))]
    public class NewMgPlannerTaskCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(SpecifiedCompletionRequirements)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Planner.Tasks.PostAsync(body, requestConfiguration =>
                {
