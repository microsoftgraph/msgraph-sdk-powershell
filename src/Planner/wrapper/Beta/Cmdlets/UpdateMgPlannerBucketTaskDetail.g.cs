...
{
    [GraphRoute("PATCH", "/planner/buckets/{plannerBucket-id}/tasks/{plannerTask-id}/details")]
    [Cmdlet(VerbsData.Update, "MgPlannerBucketTaskDetail", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Planner.Client.Models.PlannerTaskDetails))]
    public class UpdateMgPlannerBucketTaskDetailCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(References)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Planner.Buckets[PlannerBucketId].Tasks[PlannerTaskId].Details.PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Planner.Buckets[PlannerBucketId].Tasks[PlannerTaskId].Details.GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, PlannerTaskId);
