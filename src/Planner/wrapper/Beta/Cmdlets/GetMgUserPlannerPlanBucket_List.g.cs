...
{
    [GraphRoute("GET", "/users/{user-id}/planner/plans/{plannerPlan-id}/buckets")]
    [Cmdlet(VerbsCommon.Get, "MgUserPlannerPlanBucket_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Planner.Client.Models.PlannerBucket))]
    public class GetMgUserPlannerPlanBucket_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].Planner.Plans[PlannerPlanId].Buckets.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Users[UserId].Planner.Plans[PlannerPlanId].Buckets.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
