...
{
    [GraphRoute("POST", "/groups/{group-id}/planner/plans/{plannerPlan-id}/buckets")]
    [Cmdlet(VerbsCommon.New, "MgGroupPlannerPlanBucket", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Planner.Client.Models.PlannerBucket))]
    public class NewMgGroupPlannerPlanBucketCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(CreationSource)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Planner.Plans[PlannerPlanId].Buckets.PostAsync(body, requestConfiguration =>
                {
