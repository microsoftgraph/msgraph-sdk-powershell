...
namespace Microsoft.Graph.PowerShell.Planner
{
    ...
    [Cmdlet(VerbsCommon.Get, "MgPlannerRoster_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Planner.Client.Models.PlannerRoster))]
    public class GetMgPlannerRoster_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Planner.Client.Models.PlannerRosterCollectionResponse? result;
            try
            {
                result = client.Planner.Rosters.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Planner.Rosters.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
