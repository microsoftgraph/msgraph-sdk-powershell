...
{
    [GraphRoute("PATCH", "/planner/plans/{plannerPlan-id}/buckets/{plannerBucket-id}")]
    [Cmdlet(VerbsData.Update, "MgPlannerPlanBucket", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Planner.Client.Models.PlannerBucket))]
    public class UpdateMgPlannerPlanBucketCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(CreationSource)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Planner.Plans[PlannerPlanId].Buckets[PlannerBucketId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Planner.Plans[PlannerPlanId].Buckets[PlannerBucketId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, PlannerBucketId);
