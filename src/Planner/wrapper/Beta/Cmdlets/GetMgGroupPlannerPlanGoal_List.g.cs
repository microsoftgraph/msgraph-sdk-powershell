...
namespace Microsoft.Graph.PowerShell.Planner
{
    ...
    [Cmdlet(VerbsCommon.Get, "MgGroupPlannerPlanGoal_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Planner.Client.Models.PlannerGoal))]
    public class GetMgGroupPlannerPlanGoal_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Planner.Plans[PlannerPlanId].Goals.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Groups[GroupId].Planner.Plans[PlannerPlanId].Goals.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
