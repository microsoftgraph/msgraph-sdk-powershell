...
{
    [GraphRoute("GET", "/planner/plans/{plannerPlan-id}/goals/{plannerGoal-id}")]
    [Cmdlet(VerbsCommon.Get, "MgPlannerPlanGoal_Get")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Planner.Client.Models.PlannerGoal))]
    public class GetMgPlannerPlanGoal_GetCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Planner.Plans[PlannerPlanId].Goals[PlannerGoalId].GetAsync(requestConfiguration =>
                {
