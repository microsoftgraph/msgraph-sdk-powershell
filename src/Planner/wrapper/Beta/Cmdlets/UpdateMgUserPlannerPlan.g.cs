...
{
    [GraphRoute("PATCH", "/users/{user-id}/planner/plans/{plannerPlan-id}")]
    [Cmdlet(VerbsData.Update, "MgUserPlannerPlan", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Planner.Client.Models.PlannerPlan))]
    public class UpdateMgUserPlannerPlanCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(SharedWithContainers)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].Planner.Plans[PlannerPlanId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Users[UserId].Planner.Plans[PlannerPlanId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, PlannerPlanId);
