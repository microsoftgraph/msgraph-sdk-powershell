...
{
    [GraphRoute("GET", "/planner/plans/{plannerPlan-id}/historyItems")]
    [Cmdlet(VerbsCommon.Get, "MgPlannerPlanHistoryItem_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Planner.Client.Models.PlannerHistoryItem))]
    public class GetMgPlannerPlanHistoryItem_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Planner.Client.Models.PlannerHistoryItemCollectionResponse? result;
            try
            {
                result = client.Planner.Plans[PlannerPlanId].HistoryItems.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Planner.Plans[PlannerPlanId].HistoryItems.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
