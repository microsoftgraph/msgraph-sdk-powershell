...
{
    [GraphRoute("GET", "/groups/{group-id}/planner/plans/{plannerPlan-id}/buckets/{plannerBucket-id}")]
    [Cmdlet(VerbsCommon.Get, "MgGroupPlannerPlanBucket_Get")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Planner.Client.Models.PlannerBucket))]
    public class GetMgGroupPlannerPlanBucket_GetCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Planner.Plans[PlannerPlanId].Buckets[PlannerBucketId].GetAsync(requestConfiguration =>
                {
