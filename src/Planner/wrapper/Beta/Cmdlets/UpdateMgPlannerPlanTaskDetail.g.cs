...
{
    [GraphRoute("PATCH", "/planner/plans/{plannerPlan-id}/tasks/{plannerTask-id}/details")]
    [Cmdlet(VerbsData.Update, "MgPlannerPlanTaskDetail", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Planner.Client.Models.PlannerTaskDetails))]
    public class UpdateMgPlannerPlanTaskDetailCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(References)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Planner.Plans[PlannerPlanId].Tasks[PlannerTaskId].Details.PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Planner.Plans[PlannerPlanId].Tasks[PlannerTaskId].Details.GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, PlannerTaskId);
