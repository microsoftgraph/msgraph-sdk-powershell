...
{
    [GraphRoute("PATCH", "/planner/buckets/{plannerBucket-id}/tasks/{plannerTask-id}/messages/{plannerTaskChatMessage-id}")]
    [Cmdlet(VerbsData.Update, "MgPlannerBucketTaskMessage", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Planner.Client.Models.PlannerTaskChatMessage))]
    public class UpdateMgPlannerBucketTaskMessageCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Reactions)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Planner.Buckets[PlannerBucketId].Tasks[PlannerTaskId].Messages[PlannerTaskChatMessageId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Planner.Buckets[PlannerBucketId].Tasks[PlannerTaskId].Messages[PlannerTaskChatMessageId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, PlannerTaskChatMessageId);
