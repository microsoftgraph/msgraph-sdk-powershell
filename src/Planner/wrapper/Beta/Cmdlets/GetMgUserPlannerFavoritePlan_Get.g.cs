...
{
    [GraphRoute("GET", "/users/{user-id}/planner/favoritePlans/{plannerPlan-id}")]
    [Cmdlet(VerbsCommon.Get, "MgUserPlannerFavoritePlan_Get")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Planner.Client.Models.PlannerPlan))]
    public class GetMgUserPlannerFavoritePlan_GetCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].Planner.FavoritePlans[PlannerPlanId].GetAsync(requestConfiguration =>
                {
