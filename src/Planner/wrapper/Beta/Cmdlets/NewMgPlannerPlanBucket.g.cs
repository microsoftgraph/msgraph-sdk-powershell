...
{
    [GraphRoute("POST", "/planner/plans/{plannerPlan-id}/buckets")]
    [Cmdlet(VerbsCommon.New, "MgPlannerPlanBucket", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Planner.Client.Models.PlannerBucket))]
    public class NewMgPlannerPlanBucketCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(CreationSource)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Planner.Plans[PlannerPlanId].Buckets.PostAsync(body, requestConfiguration =>
                {
