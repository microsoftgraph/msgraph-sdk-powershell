...
{
    [GraphRoute("GET", "/users/{user-id}/planner/favoritePlans")]
    [Cmdlet(VerbsCommon.Get, "MgUserPlannerFavoritePlan_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Planner.Client.Models.PlannerPlan))]
    public class GetMgUserPlannerFavoritePlan_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].Planner.FavoritePlans.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Users[UserId].Planner.FavoritePlans.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
