...
{
    [GraphRoute("POST", "/users/{user-id}/planner/tasks/{plannerTask-id}/messages")]
    [Cmdlet(VerbsCommon.New, "MgUserPlannerTaskMessage", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Planner.Client.Models.PlannerTaskChatMessage))]
    public class NewMgUserPlannerTaskMessageCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Reactions)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].Planner.Tasks[PlannerTaskId].Messages.PostAsync(body, requestConfiguration =>
                {
