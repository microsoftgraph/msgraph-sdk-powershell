...
{
    [GraphRoute("POST", "/groups/{group-id}/planner/plans/{plannerPlan-id}/buckets/{plannerBucket-id}/tasks")]
    [Cmdlet(VerbsCommon.New, "MgGroupPlannerPlanBucketTask", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Planner.Client.Models.PlannerTask))]
    public class NewMgGroupPlannerPlanBucketTaskCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(SpecifiedCompletionRequirements)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Planner.Plans[PlannerPlanId].Buckets[PlannerBucketId].Tasks.PostAsync(body, requestConfiguration =>
                {
