...
namespace Microsoft.Graph.PowerShell.Planner
{
    ...
    [Cmdlet(VerbsCommon.Get, "MgGroupPlannerPlanTaskMessage_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Planner.Client.Models.PlannerTaskChatMessage))]
    public class GetMgGroupPlannerPlanTaskMessage_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Planner.Plans[PlannerPlanId].Tasks[PlannerTaskId].Messages.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Groups[GroupId].Planner.Plans[PlannerPlanId].Tasks[PlannerTaskId].Messages.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
