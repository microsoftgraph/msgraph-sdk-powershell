...
{
    [GraphRoute("PATCH", "/planner/buckets/{plannerBucket-id}/tasks/{plannerTask-id}/bucketTaskBoardFormat")]
    [Cmdlet(VerbsData.Update, "MgPlannerBucketTaskBucketTaskBoardFormat", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Planner.Client.Models.PlannerBucketTaskBoardTaskFormat))]
    public class UpdateMgPlannerBucketTaskBucketTaskBoardFormatCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(OrderHint)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Planner.Buckets[PlannerBucketId].Tasks[PlannerTaskId].BucketTaskBoardFormat.PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Planner.Buckets[PlannerBucketId].Tasks[PlannerTaskId].BucketTaskBoardFormat.GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, PlannerTaskId);
