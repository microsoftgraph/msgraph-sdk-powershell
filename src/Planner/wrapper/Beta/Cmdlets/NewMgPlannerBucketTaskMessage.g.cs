...
{
    [GraphRoute("POST", "/planner/buckets/{plannerBucket-id}/tasks/{plannerTask-id}/messages")]
    [Cmdlet(VerbsCommon.New, "MgPlannerBucketTaskMessage", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Planner.Client.Models.PlannerTaskChatMessage))]
    public class NewMgPlannerBucketTaskMessageCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Reactions)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Planner.Buckets[PlannerBucketId].Tasks[PlannerTaskId].Messages.PostAsync(body, requestConfiguration =>
                {
