...
{
    [GraphRoute("GET", "/planner/rosters/{plannerRoster-id}/plans/{plannerPlan-id}")]
    [Cmdlet(VerbsCommon.Get, "MgPlannerRosterPlan_Get")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Planner.Client.Models.PlannerPlan))]
    public class GetMgPlannerRosterPlan_GetCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Planner.Rosters[PlannerRosterId].Plans[PlannerPlanId].GetAsync(requestConfiguration =>
                {
