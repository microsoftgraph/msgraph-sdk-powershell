...
{
    [GraphRoute("GET", "/users/{user-id}/planner/plans/{plannerPlan-id}/buckets/delta()")]
    [Cmdlet(VerbsCommon.Get, "MgUserPlannerPlanBucketDelta", DefaultParameterSetName = "DeltaSync")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Planner.Client.Models.PlannerBucket))]
    public class GetMgUserPlannerPlanBucketDeltaCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            global::Microsoft.Graph.PowerShell.Planner.Client.Users.Item.Planner.Plans.Item.Buckets.Delta.DeltaGetResponse? result;
            try
            {
                result = ParameterSetName == "Resume"
                    ? client.Users[UserId].Planner.Plans[PlannerPlanId].Buckets.Delta.WithUrl(ValidateContinuationUrl(DeltaLink!, requestAdapter, nameof(DeltaLink))).GetAsDeltaGetResponseAsync(requestConfiguration =>
                        {

                AddRequestHeaders(requestConfiguration.Headers);
                        }).GetAwaiter().GetResult()
                    : client.Users[UserId].Planner.Plans[PlannerPlanId].Buckets.Delta.GetAsDeltaGetResponseAsync(requestConfiguration =>
                {
                ...
                while (true)
                {
                    ...
                    result = client.Users[UserId].Planner.Plans[PlannerPlanId].Buckets.Delta.WithUrl(nextLink).GetAsDeltaGetResponseAsync(requestConfiguration =>
                    {
