...
{
    [GraphRoute("GET", "/users/{user-id}/planner/plans/{plannerPlan-id}/tasks")]
    [Cmdlet(VerbsCommon.Get, "MgUserPlannerPlanTask_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Planner.Client.Models.PlannerTask))]
    public class GetMgUserPlannerPlanTask_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].Planner.Plans[PlannerPlanId].Tasks.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Users[UserId].Planner.Plans[PlannerPlanId].Tasks.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
