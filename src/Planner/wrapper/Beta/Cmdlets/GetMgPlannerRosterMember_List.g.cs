...
namespace Microsoft.Graph.PowerShell.Planner
{
    ...
    [Cmdlet(VerbsCommon.Get, "MgPlannerRosterMember_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Planner.Client.Models.PlannerRosterMember))]
    public class GetMgPlannerRosterMember_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Planner.Client.Models.PlannerRosterMemberCollectionResponse? result;
            try
            {
                result = client.Planner.Rosters[PlannerRosterId].Members.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Planner.Rosters[PlannerRosterId].Members.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
