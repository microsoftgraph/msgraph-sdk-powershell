...
{
    [GraphRoute("GET", "/users/{user-id}/planner/rosterPlans")]
    [Cmdlet(VerbsCommon.Get, "MgUserPlannerRosterPlan_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Planner.Client.Models.PlannerPlan))]
    public class GetMgUserPlannerRosterPlan_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].Planner.RosterPlans.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Users[UserId].Planner.RosterPlans.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
