...
{
    [GraphRoute("GET", "/users/{user-id}/planner/tasks/{plannerTask-id}")]
    [Cmdlet(VerbsCommon.Get, "MgUserPlannerTask_Get")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Planner.Client.Models.PlannerTask))]
    public class GetMgUserPlannerTask_GetCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].Planner.Tasks[PlannerTaskId].GetAsync(requestConfiguration =>
                {
