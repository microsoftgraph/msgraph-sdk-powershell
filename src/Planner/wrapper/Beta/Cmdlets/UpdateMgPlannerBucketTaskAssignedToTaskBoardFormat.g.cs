...
{
    [GraphRoute("PATCH", "/planner/buckets/{plannerBucket-id}/tasks/{plannerTask-id}/assignedToTaskBoardFormat")]
    [Cmdlet(VerbsData.Update, "MgPlannerBucketTaskAssignedToTaskBoardFormat", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Planner.Client.Models.PlannerAssignedToTaskBoardTaskFormat))]
    public class UpdateMgPlannerBucketTaskAssignedToTaskBoardFormatCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(OrderHintsByAssignee)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Planner.Buckets[PlannerBucketId].Tasks[PlannerTaskId].AssignedToTaskBoardFormat.PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Planner.Buckets[PlannerBucketId].Tasks[PlannerTaskId].AssignedToTaskBoardFormat.GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, PlannerTaskId);
