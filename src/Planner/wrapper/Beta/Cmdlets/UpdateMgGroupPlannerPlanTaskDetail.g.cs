...
{
    [GraphRoute("PATCH", "/groups/{group-id}/planner/plans/{plannerPlan-id}/tasks/{plannerTask-id}/details")]
    [Cmdlet(VerbsData.Update, "MgGroupPlannerPlanTaskDetail", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Planner.Client.Models.PlannerTaskDetails))]
    public class UpdateMgGroupPlannerPlanTaskDetailCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(References)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Planner.Plans[PlannerPlanId].Tasks[PlannerTaskId].Details.PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Groups[GroupId].Planner.Plans[PlannerPlanId].Tasks[PlannerTaskId].Details.GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, PlannerTaskId);
