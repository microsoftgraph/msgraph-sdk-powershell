...
{
    [GraphRoute("GET", "/groups/{group-id}/planner/plans/{plannerPlan-id}")]
    [Cmdlet(VerbsCommon.Get, "MgGroupPlannerPlan_Get")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Planner.Client.Models.PlannerPlan))]
    public class GetMgGroupPlannerPlan_GetCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Planner.Plans[PlannerPlanId].GetAsync(requestConfiguration =>
                {
