...
{
    [GraphRoute("PATCH", "/deviceAppManagement/managedEBooks/{managedEBook-id}/userStateSummary/{userInstallStateSummary-id}")]
    [Cmdlet(VerbsData.Update, "MgDeviceAppManagementManagedEBookUserStateSummary", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Devices.CorporateManagement.Client.Models.UserInstallStateSummary))]
    public class UpdateMgDeviceAppManagementManagedEBookUserStateSummaryCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(UserInstallStateSummaryId, "Update"))
                return;
    ...
    if (this.IsParameterBound(nameof(UserName)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.DeviceAppManagement.ManagedEBooks[ManagedEBookId].UserStateSummary[UserInstallStateSummaryId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.DeviceAppManagement.ManagedEBooks[ManagedEBookId].UserStateSummary[UserInstallStateSummaryId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException)
                {
                    ThrowGraphRequestFailed(ex, UserInstallStateSummaryId);
