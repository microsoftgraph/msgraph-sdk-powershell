...
{
    [GraphRoute("PATCH", "/deviceAppManagement/windowsInformationProtectionPolicies/{windowsInformationProtectionPolicy-id}")]
    [Cmdlet(VerbsData.Update, "MgDeviceAppManagementWindowsInformationProtectionPolicy", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    ...
    public class UpdateMgDeviceAppManagementWindowsInformationProtectionPolicyCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(WindowsInformationProtectionPolicyId, "Update"))
                return;
    ...
    if (this.IsParameterBound(nameof(PinUppercaseLetters)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.DeviceAppManagement.WindowsInformationProtectionPolicies[WindowsInformationProtectionPolicyId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.DeviceAppManagement.WindowsInformationProtectionPolicies[WindowsInformationProtectionPolicyId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, WindowsInformationProtectionPolicyId);
