...
{
    [GraphRoute("PATCH", "/users/{user-id}/managedDevices/{managedDevice-id}/logCollectionRequests/{deviceLogCollectionResponse-id}")]
    [Cmdlet(VerbsData.Update, "MgUserManagedDeviceLogCollectionResponse", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Devices.CorporateManagement.Client.Models.DeviceLogCollectionResponse))]
    public class UpdateMgUserManagedDeviceLogCollectionResponseCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Status)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].ManagedDevices[ManagedDeviceId].LogCollectionRequests[DeviceLogCollectionResponseId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Users[UserId].ManagedDevices[ManagedDeviceId].LogCollectionRequests[DeviceLogCollectionResponseId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException)
                {
                    ThrowGraphRequestFailed(ex, DeviceLogCollectionResponseId);
