...
{
    [GraphRoute("PATCH", "/deviceAppManagement/mobileApps/{mobileApp-id}/graph.managedAndroidLobApp/contentVersions/{mobileAppContent-id}/files/{mobileAppContentFile-id}")]
    [Cmdlet(VerbsData.Update, "MgDeviceAppManagementMobileAppAsManagedAndroidLobAppContentVersionFile", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Devices.CorporateManagement.Client.Models.MobileAppContentFile))]
    public class UpdateMgDeviceAppManagementMobileAppAsManagedAndroidLobAppContentVersionFileCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(UploadState)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.DeviceAppManagement.MobileApps[MobileAppId].GraphManagedAndroidLobApp.ContentVersions[MobileAppContentId].Files[MobileAppContentFileId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.DeviceAppManagement.MobileApps[MobileAppId].GraphManagedAndroidLobApp.ContentVersions[MobileAppContentId].Files[MobileAppContentFileId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, MobileAppContentFileId);
