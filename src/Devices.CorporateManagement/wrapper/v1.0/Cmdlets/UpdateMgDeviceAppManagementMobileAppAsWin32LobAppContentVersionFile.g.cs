...
{
    [GraphRoute("PATCH", "/deviceAppManagement/mobileApps/{mobileApp-id}/graph.win32LobApp/contentVersions/{mobileAppContent-id}/files/{mobileAppContentFile-id}")]
    [Cmdlet(VerbsData.Update, "MgDeviceAppManagementMobileAppAsWin32LobAppContentVersionFile", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Devices.CorporateManagement.Client.Models.MobileAppContentFile))]
    public class UpdateMgDeviceAppManagementMobileAppAsWin32LobAppContentVersionFileCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(UploadState)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.DeviceAppManagement.MobileApps[MobileAppId].GraphWin32LobApp.ContentVersions[MobileAppContentId].Files[MobileAppContentFileId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.DeviceAppManagement.MobileApps[MobileAppId].GraphWin32LobApp.ContentVersions[MobileAppContentId].Files[MobileAppContentFileId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, MobileAppContentFileId);
