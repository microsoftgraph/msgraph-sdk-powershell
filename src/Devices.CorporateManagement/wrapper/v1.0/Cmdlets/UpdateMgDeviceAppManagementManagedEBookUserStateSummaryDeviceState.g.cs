...
{
    [GraphRoute("PATCH", "/deviceAppManagement/managedEBooks/{managedEBook-id}/userStateSummary/{userInstallStateSummary-id}/deviceStates/{deviceInstallState-id}")]
    [Cmdlet(VerbsData.Update, "MgDeviceAppManagementManagedEBookUserStateSummaryDeviceState", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Devices.CorporateManagement.Client.Models.DeviceInstallState))]
    public class UpdateMgDeviceAppManagementManagedEBookUserStateSummaryDeviceStateCommand : PSCmdlet
    {
        ...
        {
            if (!ShouldProcess(DeviceInstallStateId, "Update"))
                return;
    ...
    if (this.IsParameterBound(nameof(InstallState)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.DeviceAppManagement.ManagedEBooks[ManagedEBookId].UserStateSummary[UserInstallStateSummaryId].DeviceStates[DeviceInstallStateId].PatchAsync(body, requestConfiguration =>
                {
            ...
            {
                ThrowTerminatingError(new ErrorRecord(ex, "GraphRequestFailed", ErrorCategory.InvalidOperation, DeviceInstallStateId));
                return;
            }
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.DeviceAppManagement.ManagedEBooks[ManagedEBookId].UserStateSummary[UserInstallStateSummaryId].DeviceStates[DeviceInstallStateId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex)
                {
                    ThrowTerminatingError(new ErrorRecord(ex, "GraphRequestFailed", ErrorCategory.InvalidOperation, DeviceInstallStateId));
