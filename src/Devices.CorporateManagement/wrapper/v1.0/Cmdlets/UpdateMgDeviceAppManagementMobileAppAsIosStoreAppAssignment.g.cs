...
{
    [GraphRoute("PATCH", "/deviceAppManagement/mobileApps/{mobileApp-id}/graph.iosStoreApp/assignments/{mobileAppAssignment-id}")]
    [Cmdlet(VerbsData.Update, "MgDeviceAppManagementMobileAppAsIosStoreAppAssignment", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Devices.CorporateManagement.Client.Models.MobileAppAssignment))]
    public class UpdateMgDeviceAppManagementMobileAppAsIosStoreAppAssignmentCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Target)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.DeviceAppManagement.MobileApps[MobileAppId].GraphIosStoreApp.Assignments[MobileAppAssignmentId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.DeviceAppManagement.MobileApps[MobileAppId].GraphIosStoreApp.Assignments[MobileAppAssignmentId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex)
                {
                    ThrowGraphRequestFailed(ex, MobileAppAssignmentId);
