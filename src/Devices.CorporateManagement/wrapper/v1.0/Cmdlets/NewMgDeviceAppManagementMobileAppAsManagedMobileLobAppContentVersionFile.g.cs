...
{
    [GraphRoute("POST", "/deviceAppManagement/mobileApps/{mobileApp-id}/graph.managedMobileLobApp/contentVersions/{mobileAppContent-id}/files")]
    [Cmdlet(VerbsCommon.New, "MgDeviceAppManagementMobileAppAsManagedMobileLobAppContentVersionFile", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Devices.CorporateManagement.Client.Models.MobileAppContentFile))]
    public class NewMgDeviceAppManagementMobileAppAsManagedMobileLobAppContentVersionFileCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(UploadState)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.DeviceAppManagement.MobileApps[MobileAppId].GraphManagedMobileLobApp.ContentVersions[MobileAppContentId].Files.PostAsync(body, requestConfiguration =>
                {
