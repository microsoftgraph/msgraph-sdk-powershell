...
{
    [GraphRoute("PATCH", "/deviceAppManagement/managedEBooks/{managedEBook-id}/deviceStates/{deviceInstallState-id}")]
    [Cmdlet(VerbsData.Update, "MgDeviceAppManagementManagedEBookDeviceState", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Devices.CorporateManagement.Client.Models.DeviceInstallState))]
    public class UpdateMgDeviceAppManagementManagedEBookDeviceStateCommand : PSCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(InstallState)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.DeviceAppManagement.ManagedEBooks[ManagedEBookId].DeviceStates[DeviceInstallStateId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.DeviceAppManagement.ManagedEBooks[ManagedEBookId].DeviceStates[DeviceInstallStateId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex)
                {
                    ThrowTerminatingError(new ErrorRecord(ex, "GraphRequestFailed", ErrorCategory.InvalidOperation, DeviceInstallStateId));
