...
{
    [GraphRoute("POST", "/deviceAppManagement/managedEBooks/{managedEBook-id}/userStateSummary/{userInstallStateSummary-id}/deviceStates")]
    [Cmdlet(VerbsCommon.New, "MgDeviceAppManagementManagedEBookUserStateSummaryDeviceState", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Devices.CorporateManagement.Client.Models.DeviceInstallState))]
    public class NewMgDeviceAppManagementManagedEBookUserStateSummaryDeviceStateCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(UserInstallStateSummaryId, "New"))
                return;
    ...
    if (this.IsParameterBound(nameof(InstallState)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.DeviceAppManagement.ManagedEBooks[ManagedEBookId].UserStateSummary[UserInstallStateSummaryId].DeviceStates.PostAsync(body, requestConfiguration =>
                {
