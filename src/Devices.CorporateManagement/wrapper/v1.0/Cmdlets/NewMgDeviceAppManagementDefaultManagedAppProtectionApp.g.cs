...
{
    [GraphRoute("POST", "/deviceAppManagement/defaultManagedAppProtections/{defaultManagedAppProtection-id}/apps")]
    [Cmdlet(VerbsCommon.New, "MgDeviceAppManagementDefaultManagedAppProtectionApp", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Devices.CorporateManagement.Client.Models.ManagedMobileApp))]
    public class NewMgDeviceAppManagementDefaultManagedAppProtectionAppCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(DefaultManagedAppProtectionId, "New"))
                return;
    ...
    if (this.IsParameterBound(nameof(MobileAppIdentifier)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.DeviceAppManagement.DefaultManagedAppProtections[DefaultManagedAppProtectionId].Apps.PostAsync(body, requestConfiguration =>
                {
