...
{
    [GraphRoute("PATCH", "/users/{user-id}/managedDevices/{managedDevice-id}")]
    [Cmdlet(VerbsData.Update, "MgUserManagedDevice", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Devices.CorporateManagement.Client.Models.ManagedDevice))]
    public class UpdateMgUserManagedDeviceCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(PartnerReportedThreatState)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].ManagedDevices[ManagedDeviceId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Users[UserId].ManagedDevices[ManagedDeviceId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException)
                {
                    ThrowGraphRequestFailed(ex, ManagedDeviceId);
