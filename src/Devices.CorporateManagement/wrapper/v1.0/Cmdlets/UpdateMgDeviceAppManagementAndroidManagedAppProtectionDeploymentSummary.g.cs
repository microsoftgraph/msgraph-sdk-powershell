...
{
    [GraphRoute("PATCH", "/deviceAppManagement/androidManagedAppProtections/{androidManagedAppProtection-id}/deploymentSummary")]
    [Cmdlet(VerbsData.Update, "MgDeviceAppManagementAndroidManagedAppProtectionDeploymentSummary", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Devices.CorporateManagement.Client.Models.ManagedAppPolicyDeploymentSummary))]
    public class UpdateMgDeviceAppManagementAndroidManagedAppProtectionDeploymentSummaryCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(ConfigurationDeploymentSummaryPerApp)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.DeviceAppManagement.AndroidManagedAppProtections[AndroidManagedAppProtectionId].DeploymentSummary.PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.DeviceAppManagement.AndroidManagedAppProtections[AndroidManagedAppProtectionId].DeploymentSummary.GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, AndroidManagedAppProtectionId);
