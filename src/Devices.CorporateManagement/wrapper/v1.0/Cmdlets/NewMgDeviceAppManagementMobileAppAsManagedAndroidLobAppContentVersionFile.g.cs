...
{
    [GraphRoute("POST", "/deviceAppManagement/mobileApps/{mobileApp-id}/graph.managedAndroidLobApp/contentVersions/{mobileAppContent-id}/files")]
    [Cmdlet(VerbsCommon.New, "MgDeviceAppManagementMobileAppAsManagedAndroidLobAppContentVersionFile", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Devices.CorporateManagement.Client.Models.MobileAppContentFile))]
    public class NewMgDeviceAppManagementMobileAppAsManagedAndroidLobAppContentVersionFileCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(UploadState)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.DeviceAppManagement.MobileApps[MobileAppId].GraphManagedAndroidLobApp.ContentVersions[MobileAppContentId].Files.PostAsync(body, requestConfiguration =>
                {
