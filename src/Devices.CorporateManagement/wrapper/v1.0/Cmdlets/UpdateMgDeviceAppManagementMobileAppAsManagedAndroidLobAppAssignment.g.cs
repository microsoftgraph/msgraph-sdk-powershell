...
{
    [GraphRoute("PATCH", "/deviceAppManagement/mobileApps/{mobileApp-id}/graph.managedAndroidLobApp/assignments/{mobileAppAssignment-id}")]
    [Cmdlet(VerbsData.Update, "MgDeviceAppManagementMobileAppAsManagedAndroidLobAppAssignment", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Devices.CorporateManagement.Client.Models.MobileAppAssignment))]
    public class UpdateMgDeviceAppManagementMobileAppAsManagedAndroidLobAppAssignmentCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Target)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.DeviceAppManagement.MobileApps[MobileAppId].GraphManagedAndroidLobApp.Assignments[MobileAppAssignmentId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.DeviceAppManagement.MobileApps[MobileAppId].GraphManagedAndroidLobApp.Assignments[MobileAppAssignmentId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, MobileAppAssignmentId);
