...
{
    [GraphRoute("PATCH", "/deviceAppManagement/mdmWindowsInformationProtectionPolicies/{mdmWindowsInformationProtectionPolicy-id}")]
    [Cmdlet(VerbsData.Update, "MgDeviceAppManagementMdmWindowsInformationProtectionPolicy", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    ...
    public class UpdateMgDeviceAppManagementMdmWindowsInformationProtectionPolicyCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(MdmWindowsInformationProtectionPolicyId, "Update"))
                return;
    ...
    if (this.IsParameterBound(nameof(SmbAutoEncryptedFileExtensions)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.DeviceAppManagement.MdmWindowsInformationProtectionPolicies[MdmWindowsInformationProtectionPolicyId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.DeviceAppManagement.MdmWindowsInformationProtectionPolicies[MdmWindowsInformationProtectionPolicyId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException)
                {
                    ThrowGraphRequestFailed(ex, MdmWindowsInformationProtectionPolicyId);
