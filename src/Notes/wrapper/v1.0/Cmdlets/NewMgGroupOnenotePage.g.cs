...
{
    [GraphRoute("POST", "/groups/{group-id}/onenote/pages")]
    [Cmdlet(VerbsCommon.New, "MgGroupOnenotePage", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Notes.Client.Models.OnenotePage))]
    public class NewMgGroupOnenotePageCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Links)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Onenote.Pages.PostAsync(body, requestConfiguration =>
                {
