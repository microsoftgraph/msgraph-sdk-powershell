...
{
    [GraphRoute("GET", "/groups/{group-id}/onenote/resources")]
    [Cmdlet(VerbsCommon.Get, "MgGroupOnenoteResource_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Notes.Client.Models.OnenoteResource))]
    public class GetMgGroupOnenoteResource_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Onenote.Resources.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Groups[GroupId].Onenote.Resources.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
