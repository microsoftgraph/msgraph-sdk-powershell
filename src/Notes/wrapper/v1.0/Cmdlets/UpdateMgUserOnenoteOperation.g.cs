...
{
    [GraphRoute("PATCH", "/users/{user-id}/onenote/operations/{onenoteOperation-id}")]
    [Cmdlet(VerbsData.Update, "MgUserOnenoteOperation", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Notes.Client.Models.OnenoteOperation))]
    public class UpdateMgUserOnenoteOperationCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Error)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].Onenote.Operations[OnenoteOperationId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Users[UserId].Onenote.Operations[OnenoteOperationId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex)
                {
                    ThrowGraphRequestFailed(ex, OnenoteOperationId);
