...
{
    [GraphRoute("PATCH", "/groups/{group-id}/onenote/sectionGroups/{sectionGroup-id}")]
    [Cmdlet(VerbsData.Update, "MgGroupOnenoteSectionGroup", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Notes.Client.Models.SectionGroup))]
    public class UpdateMgGroupOnenoteSectionGroupCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(LastModifiedBy)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Onenote.SectionGroups[SectionGroupId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Groups[GroupId].Onenote.SectionGroups[SectionGroupId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException)
                {
                    ThrowGraphRequestFailed(ex, SectionGroupId);
