...
{
    [GraphRoute("PATCH", "/sites/{site-id}/onenote/resources/{onenoteResource-id}")]
    [Cmdlet(VerbsData.Update, "MgSiteOnenoteResource", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Notes.Client.Models.OnenoteResource))]
    public class UpdateMgSiteOnenoteResourceCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(ContentUrl)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Sites[SiteId].Onenote.Resources[OnenoteResourceId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Sites[SiteId].Onenote.Resources[OnenoteResourceId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException)
                {
                    ThrowGraphRequestFailed(ex, OnenoteResourceId);
