...
{
    [GraphRoute("POST", "/sites/{site-id}/onenote/notebooks/{notebook-id}/sectionGroups")]
    [Cmdlet(VerbsCommon.New, "MgSiteOnenoteNotebookSectionGroup", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Notes.Client.Models.SectionGroup))]
    public class NewMgSiteOnenoteNotebookSectionGroupCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(NotebookId, "New"))
                return;
    ...
    if (this.IsParameterBound(nameof(LastModifiedBy)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Sites[SiteId].Onenote.Notebooks[NotebookId].SectionGroups.PostAsync(body, requestConfiguration =>
                {
