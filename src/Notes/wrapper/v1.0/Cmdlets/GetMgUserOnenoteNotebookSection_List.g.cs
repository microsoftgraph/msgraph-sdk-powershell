...
{
    [GraphRoute("GET", "/users/{user-id}/onenote/notebooks/{notebook-id}/sections")]
    [Cmdlet(VerbsCommon.Get, "MgUserOnenoteNotebookSection_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Notes.Client.Models.OnenoteSection))]
    public class GetMgUserOnenoteNotebookSection_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Notes.Client.Models.OnenoteSectionCollectionResponse? result;
            try
            {
                result = client.Users[UserId].Onenote.Notebooks[NotebookId].Sections.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Users[UserId].Onenote.Notebooks[NotebookId].Sections.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
