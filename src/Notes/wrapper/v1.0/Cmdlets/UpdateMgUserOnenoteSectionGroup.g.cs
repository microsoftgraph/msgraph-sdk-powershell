...
{
    [GraphRoute("PATCH", "/users/{user-id}/onenote/sectionGroups/{sectionGroup-id}")]
    [Cmdlet(VerbsData.Update, "MgUserOnenoteSectionGroup", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Notes.Client.Models.SectionGroup))]
    public class UpdateMgUserOnenoteSectionGroupCommand : PSCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(LastModifiedBy)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].Onenote.SectionGroups[SectionGroupId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Users[UserId].Onenote.SectionGroups[SectionGroupId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex)
                {
                    ThrowTerminatingError(new ErrorRecord(ex, "GraphRequestFailed", ErrorCategory.InvalidOperation, SectionGroupId));
