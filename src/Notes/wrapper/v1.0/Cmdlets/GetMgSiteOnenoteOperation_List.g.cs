...
{
    [GraphRoute("GET", "/sites/{site-id}/onenote/operations")]
    [Cmdlet(VerbsCommon.Get, "MgSiteOnenoteOperation_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Notes.Client.Models.OnenoteOperation))]
    public class GetMgSiteOnenoteOperation_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Sites[SiteId].Onenote.Operations.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Sites[SiteId].Onenote.Operations.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
