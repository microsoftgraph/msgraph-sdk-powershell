...
{
    [GraphRoute("GET", "/sites/{site-id}/onenote/sectionGroups")]
    [Cmdlet(VerbsCommon.Get, "MgSiteOnenoteSectionGroup_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Notes.Client.Models.SectionGroup))]
    public class GetMgSiteOnenoteSectionGroup_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Sites[SiteId].Onenote.SectionGroups.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Sites[SiteId].Onenote.SectionGroups.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
