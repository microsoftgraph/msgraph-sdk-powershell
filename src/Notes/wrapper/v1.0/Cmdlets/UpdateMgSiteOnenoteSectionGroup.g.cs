...
{
    [GraphRoute("PATCH", "/sites/{site-id}/onenote/sectionGroups/{sectionGroup-id}")]
    [Cmdlet(VerbsData.Update, "MgSiteOnenoteSectionGroup", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Notes.Client.Models.SectionGroup))]
    public class UpdateMgSiteOnenoteSectionGroupCommand : PSCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(LastModifiedBy)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Sites[SiteId].Onenote.SectionGroups[SectionGroupId].PatchAsync(body, requestConfiguration =>
                {
            ...
            {
                ThrowTerminatingError(new ErrorRecord(ex, "GraphRequestFailed", ErrorCategory.InvalidOperation, SectionGroupId));
                return;
            }
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Sites[SiteId].Onenote.SectionGroups[SectionGroupId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex)
                {
                    ThrowTerminatingError(new ErrorRecord(ex, "GraphRequestFailed", ErrorCategory.InvalidOperation, SectionGroupId));
