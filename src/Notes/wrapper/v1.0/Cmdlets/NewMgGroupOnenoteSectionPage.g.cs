...
{
    [GraphRoute("POST", "/groups/{group-id}/onenote/sections/{onenoteSection-id}/pages")]
    [Cmdlet(VerbsCommon.New, "MgGroupOnenoteSectionPage", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Notes.Client.Models.OnenotePage))]
    public class NewMgGroupOnenoteSectionPageCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Links)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Onenote.Sections[OnenoteSectionId].Pages.PostAsync(body, requestConfiguration =>
                {
