...
{
    [GraphRoute("POST", "/groups/{group-id}/onenote/notebooks")]
    [Cmdlet(VerbsCommon.New, "MgGroupOnenoteNotebook", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Notes.Client.Models.Notebook))]
    public class NewMgGroupOnenoteNotebookCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(UserRole)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Onenote.Notebooks.PostAsync(body, requestConfiguration =>
                {
