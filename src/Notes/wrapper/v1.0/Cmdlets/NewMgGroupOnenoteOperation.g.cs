...
{
    [GraphRoute("POST", "/groups/{group-id}/onenote/operations")]
    [Cmdlet(VerbsCommon.New, "MgGroupOnenoteOperation", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Notes.Client.Models.OnenoteOperation))]
    public class NewMgGroupOnenoteOperationCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Error)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Onenote.Operations.PostAsync(body, requestConfiguration =>
                {
