...
{
    [GraphRoute("POST", "/sites/{site-id}/onenote/sections/{onenoteSection-id}/pages")]
    [Cmdlet(VerbsCommon.New, "MgSiteOnenoteSectionPage", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Notes.Client.Models.OnenotePage))]
    public class NewMgSiteOnenoteSectionPageCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Links)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Sites[SiteId].Onenote.Sections[OnenoteSectionId].Pages.PostAsync(body, requestConfiguration =>
                {
