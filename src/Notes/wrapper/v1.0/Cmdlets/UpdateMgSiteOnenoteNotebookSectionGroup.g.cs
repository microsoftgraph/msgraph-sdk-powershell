...
{
    [GraphRoute("PATCH", "/sites/{site-id}/onenote/notebooks/{notebook-id}/sectionGroups/{sectionGroup-id}")]
    [Cmdlet(VerbsData.Update, "MgSiteOnenoteNotebookSectionGroup", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Notes.Client.Models.SectionGroup))]
    public class UpdateMgSiteOnenoteNotebookSectionGroupCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(LastModifiedBy)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Sites[SiteId].Onenote.Notebooks[NotebookId].SectionGroups[SectionGroupId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Sites[SiteId].Onenote.Notebooks[NotebookId].SectionGroups[SectionGroupId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, SectionGroupId);
