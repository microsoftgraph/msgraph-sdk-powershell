...
{
    [GraphRoute("POST", "/sites/{site-id}/onenote/notebooks")]
    [Cmdlet(VerbsCommon.New, "MgSiteOnenoteNotebook", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Notes.Client.Models.Notebook))]
    public class NewMgSiteOnenoteNotebookCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(UserRole)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Sites[SiteId].Onenote.Notebooks.PostAsync(body, requestConfiguration =>
                {
