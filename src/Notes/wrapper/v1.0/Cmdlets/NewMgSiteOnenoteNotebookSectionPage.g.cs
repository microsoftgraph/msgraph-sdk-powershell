...
{
    [GraphRoute("POST", "/sites/{site-id}/onenote/notebooks/{notebook-id}/sections/{onenoteSection-id}/pages")]
    [Cmdlet(VerbsCommon.New, "MgSiteOnenoteNotebookSectionPage", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Notes.Client.Models.OnenotePage))]
    public class NewMgSiteOnenoteNotebookSectionPageCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Links)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Sites[SiteId].Onenote.Notebooks[NotebookId].Sections[OnenoteSectionId].Pages.PostAsync(body, requestConfiguration =>
                {
