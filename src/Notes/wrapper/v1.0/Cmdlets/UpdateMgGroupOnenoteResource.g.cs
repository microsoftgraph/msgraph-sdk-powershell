...
{
    [GraphRoute("PATCH", "/groups/{group-id}/onenote/resources/{onenoteResource-id}")]
    [Cmdlet(VerbsData.Update, "MgGroupOnenoteResource", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Notes.Client.Models.OnenoteResource))]
    public class UpdateMgGroupOnenoteResourceCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(ContentUrl)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Onenote.Resources[OnenoteResourceId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Groups[GroupId].Onenote.Resources[OnenoteResourceId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex)
                {
                    ThrowGraphRequestFailed(ex, OnenoteResourceId);
