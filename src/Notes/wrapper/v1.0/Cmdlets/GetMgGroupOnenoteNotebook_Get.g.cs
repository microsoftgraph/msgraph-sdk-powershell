...
{
    [GraphRoute("GET", "/groups/{group-id}/onenote/notebooks/{notebook-id}")]
    [Cmdlet(VerbsCommon.Get, "MgGroupOnenoteNotebook_Get")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Notes.Client.Models.Notebook))]
    public class GetMgGroupOnenoteNotebook_GetCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Onenote.Notebooks[NotebookId].GetAsync(requestConfiguration =>
                {
