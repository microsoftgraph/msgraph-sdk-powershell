...
{
    [GraphRoute("POST", "/users/{user-id}/onenote/notebooks/{notebook-id}/sections/{onenoteSection-id}/pages")]
    [Cmdlet(VerbsCommon.New, "MgUserOnenoteNotebookSectionPage", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Notes.Client.Models.OnenotePage))]
    public class NewMgUserOnenoteNotebookSectionPageCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Links)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].Onenote.Notebooks[NotebookId].Sections[OnenoteSectionId].Pages.PostAsync(body, requestConfiguration =>
                {
