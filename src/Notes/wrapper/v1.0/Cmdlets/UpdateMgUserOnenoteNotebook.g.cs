...
{
    [GraphRoute("PATCH", "/users/{user-id}/onenote/notebooks/{notebook-id}")]
    [Cmdlet(VerbsData.Update, "MgUserOnenoteNotebook", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Notes.Client.Models.Notebook))]
    public class UpdateMgUserOnenoteNotebookCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(UserRole)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].Onenote.Notebooks[NotebookId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Users[UserId].Onenote.Notebooks[NotebookId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, NotebookId);
