...
{
    [GraphRoute("PATCH", "/groups/{group-id}/onenote/notebooks/{notebook-id}")]
    [Cmdlet(VerbsData.Update, "MgGroupOnenoteNotebook", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Notes.Client.Models.Notebook))]
    public class UpdateMgGroupOnenoteNotebookCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(UserRole)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Onenote.Notebooks[NotebookId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Groups[GroupId].Onenote.Notebooks[NotebookId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex)
                {
                    ThrowGraphRequestFailed(ex, NotebookId);
