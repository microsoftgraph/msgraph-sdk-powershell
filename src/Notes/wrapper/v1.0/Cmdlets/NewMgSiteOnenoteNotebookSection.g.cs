...
{
    [GraphRoute("POST", "/sites/{site-id}/onenote/notebooks/{notebook-id}/sections")]
    [Cmdlet(VerbsCommon.New, "MgSiteOnenoteNotebookSection", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Notes.Client.Models.OnenoteSection))]
    public class NewMgSiteOnenoteNotebookSectionCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Links)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Sites[SiteId].Onenote.Notebooks[NotebookId].Sections.PostAsync(body, requestConfiguration =>
                {
