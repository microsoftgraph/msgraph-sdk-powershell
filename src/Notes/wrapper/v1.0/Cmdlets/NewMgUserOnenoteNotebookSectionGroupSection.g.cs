...
{
    [GraphRoute("POST", "/users/{user-id}/onenote/notebooks/{notebook-id}/sectionGroups/{sectionGroup-id}/sections")]
    [Cmdlet(VerbsCommon.New, "MgUserOnenoteNotebookSectionGroupSection", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Notes.Client.Models.OnenoteSection))]
    public class NewMgUserOnenoteNotebookSectionGroupSectionCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Links)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].Onenote.Notebooks[NotebookId].SectionGroups[SectionGroupId].Sections.PostAsync(body, requestConfiguration =>
                {
