...
{
    [GraphRoute("POST", "/groups/{group-id}/onenote/notebooks/{notebook-id}/sections")]
    [Cmdlet(VerbsCommon.New, "MgGroupOnenoteNotebookSection", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Notes.Client.Models.OnenoteSection))]
    public class NewMgGroupOnenoteNotebookSectionCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Links)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Onenote.Notebooks[NotebookId].Sections.PostAsync(body, requestConfiguration =>
                {
