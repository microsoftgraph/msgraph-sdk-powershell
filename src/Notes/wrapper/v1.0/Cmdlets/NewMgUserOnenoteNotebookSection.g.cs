...
{
    [GraphRoute("POST", "/users/{user-id}/onenote/notebooks/{notebook-id}/sections")]
    [Cmdlet(VerbsCommon.New, "MgUserOnenoteNotebookSection", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Notes.Client.Models.OnenoteSection))]
    public class NewMgUserOnenoteNotebookSectionCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Links)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].Onenote.Notebooks[NotebookId].Sections.PostAsync(body, requestConfiguration =>
                {
