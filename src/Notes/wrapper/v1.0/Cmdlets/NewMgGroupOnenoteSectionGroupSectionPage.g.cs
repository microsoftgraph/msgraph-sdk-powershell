...
{
    [GraphRoute("POST", "/groups/{group-id}/onenote/sectionGroups/{sectionGroup-id}/sections/{onenoteSection-id}/pages")]
    [Cmdlet(VerbsCommon.New, "MgGroupOnenoteSectionGroupSectionPage", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Notes.Client.Models.OnenotePage))]
    public class NewMgGroupOnenoteSectionGroupSectionPageCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Links)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Onenote.SectionGroups[SectionGroupId].Sections[OnenoteSectionId].Pages.PostAsync(body, requestConfiguration =>
                {
