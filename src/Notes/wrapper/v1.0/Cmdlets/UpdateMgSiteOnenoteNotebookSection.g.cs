...
{
    [GraphRoute("PATCH", "/sites/{site-id}/onenote/notebooks/{notebook-id}/sections/{onenoteSection-id}")]
    [Cmdlet(VerbsData.Update, "MgSiteOnenoteNotebookSection", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Notes.Client.Models.OnenoteSection))]
    public class UpdateMgSiteOnenoteNotebookSectionCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Links)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Sites[SiteId].Onenote.Notebooks[NotebookId].Sections[OnenoteSectionId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Sites[SiteId].Onenote.Notebooks[NotebookId].Sections[OnenoteSectionId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, OnenoteSectionId);
