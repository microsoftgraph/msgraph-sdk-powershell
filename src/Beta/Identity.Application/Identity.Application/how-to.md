# How-To
This document describes how to develop for `Microsoft.Graph.Beta.Identity.Application`.

## Building `Microsoft.Graph.Beta.Identity.Application`
...
## Testing `Microsoft.Graph.Beta.Identity.Application`
...
## Packing `Microsoft.Graph.Beta.Identity.Application`
...
There are multiple scripts created for performing different actions for developing `Microsoft.Graph.Beta.Identity.Application`.
- `build-module.ps1`
  - Builds the module DLL (`./bin/Microsoft.Graph.Beta.Identity.Application.private.dll`), creates the exported cmdlets and documentation, generates custom cmdlet test stubs and exported cmdlet example stubs, and updates `./Microsoft.Graph.Beta.Identity.Application.psd1` with Azure profile information.
  - **Parameters**: [`Switch` parameters]
    - `-Run`: After building, creates an isolated PowerShell session and loads `Microsoft.Graph.Beta.Identity.Application`.
...
- `run-module.ps1`
  - Creates an isolated PowerShell session and loads `Microsoft.Graph.Beta.Identity.Application` into the session.
