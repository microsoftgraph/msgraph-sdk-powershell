# How-To
This document describes how to develop for `Microsoft.Graph.Beta.Identity.AppRoleAssignments`.

## Building `Microsoft.Graph.Beta.Identity.AppRoleAssignments`
...
## Testing `Microsoft.Graph.Beta.Identity.AppRoleAssignments`
...
## Packing `Microsoft.Graph.Beta.Identity.AppRoleAssignments`
...
There are multiple scripts created for performing different actions for developing `Microsoft.Graph.Beta.Identity.AppRoleAssignments`.
- `build-module.ps1`
  - Builds the module DLL (`./bin/Microsoft.Graph.Beta.Identity.AppRoleAssignments.private.dll`), creates the exported cmdlets and documentation, generates custom cmdlet test stubs and exported cmdlet example stubs, and updates `./Microsoft.Graph.Beta.Identity.AppRoleAssignments.psd1` with Azure profile information.
  - **Parameters**: [`Switch` parameters]
    - `-Run`: After building, creates an isolated PowerShell session and loads `Microsoft.Graph.Beta.Identity.AppRoleAssignments`.
...
- `run-module.ps1`
  - Creates an isolated PowerShell session and loads `Microsoft.Graph.Beta.Identity.AppRoleAssignments` into the session.
