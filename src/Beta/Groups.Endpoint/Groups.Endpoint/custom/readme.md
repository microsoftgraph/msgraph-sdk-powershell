# Custom
This directory contains custom implementation for non-generated cmdlets for the `Microsoft.Graph.Beta.Groups.Endpoint` module. Both scripts (`.ps1`) and C# files (`.cs`) can be implemented here. They will be used during the build process in `build-module.ps1`, and create cmdlets into the `..\exports` folder. The only generated file into this folder is the `Microsoft.Graph.Beta.Groups.Endpoint.custom.psm1`. This file should not be modified.
...
## Details
For `Microsoft.Graph.Beta.Groups.Endpoint` to use custom cmdlets, it does this two different ways. We **highly recommend** creating script cmdlets, as they are easier to write and allow access to the other exported cmdlets. C# cmdlets *cannot access exported cmdlets*.
...
These provide functionality to our HTTP pipeline and other useful features. In script, you can forward these parameters using `$PSBoundParameters` to the other cmdlets you're calling within `Microsoft.Graph.Beta.Groups.Endpoint`. For C#, follow the usage seen in the `ProcessRecordAsync` method.
...
- `Microsoft.Graph.PowerShell.Models.DoNotExportAttribute`
  - Used in C# and script cmdlets to suppress creating an exported cmdlet at build-time. These cmdlets will *not be exposed* by `Microsoft.Graph.Beta.Groups.Endpoint`.
- `Microsoft.Graph.PowerShell.Models.InternalExportAttribute`
  - Used in C# cmdlets to route exported cmdlets to the `..\internal`, which are *not exposed* by `Microsoft.Graph.Beta.Groups.Endpoint`. For more information, see [readme.md](..\internal/readme.md) in the `..\internal` folder.
