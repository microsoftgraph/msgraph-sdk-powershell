# How-To
This document describes how to develop for `Microsoft.Graph.Beta.Groups.Endpoint`.

## Building `Microsoft.Graph.Beta.Groups.Endpoint`
...
## Testing `Microsoft.Graph.Beta.Groups.Endpoint`
...
## Packing `Microsoft.Graph.Beta.Groups.Endpoint`
...
There are multiple scripts created for performing different actions for developing `Microsoft.Graph.Beta.Groups.Endpoint`.
- `build-module.ps1`
  - Builds the module DLL (`./bin/Microsoft.Graph.Beta.Groups.Endpoint.private.dll`), creates the exported cmdlets and documentation, generates custom cmdlet test stubs and exported cmdlet example stubs, and updates `./Microsoft.Graph.Beta.Groups.Endpoint.psd1` with Azure profile information.
  - **Parameters**: [`Switch` parameters]
    - `-Run`: After building, creates an isolated PowerShell session and loads `Microsoft.Graph.Beta.Groups.Endpoint`.
...
- `run-module.ps1`
  - Creates an isolated PowerShell session and loads `Microsoft.Graph.Beta.Groups.Endpoint` into the session.
