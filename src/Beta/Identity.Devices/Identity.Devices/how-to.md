# How-To
This document describes how to develop for `Microsoft.Graph.Beta.Identity.Devices`.

## Building `Microsoft.Graph.Beta.Identity.Devices`
...
## Testing `Microsoft.Graph.Beta.Identity.Devices`
...
## Packing `Microsoft.Graph.Beta.Identity.Devices`
...
There are multiple scripts created for performing different actions for developing `Microsoft.Graph.Beta.Identity.Devices`.
- `build-module.ps1`
  - Builds the module DLL (`./bin/Microsoft.Graph.Beta.Identity.Devices.private.dll`), creates the exported cmdlets and documentation, generates custom cmdlet test stubs and exported cmdlet example stubs, and updates `./Microsoft.Graph.Beta.Identity.Devices.psd1` with Azure profile information.
  - **Parameters**: [`Switch` parameters]
    - `-Run`: After building, creates an isolated PowerShell session and loads `Microsoft.Graph.Beta.Identity.Devices`.
...
- `run-module.ps1`
  - Creates an isolated PowerShell session and loads `Microsoft.Graph.Beta.Identity.Devices` into the session.
