# How-To
This document describes how to develop for `Microsoft.Graph.Beta.Identity.Domains`.

## Building `Microsoft.Graph.Beta.Identity.Domains`
...
## Testing `Microsoft.Graph.Beta.Identity.Domains`
...
## Packing `Microsoft.Graph.Beta.Identity.Domains`
...
There are multiple scripts created for performing different actions for developing `Microsoft.Graph.Beta.Identity.Domains`.
- `build-module.ps1`
  - Builds the module DLL (`./bin/Microsoft.Graph.Beta.Identity.Domains.private.dll`), creates the exported cmdlets and documentation, generates custom cmdlet test stubs and exported cmdlet example stubs, and updates `./Microsoft.Graph.Beta.Identity.Domains.psd1` with Azure profile information.
  - **Parameters**: [`Switch` parameters]
    - `-Run`: After building, creates an isolated PowerShell session and loads `Microsoft.Graph.Beta.Identity.Domains`.
...
- `run-module.ps1`
  - Creates an isolated PowerShell session and loads `Microsoft.Graph.Beta.Identity.Domains` into the session.
