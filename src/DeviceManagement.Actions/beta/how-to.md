# How-To
This document describes how to develop for `Microsoft.Graph.Beta.DeviceManagement.Actions`.

## Building `Microsoft.Graph.Beta.DeviceManagement.Actions`
...
## Testing `Microsoft.Graph.Beta.DeviceManagement.Actions`
...
## Packing `Microsoft.Graph.Beta.DeviceManagement.Actions`
...
There are multiple scripts created for performing different actions for developing `Microsoft.Graph.Beta.DeviceManagement.Actions`.
- `build-module.ps1`
  - Builds the module DLL (`./bin/Microsoft.Graph.Beta.DeviceManagement.Actions.private.dll`), creates the exported cmdlets and documentation, generates custom cmdlet test stubs and exported cmdlet example stubs, and updates `./Microsoft.Graph.Beta.DeviceManagement.Actions.psd1` with Azure profile information.
  - **Parameters**: [`Switch` parameters]
    - `-Run`: After building, creates an isolated PowerShell session and loads `Microsoft.Graph.Beta.DeviceManagement.Actions`.
...
- `run-module.ps1`
  - Creates an isolated PowerShell session and loads `Microsoft.Graph.Beta.DeviceManagement.Actions` into the session.
