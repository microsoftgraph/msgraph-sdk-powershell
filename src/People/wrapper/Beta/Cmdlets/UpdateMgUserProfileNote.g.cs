...
{
    [GraphRoute("PATCH", "/users/{user-id}/profile/notes/{personAnnotation-id}")]
    [Cmdlet(VerbsData.Update, "MgUserProfileNote", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.People.Client.Models.PersonAnnotation))]
    public class UpdateMgUserProfileNoteCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(PersonAnnotationId, "Update"))
                return;
    ...
    if (this.IsParameterBound(nameof(Detail)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].Profile.Notes[PersonAnnotationId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Users[UserId].Profile.Notes[PersonAnnotationId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, PersonAnnotationId);
