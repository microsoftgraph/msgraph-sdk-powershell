...
{
    [GraphRoute("PATCH", "/users/{user-id}/profile/awards/{personAward-id}")]
    [Cmdlet(VerbsData.Update, "MgUserProfileAward", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.People.Client.Models.PersonAward))]
    public class UpdateMgUserProfileAwardCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(PersonAwardId, "Update"))
                return;

            var body = new Microsoft.Graph.PowerShell.People.Client.Models.PersonAward();
    ...
    if (this.IsParameterBound(nameof(Sources)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].Profile.Awards[PersonAwardId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Users[UserId].Profile.Awards[PersonAwardId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, PersonAwardId);
