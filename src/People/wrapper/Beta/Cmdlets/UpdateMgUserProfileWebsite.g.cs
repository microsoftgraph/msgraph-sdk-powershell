...
{
    [GraphRoute("PATCH", "/users/{user-id}/profile/websites/{personWebsite-id}")]
    [Cmdlet(VerbsData.Update, "MgUserProfileWebsite", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.People.Client.Models.PersonWebsite))]
    public class UpdateMgUserProfileWebsiteCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(PersonWebsiteId, "Update"))
                return;
    ...
    if (this.IsParameterBound(nameof(Sources)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].Profile.Websites[PersonWebsiteId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Users[UserId].Profile.Websites[PersonWebsiteId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, PersonWebsiteId);
