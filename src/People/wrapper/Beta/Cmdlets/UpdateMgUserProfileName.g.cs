...
{
    [GraphRoute("PATCH", "/users/{user-id}/profile/names/{personName-id}")]
    [Cmdlet(VerbsData.Update, "MgUserProfileName", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.People.Client.Models.PersonName))]
    public class UpdateMgUserProfileNameCommand : GraphClientCmdlet
    {
        ...
        [Parameter(Mandatory = false)]
        public Microsoft.Graph.PowerShell.People.Client.Models.PersonNamePronounciation? Pronunciation { get; set; }
        ...
        {
            if (!ShouldProcess(PersonNameId, "Update"))
                return;

            var body = new Microsoft.Graph.PowerShell.People.Client.Models.PersonName();
    ...
    if (this.IsParameterBound(nameof(First)))
        body.First = First;
    ...
    if (this.IsParameterBound(nameof(Last)))
        body.Last = Last;

    if (this.IsParameterBound(nameof(Maiden)))
        body.Maiden = Maiden;

    if (this.IsParameterBound(nameof(Middle)))
        body.Middle = Middle;

    if (this.IsParameterBound(nameof(Nickname)))
        body.Nickname = Nickname;

    if (this.IsParameterBound(nameof(Suffix)))
        body.Suffix = Suffix;
    ...
    if (this.IsParameterBound(nameof(Pronunciation)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.People.Client.Models.PersonName? result;
            try
            {
                result = client.Users[UserId].Profile.Names[PersonNameId].PatchAsync(body, requestConfiguration =>
                {
            ...
            {
                ThrowGraphRequestFailed(ex, PersonNameId);
                return;
            }
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Users[UserId].Profile.Names[PersonNameId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, PersonNameId);
