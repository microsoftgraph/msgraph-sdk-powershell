...
{
    [GraphRoute("PATCH", "/users/{user-id}/profile/account/{userAccountInformation-id}")]
    [Cmdlet(VerbsData.Update, "MgUserProfileAccount", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.People.Client.Models.UserAccountInformation))]
    public class UpdateMgUserProfileAccountCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(UserAccountInformationId, "Update"))
                return;
    ...
    if (this.IsParameterBound(nameof(UserPersona)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].Profile.Account[UserAccountInformationId].PatchAsync(body, requestConfiguration =>
                {
            ...
            {
                ThrowGraphRequestFailed(ex, UserAccountInformationId);
                return;
            }
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Users[UserId].Profile.Account[UserAccountInformationId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, UserAccountInformationId);
