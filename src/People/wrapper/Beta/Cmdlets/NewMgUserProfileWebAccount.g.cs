...
{
    [GraphRoute("POST", "/users/{user-id}/profile/webAccounts")]
    [Cmdlet(VerbsCommon.New, "MgUserProfileWebAccount", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.People.Client.Models.WebAccount))]
    public class NewMgUserProfileWebAccountCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Service)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].Profile.WebAccounts.PostAsync(body, requestConfiguration =>
                {
