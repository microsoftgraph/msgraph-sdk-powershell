...
{
    [GraphRoute("PATCH", "/users/{user-id}/profile/skills/{skillProficiency-id}")]
    [Cmdlet(VerbsData.Update, "MgUserProfileSkill", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.People.Client.Models.SkillProficiency))]
    public class UpdateMgUserProfileSkillCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(SkillProficiencyId, "Update"))
                return;
    ...
    if (this.IsParameterBound(nameof(Proficiency)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].Profile.Skills[SkillProficiencyId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Users[UserId].Profile.Skills[SkillProficiencyId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, SkillProficiencyId);
