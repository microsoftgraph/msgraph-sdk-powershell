...
{
    [GraphRoute("GET", "/users/{user-id}/profile/addresses")]
    [Cmdlet(VerbsCommon.Get, "MgUserProfileAddress", DefaultParameterSetName = "List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.People.Client.Models.ItemAddressCollectionResponse), ParameterSetName = new[] { "List" })]
    [OutputType(typeof(Microsoft.Graph.PowerShell.People.Client.Models.ItemAddress), ParameterSetName = new[] { "Get" })]
    public class GetMgUserProfileAddressCommand : GraphClientCmdlet
    {
        ...
        // Delegates to Get-MgUserProfileAddress_Get or Get-MgUserProfileAddress_List, the two cmdlets
        // that actually call Graph.
        protected override void ProcessRecord()
        {
            var internalCmdletName = ParameterSetName == "Get" ? "Get-MgUserProfileAddress_Get" : "Get-MgUserProfileAddress_List";
            try
            {
            ...
            {
                ThrowGraphRequestFailed(ex, ParameterSetName == "Get" ? ItemAddressId : UserId);
                return;
            }
        }
    }
