...
{
    [GraphRoute("PATCH", "/users/{user-id}/profile/interests/{personInterest-id}")]
    [Cmdlet(VerbsData.Update, "MgUserProfileInterest", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.People.Client.Models.PersonInterest))]
    public class UpdateMgUserProfileInterestCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(PersonInterestId, "Update"))
                return;

            var body = new Microsoft.Graph.PowerShell.People.Client.Models.PersonInterest();
    ...
    if (this.IsParameterBound(nameof(Sources)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.People.Client.Models.PersonInterest? result;
            try
            {
                result = client.Users[UserId].Profile.Interests[PersonInterestId].PatchAsync(body, requestConfiguration =>
                {
            ...
            {
                ThrowGraphRequestFailed(ex, PersonInterestId);
                return;
            }
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Users[UserId].Profile.Interests[PersonInterestId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, PersonInterestId);
