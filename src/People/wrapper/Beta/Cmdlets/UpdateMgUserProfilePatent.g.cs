...
{
    [GraphRoute("PATCH", "/users/{user-id}/profile/patents/{itemPatent-id}")]
    [Cmdlet(VerbsData.Update, "MgUserProfilePatent", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.People.Client.Models.ItemPatent))]
    public class UpdateMgUserProfilePatentCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(ItemPatentId, "Update"))
                return;

            var body = new Microsoft.Graph.PowerShell.People.Client.Models.ItemPatent();
    ...
    if (this.IsParameterBound(nameof(IsPending)))
        body.IsPending = IsPending;
    ...
    if (this.IsParameterBound(nameof(Sources)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].Profile.Patents[ItemPatentId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Users[UserId].Profile.Patents[ItemPatentId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, ItemPatentId);
