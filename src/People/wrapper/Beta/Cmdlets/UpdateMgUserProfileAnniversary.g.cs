...
{
    [GraphRoute("PATCH", "/users/{user-id}/profile/anniversaries/{personAnnualEvent-id}")]
    [Cmdlet(VerbsData.Update, "MgUserProfileAnniversary", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.People.Client.Models.PersonAnnualEvent))]
    public class UpdateMgUserProfileAnniversaryCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(PersonAnnualEventId, "Update"))
                return;
    ...
    if (this.IsParameterBound(nameof(Type)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].Profile.Anniversaries[PersonAnnualEventId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Users[UserId].Profile.Anniversaries[PersonAnnualEventId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, PersonAnnualEventId);
