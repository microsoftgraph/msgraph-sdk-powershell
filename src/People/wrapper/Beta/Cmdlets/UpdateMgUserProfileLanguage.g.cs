...
{
    [GraphRoute("PATCH", "/users/{user-id}/profile/languages/{languageProficiency-id}")]
    [Cmdlet(VerbsData.Update, "MgUserProfileLanguage", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.People.Client.Models.LanguageProficiency))]
    public class UpdateMgUserProfileLanguageCommand : GraphClientCmdlet
    {
        [Parameter(Mandatory = true, Position = 0)]
        public string UserId { get; set; } = string.Empty;
        [Parameter(Mandatory = true, Position = 1)]
        public string LanguageProficiencyId { get; set; } = string.Empty;
        ...
        {
            if (!ShouldProcess(LanguageProficiencyId, "Update"))
                return;
    ...
    if (this.IsParameterBound(nameof(Written)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].Profile.Languages[LanguageProficiencyId].PatchAsync(body, requestConfiguration =>
                {
            ...
            {
                ThrowGraphRequestFailed(ex, LanguageProficiencyId);
                return;
            }
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Users[UserId].Profile.Languages[LanguageProficiencyId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, LanguageProficiencyId);
