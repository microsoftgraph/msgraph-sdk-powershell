...
{
    [GraphRoute("PATCH", "/users/{user-id}/profile/addresses/{itemAddress-id}")]
    [Cmdlet(VerbsData.Update, "MgUserProfileAddress", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.People.Client.Models.ItemAddress))]
    public class UpdateMgUserProfileAddressCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(ItemAddressId, "Update"))
                return;
    ...
    if (this.IsParameterBound(nameof(GeoCoordinates)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].Profile.Addresses[ItemAddressId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Users[UserId].Profile.Addresses[ItemAddressId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, ItemAddressId);
