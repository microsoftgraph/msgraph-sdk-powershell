...
{
    [GraphRoute("PATCH", "/users/{user-id}/profile/publications/{itemPublication-id}")]
    [Cmdlet(VerbsData.Update, "MgUserProfilePublication", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.People.Client.Models.ItemPublication))]
    public class UpdateMgUserProfilePublicationCommand : GraphClientCmdlet
    {
        ...
        [Parameter(Mandatory = false)]
        public global::Microsoft.Kiota.Abstractions.Date? PublishedDate { get; set; }
        ...
        {
            if (!ShouldProcess(ItemPublicationId, "Update"))
                return;

            var body = new Microsoft.Graph.PowerShell.People.Client.Models.ItemPublication();
    ...
    if (this.IsParameterBound(nameof(PublishedDate)))
    ...
    if (this.IsParameterBound(nameof(Sources)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].Profile.Publications[ItemPublicationId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Users[UserId].Profile.Publications[ItemPublicationId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, ItemPublicationId);
