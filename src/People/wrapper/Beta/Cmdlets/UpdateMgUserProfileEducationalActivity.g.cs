...
{
    [GraphRoute("PATCH", "/users/{user-id}/profile/educationalActivities/{educationalActivity-id}")]
    [Cmdlet(VerbsData.Update, "MgUserProfileEducationalActivity", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.People.Client.Models.EducationalActivity))]
    public class UpdateMgUserProfileEducationalActivityCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(EducationalActivityId, "Update"))
                return;
    ...
    if (this.IsParameterBound(nameof(Program)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].Profile.EducationalActivities[EducationalActivityId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Users[UserId].Profile.EducationalActivities[EducationalActivityId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, EducationalActivityId);
