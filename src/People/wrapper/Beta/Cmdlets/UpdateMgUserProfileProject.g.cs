...
{
    [GraphRoute("PATCH", "/users/{user-id}/profile/projects/{projectParticipation-id}")]
    [Cmdlet(VerbsData.Update, "MgUserProfileProject", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.People.Client.Models.ProjectParticipation))]
    public class UpdateMgUserProfileProjectCommand : GraphClientCmdlet
    {
        [Parameter(Mandatory = true, Position = 0)]
        public string UserId { get; set; } = string.Empty;
        [Parameter(Mandatory = true, Position = 1)]
        public string ProjectParticipationId { get; set; } = string.Empty;
        ...
        {
            if (!ShouldProcess(ProjectParticipationId, "Update"))
                return;
    ...
    if (this.IsParameterBound(nameof(Sponsors)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].Profile.Projects[ProjectParticipationId].PatchAsync(body, requestConfiguration =>
                {
            ...
            {
                ThrowGraphRequestFailed(ex, ProjectParticipationId);
                return;
            }
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Users[UserId].Profile.Projects[ProjectParticipationId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, ProjectParticipationId);
