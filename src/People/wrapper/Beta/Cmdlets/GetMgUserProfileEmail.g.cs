...
{
    [GraphRoute("GET", "/users/{user-id}/profile/emails")]
    [Cmdlet(VerbsCommon.Get, "MgUserProfileEmail", DefaultParameterSetName = "List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.People.Client.Models.ItemEmailCollectionResponse), ParameterSetName = new[] { "List" })]
    [OutputType(typeof(Microsoft.Graph.PowerShell.People.Client.Models.ItemEmail), ParameterSetName = new[] { "Get" })]
    public class GetMgUserProfileEmailCommand : GraphClientCmdlet
    {
        ...
        // Delegates to Get-MgUserProfileEmail_Get or Get-MgUserProfileEmail_List, the two cmdlets
        // that actually call Graph.
        protected override void ProcessRecord()
        {
            var internalCmdletName = ParameterSetName == "Get" ? "Get-MgUserProfileEmail_Get" : "Get-MgUserProfileEmail_List";
            try
            {
            ...
            {
                ThrowGraphRequestFailed(ex, ParameterSetName == "Get" ? ItemEmailId : UserId);
                return;
            }
        }
    }
