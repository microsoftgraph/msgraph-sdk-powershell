...
{
    [GraphRoute("PATCH", "/education/users/{educationUser-id}/rubrics/{educationRubric-id}")]
    [Cmdlet(VerbsData.Update, "MgEducationUserRubric", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Education.Client.Models.EducationRubric))]
    public class UpdateMgEducationUserRubricCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(EducationRubricId, "Update"))
                return;
    ...
    if (this.IsParameterBound(nameof(Qualities)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Education.Users[EducationUserId].Rubrics[EducationRubricId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Education.Users[EducationUserId].Rubrics[EducationRubricId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException)
                {
                    ThrowGraphRequestFailed(ex, EducationRubricId);
