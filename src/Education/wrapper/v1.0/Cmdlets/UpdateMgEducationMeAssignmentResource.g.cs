...
{
    [GraphRoute("PATCH", "/education/me/assignments/{educationAssignment-id}/resources/{educationAssignmentResource-id}")]
    [Cmdlet(VerbsData.Update, "MgEducationMeAssignmentResource", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Education.Client.Models.EducationAssignmentResource))]
    public class UpdateMgEducationMeAssignmentResourceCommand : PSCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Resource)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Education.Me.Assignments[EducationAssignmentId].Resources[EducationAssignmentResourceId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Education.Me.Assignments[EducationAssignmentId].Resources[EducationAssignmentResourceId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex)
                {
                    ThrowTerminatingError(new ErrorRecord(ex, "GraphRequestFailed", ErrorCategory.InvalidOperation, EducationAssignmentResourceId));
