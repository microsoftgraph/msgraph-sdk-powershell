...
{
    [GraphRoute("PATCH", "/education/classes/{educationClass-id}/assignments/{educationAssignment-id}/rubric")]
    [Cmdlet(VerbsData.Update, "MgEducationClassAssignmentRubric", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Education.Client.Models.EducationRubric))]
    public class UpdateMgEducationClassAssignmentRubricCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Qualities)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Education.Classes[EducationClassId].Assignments[EducationAssignmentId].Rubric.PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Education.Classes[EducationClassId].Assignments[EducationAssignmentId].Rubric.GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException)
                {
                    ThrowGraphRequestFailed(ex, EducationAssignmentId);
