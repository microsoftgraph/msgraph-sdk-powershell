...
{
    [GraphRoute("PATCH", "/education/users/{educationUser-id}/assignments/{educationAssignment-id}/resources/{educationAssignmentResource-id}")]
    [Cmdlet(VerbsData.Update, "MgEducationUserAssignmentResource", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Education.Client.Models.EducationAssignmentResource))]
    public class UpdateMgEducationUserAssignmentResourceCommand : PSCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Resource)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Education.Users[EducationUserId].Assignments[EducationAssignmentId].Resources[EducationAssignmentResourceId].PatchAsync(body, requestConfiguration =>
                {
            ...
            {
                ThrowTerminatingError(new ErrorRecord(ex, "GraphRequestFailed", ErrorCategory.InvalidOperation, EducationAssignmentResourceId));
                return;
            }
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Education.Users[EducationUserId].Assignments[EducationAssignmentId].Resources[EducationAssignmentResourceId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex)
                {
                    ThrowTerminatingError(new ErrorRecord(ex, "GraphRequestFailed", ErrorCategory.InvalidOperation, EducationAssignmentResourceId));
