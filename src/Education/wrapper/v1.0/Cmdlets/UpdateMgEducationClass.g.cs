...
{
    [GraphRoute("PATCH", "/education/classes/{educationClass-id}")]
    [Cmdlet(VerbsData.Update, "MgEducationClass", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Education.Client.Models.EducationClass))]
    public class UpdateMgEducationClassCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Term)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Education.Classes[EducationClassId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Education.Classes[EducationClassId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, EducationClassId);
