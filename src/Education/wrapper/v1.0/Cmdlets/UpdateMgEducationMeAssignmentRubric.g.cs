...
{
    [GraphRoute("PATCH", "/education/me/assignments/{educationAssignment-id}/rubric")]
    [Cmdlet(VerbsData.Update, "MgEducationMeAssignmentRubric", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Education.Client.Models.EducationRubric))]
    public class UpdateMgEducationMeAssignmentRubricCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Qualities)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Education.Me.Assignments[EducationAssignmentId].Rubric.PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Education.Me.Assignments[EducationAssignmentId].Rubric.GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException)
                {
                    ThrowGraphRequestFailed(ex, EducationAssignmentId);
