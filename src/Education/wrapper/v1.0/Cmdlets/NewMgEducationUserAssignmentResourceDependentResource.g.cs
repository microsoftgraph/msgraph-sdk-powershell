...
{
    [GraphRoute("POST", "/education/users/{educationUser-id}/assignments/{educationAssignment-id}/resources/{educationAssignmentResource-id}/dependentResources")]
    [Cmdlet(VerbsCommon.New, "MgEducationUserAssignmentResourceDependentResource", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Education.Client.Models.EducationAssignmentResource))]
    public class NewMgEducationUserAssignmentResourceDependentResourceCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(EducationAssignmentResourceId, "New"))
                return;
    ...
    if (this.IsParameterBound(nameof(Resource)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Education.Users[EducationUserId].Assignments[EducationAssignmentId].Resources[EducationAssignmentResourceId].DependentResources.PostAsync(body, requestConfiguration =>
                {
