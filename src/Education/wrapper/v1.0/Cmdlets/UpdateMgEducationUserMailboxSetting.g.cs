...
{
    [GraphRoute("PATCH", "/education/users/{educationUser-id}/user/mailboxSettings")]
    [Cmdlet(VerbsData.Update, "MgEducationUserMailboxSetting", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Education.Client.Models.MailboxSettings))]
    public class UpdateMgEducationUserMailboxSettingCommand : PSCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(WorkingHours)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Education.Users[EducationUserId].User.MailboxSettings.PatchAsync(body, requestConfiguration =>
                {
            ...
            {
                ThrowTerminatingError(new ErrorRecord(ex, "GraphRequestFailed", ErrorCategory.InvalidOperation, EducationUserId));
                return;
            }
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Education.Users[EducationUserId].User.MailboxSettings.GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex)
                {
                    ThrowTerminatingError(new ErrorRecord(ex, "GraphRequestFailed", ErrorCategory.InvalidOperation, EducationUserId));
