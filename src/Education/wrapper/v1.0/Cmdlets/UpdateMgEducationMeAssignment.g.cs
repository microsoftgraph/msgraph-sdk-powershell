...
{
    [GraphRoute("PATCH", "/education/me/assignments/{educationAssignment-id}")]
    [Cmdlet(VerbsData.Update, "MgEducationMeAssignment", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Education.Client.Models.EducationAssignment))]
    public class UpdateMgEducationMeAssignmentCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Instructions)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Education.Me.Assignments[EducationAssignmentId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Education.Me.Assignments[EducationAssignmentId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException)
                {
                    ThrowGraphRequestFailed(ex, EducationAssignmentId);
