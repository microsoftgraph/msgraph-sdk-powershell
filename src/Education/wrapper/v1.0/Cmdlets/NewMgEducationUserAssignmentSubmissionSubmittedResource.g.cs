...
{
    [GraphRoute("POST", "/education/users/{educationUser-id}/assignments/{educationAssignment-id}/submissions/{educationSubmission-id}/submittedResources")]
    [Cmdlet(VerbsCommon.New, "MgEducationUserAssignmentSubmissionSubmittedResource", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Education.Client.Models.EducationSubmissionResource))]
    public class NewMgEducationUserAssignmentSubmissionSubmittedResourceCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Resource)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Education.Users[EducationUserId].Assignments[EducationAssignmentId].Submissions[EducationSubmissionId].SubmittedResources.PostAsync(body, requestConfiguration =>
                {
