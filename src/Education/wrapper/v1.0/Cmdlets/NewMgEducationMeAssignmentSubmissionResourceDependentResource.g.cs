...
{
    [GraphRoute("POST", "/education/me/assignments/{educationAssignment-id}/submissions/{educationSubmission-id}/resources/{educationSubmissionResource-id}/dependentResources")]
    [Cmdlet(VerbsCommon.New, "MgEducationMeAssignmentSubmissionResourceDependentResource", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Education.Client.Models.EducationSubmissionResource))]
    public class NewMgEducationMeAssignmentSubmissionResourceDependentResourceCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Resource)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Education.Me.Assignments[EducationAssignmentId].Submissions[EducationSubmissionId].Resources[EducationSubmissionResourceId].DependentResources.PostAsync(body, requestConfiguration =>
                {
