...
{
    [GraphRoute("PATCH", "/education/me/rubrics/{educationRubric-id}")]
    [Cmdlet(VerbsData.Update, "MgEducationMeRubric", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Education.Client.Models.EducationRubric))]
    public class UpdateMgEducationMeRubricCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Qualities)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Education.Me.Rubrics[EducationRubricId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Education.Me.Rubrics[EducationRubricId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex)
                {
                    ThrowGraphRequestFailed(ex, EducationRubricId);
