...
{
    [GraphRoute("PATCH", "/users/{user-id}/onlineMeetings/{onlineMeeting-id}/transcripts/{callTranscript-id}")]
    [Cmdlet(VerbsData.Update, "MgUserOnlineMeetingTranscript", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.CloudCommunications.Client.Models.CallTranscript))]
    public class UpdateMgUserOnlineMeetingTranscriptCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(MeetingOrganizer)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].OnlineMeetings[OnlineMeetingId].Transcripts[CallTranscriptId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Users[UserId].OnlineMeetings[OnlineMeetingId].Transcripts[CallTranscriptId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException)
                {
                    ThrowGraphRequestFailed(ex, CallTranscriptId);
