...
{
    [GraphRoute("POST", "/users/{user-id}/onlineMeetings/{onlineMeeting-id}/transcripts")]
    [Cmdlet(VerbsCommon.New, "MgUserOnlineMeetingTranscript", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.CloudCommunications.Client.Models.CallTranscript))]
    public class NewMgUserOnlineMeetingTranscriptCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(MeetingOrganizer)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].OnlineMeetings[OnlineMeetingId].Transcripts.PostAsync(body, requestConfiguration =>
                {
