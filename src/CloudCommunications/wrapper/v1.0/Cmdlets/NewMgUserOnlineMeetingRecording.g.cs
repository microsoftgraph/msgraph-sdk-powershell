...
{
    [GraphRoute("POST", "/users/{user-id}/onlineMeetings/{onlineMeeting-id}/recordings")]
    [Cmdlet(VerbsCommon.New, "MgUserOnlineMeetingRecording", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.CloudCommunications.Client.Models.CallRecording))]
    public class NewMgUserOnlineMeetingRecordingCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(MeetingOrganizer)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].OnlineMeetings[OnlineMeetingId].Recordings.PostAsync(body, requestConfiguration =>
                {
