...
{
    [GraphRoute("POST", "/users/{user-id}/onlineMeetings")]
    [Cmdlet(VerbsCommon.New, "MgUserOnlineMeeting", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.CloudCommunications.Client.Models.OnlineMeeting))]
    public class NewMgUserOnlineMeetingCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Participants)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].OnlineMeetings.PostAsync(body, requestConfiguration =>
                {
