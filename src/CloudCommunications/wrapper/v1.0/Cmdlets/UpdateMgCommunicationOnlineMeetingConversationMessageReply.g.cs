...
{
    [GraphRoute("PATCH", "/communications/onlineMeetingConversations/{onlineMeetingEngagementConversation-id}/messages/{engagementConversationMessage-id}/replies/{engagementConversationMessage-id1}")]
    [Cmdlet(VerbsData.Update, "MgCommunicationOnlineMeetingConversationMessageReply", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.CloudCommunications.Client.Models.EngagementConversationMessage))]
    public class UpdateMgCommunicationOnlineMeetingConversationMessageReplyCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(EngagementConversationMessageId1, "Update"))
                return;
    ...
    if (this.IsParameterBound(nameof(Conversation)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Communications.OnlineMeetingConversations[OnlineMeetingEngagementConversationId].Messages[EngagementConversationMessageId].Replies[EngagementConversationMessageId1].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Communications.OnlineMeetingConversations[OnlineMeetingEngagementConversationId].Messages[EngagementConversationMessageId].Replies[EngagementConversationMessageId1].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException)
                {
                    ThrowGraphRequestFailed(ex, EngagementConversationMessageId1);
