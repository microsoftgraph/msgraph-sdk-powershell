...
{
    [GraphRoute("PATCH", "/communications/adhocCalls/{adhocCall-id}/transcripts/{callTranscript-id}")]
    [Cmdlet(VerbsData.Update, "MgCommunicationAdhocCallTranscript", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.CloudCommunications.Client.Models.CallTranscript))]
    public class UpdateMgCommunicationAdhocCallTranscriptCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(MeetingOrganizer)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Communications.AdhocCalls[AdhocCallId].Transcripts[CallTranscriptId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Communications.AdhocCalls[AdhocCallId].Transcripts[CallTranscriptId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex)
                {
                    ThrowGraphRequestFailed(ex, CallTranscriptId);
