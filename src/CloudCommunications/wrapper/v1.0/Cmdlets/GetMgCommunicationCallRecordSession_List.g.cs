...
{
    [GraphRoute("GET", "/communications/callRecords/{callRecord-id}/sessions")]
    [Cmdlet(VerbsCommon.Get, "MgCommunicationCallRecordSession_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.CloudCommunications.Client.Models.CallRecords.Session))]
    public class GetMgCommunicationCallRecordSession_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.CloudCommunications.Client.Models.CallRecords.SessionCollectionResponse? result;
            try
            {
                result = client.Communications.CallRecords[CallRecordId].Sessions.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Communications.CallRecords[CallRecordId].Sessions.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
