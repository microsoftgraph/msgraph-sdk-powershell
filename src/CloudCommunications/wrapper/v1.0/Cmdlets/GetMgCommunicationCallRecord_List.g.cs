...
{
    [GraphRoute("GET", "/communications/callRecords")]
    [Cmdlet(VerbsCommon.Get, "MgCommunicationCallRecord_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.CloudCommunications.Client.Models.CallRecords.CallRecord))]
    public class GetMgCommunicationCallRecord_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.CloudCommunications.Client.Models.CallRecords.CallRecordCollectionResponse? result;
            try
            {
                result = client.Communications.CallRecords.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Communications.CallRecords.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
