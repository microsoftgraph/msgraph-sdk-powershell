...
{
    [GraphRoute("PATCH", "/users/{user-id}/presence")]
    [Cmdlet(VerbsData.Update, "MgUserPresence", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.CloudCommunications.Client.Models.Presence))]
    public class UpdateMgUserPresenceCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(WorkLocation)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].Presence.PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Users[UserId].Presence.GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex)
                {
                    ThrowGraphRequestFailed(ex, UserId);
