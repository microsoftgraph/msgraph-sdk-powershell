...
{
    [GraphRoute("POST", "/communications/onlineMeetings/{onlineMeeting-id}/transcripts")]
    [Cmdlet(VerbsCommon.New, "MgCommunicationOnlineMeetingTranscript", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.CloudCommunications.Client.Models.CallTranscript))]
    public class NewMgCommunicationOnlineMeetingTranscriptCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(MeetingOrganizer)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Communications.OnlineMeetings[OnlineMeetingId].Transcripts.PostAsync(body, requestConfiguration =>
                {
