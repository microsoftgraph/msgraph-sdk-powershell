...
{
    [GraphRoute("POST", "/communications/onlineMeetings/{onlineMeeting-id}/attendanceReports")]
    [Cmdlet(VerbsCommon.New, "MgCommunicationOnlineMeetingAttendanceReport", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.CloudCommunications.Client.Models.MeetingAttendanceReport))]
    public class NewMgCommunicationOnlineMeetingAttendanceReportCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(ExternalEventInformation)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Communications.OnlineMeetings[OnlineMeetingId].AttendanceReports.PostAsync(body, requestConfiguration =>
                {
