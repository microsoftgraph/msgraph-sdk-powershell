...
{
    [GraphRoute("POST", "/communications/adhocCalls/{adhocCall-id}/transcripts")]
    [Cmdlet(VerbsCommon.New, "MgCommunicationAdhocCallTranscript", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.CloudCommunications.Client.Models.CallTranscript))]
    public class NewMgCommunicationAdhocCallTranscriptCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(AdhocCallId, "New"))
                return;
    ...
    if (this.IsParameterBound(nameof(MeetingOrganizer)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Communications.AdhocCalls[AdhocCallId].Transcripts.PostAsync(body, requestConfiguration =>
                {
