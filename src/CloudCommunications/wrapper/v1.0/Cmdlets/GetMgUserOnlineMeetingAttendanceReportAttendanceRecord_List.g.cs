...
{
    [GraphRoute("GET", "/users/{user-id}/onlineMeetings/{onlineMeeting-id}/attendanceReports/{meetingAttendanceReport-id}/attendanceRecords")]
    [Cmdlet(VerbsCommon.Get, "MgUserOnlineMeetingAttendanceReportAttendanceRecord_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.CloudCommunications.Client.Models.AttendanceRecord))]
    public class GetMgUserOnlineMeetingAttendanceReportAttendanceRecord_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.CloudCommunications.Client.Models.AttendanceRecordCollectionResponse? result;
            try
            {
                result = client.Users[UserId].OnlineMeetings[OnlineMeetingId].AttendanceReports[MeetingAttendanceReportId].AttendanceRecords.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Users[UserId].OnlineMeetings[OnlineMeetingId].AttendanceReports[MeetingAttendanceReportId].AttendanceRecords.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
