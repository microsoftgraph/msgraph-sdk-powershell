...
{
    [GraphRoute("PATCH", "/communications/adhocCalls/{adhocCall-id}/recordings/{callRecording-id}")]
    [Cmdlet(VerbsData.Update, "MgCommunicationAdhocCallRecording", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.CloudCommunications.Client.Models.CallRecording))]
    public class UpdateMgCommunicationAdhocCallRecordingCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(MeetingOrganizer)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Communications.AdhocCalls[AdhocCallId].Recordings[CallRecordingId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Communications.AdhocCalls[AdhocCallId].Recordings[CallRecordingId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex)
                {
                    ThrowGraphRequestFailed(ex, CallRecordingId);
