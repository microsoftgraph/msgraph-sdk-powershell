...
{
    [GraphRoute("GET", "/communications/callRecords/{callRecord-id}/participants_v2")]
    [Cmdlet(VerbsCommon.Get, "MgCommunicationCallRecordParticipantV2_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.CloudCommunications.Client.Models.CallRecords.Participant))]
    public class GetMgCommunicationCallRecordParticipantV2_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.CloudCommunications.Client.Models.CallRecords.ParticipantCollectionResponse? result;
            try
            {
                result = client.Communications.CallRecords[CallRecordId].Participants_v2.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Communications.CallRecords[CallRecordId].Participants_v2.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
