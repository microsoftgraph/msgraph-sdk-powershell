...
{
    [GraphRoute("POST", "/users/{user-id}/settings/workHoursAndLocations/recurrences")]
    [Cmdlet(VerbsCommon.New, "MgUserSettingWorkHourAndLocationRecurrence", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Users.Client.Models.WorkPlanRecurrence))]
    public class NewMgUserSettingWorkHourAndLocationRecurrenceCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(WorkLocationType)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].Settings.WorkHoursAndLocations.Recurrences.PostAsync(body, requestConfiguration =>
                {
