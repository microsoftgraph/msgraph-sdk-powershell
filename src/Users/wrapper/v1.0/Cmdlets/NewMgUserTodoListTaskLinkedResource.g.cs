...
{
    [GraphRoute("POST", "/users/{user-id}/todo/lists/{todoTaskList-id}/tasks/{todoTask-id}/linkedResources")]
    [Cmdlet(VerbsCommon.New, "MgUserTodoListTaskLinkedResource", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Users.Client.Models.LinkedResource))]
    public class NewMgUserTodoListTaskLinkedResourceCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(WebUrl)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].Todo.Lists[TodoTaskListId].Tasks[TodoTaskId].LinkedResources.PostAsync(body, requestConfiguration =>
                {
