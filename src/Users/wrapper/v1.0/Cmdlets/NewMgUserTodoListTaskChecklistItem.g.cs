...
{
    [GraphRoute("POST", "/users/{user-id}/todo/lists/{todoTaskList-id}/tasks/{todoTask-id}/checklistItems")]
    [Cmdlet(VerbsCommon.New, "MgUserTodoListTaskChecklistItem", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Users.Client.Models.ChecklistItem))]
    public class NewMgUserTodoListTaskChecklistItemCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(IsChecked)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].Todo.Lists[TodoTaskListId].Tasks[TodoTaskId].ChecklistItems.PostAsync(body, requestConfiguration =>
                {
