...
{
    [GraphRoute("POST", "/users/{user-id}/settings/storage/quota/services")]
    [Cmdlet(VerbsCommon.New, "MgUserSettingStorageQuotaService", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Users.Client.Models.ServiceStorageQuotaBreakdown))]
    public class NewMgUserSettingStorageQuotaServiceCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Used)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].Settings.Storage.Quota.Services.PostAsync(body, requestConfiguration =>
                {
