...
{
    [GraphRoute("PUT", "/users/{user-id}/settings/workHoursAndLocations/occurrences/{workPlanOccurrence-id}")]
    [Cmdlet(VerbsCommon.Set, "MgUserSettingWorkHourAndLocationOccurrence", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Users.Client.Models.WorkPlanOccurrence))]
    public class SetMgUserSettingWorkHourAndLocationOccurrenceCommand : PSCmdlet
    {
        ...
        {
            if (!ShouldProcess(WorkPlanOccurrenceId, "Set"))
                return;
    ...
    if (this.IsParameterBound(nameof(WorkLocationType)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].Settings.WorkHoursAndLocations.Occurrences[WorkPlanOccurrenceId].PutAsync(body, requestConfiguration =>
                {
            ...
            {
                ThrowTerminatingError(new ErrorRecord(ex, "GraphRequestFailed", ErrorCategory.InvalidOperation, WorkPlanOccurrenceId));
                return;
            }
