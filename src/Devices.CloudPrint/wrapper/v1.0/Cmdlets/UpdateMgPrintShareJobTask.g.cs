...
{
    [GraphRoute("PATCH", "/print/shares/{printerShare-id}/jobs/{printJob-id}/tasks/{printTask-id}")]
    [Cmdlet(VerbsData.Update, "MgPrintShareJobTask", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Devices.CloudPrint.Client.Models.PrintTask))]
    public class UpdateMgPrintShareJobTaskCommand : PSCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Trigger)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Print.Shares[PrinterShareId].Jobs[PrintJobId].Tasks[PrintTaskId].PatchAsync(body, requestConfiguration =>
                {
            ...
            {
                ThrowTerminatingError(new ErrorRecord(ex, "GraphRequestFailed", ErrorCategory.InvalidOperation, PrintTaskId));
                return;
            }
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Print.Shares[PrinterShareId].Jobs[PrintJobId].Tasks[PrintTaskId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex)
                {
                    ThrowTerminatingError(new ErrorRecord(ex, "GraphRequestFailed", ErrorCategory.InvalidOperation, PrintTaskId));
