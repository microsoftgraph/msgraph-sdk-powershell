...
{
    [GraphRoute("GET", "/print/shares/{printerShare-id}/allowedGroups/$ref")]
    [Cmdlet(VerbsCommon.Get, "MgPrintShareAllowedGroupByRef")]
    [OutputType(typeof(string))]
    public class GetMgPrintShareAllowedGroupByRefCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Devices.CloudPrint.Client.Models.StringCollectionResponse? result;
            try
            {
                result = client.Print.Shares[PrinterShareId].AllowedGroups.Ref.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Print.Shares[PrinterShareId].AllowedGroups.Ref.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
