...
{
    [GraphRoute("POST", "/print/shares/{printerShare-id}/jobs/{printJob-id}/tasks")]
    [Cmdlet(VerbsCommon.New, "MgPrintShareJobTask", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Devices.CloudPrint.Client.Models.PrintTask))]
    public class NewMgPrintShareJobTaskCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Trigger)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Print.Shares[PrinterShareId].Jobs[PrintJobId].Tasks.PostAsync(body, requestConfiguration =>
                {
