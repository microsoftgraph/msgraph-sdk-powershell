...
{
    [GraphRoute("PATCH", "/print/shares/{printerShare-id}/jobs/{printJob-id}")]
    [Cmdlet(VerbsData.Update, "MgPrintShareJob", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Devices.CloudPrint.Client.Models.PrintJob))]
    public class UpdateMgPrintShareJobCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Status)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Print.Shares[PrinterShareId].Jobs[PrintJobId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Print.Shares[PrinterShareId].Jobs[PrintJobId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException)
                {
                    ThrowGraphRequestFailed(ex, PrintJobId);
