...
{
    [GraphRoute("GET", "/print/printers/{printer-id}/connectors")]
    [Cmdlet(VerbsCommon.Get, "MgPrintPrinterConnector_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Devices.CloudPrint.Client.Models.PrintConnector))]
    public class GetMgPrintPrinterConnector_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Devices.CloudPrint.Client.Models.PrintConnectorCollectionResponse? result;
            try
            {
                result = client.Print.Printers[PrinterId].Connectors.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Print.Printers[PrinterId].Connectors.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
