...
{
    [GraphRoute("GET", "/print/shares/{printerShare-id}/allowedGroups/{group-id}/serviceProvisioningErrors")]
    [Cmdlet(VerbsCommon.Get, "MgPrintShareAllowedGroupServiceProvisioningError")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Devices.CloudPrint.Client.Models.ServiceProvisioningError))]
    public class GetMgPrintShareAllowedGroupServiceProvisioningErrorCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Devices.CloudPrint.Client.Models.ServiceProvisioningErrorCollectionResponse? result;
            try
            {
                result = client.Print.Shares[PrinterShareId].AllowedGroups[GroupId].ServiceProvisioningErrors.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Print.Shares[PrinterShareId].AllowedGroups[GroupId].ServiceProvisioningErrors.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
