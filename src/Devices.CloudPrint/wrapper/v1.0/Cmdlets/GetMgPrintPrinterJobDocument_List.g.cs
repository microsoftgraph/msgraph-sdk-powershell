...
{
    [GraphRoute("GET", "/print/printers/{printer-id}/jobs/{printJob-id}/documents")]
    [Cmdlet(VerbsCommon.Get, "MgPrintPrinterJobDocument_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Devices.CloudPrint.Client.Models.PrintDocument))]
    public class GetMgPrintPrinterJobDocument_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Devices.CloudPrint.Client.Models.PrintDocumentCollectionResponse? result;
            try
            {
                result = client.Print.Printers[PrinterId].Jobs[PrintJobId].Documents.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Print.Printers[PrinterId].Jobs[PrintJobId].Documents.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
