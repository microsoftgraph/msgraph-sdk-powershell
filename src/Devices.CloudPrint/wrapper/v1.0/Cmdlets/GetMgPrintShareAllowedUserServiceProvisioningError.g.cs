...
{
    [GraphRoute("GET", "/print/shares/{printerShare-id}/allowedUsers/{user-id}/serviceProvisioningErrors")]
    [Cmdlet(VerbsCommon.Get, "MgPrintShareAllowedUserServiceProvisioningError")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Devices.CloudPrint.Client.Models.ServiceProvisioningError))]
    public class GetMgPrintShareAllowedUserServiceProvisioningErrorCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Print.Shares[PrinterShareId].AllowedUsers[UserId].ServiceProvisioningErrors.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Print.Shares[PrinterShareId].AllowedUsers[UserId].ServiceProvisioningErrors.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
