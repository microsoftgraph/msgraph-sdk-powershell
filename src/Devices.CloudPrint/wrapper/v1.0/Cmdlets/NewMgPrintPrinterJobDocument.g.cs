...
{
    [GraphRoute("POST", "/print/printers/{printer-id}/jobs/{printJob-id}/documents")]
    [Cmdlet(VerbsCommon.New, "MgPrintPrinterJobDocument", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Devices.CloudPrint.Client.Models.PrintDocument))]
    public class NewMgPrintPrinterJobDocumentCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(UploadedDateTime)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Print.Printers[PrinterId].Jobs[PrintJobId].Documents.PostAsync(body, requestConfiguration =>
                {
