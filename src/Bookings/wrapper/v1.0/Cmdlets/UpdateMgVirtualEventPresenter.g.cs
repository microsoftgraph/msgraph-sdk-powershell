...
{
    [GraphRoute("PATCH", "/solutions/virtualEvents/events/{virtualEvent-id}/presenters/{virtualEventPresenter-id}")]
    [Cmdlet(VerbsData.Update, "MgVirtualEventPresenter", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Bookings.Client.Models.VirtualEventPresenter))]
    public class UpdateMgVirtualEventPresenterCommand : PSCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(PresenterDetails)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Solutions.VirtualEvents.Events[VirtualEventId].Presenters[VirtualEventPresenterId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Solutions.VirtualEvents.Events[VirtualEventId].Presenters[VirtualEventPresenterId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex)
                {
                    ThrowTerminatingError(new ErrorRecord(ex, "GraphRequestFailed", ErrorCategory.InvalidOperation, VirtualEventPresenterId));
