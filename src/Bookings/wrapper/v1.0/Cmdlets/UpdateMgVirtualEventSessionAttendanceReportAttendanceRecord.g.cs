...
{
    [GraphRoute("PATCH", "/solutions/virtualEvents/events/{virtualEvent-id}/sessions/{virtualEventSession-id}/attendanceReports/{meetingAttendanceReport-id}/attendanceRecords/{attendanceRecord-id}")]
    [Cmdlet(VerbsData.Update, "MgVirtualEventSessionAttendanceReportAttendanceRecord", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Bookings.Client.Models.AttendanceRecord))]
    public class UpdateMgVirtualEventSessionAttendanceReportAttendanceRecordCommand : PSCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Identity)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Solutions.VirtualEvents.Events[VirtualEventId].Sessions[VirtualEventSessionId].AttendanceReports[MeetingAttendanceReportId].AttendanceRecords[AttendanceRecordId].PatchAsync(body, requestConfiguration =>
                {
            ...
            {
                ThrowTerminatingError(new ErrorRecord(ex, "GraphRequestFailed", ErrorCategory.InvalidOperation, AttendanceRecordId));
                return;
            }
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Solutions.VirtualEvents.Events[VirtualEventId].Sessions[VirtualEventSessionId].AttendanceReports[MeetingAttendanceReportId].AttendanceRecords[AttendanceRecordId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex)
                {
                    ThrowTerminatingError(new ErrorRecord(ex, "GraphRequestFailed", ErrorCategory.InvalidOperation, AttendanceRecordId));
