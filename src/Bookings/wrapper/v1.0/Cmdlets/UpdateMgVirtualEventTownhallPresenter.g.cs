...
{
    [GraphRoute("PATCH", "/solutions/virtualEvents/townhalls/{virtualEventTownhall-id}/presenters/{virtualEventPresenter-id}")]
    [Cmdlet(VerbsData.Update, "MgVirtualEventTownhallPresenter", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Bookings.Client.Models.VirtualEventPresenter))]
    public class UpdateMgVirtualEventTownhallPresenterCommand : PSCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(PresenterDetails)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Solutions.VirtualEvents.Townhalls[VirtualEventTownhallId].Presenters[VirtualEventPresenterId].PatchAsync(body, requestConfiguration =>
                {
            ...
            {
                ThrowTerminatingError(new ErrorRecord(ex, "GraphRequestFailed", ErrorCategory.InvalidOperation, VirtualEventPresenterId));
                return;
            }
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Solutions.VirtualEvents.Townhalls[VirtualEventTownhallId].Presenters[VirtualEventPresenterId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex)
                {
                    ThrowTerminatingError(new ErrorRecord(ex, "GraphRequestFailed", ErrorCategory.InvalidOperation, VirtualEventPresenterId));
