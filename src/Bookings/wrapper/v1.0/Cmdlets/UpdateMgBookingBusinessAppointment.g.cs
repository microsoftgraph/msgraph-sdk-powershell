...
{
    [GraphRoute("PATCH", "/solutions/bookingBusinesses/{bookingBusiness-id}/appointments/{bookingAppointment-id}")]
    [Cmdlet(VerbsData.Update, "MgBookingBusinessAppointment", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Bookings.Client.Models.BookingAppointment))]
    public class UpdateMgBookingBusinessAppointmentCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(StartDateTime)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Solutions.BookingBusinesses[BookingBusinessId].Appointments[BookingAppointmentId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Solutions.BookingBusinesses[BookingBusinessId].Appointments[BookingAppointmentId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex)
                {
                    ThrowGraphRequestFailed(ex, BookingAppointmentId);
