...
{
    [GraphRoute("PATCH", "/solutions/virtualEvents/webinars/{virtualEventWebinar-id}/sessions/{virtualEventSession-id}")]
    [Cmdlet(VerbsData.Update, "MgVirtualEventWebinarSession", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Bookings.Client.Models.VirtualEventSession))]
    public class UpdateMgVirtualEventWebinarSessionCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(StartDateTime)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Solutions.VirtualEvents.Webinars[VirtualEventWebinarId].Sessions[VirtualEventSessionId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Solutions.VirtualEvents.Webinars[VirtualEventWebinarId].Sessions[VirtualEventSessionId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException)
                {
                    ThrowGraphRequestFailed(ex, VirtualEventSessionId);
