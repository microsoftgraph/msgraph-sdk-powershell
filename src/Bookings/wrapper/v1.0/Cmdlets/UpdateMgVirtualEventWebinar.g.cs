...
{
    [GraphRoute("PATCH", "/solutions/virtualEvents/webinars/{virtualEventWebinar-id}")]
    [Cmdlet(VerbsData.Update, "MgVirtualEventWebinar", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Bookings.Client.Models.VirtualEventWebinar))]
    public class UpdateMgVirtualEventWebinarCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(CoOrganizers)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Solutions.VirtualEvents.Webinars[VirtualEventWebinarId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Solutions.VirtualEvents.Webinars[VirtualEventWebinarId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, VirtualEventWebinarId);
