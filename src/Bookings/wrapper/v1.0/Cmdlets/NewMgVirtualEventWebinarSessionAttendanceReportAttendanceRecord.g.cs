...
{
    [GraphRoute("POST", "/solutions/virtualEvents/webinars/{virtualEventWebinar-id}/sessions/{virtualEventSession-id}/attendanceReports/{meetingAttendanceReport-id}/attendanceRecords")]
    [Cmdlet(VerbsCommon.New, "MgVirtualEventWebinarSessionAttendanceReportAttendanceRecord", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Bookings.Client.Models.AttendanceRecord))]
    public class NewMgVirtualEventWebinarSessionAttendanceReportAttendanceRecordCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Identity)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Solutions.VirtualEvents.Webinars[VirtualEventWebinarId].Sessions[VirtualEventSessionId].AttendanceReports[MeetingAttendanceReportId].AttendanceRecords.PostAsync(body, requestConfiguration =>
                {
