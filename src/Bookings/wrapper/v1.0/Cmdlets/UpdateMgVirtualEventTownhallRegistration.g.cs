...
{
    [GraphRoute("PATCH", "/solutions/virtualEvents/townhalls/{virtualEventTownhall-id}/registrations/{virtualEventRegistration-id}")]
    [Cmdlet(VerbsData.Update, "MgVirtualEventTownhallRegistration", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Bookings.Client.Models.VirtualEventRegistration))]
    public class UpdateMgVirtualEventTownhallRegistrationCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Status)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Solutions.VirtualEvents.Townhalls[VirtualEventTownhallId].Registrations[VirtualEventRegistrationId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Solutions.VirtualEvents.Townhalls[VirtualEventTownhallId].Registrations[VirtualEventRegistrationId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, VirtualEventRegistrationId);
