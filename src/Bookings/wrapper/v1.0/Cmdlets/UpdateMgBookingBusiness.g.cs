...
{
    [GraphRoute("PATCH", "/solutions/bookingBusinesses/{bookingBusiness-id}")]
    [Cmdlet(VerbsData.Update, "MgBookingBusiness", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Bookings.Client.Models.BookingBusiness))]
    public class UpdateMgBookingBusinessCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(BookingBusinessId, "Update"))
                return;
    ...
    if (this.IsParameterBound(nameof(SchedulingPolicy)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Solutions.BookingBusinesses[BookingBusinessId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Solutions.BookingBusinesses[BookingBusinessId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException)
                {
                    ThrowGraphRequestFailed(ex, BookingBusinessId);
