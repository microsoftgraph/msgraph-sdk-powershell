...
{
    [GraphRoute("PATCH", "/solutions/virtualEvents/townhalls/{virtualEventTownhall-id}/sessions/{virtualEventSession-id}/attendanceReports/{meetingAttendanceReport-id}")]
    [Cmdlet(VerbsData.Update, "MgVirtualEventTownhallSessionAttendanceReport", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Bookings.Client.Models.MeetingAttendanceReport))]
    public class UpdateMgVirtualEventTownhallSessionAttendanceReportCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(ExternalEventInformation)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Solutions.VirtualEvents.Townhalls[VirtualEventTownhallId].Sessions[VirtualEventSessionId].AttendanceReports[MeetingAttendanceReportId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Solutions.VirtualEvents.Townhalls[VirtualEventTownhallId].Sessions[VirtualEventSessionId].AttendanceReports[MeetingAttendanceReportId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException)
                {
                    ThrowGraphRequestFailed(ex, MeetingAttendanceReportId);
