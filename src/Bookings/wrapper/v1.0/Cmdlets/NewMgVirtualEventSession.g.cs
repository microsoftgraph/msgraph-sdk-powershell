...
{
    [GraphRoute("POST", "/solutions/virtualEvents/events/{virtualEvent-id}/sessions")]
    [Cmdlet(VerbsCommon.New, "MgVirtualEventSession", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Bookings.Client.Models.VirtualEventSession))]
    public class NewMgVirtualEventSessionCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(StartDateTime)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Solutions.VirtualEvents.Events[VirtualEventId].Sessions.PostAsync(body, requestConfiguration =>
                {
