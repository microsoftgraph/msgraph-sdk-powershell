...
{
    [GraphRoute("POST", "/solutions/virtualEvents/events/{virtualEvent-id}/sessions/{virtualEventSession-id}/attendanceReports")]
    [Cmdlet(VerbsCommon.New, "MgVirtualEventSessionAttendanceReport", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Bookings.Client.Models.MeetingAttendanceReport))]
    public class NewMgVirtualEventSessionAttendanceReportCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(ExternalEventInformation)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Solutions.VirtualEvents.Events[VirtualEventId].Sessions[VirtualEventSessionId].AttendanceReports.PostAsync(body, requestConfiguration =>
                {
