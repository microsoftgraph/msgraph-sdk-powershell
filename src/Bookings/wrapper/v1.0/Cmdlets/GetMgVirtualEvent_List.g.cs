...
{
    [GraphRoute("GET", "/solutions/virtualEvents/events")]
    [Cmdlet(VerbsCommon.Get, "MgVirtualEvent_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Bookings.Client.Models.VirtualEvent))]
    public class GetMgVirtualEvent_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Bookings.Client.Models.VirtualEventCollectionResponse? result;
            try
            {
                result = client.Solutions.VirtualEvents.Events.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Solutions.VirtualEvents.Events.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
