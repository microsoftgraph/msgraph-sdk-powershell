...
{
    [GraphRoute("GET", "/solutions/virtualEvents/events/{virtualEvent-id}/presenters")]
    [Cmdlet(VerbsCommon.Get, "MgVirtualEventPresenter_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Bookings.Client.Models.VirtualEventPresenter))]
    public class GetMgVirtualEventPresenter_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Solutions.VirtualEvents.Events[VirtualEventId].Presenters.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Solutions.VirtualEvents.Events[VirtualEventId].Presenters.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
