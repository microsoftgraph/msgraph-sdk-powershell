# How-To
This document describes how to develop for `Microsoft.Graph.Beta.DeviceManagement`.

## Building `Microsoft.Graph.Beta.DeviceManagement`
...
## Testing `Microsoft.Graph.Beta.DeviceManagement`
...
## Packing `Microsoft.Graph.Beta.DeviceManagement`
...
There are multiple scripts created for performing different actions for developing `Microsoft.Graph.Beta.DeviceManagement`.
- `build-module.ps1`
  - Builds the module DLL (`./bin/Microsoft.Graph.Beta.DeviceManagement.private.dll`), creates the exported cmdlets and documentation, generates custom cmdlet test stubs and exported cmdlet example stubs, and updates `./Microsoft.Graph.Beta.DeviceManagement.psd1` with Azure profile information.
  - **Parameters**: [`Switch` parameters]
    - `-Run`: After building, creates an isolated PowerShell session and loads `Microsoft.Graph.Beta.DeviceManagement`.
...
- `run-module.ps1`
  - Creates an isolated PowerShell session and loads `Microsoft.Graph.Beta.DeviceManagement` into the session.
