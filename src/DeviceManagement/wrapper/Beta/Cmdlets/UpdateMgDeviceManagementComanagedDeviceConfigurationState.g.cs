...
{
    [GraphRoute("PATCH", "/deviceManagement/comanagedDevices/{managedDevice-id}/deviceConfigurationStates/{deviceConfigurationState-id}")]
    [Cmdlet(VerbsData.Update, "MgDeviceManagementComanagedDeviceConfigurationState", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.DeviceManagement.Client.Models.DeviceConfigurationState))]
    public class UpdateMgDeviceManagementComanagedDeviceConfigurationStateCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(State)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.DeviceManagement.ComanagedDevices[ManagedDeviceId].DeviceConfigurationStates[DeviceConfigurationStateId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.DeviceManagement.ComanagedDevices[ManagedDeviceId].DeviceConfigurationStates[DeviceConfigurationStateId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, DeviceConfigurationStateId);
