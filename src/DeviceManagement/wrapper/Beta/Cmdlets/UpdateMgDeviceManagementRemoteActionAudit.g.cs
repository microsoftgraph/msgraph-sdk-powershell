...
{
    [GraphRoute("PATCH", "/deviceManagement/remoteActionAudits/{remoteActionAudit-id}")]
    [Cmdlet(VerbsData.Update, "MgDeviceManagementRemoteActionAudit", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.DeviceManagement.Client.Models.RemoteActionAudit))]
    public class UpdateMgDeviceManagementRemoteActionAuditCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(RemoteActionAuditId, "Update"))
                return;
    ...
    if (this.IsParameterBound(nameof(DeviceActionCategory)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.DeviceManagement.RemoteActionAudits[RemoteActionAuditId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.DeviceManagement.RemoteActionAudits[RemoteActionAuditId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, RemoteActionAuditId);
