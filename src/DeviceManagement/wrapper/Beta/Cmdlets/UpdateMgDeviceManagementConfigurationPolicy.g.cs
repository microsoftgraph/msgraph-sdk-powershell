...
{
    [GraphRoute("PATCH", "/deviceManagement/configurationPolicies/{deviceManagementConfigurationPolicy-id}")]
    [Cmdlet(VerbsData.Update, "MgDeviceManagementConfigurationPolicy", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.DeviceManagement.Client.Models.DeviceManagementConfigurationPolicy))]
    public class UpdateMgDeviceManagementConfigurationPolicyCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(DeviceManagementConfigurationPolicyId, "Update"))
                return;
    ...
    if (this.IsParameterBound(nameof(TemplateReference)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.DeviceManagement.ConfigurationPolicies[DeviceManagementConfigurationPolicyId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.DeviceManagement.ConfigurationPolicies[DeviceManagementConfigurationPolicyId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, DeviceManagementConfigurationPolicyId);
