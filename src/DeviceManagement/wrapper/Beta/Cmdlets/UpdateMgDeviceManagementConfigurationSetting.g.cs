...
{
    [GraphRoute("PATCH", "/deviceManagement/configurationSettings/{deviceManagementConfigurationSettingDefinition-id}")]
    [Cmdlet(VerbsData.Update, "MgDeviceManagementConfigurationSetting", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    ...
    public class UpdateMgDeviceManagementConfigurationSettingCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Visibility)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.DeviceManagement.ConfigurationSettings[DeviceManagementConfigurationSettingDefinitionId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.DeviceManagement.ConfigurationSettings[DeviceManagementConfigurationSettingDefinitionId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, DeviceManagementConfigurationSettingDefinitionId);
