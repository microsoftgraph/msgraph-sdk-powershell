...
{
    [GraphRoute("POST", "/deviceManagement/configurationPolicies/{deviceManagementConfigurationPolicy-id}/settings")]
    [Cmdlet(VerbsCommon.New, "MgDeviceManagementConfigurationPolicySetting", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.DeviceManagement.Client.Models.DeviceManagementConfigurationSetting))]
    public class NewMgDeviceManagementConfigurationPolicySettingCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(DeviceManagementConfigurationPolicyId, "New"))
                return;
    ...
    if (this.IsParameterBound(nameof(SettingInstance)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.DeviceManagement.ConfigurationPolicies[DeviceManagementConfigurationPolicyId].Settings.PostAsync(body, requestConfiguration =>
                {
