...
{
    [GraphRoute("PATCH", "/deviceManagement/macOSSoftwareUpdateAccountSummaries/{macOSSoftwareUpdateAccountSummary-id}/categorySummaries/{macOSSoftwareUpdateCategorySummary-id}/updateStateSummaries/{macOSSoftwareUpdateStateSummary-id}")]
    [Cmdlet(VerbsData.Update, "MgDeviceManagementMacOSSoftwareUpdateAccountSummaryCategorySummaryUpdateStateSummary", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.DeviceManagement.Client.Models.MacOSSoftwareUpdateStateSummary))]
    public class UpdateMgDeviceManagementMacOSSoftwareUpdateAccountSummaryCategorySummaryUpdateStateSummaryCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(MacOSSoftwareUpdateStateSummaryId, "Update"))
                return;
    ...
    if (this.IsParameterBound(nameof(UpdateCategory)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.DeviceManagement.MacOSSoftwareUpdateAccountSummaries[MacOSSoftwareUpdateAccountSummaryId].CategorySummaries[MacOSSoftwareUpdateCategorySummaryId].UpdateStateSummaries[MacOSSoftwareUpdateStateSummaryId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.DeviceManagement.MacOSSoftwareUpdateAccountSummaries[MacOSSoftwareUpdateAccountSummaryId].CategorySummaries[MacOSSoftwareUpdateCategorySummaryId].UpdateStateSummaries[MacOSSoftwareUpdateStateSummaryId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, MacOSSoftwareUpdateStateSummaryId);
