...
{
    [GraphRoute("PATCH", "/deviceManagement/templates/{deviceManagementTemplate-id}")]
    [Cmdlet(VerbsData.Update, "MgDeviceManagementTemplate", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.DeviceManagement.Client.Models.DeviceManagementTemplate))]
    public class UpdateMgDeviceManagementTemplateCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(DeviceManagementTemplateId, "Update"))
                return;
    ...
    if (this.IsParameterBound(nameof(TemplateType)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.DeviceManagement.Templates[DeviceManagementTemplateId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.DeviceManagement.Templates[DeviceManagementTemplateId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, DeviceManagementTemplateId);
