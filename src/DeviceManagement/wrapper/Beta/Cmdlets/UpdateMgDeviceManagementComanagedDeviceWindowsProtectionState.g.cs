...
{
    [GraphRoute("PATCH", "/deviceManagement/comanagedDevices/{managedDevice-id}/windowsProtectionState")]
    [Cmdlet(VerbsData.Update, "MgDeviceManagementComanagedDeviceWindowsProtectionState", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.DeviceManagement.Client.Models.WindowsProtectionState))]
    public class UpdateMgDeviceManagementComanagedDeviceWindowsProtectionStateCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(ProductStatus)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.DeviceManagement.ComanagedDevices[ManagedDeviceId].WindowsProtectionState.PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.DeviceManagement.ComanagedDevices[ManagedDeviceId].WindowsProtectionState.GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, ManagedDeviceId);
