...
{
    [GraphRoute("POST", "/deviceManagement/windowsQualityUpdateProfiles/{windowsQualityUpdateProfile-id}/assignments")]
    [Cmdlet(VerbsCommon.New, "MgDeviceManagementWindowsQualityUpdateProfileAssignment", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.DeviceManagement.Client.Models.WindowsQualityUpdateProfileAssignment))]
    public class NewMgDeviceManagementWindowsQualityUpdateProfileAssignmentCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(WindowsQualityUpdateProfileId, "New"))
                return;
    ...
    if (this.IsParameterBound(nameof(Target)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.DeviceManagement.WindowsQualityUpdateProfiles[WindowsQualityUpdateProfileId].Assignments.PostAsync(body, requestConfiguration =>
                {
