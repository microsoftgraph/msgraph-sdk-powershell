...
{
    [GraphRoute("PATCH", "/deviceManagement/deviceShellScripts/{deviceShellScript-id}/deviceRunStates/{deviceManagementScriptDeviceState-id}")]
    [Cmdlet(VerbsData.Update, "MgDeviceManagementDeviceShellScriptDeviceRunState", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.DeviceManagement.Client.Models.DeviceManagementScriptDeviceState))]
    public class UpdateMgDeviceManagementDeviceShellScriptDeviceRunStateCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(RunState)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.DeviceManagement.DeviceShellScripts[DeviceShellScriptId].DeviceRunStates[DeviceManagementScriptDeviceStateId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.DeviceManagement.DeviceShellScripts[DeviceShellScriptId].DeviceRunStates[DeviceManagementScriptDeviceStateId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, DeviceManagementScriptDeviceStateId);
