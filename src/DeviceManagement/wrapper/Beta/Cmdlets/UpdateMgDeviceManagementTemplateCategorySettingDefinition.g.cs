...
{
    [GraphRoute("PATCH", "/deviceManagement/templates/{deviceManagementTemplate-id}/categories/{deviceManagementTemplateSettingCategory-id}/settingDefinitions/{deviceManagementSettingDefinition-id}")]
    [Cmdlet(VerbsData.Update, "MgDeviceManagementTemplateCategorySettingDefinition", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.DeviceManagement.Client.Models.DeviceManagementSettingDefinition))]
    public class UpdateMgDeviceManagementTemplateCategorySettingDefinitionCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(ValueType)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.DeviceManagement.Templates[DeviceManagementTemplateId].Categories[DeviceManagementTemplateSettingCategoryId].SettingDefinitions[DeviceManagementSettingDefinitionId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.DeviceManagement.Templates[DeviceManagementTemplateId].Categories[DeviceManagementTemplateSettingCategoryId].SettingDefinitions[DeviceManagementSettingDefinitionId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, DeviceManagementSettingDefinitionId);
