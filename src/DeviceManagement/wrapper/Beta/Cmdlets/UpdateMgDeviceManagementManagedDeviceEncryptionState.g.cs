...
{
    [GraphRoute("PATCH", "/deviceManagement/managedDeviceEncryptionStates/{managedDeviceEncryptionState-id}")]
    [Cmdlet(VerbsData.Update, "MgDeviceManagementManagedDeviceEncryptionState", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.DeviceManagement.Client.Models.ManagedDeviceEncryptionState))]
    public class UpdateMgDeviceManagementManagedDeviceEncryptionStateCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(ManagedDeviceEncryptionStateId, "Update"))
                return;
    ...
    if (this.IsParameterBound(nameof(PolicyDetails)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.DeviceManagement.ManagedDeviceEncryptionStates[ManagedDeviceEncryptionStateId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.DeviceManagement.ManagedDeviceEncryptionStates[ManagedDeviceEncryptionStateId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, ManagedDeviceEncryptionStateId);
