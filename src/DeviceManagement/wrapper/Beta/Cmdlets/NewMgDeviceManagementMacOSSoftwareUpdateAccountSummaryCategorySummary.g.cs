...
{
    [GraphRoute("POST", "/deviceManagement/macOSSoftwareUpdateAccountSummaries/{macOSSoftwareUpdateAccountSummary-id}/categorySummaries")]
    [Cmdlet(VerbsCommon.New, "MgDeviceManagementMacOSSoftwareUpdateAccountSummaryCategorySummary", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.DeviceManagement.Client.Models.MacOSSoftwareUpdateCategorySummary))]
    public class NewMgDeviceManagementMacOSSoftwareUpdateAccountSummaryCategorySummaryCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(MacOSSoftwareUpdateAccountSummaryId, "New"))
                return;
    ...
    if (this.IsParameterBound(nameof(UpdateCategory)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.DeviceManagement.MacOSSoftwareUpdateAccountSummaries[MacOSSoftwareUpdateAccountSummaryId].CategorySummaries.PostAsync(body, requestConfiguration =>
                {
