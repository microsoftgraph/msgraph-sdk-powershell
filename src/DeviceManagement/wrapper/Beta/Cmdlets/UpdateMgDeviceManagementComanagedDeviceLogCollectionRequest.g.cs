...
{
    [GraphRoute("PATCH", "/deviceManagement/comanagedDevices/{managedDevice-id}/logCollectionRequests/{deviceLogCollectionResponse-id}")]
    [Cmdlet(VerbsData.Update, "MgDeviceManagementComanagedDeviceLogCollectionRequest", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.DeviceManagement.Client.Models.DeviceLogCollectionResponse))]
    public class UpdateMgDeviceManagementComanagedDeviceLogCollectionRequestCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Status)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.DeviceManagement.ComanagedDevices[ManagedDeviceId].LogCollectionRequests[DeviceLogCollectionResponseId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.DeviceManagement.ComanagedDevices[ManagedDeviceId].LogCollectionRequests[DeviceLogCollectionResponseId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, DeviceLogCollectionResponseId);
