...
{
    [GraphRoute("PATCH", "/deviceManagement/reusableSettings/{deviceManagementConfigurationSettingDefinition-id}")]
    [Cmdlet(VerbsData.Update, "MgDeviceManagementReusableSetting", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    ...
    public class UpdateMgDeviceManagementReusableSettingCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Visibility)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.DeviceManagement.ReusableSettings[DeviceManagementConfigurationSettingDefinitionId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.DeviceManagement.ReusableSettings[DeviceManagementConfigurationSettingDefinitionId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, DeviceManagementConfigurationSettingDefinitionId);
