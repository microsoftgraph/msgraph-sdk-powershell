...
{
    [GraphRoute("POST", "/deviceManagement/intents/{deviceManagementIntent-id}/categories/{deviceManagementIntentSettingCategory-id}/settings")]
    [Cmdlet(VerbsCommon.New, "MgDeviceManagementIntentCategorySetting", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.DeviceManagement.Client.Models.DeviceManagementSettingInstance))]
    public class NewMgDeviceManagementIntentCategorySettingCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(DeviceManagementIntentSettingCategoryId, "New"))
                return;
    ...
    if (this.IsParameterBound(nameof(ValueJson)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.DeviceManagement.Intents[DeviceManagementIntentId].Categories[DeviceManagementIntentSettingCategoryId].Settings.PostAsync(body, requestConfiguration =>
                {
