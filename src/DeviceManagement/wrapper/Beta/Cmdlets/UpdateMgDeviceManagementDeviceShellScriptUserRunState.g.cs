...
{
    [GraphRoute("PATCH", "/deviceManagement/deviceShellScripts/{deviceShellScript-id}/userRunStates/{deviceManagementScriptUserState-id}")]
    [Cmdlet(VerbsData.Update, "MgDeviceManagementDeviceShellScriptUserRunState", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.DeviceManagement.Client.Models.DeviceManagementScriptUserState))]
    public class UpdateMgDeviceManagementDeviceShellScriptUserRunStateCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(UserPrincipalName)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.DeviceManagement.DeviceShellScripts[DeviceShellScriptId].UserRunStates[DeviceManagementScriptUserStateId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.DeviceManagement.DeviceShellScripts[DeviceShellScriptId].UserRunStates[DeviceManagementScriptUserStateId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, DeviceManagementScriptUserStateId);
