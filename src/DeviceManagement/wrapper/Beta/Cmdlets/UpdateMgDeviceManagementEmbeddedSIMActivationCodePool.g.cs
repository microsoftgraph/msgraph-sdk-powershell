...
{
    [GraphRoute("PATCH", "/deviceManagement/embeddedSIMActivationCodePools/{embeddedSIMActivationCodePool-id}")]
    [Cmdlet(VerbsData.Update, "MgDeviceManagementEmbeddedSIMActivationCodePool", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.DeviceManagement.Client.Models.EmbeddedSIMActivationCodePool))]
    public class UpdateMgDeviceManagementEmbeddedSIMActivationCodePoolCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(EmbeddedSIMActivationCodePoolId, "Update"))
                return;
    ...
    if (this.IsParameterBound(nameof(ActivationCodes)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.DeviceManagement.EmbeddedSIMActivationCodePools[EmbeddedSIMActivationCodePoolId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.DeviceManagement.EmbeddedSIMActivationCodePools[EmbeddedSIMActivationCodePoolId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, EmbeddedSIMActivationCodePoolId);
