...
{
    [GraphRoute("PATCH", "/deviceManagement/deviceManagementScripts/{deviceManagementScript-id}/userRunStates/{deviceManagementScriptUserState-id}/deviceRunStates/{deviceManagementScriptDeviceState-id}")]
    [Cmdlet(VerbsData.Update, "MgDeviceManagementDeviceManagementScriptUserRunStateDeviceRunState", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.DeviceManagement.Client.Models.DeviceManagementScriptDeviceState))]
    public class UpdateMgDeviceManagementDeviceManagementScriptUserRunStateDeviceRunStateCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(RunState)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.DeviceManagement.DeviceManagementScripts[DeviceManagementScriptId].UserRunStates[DeviceManagementScriptUserStateId].DeviceRunStates[DeviceManagementScriptDeviceStateId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.DeviceManagement.DeviceManagementScripts[DeviceManagementScriptId].UserRunStates[DeviceManagementScriptUserStateId].DeviceRunStates[DeviceManagementScriptDeviceStateId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, DeviceManagementScriptDeviceStateId);
