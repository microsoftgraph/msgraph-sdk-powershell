...
{
    [GraphRoute("PATCH", "/deviceManagement/derivedCredentials/{deviceManagementDerivedCredentialSettings-id}")]
    [Cmdlet(VerbsData.Update, "MgDeviceManagementDerivedCredential", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.DeviceManagement.Client.Models.DeviceManagementDerivedCredentialSettings))]
    public class UpdateMgDeviceManagementDerivedCredentialCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(DeviceManagementDerivedCredentialSettingsId, "Update"))
                return;
    ...
    if (this.IsParameterBound(nameof(NotificationType)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.DeviceManagement.DerivedCredentials[DeviceManagementDerivedCredentialSettingsId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.DeviceManagement.DerivedCredentials[DeviceManagementDerivedCredentialSettingsId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, DeviceManagementDerivedCredentialSettingsId);
