...
{
    [GraphRoute("PATCH", "/deviceManagement/intents/{deviceManagementIntent-id}/categories/{deviceManagementIntentSettingCategory-id}/settingDefinitions/{deviceManagementSettingDefinition-id}")]
    [Cmdlet(VerbsData.Update, "MgDeviceManagementIntentCategorySettingDefinition", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.DeviceManagement.Client.Models.DeviceManagementSettingDefinition))]
    public class UpdateMgDeviceManagementIntentCategorySettingDefinitionCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(ValueType)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.DeviceManagement.Intents[DeviceManagementIntentId].Categories[DeviceManagementIntentSettingCategoryId].SettingDefinitions[DeviceManagementSettingDefinitionId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.DeviceManagement.Intents[DeviceManagementIntentId].Categories[DeviceManagementIntentSettingCategoryId].SettingDefinitions[DeviceManagementSettingDefinitionId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, DeviceManagementSettingDefinitionId);
