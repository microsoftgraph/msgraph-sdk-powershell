...
{
    [GraphRoute("PATCH", "/deviceManagement/deviceConfigurations/{deviceConfiguration-id}/deviceSettingStateSummaries/{settingStateDeviceSummary-id}")]
    [Cmdlet(VerbsData.Update, "MgDeviceManagementDeviceConfigurationDeviceSettingStateSummary", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.DeviceManagement.Client.Models.SettingStateDeviceSummary))]
    public class UpdateMgDeviceManagementDeviceConfigurationDeviceSettingStateSummaryCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(UnknownDeviceCount)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.DeviceManagement.DeviceConfigurations[DeviceConfigurationId].DeviceSettingStateSummaries[SettingStateDeviceSummaryId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.DeviceManagement.DeviceConfigurations[DeviceConfigurationId].DeviceSettingStateSummaries[SettingStateDeviceSummaryId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, SettingStateDeviceSummaryId);
