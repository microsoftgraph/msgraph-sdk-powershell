...
{
    [GraphRoute("PATCH", "/deviceManagement/managedDevices/{managedDevice-id}/securityBaselineStates/{securityBaselineState-id}/settingStates/{securityBaselineSettingState-id}")]
    [Cmdlet(VerbsData.Update, "MgDeviceManagementManagedDeviceSecurityBaselineStateSettingState", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.DeviceManagement.Client.Models.SecurityBaselineSettingState))]
    public class UpdateMgDeviceManagementManagedDeviceSecurityBaselineStateSettingStateCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(State)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.DeviceManagement.ManagedDevices[ManagedDeviceId].SecurityBaselineStates[SecurityBaselineStateId].SettingStates[SecurityBaselineSettingStateId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.DeviceManagement.ManagedDevices[ManagedDeviceId].SecurityBaselineStates[SecurityBaselineStateId].SettingStates[SecurityBaselineSettingStateId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, SecurityBaselineSettingStateId);
