...
{
    [GraphRoute("PATCH", "/deviceManagement/deviceHealthScripts/{deviceHealthScript-id}")]
    [Cmdlet(VerbsData.Update, "MgDeviceManagementDeviceHealthScript", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.DeviceManagement.Client.Models.DeviceHealthScript))]
    public class UpdateMgDeviceManagementDeviceHealthScriptCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(RunAsAccount)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.DeviceManagement.DeviceHealthScripts[DeviceHealthScriptId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.DeviceManagement.DeviceHealthScripts[DeviceHealthScriptId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, DeviceHealthScriptId);
