...
{
    [GraphRoute("PATCH", "/deviceManagement/comanagedDevices/{managedDevice-id}/securityBaselineStates/{securityBaselineState-id}/settingStates/{securityBaselineSettingState-id}")]
    [Cmdlet(VerbsData.Update, "MgDeviceManagementComanagedDeviceSecurityBaselineStateSettingState", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.DeviceManagement.Client.Models.SecurityBaselineSettingState))]
    public class UpdateMgDeviceManagementComanagedDeviceSecurityBaselineStateSettingStateCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(SecurityBaselineSettingStateId, "Update"))
                return;
    ...
    if (this.IsParameterBound(nameof(State)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.DeviceManagement.ComanagedDevices[ManagedDeviceId].SecurityBaselineStates[SecurityBaselineStateId].SettingStates[SecurityBaselineSettingStateId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.DeviceManagement.ComanagedDevices[ManagedDeviceId].SecurityBaselineStates[SecurityBaselineStateId].SettingStates[SecurityBaselineSettingStateId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, SecurityBaselineSettingStateId);
