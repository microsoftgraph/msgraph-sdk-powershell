...
{
    [GraphRoute("PATCH", "/deviceManagement/microsoftTunnelConfigurations/{microsoftTunnelConfiguration-id}")]
    [Cmdlet(VerbsData.Update, "MgDeviceManagementMicrosoftTunnelConfiguration", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.DeviceManagement.Client.Models.MicrosoftTunnelConfiguration))]
    public class UpdateMgDeviceManagementMicrosoftTunnelConfigurationCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(MicrosoftTunnelConfigurationId, "Update"))
                return;
    ...
    if (this.IsParameterBound(nameof(AdvancedSettings)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.DeviceManagement.MicrosoftTunnelConfigurations[MicrosoftTunnelConfigurationId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.DeviceManagement.MicrosoftTunnelConfigurations[MicrosoftTunnelConfigurationId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, MicrosoftTunnelConfigurationId);
