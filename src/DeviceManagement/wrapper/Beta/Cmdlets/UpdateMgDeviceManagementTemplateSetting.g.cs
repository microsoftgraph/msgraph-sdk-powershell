...
{
    [GraphRoute("PATCH", "/deviceManagement/templates/{deviceManagementTemplate-id}/settings/{deviceManagementSettingInstance-id}")]
    [Cmdlet(VerbsData.Update, "MgDeviceManagementTemplateSetting", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.DeviceManagement.Client.Models.DeviceManagementSettingInstance))]
    public class UpdateMgDeviceManagementTemplateSettingCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(ValueJson)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.DeviceManagement.Templates[DeviceManagementTemplateId].Settings[DeviceManagementSettingInstanceId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.DeviceManagement.Templates[DeviceManagementTemplateId].Settings[DeviceManagementSettingInstanceId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, DeviceManagementSettingInstanceId);
