...
{
    [GraphRoute("PATCH", "/deviceManagement/assignmentFilters/{deviceAndAppManagementAssignmentFilter-id}")]
    [Cmdlet(VerbsData.Update, "MgDeviceManagementAssignmentFilter", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.DeviceManagement.Client.Models.DeviceAndAppManagementAssignmentFilter))]
    public class UpdateMgDeviceManagementAssignmentFilterCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(DeviceAndAppManagementAssignmentFilterId, "Update"))
                return;
    ...
    if (this.IsParameterBound(nameof(Platform)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.DeviceManagement.AssignmentFilters[DeviceAndAppManagementAssignmentFilterId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.DeviceManagement.AssignmentFilters[DeviceAndAppManagementAssignmentFilterId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, DeviceAndAppManagementAssignmentFilterId);
