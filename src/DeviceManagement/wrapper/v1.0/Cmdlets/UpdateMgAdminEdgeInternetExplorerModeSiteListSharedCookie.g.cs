...
{
    [GraphRoute("PATCH", "/admin/edge/internetExplorerMode/siteLists/{browserSiteList-id}/sharedCookies/{browserSharedCookie-id}")]
    [Cmdlet(VerbsData.Update, "MgAdminEdgeInternetExplorerModeSiteListSharedCookie", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.DeviceManagement.Client.Models.BrowserSharedCookie))]
    public class UpdateMgAdminEdgeInternetExplorerModeSiteListSharedCookieCommand : PSCmdlet
    {
        ...
        {
            if (!ShouldProcess(BrowserSharedCookieId, "Update"))
                return;
    ...
    if (this.IsParameterBound(nameof(Status)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Admin.Edge.InternetExplorerMode.SiteLists[BrowserSiteListId].SharedCookies[BrowserSharedCookieId].PatchAsync(body, requestConfiguration =>
                {
            ...
            {
                ThrowTerminatingError(new ErrorRecord(ex, "GraphRequestFailed", ErrorCategory.InvalidOperation, BrowserSharedCookieId));
                return;
            }
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Admin.Edge.InternetExplorerMode.SiteLists[BrowserSiteListId].SharedCookies[BrowserSharedCookieId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex)
                {
                    ThrowTerminatingError(new ErrorRecord(ex, "GraphRequestFailed", ErrorCategory.InvalidOperation, BrowserSharedCookieId));
