...
#>
function Select-MgBetaEntitlementManagementAccessPackage {
[OutputType([Microsoft.Graph.Beta.PowerShell.Models.IMicrosoftGraphAccessPackage])]
[CmdletBinding(PositionalBinding=$false, ConfirmImpact='Medium')]
param(
    [Parameter (ValueFromPipeline=$true)]
    [Microsoft.Graph.Beta.PowerShell.Models.MicrosoftGraphAccessPackage[]]$AccessPackage,
...
process {
    ...
    if ($policyEvaluation) {
        ...
        foreach ($p in $AccessPackage.AccessPackageAssignmentPolicies) {
            ...
            $thisMatch = @(Select-MgBetaEntitlementManagementAccessPackageAssignmentPolicy -ScopeType $PolicyWithScopeType -NoApprovalRequiredForRequest:$PolicyWithNoApprovalRequiredForRequest -Policy $p)
