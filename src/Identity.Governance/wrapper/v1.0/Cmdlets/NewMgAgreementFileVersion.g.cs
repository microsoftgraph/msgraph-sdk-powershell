...
{
    [GraphRoute("POST", "/agreements/{agreement-id}/files/{agreementFileLocalization-id}/versions")]
    [Cmdlet(VerbsCommon.New, "MgAgreementFileVersion", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.Governance.Client.Models.AgreementFileVersion))]
    public class NewMgAgreementFileVersionCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(FileData)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Agreements[AgreementId].Files[AgreementFileLocalizationId].Versions.PostAsync(body, requestConfiguration =>
                {
