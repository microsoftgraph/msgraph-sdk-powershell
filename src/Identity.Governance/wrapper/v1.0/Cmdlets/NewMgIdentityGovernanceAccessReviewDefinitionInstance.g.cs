...
{
    [GraphRoute("POST", "/identityGovernance/accessReviews/definitions/{accessReviewScheduleDefinition-id}/instances")]
    [Cmdlet(VerbsCommon.New, "MgIdentityGovernanceAccessReviewDefinitionInstance", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.Governance.Client.Models.AccessReviewInstance))]
    public class NewMgIdentityGovernanceAccessReviewDefinitionInstanceCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(AccessReviewScheduleDefinitionId, "New"))
                return;
    ...
    if (this.IsParameterBound(nameof(Scope)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.IdentityGovernance.AccessReviews.Definitions[AccessReviewScheduleDefinitionId].Instances.PostAsync(body, requestConfiguration =>
                {
