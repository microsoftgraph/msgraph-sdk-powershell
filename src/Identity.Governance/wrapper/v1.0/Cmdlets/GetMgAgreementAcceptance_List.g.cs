...
{
    [GraphRoute("GET", "/agreements/{agreement-id}/acceptances")]
    [Cmdlet(VerbsCommon.Get, "MgAgreementAcceptance_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.Governance.Client.Models.AgreementAcceptance))]
    public class GetMgAgreementAcceptance_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Agreements[AgreementId].Acceptances.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Agreements[AgreementId].Acceptances.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
