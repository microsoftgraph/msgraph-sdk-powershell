...
{
    [GraphRoute("POST", "/identityGovernance/entitlementManagement/resourceEnvironments/{accessPackageResourceEnvironment-id}/resources")]
    [Cmdlet(VerbsCommon.New, "MgEntitlementManagementResourceEnvironmentResource", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.Governance.Client.Models.AccessPackageResource))]
    public class NewMgEntitlementManagementResourceEnvironmentResourceCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(AccessPackageResourceEnvironmentId, "New"))
                return;
    ...
    if (this.IsParameterBound(nameof(Attributes)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.IdentityGovernance.EntitlementManagement.ResourceEnvironments[AccessPackageResourceEnvironmentId].Resources.PostAsync(body, requestConfiguration =>
                {
