...
{
    [GraphRoute("PATCH", "/identityGovernance/privilegedAccess/group/assignmentSchedules/{privilegedAccessGroupAssignmentSchedule-id}")]
    [Cmdlet(VerbsData.Update, "MgIdentityGovernancePrivilegedAccessGroupAssignmentSchedule", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.Governance.Client.Models.PrivilegedAccessGroupAssignmentSchedule))]
    public class UpdateMgIdentityGovernancePrivilegedAccessGroupAssignmentScheduleCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(PrivilegedAccessGroupAssignmentScheduleId, "Update"))
                return;
    ...
    if (this.IsParameterBound(nameof(MemberType)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.IdentityGovernance.PrivilegedAccess.Group.AssignmentSchedules[PrivilegedAccessGroupAssignmentScheduleId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.IdentityGovernance.PrivilegedAccess.Group.AssignmentSchedules[PrivilegedAccessGroupAssignmentScheduleId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException)
                {
                    ThrowGraphRequestFailed(ex, PrivilegedAccessGroupAssignmentScheduleId);
