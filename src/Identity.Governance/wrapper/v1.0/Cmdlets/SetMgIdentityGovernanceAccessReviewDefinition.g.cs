...
{
    [GraphRoute("PUT", "/identityGovernance/accessReviews/definitions/{accessReviewScheduleDefinition-id}")]
    [Cmdlet(VerbsCommon.Set, "MgIdentityGovernanceAccessReviewDefinition", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.Governance.Client.Models.AccessReviewScheduleDefinition))]
    public class SetMgIdentityGovernanceAccessReviewDefinitionCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(AccessReviewScheduleDefinitionId, "Set"))
                return;
    ...
    if (this.IsParameterBound(nameof(StageSettings)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.IdentityGovernance.AccessReviews.Definitions[AccessReviewScheduleDefinitionId].PutAsync(body, requestConfiguration =>
                {
