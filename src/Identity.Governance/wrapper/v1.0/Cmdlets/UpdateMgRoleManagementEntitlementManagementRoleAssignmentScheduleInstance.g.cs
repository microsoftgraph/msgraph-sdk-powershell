...
{
    [GraphRoute("PATCH", "/roleManagement/entitlementManagement/roleAssignmentScheduleInstances/{unifiedRoleAssignmentScheduleInstance-id}")]
    [Cmdlet(VerbsData.Update, "MgRoleManagementEntitlementManagementRoleAssignmentScheduleInstance", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.Governance.Client.Models.UnifiedRoleAssignmentScheduleInstance))]
    public class UpdateMgRoleManagementEntitlementManagementRoleAssignmentScheduleInstanceCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(StartDateTime)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.RoleManagement.EntitlementManagement.RoleAssignmentScheduleInstances[UnifiedRoleAssignmentScheduleInstanceId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.RoleManagement.EntitlementManagement.RoleAssignmentScheduleInstances[UnifiedRoleAssignmentScheduleInstanceId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, UnifiedRoleAssignmentScheduleInstanceId);
