...
{
    [GraphRoute("PATCH", "/identityGovernance/catalogs/{accessPackageCatalog-id}/customWorkflowExtensions/{customCalloutExtension-id}")]
    [Cmdlet(VerbsData.Update, "MgIdentityGovernanceCatalogCustomWorkflowExtension", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.Governance.Client.Models.CustomCalloutExtension))]
    public class UpdateMgIdentityGovernanceCatalogCustomWorkflowExtensionCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(EndpointConfiguration)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.IdentityGovernance.Catalogs[AccessPackageCatalogId].CustomWorkflowExtensions[CustomCalloutExtensionId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.IdentityGovernance.Catalogs[AccessPackageCatalogId].CustomWorkflowExtensions[CustomCalloutExtensionId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, CustomCalloutExtensionId);
