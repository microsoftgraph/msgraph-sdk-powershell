...
{
    [GraphRoute("POST", "/roleManagement/entitlementManagement/roleDefinitions/{unifiedRoleDefinition-id}/inheritsPermissionsFrom")]
    [Cmdlet(VerbsCommon.New, "MgRoleManagementEntitlementManagementRoleDefinitionInheritPermissionFrom", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.Governance.Client.Models.UnifiedRoleDefinition))]
    public class NewMgRoleManagementEntitlementManagementRoleDefinitionInheritPermissionFromCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(RolePermissions)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.RoleManagement.EntitlementManagement.RoleDefinitions[UnifiedRoleDefinitionId].InheritsPermissionsFrom.PostAsync(body, requestConfiguration =>
                {
