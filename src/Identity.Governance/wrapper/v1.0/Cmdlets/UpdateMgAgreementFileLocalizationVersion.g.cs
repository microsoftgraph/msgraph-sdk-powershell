...
{
    [GraphRoute("PATCH", "/agreements/{agreement-id}/file/localizations/{agreementFileLocalization-id}/versions/{agreementFileVersion-id}")]
    [Cmdlet(VerbsData.Update, "MgAgreementFileLocalizationVersion", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.Governance.Client.Models.AgreementFileVersion))]
    public class UpdateMgAgreementFileLocalizationVersionCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(FileData)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Agreements[AgreementId].File.Localizations[AgreementFileLocalizationId].Versions[AgreementFileVersionId].PatchAsync(body, requestConfiguration =>
                {
