...
{
    [GraphRoute("POST", "/identityGovernance/entitlementManagement/catalogs/{accessPackageCatalog-id}/resourceRoles/{accessPackageResourceRole-id}/resource/scopes")]
    [Cmdlet(VerbsCommon.New, "MgEntitlementManagementCatalogResourceRoleResourceScope", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.Governance.Client.Models.AccessPackageResourceScope))]
    public class NewMgEntitlementManagementCatalogResourceRoleResourceScopeCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(OriginSystem)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.IdentityGovernance.EntitlementManagement.Catalogs[AccessPackageCatalogId].ResourceRoles[AccessPackageResourceRoleId].Resource.Scopes.PostAsync(body, requestConfiguration =>
                {
