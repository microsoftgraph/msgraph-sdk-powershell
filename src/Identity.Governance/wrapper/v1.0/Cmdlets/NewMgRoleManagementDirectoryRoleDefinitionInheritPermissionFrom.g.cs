...
{
    [GraphRoute("POST", "/roleManagement/directory/roleDefinitions/{unifiedRoleDefinition-id}/inheritsPermissionsFrom")]
    [Cmdlet(VerbsCommon.New, "MgRoleManagementDirectoryRoleDefinitionInheritPermissionFrom", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.Governance.Client.Models.UnifiedRoleDefinition))]
    public class NewMgRoleManagementDirectoryRoleDefinitionInheritPermissionFromCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(UnifiedRoleDefinitionId, "New"))
                return;
    ...
    if (this.IsParameterBound(nameof(RolePermissions)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.RoleManagement.Directory.RoleDefinitions[UnifiedRoleDefinitionId].InheritsPermissionsFrom.PostAsync(body, requestConfiguration =>
                {
