...
{
    [GraphRoute("POST", "/identityGovernance/accessReviews/unified/instances/{accessReviewInstance-id}/stages/{accessReviewStage-id}/decisions/{accessReviewInstanceDecisionItem-id}/insights")]
    [Cmdlet(VerbsCommon.New, "MgIdentityGovernanceAccessReviewUnifiedInstanceStageDecisionInsight", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.Governance.Client.Models.GovernanceInsight))]
    public class NewMgIdentityGovernanceAccessReviewUnifiedInstanceStageDecisionInsightCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(InsightCreatedDateTime)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.IdentityGovernance.AccessReviews.Unified.Instances[AccessReviewInstanceId].Stages[AccessReviewStageId].Decisions[AccessReviewInstanceDecisionItemId].Insights.PostAsync(body, requestConfiguration =>
                {
