...
{
    [GraphRoute("PATCH", "/agreements/{agreement-id}/files/{agreementFileLocalization-id}/versions/{agreementFileVersion-id}")]
    [Cmdlet(VerbsData.Update, "MgAgreementFileVersion", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.Governance.Client.Models.AgreementFileVersion))]
    public class UpdateMgAgreementFileVersionCommand : PSCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(FileData)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Agreements[AgreementId].Files[AgreementFileLocalizationId].Versions[AgreementFileVersionId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Agreements[AgreementId].Files[AgreementFileLocalizationId].Versions[AgreementFileVersionId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex)
                {
                    ThrowTerminatingError(new ErrorRecord(ex, "GraphRequestFailed", ErrorCategory.InvalidOperation, AgreementFileVersionId));
