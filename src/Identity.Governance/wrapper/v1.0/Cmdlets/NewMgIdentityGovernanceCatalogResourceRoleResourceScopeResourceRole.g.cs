...
{
    [GraphRoute("POST", "/identityGovernance/catalogs/{accessPackageCatalog-id}/resourceRoles/{accessPackageResourceRole-id}/resource/scopes/{accessPackageResourceScope-id}/resource/roles")]
    [Cmdlet(VerbsCommon.New, "MgIdentityGovernanceCatalogResourceRoleResourceScopeResourceRole", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.Governance.Client.Models.AccessPackageResourceRole))]
    public class NewMgIdentityGovernanceCatalogResourceRoleResourceScopeResourceRoleCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Type)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.IdentityGovernance.Catalogs[AccessPackageCatalogId].ResourceRoles[AccessPackageResourceRoleId].Resource.Scopes[AccessPackageResourceScopeId].Resource.Roles.PostAsync(body, requestConfiguration =>
                {
