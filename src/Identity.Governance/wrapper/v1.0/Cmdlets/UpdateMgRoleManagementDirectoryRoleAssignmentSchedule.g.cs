...
{
    [GraphRoute("PATCH", "/roleManagement/directory/roleAssignmentSchedules/{unifiedRoleAssignmentSchedule-id}")]
    [Cmdlet(VerbsData.Update, "MgRoleManagementDirectoryRoleAssignmentSchedule", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.Governance.Client.Models.UnifiedRoleAssignmentSchedule))]
    public class UpdateMgRoleManagementDirectoryRoleAssignmentScheduleCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(ScheduleInfo)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.RoleManagement.Directory.RoleAssignmentSchedules[UnifiedRoleAssignmentScheduleId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.RoleManagement.Directory.RoleAssignmentSchedules[UnifiedRoleAssignmentScheduleId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex)
                {
                    ThrowGraphRequestFailed(ex, UnifiedRoleAssignmentScheduleId);
