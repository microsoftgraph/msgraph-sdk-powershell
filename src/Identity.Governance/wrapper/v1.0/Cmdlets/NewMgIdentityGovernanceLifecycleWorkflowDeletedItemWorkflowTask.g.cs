...
{
    [GraphRoute("POST", "/identityGovernance/lifecycleWorkflows/deletedItems/workflows/{workflow-id}/tasks")]
    [Cmdlet(VerbsCommon.New, "MgIdentityGovernanceLifecycleWorkflowDeletedItemWorkflowTask", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.Governance.Client.Models.IdentityGovernance.TaskObject))]
    public class NewMgIdentityGovernanceLifecycleWorkflowDeletedItemWorkflowTaskCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(WorkflowId, "New"))
                return;
    ...
    if (this.IsParameterBound(nameof(Category)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.IdentityGovernance.LifecycleWorkflows.DeletedItems.Workflows[WorkflowId].Tasks.PostAsync(body, requestConfiguration =>
                {
