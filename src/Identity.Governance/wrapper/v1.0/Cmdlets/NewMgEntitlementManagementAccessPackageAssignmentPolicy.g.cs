...
{
    [GraphRoute("POST", "/identityGovernance/entitlementManagement/accessPackages/{accessPackage-id}/assignmentPolicies")]
    [Cmdlet(VerbsCommon.New, "MgEntitlementManagementAccessPackageAssignmentPolicy", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.Governance.Client.Models.AccessPackageAssignmentPolicy))]
    public class NewMgEntitlementManagementAccessPackageAssignmentPolicyCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(SpecificAllowedTargets)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.IdentityGovernance.EntitlementManagement.AccessPackages[AccessPackageId].AssignmentPolicies.PostAsync(body, requestConfiguration =>
                {
