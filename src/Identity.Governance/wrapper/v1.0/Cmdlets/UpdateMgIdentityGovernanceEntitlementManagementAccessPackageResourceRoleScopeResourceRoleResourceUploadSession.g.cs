...
{
    [GraphRoute("PATCH", "/identityGovernance/entitlementManagement/accessPackages/{accessPackage-id}/resourceRoleScopes/{accessPackageResourceRoleScope-id}/scope/resource/roles/{accessPackageResourceRole-id}/resource/uploadSessions/{customDataProvidedResourceUploadSession-id}")]
    [Cmdlet(VerbsData.Update, "MgIdentityGovernanceEntitlementManagementAccessPackageResourceRoleScopeResourceRoleResourceUploadSession", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.Governance.Client.Models.CustomDataProvidedResourceUploadSession))]
    public class UpdateMgIdentityGovernanceEntitlementManagementAccessPackageResourceRoleScopeResourceRoleResourceUploadSessionCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Status)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.IdentityGovernance.EntitlementManagement.AccessPackages[AccessPackageId].ResourceRoleScopes[AccessPackageResourceRoleScopeId].Scope.Resource.Roles[AccessPackageResourceRoleId].Resource.UploadSessions[CustomDataProvidedResourceUploadSessionId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.IdentityGovernance.EntitlementManagement.AccessPackages[AccessPackageId].ResourceRoleScopes[AccessPackageResourceRoleScopeId].Scope.Resource.Roles[AccessPackageResourceRoleId].Resource.UploadSessions[CustomDataProvidedResourceUploadSessionId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, CustomDataProvidedResourceUploadSessionId);
