...
{
    [GraphRoute("PATCH", "/roleManagement/entitlementManagement/roleDefinitions/{unifiedRoleDefinition-id}/inheritsPermissionsFrom/{unifiedRoleDefinition-id1}")]
    [Cmdlet(VerbsData.Update, "MgRoleManagementEntitlementManagementRoleDefinitionInheritPermissionFrom", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.Governance.Client.Models.UnifiedRoleDefinition))]
    public class UpdateMgRoleManagementEntitlementManagementRoleDefinitionInheritPermissionFromCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(RolePermissions)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.RoleManagement.EntitlementManagement.RoleDefinitions[UnifiedRoleDefinitionId].InheritsPermissionsFrom[UnifiedRoleDefinitionId1].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.RoleManagement.EntitlementManagement.RoleDefinitions[UnifiedRoleDefinitionId].InheritsPermissionsFrom[UnifiedRoleDefinitionId1].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException)
                {
                    ThrowGraphRequestFailed(ex, UnifiedRoleDefinitionId1);
