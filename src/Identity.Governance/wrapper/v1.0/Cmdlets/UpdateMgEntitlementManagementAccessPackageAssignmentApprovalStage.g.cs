...
{
    [GraphRoute("PATCH", "/identityGovernance/entitlementManagement/accessPackageAssignmentApprovals/{approval-id}/stages/{approvalStage-id}")]
    [Cmdlet(VerbsData.Update, "MgEntitlementManagementAccessPackageAssignmentApprovalStage", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.Governance.Client.Models.ApprovalStage))]
    public class UpdateMgEntitlementManagementAccessPackageAssignmentApprovalStageCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(ReviewedBy)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.IdentityGovernance.EntitlementManagement.AccessPackageAssignmentApprovals[ApprovalId].Stages[ApprovalStageId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.IdentityGovernance.EntitlementManagement.AccessPackageAssignmentApprovals[ApprovalId].Stages[ApprovalStageId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException)
                {
                    ThrowGraphRequestFailed(ex, ApprovalStageId);
