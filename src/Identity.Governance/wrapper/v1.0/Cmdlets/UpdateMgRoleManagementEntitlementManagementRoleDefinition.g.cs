...
{
    [GraphRoute("PATCH", "/roleManagement/entitlementManagement/roleDefinitions/{unifiedRoleDefinition-id}")]
    [Cmdlet(VerbsData.Update, "MgRoleManagementEntitlementManagementRoleDefinition", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.Governance.Client.Models.UnifiedRoleDefinition))]
    public class UpdateMgRoleManagementEntitlementManagementRoleDefinitionCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(RolePermissions)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.RoleManagement.EntitlementManagement.RoleDefinitions[UnifiedRoleDefinitionId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.RoleManagement.EntitlementManagement.RoleDefinitions[UnifiedRoleDefinitionId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex)
                {
                    ThrowGraphRequestFailed(ex, UnifiedRoleDefinitionId);
