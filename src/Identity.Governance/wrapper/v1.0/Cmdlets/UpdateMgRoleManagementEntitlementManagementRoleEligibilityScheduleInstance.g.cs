...
{
    [GraphRoute("PATCH", "/roleManagement/entitlementManagement/roleEligibilityScheduleInstances/{unifiedRoleEligibilityScheduleInstance-id}")]
    [Cmdlet(VerbsData.Update, "MgRoleManagementEntitlementManagementRoleEligibilityScheduleInstance", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.Governance.Client.Models.UnifiedRoleEligibilityScheduleInstance))]
    public class UpdateMgRoleManagementEntitlementManagementRoleEligibilityScheduleInstanceCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(StartDateTime)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.RoleManagement.EntitlementManagement.RoleEligibilityScheduleInstances[UnifiedRoleEligibilityScheduleInstanceId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.RoleManagement.EntitlementManagement.RoleEligibilityScheduleInstances[UnifiedRoleEligibilityScheduleInstanceId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException)
                {
                    ThrowGraphRequestFailed(ex, UnifiedRoleEligibilityScheduleInstanceId);
