...
{
    [GraphRoute("PATCH", "/identityGovernance/lifecycleWorkflows/workflows/{workflow-id}/subjectProcessingResults/{subjectProcessingResult-id}/taskProcessingResults/{taskProcessingResult-id}/subject/mailboxSettings")]
    [Cmdlet(VerbsData.Update, "MgIdentityGovernanceLifecycleWorkflowSubjectProcessingResultTaskProcessingResultSubjectMailboxSetting", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.Governance.Client.Models.MailboxSettings))]
    public class UpdateMgIdentityGovernanceLifecycleWorkflowSubjectProcessingResultTaskProcessingResultSubjectMailboxSettingCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(WorkingHours)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.IdentityGovernance.LifecycleWorkflows.Workflows[WorkflowId].SubjectProcessingResults[SubjectProcessingResultId].TaskProcessingResults[TaskProcessingResultId].Subject.MailboxSettings.PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.IdentityGovernance.LifecycleWorkflows.Workflows[WorkflowId].SubjectProcessingResults[SubjectProcessingResultId].TaskProcessingResults[TaskProcessingResultId].Subject.MailboxSettings.GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, TaskProcessingResultId);
