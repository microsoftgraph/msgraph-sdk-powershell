...
{
    [GraphRoute("PATCH", "/identityGovernance/entitlementManagement/catalogs/{accessPackageCatalog-id}")]
    [Cmdlet(VerbsData.Update, "MgEntitlementManagementCatalog", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.Governance.Client.Models.AccessPackageCatalog))]
    public class UpdateMgEntitlementManagementCatalogCommand : PSCmdlet
    {
        ...
        {
            if (!ShouldProcess(AccessPackageCatalogId, "Update"))
                return;
    ...
    if (this.IsParameterBound(nameof(State)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.IdentityGovernance.EntitlementManagement.Catalogs[AccessPackageCatalogId].PatchAsync(body, requestConfiguration =>
                {
            ...
            {
                ThrowTerminatingError(new ErrorRecord(ex, "GraphRequestFailed", ErrorCategory.InvalidOperation, AccessPackageCatalogId));
                return;
            }
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.IdentityGovernance.EntitlementManagement.Catalogs[AccessPackageCatalogId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex)
                {
                    ThrowTerminatingError(new ErrorRecord(ex, "GraphRequestFailed", ErrorCategory.InvalidOperation, AccessPackageCatalogId));
