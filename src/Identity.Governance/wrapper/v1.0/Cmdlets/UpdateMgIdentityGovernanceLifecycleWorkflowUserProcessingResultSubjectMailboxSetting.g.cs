...
{
    [GraphRoute("PATCH", "/identityGovernance/lifecycleWorkflows/workflows/{workflow-id}/userProcessingResults/{userProcessingResult-id}/subject/mailboxSettings")]
    [Cmdlet(VerbsData.Update, "MgIdentityGovernanceLifecycleWorkflowUserProcessingResultSubjectMailboxSetting", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.Governance.Client.Models.MailboxSettings))]
    public class UpdateMgIdentityGovernanceLifecycleWorkflowUserProcessingResultSubjectMailboxSettingCommand : PSCmdlet
    {
        ...
        {
            if (!ShouldProcess(UserProcessingResultId, "Update"))
                return;
    ...
    if (this.IsParameterBound(nameof(WorkingHours)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.IdentityGovernance.LifecycleWorkflows.Workflows[WorkflowId].UserProcessingResults[UserProcessingResultId].Subject.MailboxSettings.PatchAsync(body, requestConfiguration =>
                {
            ...
            {
                ThrowTerminatingError(new ErrorRecord(ex, "GraphRequestFailed", ErrorCategory.InvalidOperation, UserProcessingResultId));
                return;
            }
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.IdentityGovernance.LifecycleWorkflows.Workflows[WorkflowId].UserProcessingResults[UserProcessingResultId].Subject.MailboxSettings.GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex)
                {
                    ThrowTerminatingError(new ErrorRecord(ex, "GraphRequestFailed", ErrorCategory.InvalidOperation, UserProcessingResultId));
