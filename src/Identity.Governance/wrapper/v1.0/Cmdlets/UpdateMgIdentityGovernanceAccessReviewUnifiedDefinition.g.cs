...
{
    [GraphRoute("PATCH", "/identityGovernance/accessReviews/unified/definitions/{accessReviewScheduleDefinition-id}")]
    [Cmdlet(VerbsData.Update, "MgIdentityGovernanceAccessReviewUnifiedDefinition", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.Governance.Client.Models.AccessReviewScheduleDefinition))]
    public class UpdateMgIdentityGovernanceAccessReviewUnifiedDefinitionCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(AccessReviewScheduleDefinitionId, "Update"))
                return;
    ...
    if (this.IsParameterBound(nameof(StageSettings)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.IdentityGovernance.AccessReviews.Unified.Definitions[AccessReviewScheduleDefinitionId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.IdentityGovernance.AccessReviews.Unified.Definitions[AccessReviewScheduleDefinitionId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, AccessReviewScheduleDefinitionId);
