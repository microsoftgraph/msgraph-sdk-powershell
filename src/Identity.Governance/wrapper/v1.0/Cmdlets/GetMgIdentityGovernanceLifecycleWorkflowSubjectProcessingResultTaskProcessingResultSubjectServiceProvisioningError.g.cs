...
{
    [GraphRoute("GET", "/identityGovernance/lifecycleWorkflows/workflows/{workflow-id}/subjectProcessingResults/{subjectProcessingResult-id}/taskProcessingResults/{taskProcessingResult-id}/subject/serviceProvisioningErrors")]
    [Cmdlet(VerbsCommon.Get, "MgIdentityGovernanceLifecycleWorkflowSubjectProcessingResultTaskProcessingResultSubjectServiceProvisioningError")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.Governance.Client.Models.ServiceProvisioningError))]
    public class GetMgIdentityGovernanceLifecycleWorkflowSubjectProcessingResultTaskProcessingResultSubjectServiceProvisioningErrorCommand : GraphClientCmdlet
    {
        [Parameter(Mandatory = true, Position = 0)]
        public string WorkflowId { get; set; } = string.Empty;
        [Parameter(Mandatory = true, Position = 1)]
        public string SubjectProcessingResultId { get; set; } = string.Empty;
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Identity.Governance.Client.Models.ServiceProvisioningErrorCollectionResponse? result;
            try
            {
                result = client.IdentityGovernance.LifecycleWorkflows.Workflows[WorkflowId].SubjectProcessingResults[SubjectProcessingResultId].TaskProcessingResults[TaskProcessingResultId].Subject.ServiceProvisioningErrors.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.IdentityGovernance.LifecycleWorkflows.Workflows[WorkflowId].SubjectProcessingResults[SubjectProcessingResultId].TaskProcessingResults[TaskProcessingResultId].Subject.ServiceProvisioningErrors.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
