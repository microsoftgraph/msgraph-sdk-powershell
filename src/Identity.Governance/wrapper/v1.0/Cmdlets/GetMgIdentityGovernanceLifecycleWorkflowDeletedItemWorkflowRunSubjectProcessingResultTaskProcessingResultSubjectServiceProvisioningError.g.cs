...
{
    [GraphRoute("GET", "/identityGovernance/lifecycleWorkflows/deletedItems/workflows/{workflow-id}/runs/{run-id}/subjectProcessingResults/{subjectProcessingResult-id}/taskProcessingResults/{taskProcessingResult-id}/subject/serviceProvisioningErrors")]
    [Cmdlet(VerbsCommon.Get, "MgIdentityGovernanceLifecycleWorkflowDeletedItemWorkflowRunSubjectProcessingResultTaskProcessingResultSubjectServiceProvisioningError")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.Governance.Client.Models.ServiceProvisioningError))]
    public class GetMgIdentityGovernanceLifecycleWorkflowDeletedItemWorkflowRunSubjectProcessingResultTaskProcessingResultSubjectServiceProvisioningErrorCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.IdentityGovernance.LifecycleWorkflows.DeletedItems.Workflows[WorkflowId].Runs[RunId].SubjectProcessingResults[SubjectProcessingResultId].TaskProcessingResults[TaskProcessingResultId].Subject.ServiceProvisioningErrors.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.IdentityGovernance.LifecycleWorkflows.DeletedItems.Workflows[WorkflowId].Runs[RunId].SubjectProcessingResults[SubjectProcessingResultId].TaskProcessingResults[TaskProcessingResultId].Subject.ServiceProvisioningErrors.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
