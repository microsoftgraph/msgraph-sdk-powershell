...
{
    [GraphRoute("POST", "/identityGovernance/catalogs/{accessPackageCatalog-id}/resourceScopes/{accessPackageResourceScope-id}/resource/roles/{accessPackageResourceRole-id}/resource/scopes")]
    [Cmdlet(VerbsCommon.New, "MgIdentityGovernanceCatalogResourceScopeResourceRoleResourceScope", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.Governance.Client.Models.AccessPackageResourceScope))]
    public class NewMgIdentityGovernanceCatalogResourceScopeResourceRoleResourceScopeCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(OriginSystem)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.IdentityGovernance.Catalogs[AccessPackageCatalogId].ResourceScopes[AccessPackageResourceScopeId].Resource.Roles[AccessPackageResourceRoleId].Resource.Scopes.PostAsync(body, requestConfiguration =>
                {
