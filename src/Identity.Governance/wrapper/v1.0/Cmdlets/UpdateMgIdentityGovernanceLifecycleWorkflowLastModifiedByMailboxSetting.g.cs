...
{
    [GraphRoute("PATCH", "/identityGovernance/lifecycleWorkflows/workflows/{workflow-id}/lastModifiedBy/mailboxSettings")]
    [Cmdlet(VerbsData.Update, "MgIdentityGovernanceLifecycleWorkflowLastModifiedByMailboxSetting", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.Governance.Client.Models.MailboxSettings))]
    public class UpdateMgIdentityGovernanceLifecycleWorkflowLastModifiedByMailboxSettingCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(WorkingHours)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.IdentityGovernance.LifecycleWorkflows.Workflows[WorkflowId].LastModifiedBy.MailboxSettings.PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.IdentityGovernance.LifecycleWorkflows.Workflows[WorkflowId].LastModifiedBy.MailboxSettings.GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException)
                {
                    ThrowGraphRequestFailed(ex, WorkflowId);
