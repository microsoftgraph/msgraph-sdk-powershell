...
{
    [GraphRoute("POST", "/identityGovernance/entitlementManagement/resourceRequests/{accessPackageResourceRequest-id}/catalog/resourceScopes")]
    [Cmdlet(VerbsCommon.New, "MgEntitlementManagementResourceRequestCatalogResourceScope", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.Governance.Client.Models.AccessPackageResourceScope))]
    public class NewMgEntitlementManagementResourceRequestCatalogResourceScopeCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(OriginSystem)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.IdentityGovernance.EntitlementManagement.ResourceRequests[AccessPackageResourceRequestId].Catalog.ResourceScopes.PostAsync(body, requestConfiguration =>
                {
