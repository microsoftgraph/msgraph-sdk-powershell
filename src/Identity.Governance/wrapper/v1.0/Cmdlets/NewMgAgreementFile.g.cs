...
{
    [GraphRoute("POST", "/agreements/{agreement-id}/files")]
    [Cmdlet(VerbsCommon.New, "MgAgreementFile", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.Governance.Client.Models.AgreementFileLocalization))]
    public class NewMgAgreementFileCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(FileData)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Agreements[AgreementId].Files.PostAsync(body, requestConfiguration =>
                {
