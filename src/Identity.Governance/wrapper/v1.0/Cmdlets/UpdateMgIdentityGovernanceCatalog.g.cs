...
{
    [GraphRoute("PATCH", "/identityGovernance/catalogs/{accessPackageCatalog-id}")]
    [Cmdlet(VerbsData.Update, "MgIdentityGovernanceCatalog", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.Governance.Client.Models.AccessPackageCatalog))]
    public class UpdateMgIdentityGovernanceCatalogCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(State)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.IdentityGovernance.Catalogs[AccessPackageCatalogId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.IdentityGovernance.Catalogs[AccessPackageCatalogId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, AccessPackageCatalogId);
