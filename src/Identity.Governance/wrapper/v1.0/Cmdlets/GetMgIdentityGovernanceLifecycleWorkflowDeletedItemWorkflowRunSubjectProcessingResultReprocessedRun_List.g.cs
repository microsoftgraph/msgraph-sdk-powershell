...
{
    [GraphRoute("GET", "/identityGovernance/lifecycleWorkflows/deletedItems/workflows/{workflow-id}/runs/{run-id}/subjectProcessingResults/{subjectProcessingResult-id}/reprocessedRuns")]
    [Cmdlet(VerbsCommon.Get, "MgIdentityGovernanceLifecycleWorkflowDeletedItemWorkflowRunSubjectProcessingResultReprocessedRun_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.Governance.Client.Models.IdentityGovernance.Run))]
    public class GetMgIdentityGovernanceLifecycleWorkflowDeletedItemWorkflowRunSubjectProcessingResultReprocessedRun_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.IdentityGovernance.LifecycleWorkflows.DeletedItems.Workflows[WorkflowId].Runs[RunId].SubjectProcessingResults[SubjectProcessingResultId].ReprocessedRuns.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.IdentityGovernance.LifecycleWorkflows.DeletedItems.Workflows[WorkflowId].Runs[RunId].SubjectProcessingResults[SubjectProcessingResultId].ReprocessedRuns.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
