...
{
    [GraphRoute("PATCH", "/agreements/{agreement-id}/file/localizations/{agreementFileLocalization-id}")]
    [Cmdlet(VerbsData.Update, "MgAgreementFileLocalization", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.Governance.Client.Models.AgreementFileLocalization))]
    public class UpdateMgAgreementFileLocalizationCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(FileData)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Agreements[AgreementId].File.Localizations[AgreementFileLocalizationId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Agreements[AgreementId].File.Localizations[AgreementFileLocalizationId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, AgreementFileLocalizationId);
