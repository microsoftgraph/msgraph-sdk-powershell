...
{
    [GraphRoute("POST", "/identityGovernance/accessReviews/unified/definitions/{accessReviewScheduleDefinition-id}/instances/{accessReviewInstance-id}/stages/{accessReviewStage-id}/decisions")]
    [Cmdlet(VerbsCommon.New, "MgIdentityGovernanceAccessReviewUnifiedDefinitionInstanceStageDecision", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.Governance.Client.Models.AccessReviewInstanceDecisionItem))]
    public class NewMgIdentityGovernanceAccessReviewUnifiedDefinitionInstanceStageDecisionCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(AccessReviewStageId, "New"))
                return;
    ...
    if (this.IsParameterBound(nameof(ReviewedBy)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.IdentityGovernance.AccessReviews.Unified.Definitions[AccessReviewScheduleDefinitionId].Instances[AccessReviewInstanceId].Stages[AccessReviewStageId].Decisions.PostAsync(body, requestConfiguration =>
                {
