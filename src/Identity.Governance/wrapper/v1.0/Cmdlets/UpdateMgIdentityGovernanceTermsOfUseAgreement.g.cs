...
{
    [GraphRoute("PATCH", "/identityGovernance/termsOfUse/agreements/{agreement-id}")]
    [Cmdlet(VerbsData.Update, "MgIdentityGovernanceTermsOfUseAgreement", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.Governance.Client.Models.Agreement))]
    public class UpdateMgIdentityGovernanceTermsOfUseAgreementCommand : PSCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(TermsExpiration)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.IdentityGovernance.TermsOfUse.Agreements[AgreementId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.IdentityGovernance.TermsOfUse.Agreements[AgreementId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex)
                {
                    ThrowTerminatingError(new ErrorRecord(ex, "GraphRequestFailed", ErrorCategory.InvalidOperation, AgreementId));
