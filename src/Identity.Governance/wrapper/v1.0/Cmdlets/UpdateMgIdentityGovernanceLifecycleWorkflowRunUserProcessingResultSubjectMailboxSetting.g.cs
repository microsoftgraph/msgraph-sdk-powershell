...
{
    [GraphRoute("PATCH", "/identityGovernance/lifecycleWorkflows/workflows/{workflow-id}/runs/{run-id}/userProcessingResults/{userProcessingResult-id}/subject/mailboxSettings")]
    [Cmdlet(VerbsData.Update, "MgIdentityGovernanceLifecycleWorkflowRunUserProcessingResultSubjectMailboxSetting", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.Governance.Client.Models.MailboxSettings))]
    public class UpdateMgIdentityGovernanceLifecycleWorkflowRunUserProcessingResultSubjectMailboxSettingCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(WorkingHours)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.IdentityGovernance.LifecycleWorkflows.Workflows[WorkflowId].Runs[RunId].UserProcessingResults[UserProcessingResultId].Subject.MailboxSettings.PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.IdentityGovernance.LifecycleWorkflows.Workflows[WorkflowId].Runs[RunId].UserProcessingResults[UserProcessingResultId].Subject.MailboxSettings.GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, UserProcessingResultId);
