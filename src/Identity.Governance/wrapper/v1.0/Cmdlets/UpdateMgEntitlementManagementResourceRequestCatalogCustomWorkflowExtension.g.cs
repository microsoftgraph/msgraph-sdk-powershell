...
{
    [GraphRoute("PATCH", "/identityGovernance/entitlementManagement/resourceRequests/{accessPackageResourceRequest-id}/catalog/customWorkflowExtensions/{customCalloutExtension-id}")]
    [Cmdlet(VerbsData.Update, "MgEntitlementManagementResourceRequestCatalogCustomWorkflowExtension", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.Governance.Client.Models.CustomCalloutExtension))]
    public class UpdateMgEntitlementManagementResourceRequestCatalogCustomWorkflowExtensionCommand : PSCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(EndpointConfiguration)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.IdentityGovernance.EntitlementManagement.ResourceRequests[AccessPackageResourceRequestId].Catalog.CustomWorkflowExtensions[CustomCalloutExtensionId].PatchAsync(body, requestConfiguration =>
                {
            ...
            {
                ThrowTerminatingError(new ErrorRecord(ex, "GraphRequestFailed", ErrorCategory.InvalidOperation, CustomCalloutExtensionId));
                return;
            }
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.IdentityGovernance.EntitlementManagement.ResourceRequests[AccessPackageResourceRequestId].Catalog.CustomWorkflowExtensions[CustomCalloutExtensionId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex)
                {
                    ThrowTerminatingError(new ErrorRecord(ex, "GraphRequestFailed", ErrorCategory.InvalidOperation, CustomCalloutExtensionId));
