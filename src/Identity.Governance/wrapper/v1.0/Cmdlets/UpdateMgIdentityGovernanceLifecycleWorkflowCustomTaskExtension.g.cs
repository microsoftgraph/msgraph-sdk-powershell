...
{
    [GraphRoute("PATCH", "/identityGovernance/lifecycleWorkflows/customTaskExtensions/{customTaskExtension-id}")]
    [Cmdlet(VerbsData.Update, "MgIdentityGovernanceLifecycleWorkflowCustomTaskExtension", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.Governance.Client.Models.IdentityGovernance.CustomTaskExtension))]
    public class UpdateMgIdentityGovernanceLifecycleWorkflowCustomTaskExtensionCommand : PSCmdlet
    {
        ...
        {
            if (!ShouldProcess(CustomTaskExtensionId, "Update"))
                return;
    ...
    if (this.IsParameterBound(nameof(CallbackConfiguration)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.IdentityGovernance.LifecycleWorkflows.CustomTaskExtensions[CustomTaskExtensionId].PatchAsync(body, requestConfiguration =>
                {
