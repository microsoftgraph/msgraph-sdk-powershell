...
{
    [GraphRoute("GET", "/groups/{group-id}/members")]
    [Cmdlet(VerbsCommon.Get, "MgGroupMember")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Groups.Client.Models.DirectoryObject))]
    public class GetMgGroupMemberCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Groups.Client.Models.DirectoryObjectCollectionResponse? result;
            try
            {
                result = client.Groups[GroupId].Members.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Groups[GroupId].Members.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
