...
{
    [GraphRoute("GET", "/groups/{group-id}/owners/graph.device")]
    [Cmdlet(VerbsCommon.Get, "MgGroupOwnerAsDevice_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Groups.Client.Models.Device))]
    public class GetMgGroupOwnerAsDevice_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Owners.GraphDevice.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Groups[GroupId].Owners.GraphDevice.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
