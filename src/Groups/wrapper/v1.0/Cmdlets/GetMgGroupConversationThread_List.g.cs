...
{
    [GraphRoute("GET", "/groups/{group-id}/conversations/{conversation-id}/threads")]
    [Cmdlet(VerbsCommon.Get, "MgGroupConversationThread_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Groups.Client.Models.ConversationThread))]
    public class GetMgGroupConversationThread_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Conversations[ConversationId].Threads.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Groups[GroupId].Conversations[ConversationId].Threads.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
