...
{
    [GraphRoute("PATCH", "/groups/{group-id}/conversations/{conversation-id}/threads/{conversationThread-id}")]
    [Cmdlet(VerbsData.Update, "MgGroupConversationThread", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Groups.Client.Models.ConversationThread))]
    public class UpdateMgGroupConversationThreadCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(ToRecipients)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Conversations[ConversationId].Threads[ConversationThreadId].PatchAsync(body, requestConfiguration =>
                {
