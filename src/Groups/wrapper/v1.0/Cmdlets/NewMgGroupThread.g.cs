...
{
    [GraphRoute("POST", "/groups/{group-id}/threads")]
    [Cmdlet(VerbsCommon.New, "MgGroupThread", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Groups.Client.Models.ConversationThread))]
    public class NewMgGroupThreadCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(ToRecipients)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Threads.PostAsync(body, requestConfiguration =>
                {
