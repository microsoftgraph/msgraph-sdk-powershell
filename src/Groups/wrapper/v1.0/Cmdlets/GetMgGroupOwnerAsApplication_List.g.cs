...
{
    [GraphRoute("GET", "/groups/{group-id}/owners/graph.application")]
    [Cmdlet(VerbsCommon.Get, "MgGroupOwnerAsApplication_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Groups.Client.Models.Application))]
    public class GetMgGroupOwnerAsApplication_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Owners.GraphApplication.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Groups[GroupId].Owners.GraphApplication.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
