...
{
    [GraphRoute("POST", "/groups/{group-id}/threads/{conversationThread-id}/posts/{post-id}/attachments")]
    [Cmdlet(VerbsCommon.New, "MgGroupThreadPostAttachment", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Groups.Client.Models.Attachment))]
    public class NewMgGroupThreadPostAttachmentCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Size)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Threads[ConversationThreadId].Posts[PostId].Attachments.PostAsync(body, requestConfiguration =>
                {
