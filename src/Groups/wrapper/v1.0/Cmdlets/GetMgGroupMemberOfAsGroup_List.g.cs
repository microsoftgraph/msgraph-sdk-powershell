...
{
    [GraphRoute("GET", "/groups/{group-id}/memberOf/graph.group")]
    [Cmdlet(VerbsCommon.Get, "MgGroupMemberOfAsGroup_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Groups.Client.Models.Group))]
    public class GetMgGroupMemberOfAsGroup_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].MemberOf.GraphGroup.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Groups[GroupId].MemberOf.GraphGroup.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
