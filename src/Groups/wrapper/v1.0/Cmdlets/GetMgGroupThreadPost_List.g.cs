...
{
    [GraphRoute("GET", "/groups/{group-id}/threads/{conversationThread-id}/posts")]
    [Cmdlet(VerbsCommon.Get, "MgGroupThreadPost_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Groups.Client.Models.Post))]
    public class GetMgGroupThreadPost_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Groups.Client.Models.PostCollectionResponse? result;
            try
            {
                result = client.Groups[GroupId].Threads[ConversationThreadId].Posts.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Groups[GroupId].Threads[ConversationThreadId].Posts.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
