...
{
    [GraphRoute("GET", "/groups/{group-id}/membersWithLicenseErrors/graph.device")]
    [Cmdlet(VerbsCommon.Get, "MgGroupMemberWithLicenseErrorAsDevice_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Groups.Client.Models.Device))]
    public class GetMgGroupMemberWithLicenseErrorAsDevice_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Groups.Client.Models.DeviceCollectionResponse? result;
            try
            {
                result = client.Groups[GroupId].MembersWithLicenseErrors.GraphDevice.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Groups[GroupId].MembersWithLicenseErrors.GraphDevice.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
