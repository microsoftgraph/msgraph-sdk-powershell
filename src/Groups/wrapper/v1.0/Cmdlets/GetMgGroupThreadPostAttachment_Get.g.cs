...
{
    [GraphRoute("GET", "/groups/{group-id}/threads/{conversationThread-id}/posts/{post-id}/attachments/{attachment-id}")]
    [Cmdlet(VerbsCommon.Get, "MgGroupThreadPostAttachment_Get")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Groups.Client.Models.Attachment))]
    public class GetMgGroupThreadPostAttachment_GetCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Threads[ConversationThreadId].Posts[PostId].Attachments[AttachmentId].GetAsync(requestConfiguration =>
                {
