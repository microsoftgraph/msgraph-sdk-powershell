...
{
    [GraphRoute("GET", "/groups/{group-id}/members/$ref")]
    [Cmdlet(VerbsCommon.Get, "MgGroupMemberByRef")]
    [OutputType(typeof(string))]
    public class GetMgGroupMemberByRefCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Members.Ref.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Groups[GroupId].Members.Ref.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
