...
{
    [GraphRoute("GET", "/groups/{group-id}/groupLifecyclePolicies")]
    [Cmdlet(VerbsCommon.Get, "MgGroupLifecyclePolicyByGroup")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Groups.Client.Models.GroupLifecyclePolicy))]
    public class GetMgGroupLifecyclePolicyByGroupCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].GroupLifecyclePolicies.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Groups[GroupId].GroupLifecyclePolicies.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
