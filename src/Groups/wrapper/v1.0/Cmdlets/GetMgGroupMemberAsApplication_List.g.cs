...
{
    [GraphRoute("GET", "/groups/{group-id}/members/graph.application")]
    [Cmdlet(VerbsCommon.Get, "MgGroupMemberAsApplication_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Groups.Client.Models.Application))]
    public class GetMgGroupMemberAsApplication_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Groups.Client.Models.ApplicationCollectionResponse? result;
            try
            {
                result = client.Groups[GroupId].Members.GraphApplication.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Groups[GroupId].Members.GraphApplication.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
