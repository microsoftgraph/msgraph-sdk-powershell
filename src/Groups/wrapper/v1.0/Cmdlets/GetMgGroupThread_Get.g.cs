...
{
    [GraphRoute("GET", "/groups/{group-id}/threads/{conversationThread-id}")]
    [Cmdlet(VerbsCommon.Get, "MgGroupThread_Get")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Groups.Client.Models.ConversationThread))]
    public class GetMgGroupThread_GetCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Threads[ConversationThreadId].GetAsync(requestConfiguration =>
                {
