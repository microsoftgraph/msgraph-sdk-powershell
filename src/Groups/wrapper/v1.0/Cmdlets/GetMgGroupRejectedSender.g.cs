...
{
    [GraphRoute("GET", "/groups/{group-id}/rejectedSenders")]
    [Cmdlet(VerbsCommon.Get, "MgGroupRejectedSender")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Groups.Client.Models.DirectoryObject))]
    public class GetMgGroupRejectedSenderCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].RejectedSenders.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Groups[GroupId].RejectedSenders.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
