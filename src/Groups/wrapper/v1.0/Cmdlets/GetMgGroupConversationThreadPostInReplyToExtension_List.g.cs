...
{
    [GraphRoute("GET", "/groups/{group-id}/conversations/{conversation-id}/threads/{conversationThread-id}/posts/{post-id}/inReplyTo/extensions")]
    [Cmdlet(VerbsCommon.Get, "MgGroupConversationThreadPostInReplyToExtension_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Groups.Client.Models.Extension))]
    public class GetMgGroupConversationThreadPostInReplyToExtension_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Groups.Client.Models.ExtensionCollectionResponse? result;
            try
            {
                result = client.Groups[GroupId].Conversations[ConversationId].Threads[ConversationThreadId].Posts[PostId].InReplyTo.Extensions.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Groups[GroupId].Conversations[ConversationId].Threads[ConversationThreadId].Posts[PostId].InReplyTo.Extensions.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
