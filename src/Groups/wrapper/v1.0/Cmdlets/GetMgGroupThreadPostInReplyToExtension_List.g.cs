...
{
    [GraphRoute("GET", "/groups/{group-id}/threads/{conversationThread-id}/posts/{post-id}/inReplyTo/extensions")]
    [Cmdlet(VerbsCommon.Get, "MgGroupThreadPostInReplyToExtension_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Groups.Client.Models.Extension))]
    public class GetMgGroupThreadPostInReplyToExtension_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Threads[ConversationThreadId].Posts[PostId].InReplyTo.Extensions.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Groups[GroupId].Threads[ConversationThreadId].Posts[PostId].InReplyTo.Extensions.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
