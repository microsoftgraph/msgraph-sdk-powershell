...
{
    [GraphRoute("GET", "/groups/{group-id}/rejectedSenders/$ref")]
    [Cmdlet(VerbsCommon.Get, "MgGroupRejectedSenderByRef")]
    [OutputType(typeof(string))]
    public class GetMgGroupRejectedSenderByRefCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Groups.Client.Models.StringCollectionResponse? result;
            try
            {
                result = client.Groups[GroupId].RejectedSenders.Ref.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Groups[GroupId].RejectedSenders.Ref.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
