...
{
    [GraphRoute("GET", "/groups/{group-id}/owners/graph.servicePrincipal")]
    [Cmdlet(VerbsCommon.Get, "MgGroupOwnerAsServicePrincipal_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Groups.Client.Models.ServicePrincipal))]
    public class GetMgGroupOwnerAsServicePrincipal_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Owners.GraphServicePrincipal.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Groups[GroupId].Owners.GraphServicePrincipal.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
