...
{
    [GraphRoute("GET", "/groups/{group-id}/transitiveMembers/graph.application")]
    [Cmdlet(VerbsCommon.Get, "MgGroupTransitiveMemberAsApplication_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Groups.Client.Models.Application))]
    public class GetMgGroupTransitiveMemberAsApplication_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].TransitiveMembers.GraphApplication.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Groups[GroupId].TransitiveMembers.GraphApplication.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
