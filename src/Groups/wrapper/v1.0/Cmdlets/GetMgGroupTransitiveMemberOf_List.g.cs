...
{
    [GraphRoute("GET", "/groups/{group-id}/transitiveMemberOf")]
    [Cmdlet(VerbsCommon.Get, "MgGroupTransitiveMemberOf_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Groups.Client.Models.DirectoryObject))]
    public class GetMgGroupTransitiveMemberOf_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].TransitiveMemberOf.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Groups[GroupId].TransitiveMemberOf.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
