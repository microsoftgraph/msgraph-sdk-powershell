...
{
    [GraphRoute("GET", "/groups/{group-id}/conversations")]
    [Cmdlet(VerbsCommon.Get, "MgGroupConversation_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Groups.Client.Models.Conversation))]
    public class GetMgGroupConversation_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Groups.Client.Models.ConversationCollectionResponse? result;
            try
            {
                result = client.Groups[GroupId].Conversations.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Groups[GroupId].Conversations.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
