...
{
    [GraphRoute("POST", "/groups/{group-id}/conversations/{conversation-id}/threads/{conversationThread-id}/posts/{post-id}/attachments")]
    [Cmdlet(VerbsCommon.New, "MgGroupConversationThreadPostAttachment", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Groups.Client.Models.Attachment))]
    public class NewMgGroupConversationThreadPostAttachmentCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Size)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Conversations[ConversationId].Threads[ConversationThreadId].Posts[PostId].Attachments.PostAsync(body, requestConfiguration =>
                {
