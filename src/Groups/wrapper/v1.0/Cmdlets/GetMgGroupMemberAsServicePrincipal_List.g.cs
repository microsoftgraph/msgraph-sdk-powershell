...
{
    [GraphRoute("GET", "/groups/{group-id}/members/graph.servicePrincipal")]
    [Cmdlet(VerbsCommon.Get, "MgGroupMemberAsServicePrincipal_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Groups.Client.Models.ServicePrincipal))]
    public class GetMgGroupMemberAsServicePrincipal_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Groups.Client.Models.ServicePrincipalCollectionResponse? result;
            try
            {
                result = client.Groups[GroupId].Members.GraphServicePrincipal.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Groups[GroupId].Members.GraphServicePrincipal.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
