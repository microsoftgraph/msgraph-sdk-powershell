...
{
    [GraphRoute("GET", "/groupLifecyclePolicies")]
    [Cmdlet(VerbsCommon.Get, "MgGroupLifecyclePolicy_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Groups.Client.Models.GroupLifecyclePolicy))]
    public class GetMgGroupLifecyclePolicy_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Groups.Client.Models.GroupLifecyclePolicyCollectionResponse? result;
            try
            {
                result = client.GroupLifecyclePolicies.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.GroupLifecyclePolicies.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
