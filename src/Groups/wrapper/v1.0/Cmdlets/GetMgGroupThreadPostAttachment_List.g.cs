...
{
    [GraphRoute("GET", "/groups/{group-id}/threads/{conversationThread-id}/posts/{post-id}/attachments")]
    [Cmdlet(VerbsCommon.Get, "MgGroupThreadPostAttachment_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Groups.Client.Models.Attachment))]
    public class GetMgGroupThreadPostAttachment_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Groups.Client.Models.AttachmentCollectionResponse? result;
            try
            {
                result = client.Groups[GroupId].Threads[ConversationThreadId].Posts[PostId].Attachments.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Groups[GroupId].Threads[ConversationThreadId].Posts[PostId].Attachments.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
