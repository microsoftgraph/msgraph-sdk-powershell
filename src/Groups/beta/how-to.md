# How-To
This document describes how to develop for `Microsoft.Graph.Beta.Groups`.

## Building `Microsoft.Graph.Beta.Groups`
...
## Testing `Microsoft.Graph.Beta.Groups`
...
## Packing `Microsoft.Graph.Beta.Groups`
...
There are multiple scripts created for performing different actions for developing `Microsoft.Graph.Beta.Groups`.
- `build-module.ps1`
  - Builds the module DLL (`./bin/Microsoft.Graph.Beta.Groups.private.dll`), creates the exported cmdlets and documentation, generates custom cmdlet test stubs and exported cmdlet example stubs, and updates `./Microsoft.Graph.Beta.Groups.psd1` with Azure profile information.
  - **Parameters**: [`Switch` parameters]
    - `-Run`: After building, creates an isolated PowerShell session and loads `Microsoft.Graph.Beta.Groups`.
...
- `run-module.ps1`
  - Creates an isolated PowerShell session and loads `Microsoft.Graph.Beta.Groups` into the session.
