...
{
    [GraphRoute("POST", "/drives/{drive-id}/items")]
    [Cmdlet(VerbsCommon.New, "MgDriveItem", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Files.Client.Models.DriveItem))]
    public class NewMgDriveItemCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Video)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Drives[DriveId].Items.PostAsync(body, requestConfiguration =>
                {
