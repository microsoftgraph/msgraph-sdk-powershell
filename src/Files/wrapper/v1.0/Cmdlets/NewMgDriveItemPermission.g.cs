...
{
    [GraphRoute("POST", "/drives/{drive-id}/items/{driveItem-id}/permissions")]
    [Cmdlet(VerbsCommon.New, "MgDriveItemPermission", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Files.Client.Models.Permission))]
    public class NewMgDriveItemPermissionCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Link)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Drives[DriveId].Items[DriveItemId].Permissions.PostAsync(body, requestConfiguration =>
                {
