...
{
    [GraphRoute("POST", "/shares/{sharedDriveItem-id}/list/items/{listItem-id}/documentSetVersions")]
    [Cmdlet(VerbsCommon.New, "MgShareListItemDocumentSetVersion", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Files.Client.Models.DocumentSetVersion))]
    public class NewMgShareListItemDocumentSetVersionCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Items)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Shares[SharedDriveItemId].List.Items[ListItemId].DocumentSetVersions.PostAsync(body, requestConfiguration =>
                {
