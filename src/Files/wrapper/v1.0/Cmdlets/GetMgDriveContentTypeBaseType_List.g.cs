...
{
    [GraphRoute("GET", "/drives/{drive-id}/list/contentTypes/{contentType-id}/baseTypes")]
    [Cmdlet(VerbsCommon.Get, "MgDriveContentTypeBaseType_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Files.Client.Models.ContentType))]
    public class GetMgDriveContentTypeBaseType_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Files.Client.Models.ContentTypeCollectionResponse? result;
            try
            {
                result = client.Drives[DriveId].List.ContentTypes[ContentTypeId].BaseTypes.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Drives[DriveId].List.ContentTypes[ContentTypeId].BaseTypes.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
