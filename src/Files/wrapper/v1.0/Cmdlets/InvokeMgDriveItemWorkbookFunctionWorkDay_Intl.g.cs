...
{
    [GraphRoute("POST", "/drives/{drive-id}/items/{driveItem-id}/workbook/functions/workDay_Intl")]
    [Cmdlet(VerbsLifecycle.Invoke, "MgDriveItemWorkbookFunctionWorkDay_Intl", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Files.Client.Models.WorkbookFunctionResult))]
    public class InvokeMgDriveItemWorkbookFunctionWorkDay_IntlCommand : GraphClientCmdlet
    {
        ...
        protected override void ProcessRecord()
        {
            ...
            var body = new global::Microsoft.Graph.PowerShell.Files.Client.Drives.Item.Items.Item.Workbook.Functions.WorkDay_Intl.WorkDay_IntlPostRequestBody();
    ...
    if (this.IsParameterBound(nameof(Days)))
    {
        var untypedDays = UntypedValue.From(Days);
        if (untypedDays is not null)
            body.Days = untypedDays;
    }
    ...
    if (this.IsParameterBound(nameof(Holidays)))
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Drives[DriveId].Items[DriveItemId].Workbook.Functions.WorkDay_Intl.PostAsync(body, requestConfiguration =>
                {
