...
{
    [GraphRoute("GET", "/shares/{sharedDriveItem-id}/list/createdByUser/serviceProvisioningErrors")]
    [Cmdlet(VerbsCommon.Get, "MgShareListCreatedByUserServiceProvisioningError")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Files.Client.Models.ServiceProvisioningError))]
    public class GetMgShareListCreatedByUserServiceProvisioningErrorCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Shares[SharedDriveItemId].List.CreatedByUser.ServiceProvisioningErrors.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Shares[SharedDriveItemId].List.CreatedByUser.ServiceProvisioningErrors.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
