...
{
    [GraphRoute("POST", "/drives/{drive-id}/list/operations")]
    [Cmdlet(VerbsCommon.New, "MgDriveListOperation", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Files.Client.Models.RichLongRunningOperation))]
    public class NewMgDriveListOperationCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Error)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Drives[DriveId].List.Operations.PostAsync(body, requestConfiguration =>
                {
