...
{
    [GraphRoute("POST", "/drives/{drive-id}/list/items")]
    [Cmdlet(VerbsCommon.New, "MgDriveListItem", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Files.Client.Models.ListItem))]
    public class NewMgDriveListItemCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(SharepointIds)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Drives[DriveId].List.Items.PostAsync(body, requestConfiguration =>
                {
