...
{
    [GraphRoute("POST", "/drives/{drive-id}/list/subscriptions")]
    [Cmdlet(VerbsCommon.New, "MgDriveListSubscription", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Files.Client.Models.Subscription))]
    public class NewMgDriveListSubscriptionCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Resource)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Drives[DriveId].List.Subscriptions.PostAsync(body, requestConfiguration =>
                {
