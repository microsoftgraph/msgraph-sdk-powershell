...
{
    [GraphRoute("PATCH", "/drives/{drive-id}/list/items/{listItem-id}")]
    [Cmdlet(VerbsData.Update, "MgDriveListItem", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Files.Client.Models.ListItem))]
    public class UpdateMgDriveListItemCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(SharepointIds)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Drives[DriveId].List.Items[ListItemId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Drives[DriveId].List.Items[ListItemId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, ListItemId);
