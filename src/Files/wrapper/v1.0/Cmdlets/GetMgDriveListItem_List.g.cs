...
{
    [GraphRoute("GET", "/drives/{drive-id}/list/items")]
    [Cmdlet(VerbsCommon.Get, "MgDriveListItem_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Files.Client.Models.ListItem))]
    public class GetMgDriveListItem_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Files.Client.Models.ListItemCollectionResponse? result;
            try
            {
                result = client.Drives[DriveId].List.Items.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Drives[DriveId].List.Items.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
