...
{
    [GraphRoute("POST", "/drives/{drive-id}/items/{driveItem-id}/subscriptions")]
    [Cmdlet(VerbsCommon.New, "MgDriveItemSubscription", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Files.Client.Models.Subscription))]
    public class NewMgDriveItemSubscriptionCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Resource)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Drives[DriveId].Items[DriveItemId].Subscriptions.PostAsync(body, requestConfiguration =>
                {
