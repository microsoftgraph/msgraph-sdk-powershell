...
{
    [GraphRoute("PATCH", "/shares/{sharedDriveItem-id}/list/contentTypes/{contentType-id}")]
    [Cmdlet(VerbsData.Update, "MgShareListContentType", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Files.Client.Models.ContentType))]
    public class UpdateMgShareListContentTypeCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Order)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Shares[SharedDriveItemId].List.ContentTypes[ContentTypeId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Shares[SharedDriveItemId].List.ContentTypes[ContentTypeId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException)
                {
                    ThrowGraphRequestFailed(ex, ContentTypeId);
