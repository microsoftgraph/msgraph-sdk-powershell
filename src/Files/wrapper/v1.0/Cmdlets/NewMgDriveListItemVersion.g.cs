...
{
    [GraphRoute("POST", "/drives/{drive-id}/list/items/{listItem-id}/versions")]
    [Cmdlet(VerbsCommon.New, "MgDriveListItemVersion", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Files.Client.Models.ListItemVersion))]
    public class NewMgDriveListItemVersionCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Publication)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Drives[DriveId].List.Items[ListItemId].Versions.PostAsync(body, requestConfiguration =>
                {
