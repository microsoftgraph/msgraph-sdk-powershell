...
{
    [GraphRoute("PATCH", "/drives/{drive-id}/items/{driveItem-id}/permissions/{permission-id}")]
    [Cmdlet(VerbsData.Update, "MgDriveItemPermission", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Files.Client.Models.Permission))]
    public class UpdateMgDriveItemPermissionCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Link)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Drives[DriveId].Items[DriveItemId].Permissions[PermissionId].PatchAsync(body, requestConfiguration =>
                {
