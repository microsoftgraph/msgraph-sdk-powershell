...
{
    [GraphRoute("GET", "/drives/{drive-id}/list/subscriptions")]
    [Cmdlet(VerbsCommon.Get, "MgDriveListSubscription_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Files.Client.Models.Subscription))]
    public class GetMgDriveListSubscription_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Drives[DriveId].List.Subscriptions.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Drives[DriveId].List.Subscriptions.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
