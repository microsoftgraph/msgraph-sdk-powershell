...
{
    [GraphRoute("PATCH", "/shares/{sharedDriveItem-id}/list")]
    [Cmdlet(VerbsData.Update, "MgShareList", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Files.Client.Models.List))]
    public class UpdateMgShareListCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(System)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Shares[SharedDriveItemId].List.PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Shares[SharedDriveItemId].List.GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex)
                {
                    ThrowGraphRequestFailed(ex, SharedDriveItemId);
