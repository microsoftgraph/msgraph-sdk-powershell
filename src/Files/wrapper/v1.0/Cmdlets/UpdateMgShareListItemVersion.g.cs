...
{
    [GraphRoute("PATCH", "/shares/{sharedDriveItem-id}/list/items/{listItem-id}/versions/{listItemVersion-id}")]
    [Cmdlet(VerbsData.Update, "MgShareListItemVersion", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Files.Client.Models.ListItemVersion))]
    public class UpdateMgShareListItemVersionCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Publication)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Shares[SharedDriveItemId].List.Items[ListItemId].Versions[ListItemVersionId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Shares[SharedDriveItemId].List.Items[ListItemId].Versions[ListItemVersionId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, ListItemVersionId);
