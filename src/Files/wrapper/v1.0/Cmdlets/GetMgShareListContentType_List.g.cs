...
{
    [GraphRoute("GET", "/shares/{sharedDriveItem-id}/list/contentTypes")]
    [Cmdlet(VerbsCommon.Get, "MgShareListContentType_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Files.Client.Models.ContentType))]
    public class GetMgShareListContentType_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Shares[SharedDriveItemId].List.ContentTypes.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Shares[SharedDriveItemId].List.ContentTypes.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
