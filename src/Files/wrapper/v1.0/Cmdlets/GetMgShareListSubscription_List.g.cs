...
{
    [GraphRoute("GET", "/shares/{sharedDriveItem-id}/list/subscriptions")]
    [Cmdlet(VerbsCommon.Get, "MgShareListSubscription_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Files.Client.Models.Subscription))]
    public class GetMgShareListSubscription_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Shares[SharedDriveItemId].List.Subscriptions.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Shares[SharedDriveItemId].List.Subscriptions.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
