...
{
    [GraphRoute("GET", "/drives/{drive-id}/items/{driveItem-id}/thumbnails")]
    [Cmdlet(VerbsCommon.Get, "MgDriveItemThumbnail_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Files.Client.Models.ThumbnailSet))]
    public class GetMgDriveItemThumbnail_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Files.Client.Models.ThumbnailSetCollectionResponse? result;
            try
            {
                result = client.Drives[DriveId].Items[DriveItemId].Thumbnails.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Drives[DriveId].Items[DriveItemId].Thumbnails.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
