...
{
    [GraphRoute("POST", "/shares/{sharedDriveItem-id}/list/contentTypes")]
    [Cmdlet(VerbsCommon.New, "MgShareListContentType", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Files.Client.Models.ContentType))]
    public class NewMgShareListContentTypeCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Order)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Shares[SharedDriveItemId].List.ContentTypes.PostAsync(body, requestConfiguration =>
                {
