...
{
    [GraphRoute("GET", "/drives/{drive-id}/list/contentTypes")]
    [Cmdlet(VerbsCommon.Get, "MgDriveListContentType_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Files.Client.Models.ContentType))]
    public class GetMgDriveListContentType_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Drives[DriveId].List.ContentTypes.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Drives[DriveId].List.ContentTypes.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
