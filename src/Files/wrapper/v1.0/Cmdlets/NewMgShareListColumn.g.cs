...
{
    [GraphRoute("POST", "/shares/{sharedDriveItem-id}/list/columns")]
    [Cmdlet(VerbsCommon.New, "MgShareListColumn", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Files.Client.Models.ColumnDefinition))]
    public class NewMgShareListColumnCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Validation)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Shares[SharedDriveItemId].List.Columns.PostAsync(body, requestConfiguration =>
                {
