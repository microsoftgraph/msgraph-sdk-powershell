...
{
    [GraphRoute("POST", "/drives/{drive-id}/list/contentTypes/{contentType-id}/columns")]
    [Cmdlet(VerbsCommon.New, "MgDriveListContentTypeColumn", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Files.Client.Models.ColumnDefinition))]
    public class NewMgDriveListContentTypeColumnCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Validation)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Drives[DriveId].List.ContentTypes[ContentTypeId].Columns.PostAsync(body, requestConfiguration =>
                {
