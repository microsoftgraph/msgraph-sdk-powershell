...
{
    [GraphRoute("GET", "/drives/{drive-id}/list/items/{listItem-id}/documentSetVersions")]
    [Cmdlet(VerbsCommon.Get, "MgDriveListItemDocumentSetVersion_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Files.Client.Models.DocumentSetVersion))]
    public class GetMgDriveListItemDocumentSetVersion_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Drives[DriveId].List.Items[ListItemId].DocumentSetVersions.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Drives[DriveId].List.Items[ListItemId].DocumentSetVersions.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
