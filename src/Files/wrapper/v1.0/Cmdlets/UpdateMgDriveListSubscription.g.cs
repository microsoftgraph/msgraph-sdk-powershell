...
{
    [GraphRoute("PATCH", "/drives/{drive-id}/list/subscriptions/{subscription-id}")]
    [Cmdlet(VerbsData.Update, "MgDriveListSubscription", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Files.Client.Models.Subscription))]
    public class UpdateMgDriveListSubscriptionCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Resource)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Drives[DriveId].List.Subscriptions[SubscriptionId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Drives[DriveId].List.Subscriptions[SubscriptionId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex)
                {
                    ThrowGraphRequestFailed(ex, SubscriptionId);
