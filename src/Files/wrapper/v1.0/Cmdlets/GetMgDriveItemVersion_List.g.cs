...
{
    [GraphRoute("GET", "/drives/{drive-id}/items/{driveItem-id}/versions")]
    [Cmdlet(VerbsCommon.Get, "MgDriveItemVersion_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Files.Client.Models.DriveItemVersion))]
    public class GetMgDriveItemVersion_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Files.Client.Models.DriveItemVersionCollectionResponse? result;
            try
            {
                result = client.Drives[DriveId].Items[DriveItemId].Versions.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Drives[DriveId].Items[DriveItemId].Versions.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
