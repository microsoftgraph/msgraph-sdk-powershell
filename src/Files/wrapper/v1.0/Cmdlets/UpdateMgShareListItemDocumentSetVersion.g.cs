...
{
    [GraphRoute("PATCH", "/shares/{sharedDriveItem-id}/list/items/{listItem-id}/documentSetVersions/{documentSetVersion-id}")]
    [Cmdlet(VerbsData.Update, "MgShareListItemDocumentSetVersion", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Files.Client.Models.DocumentSetVersion))]
    public class UpdateMgShareListItemDocumentSetVersionCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Items)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Shares[SharedDriveItemId].List.Items[ListItemId].DocumentSetVersions[DocumentSetVersionId].PatchAsync(body, requestConfiguration =>
                {
