...
{
    [GraphRoute("PATCH", "/shares/{sharedDriveItem-id}/list/operations/{richLongRunningOperation-id}")]
    [Cmdlet(VerbsData.Update, "MgShareListOperation", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Files.Client.Models.RichLongRunningOperation))]
    public class UpdateMgShareListOperationCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Error)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Shares[SharedDriveItemId].List.Operations[RichLongRunningOperationId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Shares[SharedDriveItemId].List.Operations[RichLongRunningOperationId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex)
                {
                    ThrowGraphRequestFailed(ex, RichLongRunningOperationId);
