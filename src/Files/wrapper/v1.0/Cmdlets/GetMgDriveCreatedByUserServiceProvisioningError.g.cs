...
{
    [GraphRoute("GET", "/drives/{drive-id}/createdByUser/serviceProvisioningErrors")]
    [Cmdlet(VerbsCommon.Get, "MgDriveCreatedByUserServiceProvisioningError")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Files.Client.Models.ServiceProvisioningError))]
    public class GetMgDriveCreatedByUserServiceProvisioningErrorCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Drives[DriveId].CreatedByUser.ServiceProvisioningErrors.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Drives[DriveId].CreatedByUser.ServiceProvisioningErrors.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
