...
{
    [GraphRoute("GET", "/drives/{drive-id}/list/items/delta()")]
    [Cmdlet(VerbsCommon.Get, "MgDriveListItemDelta", DefaultParameterSetName = "DeltaSync")]
    [OutputType(typeof(global::Microsoft.Graph.PowerShell.Files.Client.Drives.Item.List.Items.Delta.DeltaGetResponse))]
    public class GetMgDriveListItemDeltaCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            global::Microsoft.Graph.PowerShell.Files.Client.Drives.Item.List.Items.Delta.DeltaGetResponse? result;
            try
            {
                result = ParameterSetName == "Resume"
                    ? client.Drives[DriveId].List.Items.Delta.WithUrl(ValidateContinuationUrl(DeltaLink!, requestAdapter, nameof(DeltaLink))).GetAsDeltaGetResponseAsync(requestConfiguration =>
                        {

                AddRequestHeaders(requestConfiguration.Headers);
                        }).GetAwaiter().GetResult()
                    : client.Drives[DriveId].List.Items.Delta.GetAsDeltaGetResponseAsync(requestConfiguration =>
                {
                ...
                while (true)
                {
                    ...
                    result = client.Drives[DriveId].List.Items.Delta.WithUrl(nextLink).GetAsDeltaGetResponseAsync(requestConfiguration =>
                    {
