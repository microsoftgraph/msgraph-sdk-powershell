...
{
    [GraphRoute("PATCH", "/drives/{drive-id}/list/columns/{columnDefinition-id}")]
    [Cmdlet(VerbsData.Update, "MgDriveListColumn", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Files.Client.Models.ColumnDefinition))]
    public class UpdateMgDriveListColumnCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Validation)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Drives[DriveId].List.Columns[ColumnDefinitionId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Drives[DriveId].List.Columns[ColumnDefinitionId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException)
                {
                    ThrowGraphRequestFailed(ex, ColumnDefinitionId);
