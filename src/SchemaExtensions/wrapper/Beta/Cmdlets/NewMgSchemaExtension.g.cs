...
{
    [GraphRoute("POST", "/schemaExtensions")]
    [Cmdlet(VerbsCommon.New, "MgSchemaExtension", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.SchemaExtensions.Client.Models.SchemaExtension))]
    public class NewMgSchemaExtensionCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Properties)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.SchemaExtensions.PostAsync(body, requestConfiguration =>
                {
