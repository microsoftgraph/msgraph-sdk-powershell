...
{
    [GraphRoute("PATCH", "/subscriptions/{subscription-id}")]
    [Cmdlet(VerbsData.Update, "MgSubscription", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.ChangeNotifications.Client.Models.Subscription))]
    public class UpdateMgSubscriptionCommand : GraphClientCmdlet
    {
        ...
        [Parameter(Mandatory = false)]
        public string? WebPushEncryptionP256dhPublicKey { get; set; }

        [Parameter(Mandatory = false)]
        public string? WebPushEncryptionSecret { get; set; }
        ...
        protected override void ProcessRecord()
        {
            ...
            var body = new Microsoft.Graph.PowerShell.ChangeNotifications.Client.Models.Subscription();
    ...
    if (this.IsParameterBound(nameof(NotificationContentType)))
        body.NotificationContentType = NotificationContentType;
    ...
    if (this.IsParameterBound(nameof(VapidPublicKey)))
        body.VapidPublicKey = VapidPublicKey;

    if (this.IsParameterBound(nameof(WebPushEncryptionP256dhPublicKey)))
        body.WebPushEncryptionP256dhPublicKey = WebPushEncryptionP256dhPublicKey;

    if (this.IsParameterBound(nameof(WebPushEncryptionSecret)))
        body.WebPushEncryptionSecret = WebPushEncryptionSecret;
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Subscriptions[SubscriptionId].PatchAsync(body, requestConfiguration =>
                {
