...
{
    [GraphRoute("GET", "/tenantRelationships/delegatedAdminRelationships/{delegatedAdminRelationship-id}/accessAssignments")]
    [Cmdlet(VerbsCommon.Get, "MgTenantRelationshipDelegatedAdminRelationshipAccessAssignment_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.Partner.Client.Models.DelegatedAdminAccessAssignment))]
    public class GetMgTenantRelationshipDelegatedAdminRelationshipAccessAssignment_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Identity.Partner.Client.Models.DelegatedAdminAccessAssignmentCollectionResponse? result;
            try
            {
                result = client.TenantRelationships.DelegatedAdminRelationships[DelegatedAdminRelationshipId].AccessAssignments.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.TenantRelationships.DelegatedAdminRelationships[DelegatedAdminRelationshipId].AccessAssignments.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
