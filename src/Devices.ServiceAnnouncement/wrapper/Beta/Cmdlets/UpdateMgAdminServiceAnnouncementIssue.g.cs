...
{
    [GraphRoute("PATCH", "/admin/serviceAnnouncement/issues/{serviceHealthIssue-id}")]
    [Cmdlet(VerbsData.Update, "MgAdminServiceAnnouncementIssue", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Devices.ServiceAnnouncement.Client.Models.ServiceHealthIssue))]
    public class UpdateMgAdminServiceAnnouncementIssueCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Status)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Admin.ServiceAnnouncement.Issues[ServiceHealthIssueId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Admin.ServiceAnnouncement.Issues[ServiceHealthIssueId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, ServiceHealthIssueId);
