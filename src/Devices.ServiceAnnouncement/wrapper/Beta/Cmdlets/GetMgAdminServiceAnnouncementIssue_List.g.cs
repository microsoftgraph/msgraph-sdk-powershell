...
{
    [GraphRoute("GET", "/admin/serviceAnnouncement/issues")]
    [Cmdlet(VerbsCommon.Get, "MgAdminServiceAnnouncementIssue_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Devices.ServiceAnnouncement.Client.Models.ServiceHealthIssue))]
    public class GetMgAdminServiceAnnouncementIssue_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Devices.ServiceAnnouncement.Client.Models.ServiceHealthIssueCollectionResponse? result;
            try
            {
                result = client.Admin.ServiceAnnouncement.Issues.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Admin.ServiceAnnouncement.Issues.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
