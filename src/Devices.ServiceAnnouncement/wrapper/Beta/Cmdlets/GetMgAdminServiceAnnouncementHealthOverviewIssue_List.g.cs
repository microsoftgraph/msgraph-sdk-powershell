...
namespace Microsoft.Graph.PowerShell.Devices.ServiceAnnouncement
{
    ...
    [Cmdlet(VerbsCommon.Get, "MgAdminServiceAnnouncementHealthOverviewIssue_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Devices.ServiceAnnouncement.Client.Models.ServiceHealthIssue))]
    public class GetMgAdminServiceAnnouncementHealthOverviewIssue_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Admin.ServiceAnnouncement.HealthOverviews[ServiceHealthId].Issues.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Admin.ServiceAnnouncement.HealthOverviews[ServiceHealthId].Issues.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
