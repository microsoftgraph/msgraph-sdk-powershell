...
{
    [GraphRoute("GET", "/users/{user-id}/contactFolders/{contactFolder-id}/contacts/{contact-id}")]
    [Cmdlet(VerbsCommon.Get, "MgUserContactFolderContact_Get")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.PersonalContacts.Client.Models.Contact))]
    public class GetMgUserContactFolderContact_GetCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].ContactFolders[ContactFolderId].Contacts[ContactId].GetAsync(requestConfiguration =>
                {
