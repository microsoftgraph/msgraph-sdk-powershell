...
{
    [GraphRoute("GET", "/users/{user-id}/contactFolders/{contactFolder-id}/contacts/{contact-id}/photo")]
    [Cmdlet(VerbsCommon.Get, "MgUserContactFolderContactPhoto")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.PersonalContacts.Client.Models.ProfilePhoto))]
    public class GetMgUserContactFolderContactPhotoCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].ContactFolders[ContactFolderId].Contacts[ContactId].Photo.GetAsync(requestConfiguration =>
                {
