...
{
    [GraphRoute("GET", "/users/{user-id}/contactFolders/{contactFolder-id}/childFolders")]
    [Cmdlet(VerbsCommon.Get, "MgUserContactFolderChildFolder_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.PersonalContacts.Client.Models.ContactFolder))]
    public class GetMgUserContactFolderChildFolder_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.PersonalContacts.Client.Models.ContactFolderCollectionResponse? result;
            try
            {
                result = client.Users[UserId].ContactFolders[ContactFolderId].ChildFolders.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Users[UserId].ContactFolders[ContactFolderId].ChildFolders.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
