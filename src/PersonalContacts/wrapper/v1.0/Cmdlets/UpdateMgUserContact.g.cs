...
{
    [GraphRoute("PATCH", "/users/{user-id}/contacts/{contact-id}")]
    [Cmdlet(VerbsData.Update, "MgUserContact", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.PersonalContacts.Client.Models.Contact))]
    public class UpdateMgUserContactCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(TertiaryEmailAddress)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].Contacts[ContactId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Users[UserId].Contacts[ContactId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException)
                {
                    ThrowGraphRequestFailed(ex, ContactId);
