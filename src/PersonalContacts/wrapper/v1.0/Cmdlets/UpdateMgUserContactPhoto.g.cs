...
{
    [GraphRoute("PATCH", "/users/{user-id}/contacts/{contact-id}/photo")]
    [Cmdlet(VerbsData.Update, "MgUserContactPhoto", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.PersonalContacts.Client.Models.ProfilePhoto))]
    public class UpdateMgUserContactPhotoCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Width)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].Contacts[ContactId].Photo.PatchAsync(body, requestConfiguration =>
                {
