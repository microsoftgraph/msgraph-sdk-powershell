...
{
    [GraphRoute("GET", "/users/{user-id}/contactFolders/{contactFolder-id}/contacts/{contact-id}/extensions/{extension-id}")]
    [Cmdlet(VerbsCommon.Get, "MgUserContactFolderContactExtension_Get")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.PersonalContacts.Client.Models.Extension))]
    public class GetMgUserContactFolderContactExtension_GetCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].ContactFolders[ContactFolderId].Contacts[ContactId].Extensions[ExtensionId].GetAsync(requestConfiguration =>
                {
