...
{
    [GraphRoute("POST", "/users/{user-id}/contactFolders/{contactFolder-id}/childFolders/{contactFolder-id1}/contacts")]
    [Cmdlet(VerbsCommon.New, "MgUserContactFolderChildFolderContact", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.PersonalContacts.Client.Models.Contact))]
    public class NewMgUserContactFolderChildFolderContactCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(ContactFolderId1, "New"))
                return;
    ...
    if (this.IsParameterBound(nameof(TertiaryEmailAddress)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].ContactFolders[ContactFolderId].ChildFolders[ContactFolderId1].Contacts.PostAsync(body, requestConfiguration =>
                {
