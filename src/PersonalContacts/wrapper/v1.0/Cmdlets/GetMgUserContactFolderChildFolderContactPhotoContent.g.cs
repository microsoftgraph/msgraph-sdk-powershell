...
{
    [GraphRoute("GET", "/users/{user-id}/contactFolders/{contactFolder-id}/childFolders/{contactFolder-id1}/contacts/{contact-id}/photo/$value")]
    [Cmdlet(VerbsCommon.Get, "MgUserContactFolderChildFolderContactPhotoContent")]
    [OutputType(typeof(byte[]))]
    public class GetMgUserContactFolderChildFolderContactPhotoContentCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].ContactFolders[ContactFolderId].ChildFolders[ContactFolderId1].Contacts[ContactId].Photo.Content.GetAsync(requestConfiguration =>
                {
