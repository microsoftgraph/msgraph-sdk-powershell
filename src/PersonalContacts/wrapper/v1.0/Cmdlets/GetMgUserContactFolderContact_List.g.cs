...
{
    [GraphRoute("GET", "/users/{user-id}/contactFolders/{contactFolder-id}/contacts")]
    [Cmdlet(VerbsCommon.Get, "MgUserContactFolderContact_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.PersonalContacts.Client.Models.Contact))]
    public class GetMgUserContactFolderContact_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.PersonalContacts.Client.Models.ContactCollectionResponse? result;
            try
            {
                result = client.Users[UserId].ContactFolders[ContactFolderId].Contacts.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Users[UserId].ContactFolders[ContactFolderId].Contacts.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
