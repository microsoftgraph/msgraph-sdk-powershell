...
{
    [GraphRoute("POST", "/users/{user-id}/contactFolders/{contactFolder-id}/childFolders")]
    [Cmdlet(VerbsCommon.New, "MgUserContactFolderChildFolder", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.PersonalContacts.Client.Models.ContactFolder))]
    public class NewMgUserContactFolderChildFolderCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(ParentFolderId)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].ContactFolders[ContactFolderId].ChildFolders.PostAsync(body, requestConfiguration =>
                {
