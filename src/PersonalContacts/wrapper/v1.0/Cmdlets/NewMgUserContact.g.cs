...
{
    [GraphRoute("POST", "/users/{user-id}/contacts")]
    [Cmdlet(VerbsCommon.New, "MgUserContact", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.PersonalContacts.Client.Models.Contact))]
    public class NewMgUserContactCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(TertiaryEmailAddress)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].Contacts.PostAsync(body, requestConfiguration =>
                {
