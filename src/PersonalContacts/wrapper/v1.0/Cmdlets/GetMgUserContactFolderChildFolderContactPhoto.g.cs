...
{
    [GraphRoute("GET", "/users/{user-id}/contactFolders/{contactFolder-id}/childFolders/{contactFolder-id1}/contacts/{contact-id}/photo")]
    [Cmdlet(VerbsCommon.Get, "MgUserContactFolderChildFolderContactPhoto")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.PersonalContacts.Client.Models.ProfilePhoto))]
    public class GetMgUserContactFolderChildFolderContactPhotoCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].ContactFolders[ContactFolderId].ChildFolders[ContactFolderId1].Contacts[ContactId].Photo.GetAsync(requestConfiguration =>
                {
