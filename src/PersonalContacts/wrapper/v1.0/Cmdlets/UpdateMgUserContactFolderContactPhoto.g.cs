...
{
    [GraphRoute("PATCH", "/users/{user-id}/contactFolders/{contactFolder-id}/contacts/{contact-id}/photo")]
    [Cmdlet(VerbsData.Update, "MgUserContactFolderContactPhoto", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.PersonalContacts.Client.Models.ProfilePhoto))]
    public class UpdateMgUserContactFolderContactPhotoCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Width)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].ContactFolders[ContactFolderId].Contacts[ContactId].Photo.PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Users[UserId].ContactFolders[ContactFolderId].Contacts[ContactId].Photo.GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, ContactId);
