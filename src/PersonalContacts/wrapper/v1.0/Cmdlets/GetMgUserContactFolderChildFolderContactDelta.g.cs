...
namespace Microsoft.Graph.PowerShell.PersonalContacts
{
    ...
    [Cmdlet(VerbsCommon.Get, "MgUserContactFolderChildFolderContactDelta", DefaultParameterSetName = "DeltaSync")]
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = ParameterSetName == "Resume"
                    ? client.Users[UserId].ContactFolders[ContactFolderId].ChildFolders[ContactFolderId1].Contacts.Delta.WithUrl(ValidateContinuationUrl(DeltaLink!, requestAdapter, nameof(DeltaLink))).GetAsDeltaGetResponseAsync(requestConfiguration =>
                        {

                AddRequestHeaders(requestConfiguration.Headers);
                        }).GetAwaiter().GetResult()
                    : client.Users[UserId].ContactFolders[ContactFolderId].ChildFolders[ContactFolderId1].Contacts.Delta.GetAsDeltaGetResponseAsync(requestConfiguration =>
                {
                ...
                while (true)
                {
                    ...
                    result = client.Users[UserId].ContactFolders[ContactFolderId].ChildFolders[ContactFolderId1].Contacts.Delta.WithUrl(nextLink).GetAsDeltaGetResponseAsync(requestConfiguration =>
                    {
