...
{
    [GraphRoute("GET", "/privacy/subjectRightsRequests/{subjectRightsRequest-id}/collaborators/{user-id}/serviceProvisioningErrors")]
    [Cmdlet(VerbsCommon.Get, "MgPrivacySubjectRightsRequestCollaboratorServiceProvisioningError")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Compliance.Client.Models.ServiceProvisioningError))]
    public class GetMgPrivacySubjectRightsRequestCollaboratorServiceProvisioningErrorCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Privacy.SubjectRightsRequests[SubjectRightsRequestId].Collaborators[UserId].ServiceProvisioningErrors.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Privacy.SubjectRightsRequests[SubjectRightsRequestId].Collaborators[UserId].ServiceProvisioningErrors.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
