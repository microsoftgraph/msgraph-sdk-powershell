...
{
    [GraphRoute("GET", "/privacy/subjectRightsRequests/{subjectRightsRequest-id}/notes")]
    [Cmdlet(VerbsCommon.Get, "MgPrivacySubjectRightsRequestNote_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Compliance.Client.Models.AuthoredNote))]
    public class GetMgPrivacySubjectRightsRequestNote_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Compliance.Client.Models.AuthoredNoteCollectionResponse? result;
            try
            {
                result = client.Privacy.SubjectRightsRequests[SubjectRightsRequestId].Notes.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Privacy.SubjectRightsRequests[SubjectRightsRequestId].Notes.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
