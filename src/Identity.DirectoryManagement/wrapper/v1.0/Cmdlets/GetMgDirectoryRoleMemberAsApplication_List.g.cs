...
{
    [GraphRoute("GET", "/directoryRoles/{directoryRole-id}/members/graph.application")]
    [Cmdlet(VerbsCommon.Get, "MgDirectoryRoleMemberAsApplication_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.DirectoryManagement.Client.Models.Application))]
    public class GetMgDirectoryRoleMemberAsApplication_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.DirectoryRoles[DirectoryRoleId].Members.GraphApplication.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.DirectoryRoles[DirectoryRoleId].Members.GraphApplication.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
