...
{
    [GraphRoute("PATCH", "/domains/{domain-id}/verificationDnsRecords/{domainDnsRecord-id}")]
    [Cmdlet(VerbsData.Update, "MgDomainVerificationDnsRecord", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.DirectoryManagement.Client.Models.DomainDnsRecord))]
    public class UpdateMgDomainVerificationDnsRecordCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Ttl)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Domains[DomainId].VerificationDnsRecords[DomainDnsRecordId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Domains[DomainId].VerificationDnsRecords[DomainDnsRecordId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex)
                {
                    ThrowGraphRequestFailed(ex, DomainDnsRecordId);
