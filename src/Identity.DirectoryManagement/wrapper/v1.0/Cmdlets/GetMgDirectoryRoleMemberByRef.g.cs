...
{
    [GraphRoute("GET", "/directoryRoles/{directoryRole-id}/members/$ref")]
    [Cmdlet(VerbsCommon.Get, "MgDirectoryRoleMemberByRef")]
    [OutputType(typeof(string))]
    public class GetMgDirectoryRoleMemberByRefCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Identity.DirectoryManagement.Client.Models.StringCollectionResponse? result;
            try
            {
                result = client.DirectoryRoles[DirectoryRoleId].Members.Ref.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.DirectoryRoles[DirectoryRoleId].Members.Ref.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
