...
{
    [GraphRoute("GET", "/devices/{device-id}/memberOf/graph.administrativeUnit")]
    [Cmdlet(VerbsCommon.Get, "MgDeviceMemberOfAsAdministrativeUnit_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.DirectoryManagement.Client.Models.AdministrativeUnit))]
    public class GetMgDeviceMemberOfAsAdministrativeUnit_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Devices[DeviceId].MemberOf.GraphAdministrativeUnit.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Devices[DeviceId].MemberOf.GraphAdministrativeUnit.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
