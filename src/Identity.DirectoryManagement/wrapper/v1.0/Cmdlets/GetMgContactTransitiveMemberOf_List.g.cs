...
{
    [GraphRoute("GET", "/contacts/{orgContact-id}/transitiveMemberOf")]
    [Cmdlet(VerbsCommon.Get, "MgContactTransitiveMemberOf_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.DirectoryManagement.Client.Models.DirectoryObject))]
    public class GetMgContactTransitiveMemberOf_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Identity.DirectoryManagement.Client.Models.DirectoryObjectCollectionResponse? result;
            try
            {
                result = client.Contacts[OrgContactId].TransitiveMemberOf.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Contacts[OrgContactId].TransitiveMemberOf.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
