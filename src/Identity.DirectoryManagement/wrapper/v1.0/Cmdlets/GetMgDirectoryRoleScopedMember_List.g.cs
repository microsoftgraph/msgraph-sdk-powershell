...
{
    [GraphRoute("GET", "/directoryRoles/{directoryRole-id}/scopedMembers")]
    [Cmdlet(VerbsCommon.Get, "MgDirectoryRoleScopedMember_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.DirectoryManagement.Client.Models.ScopedRoleMembership))]
    public class GetMgDirectoryRoleScopedMember_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.DirectoryRoles[DirectoryRoleId].ScopedMembers.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.DirectoryRoles[DirectoryRoleId].ScopedMembers.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
