...
{
    [GraphRoute("GET", "/devices/{device-id}/transitiveMemberOf")]
    [Cmdlet(VerbsCommon.Get, "MgDeviceTransitiveMemberOf_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.DirectoryManagement.Client.Models.DirectoryObject))]
    public class GetMgDeviceTransitiveMemberOf_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Devices[DeviceId].TransitiveMemberOf.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Devices[DeviceId].TransitiveMemberOf.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
