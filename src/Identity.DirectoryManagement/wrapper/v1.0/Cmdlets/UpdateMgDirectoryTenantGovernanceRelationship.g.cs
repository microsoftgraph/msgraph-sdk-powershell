...
{
    [GraphRoute("PATCH", "/directory/tenantGovernance/governanceRelationships/{governanceRelationship-id}")]
    [Cmdlet(VerbsData.Update, "MgDirectoryTenantGovernanceRelationship", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.DirectoryManagement.Client.Models.GovernanceRelationship))]
    public class UpdateMgDirectoryTenantGovernanceRelationshipCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(GovernanceRelationshipId, "Update"))
                return;
    ...
    if (this.IsParameterBound(nameof(Status)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Directory.TenantGovernance.GovernanceRelationships[GovernanceRelationshipId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Directory.TenantGovernance.GovernanceRelationships[GovernanceRelationshipId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, GovernanceRelationshipId);
