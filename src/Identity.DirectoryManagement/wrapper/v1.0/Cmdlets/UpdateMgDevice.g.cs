...
{
    [GraphRoute("PATCH", "/devices/{device-id}")]
    [Cmdlet(VerbsData.Update, "MgDevice", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.DirectoryManagement.Client.Models.Device))]
    public class UpdateMgDeviceCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(DeviceId, "Update"))
                return;
    ...
    if (this.IsParameterBound(nameof(DeviceId1)))
        body.DeviceId = DeviceId1;
    ...
    if (this.IsParameterBound(nameof(AlternativeSecurityIds)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Devices[DeviceId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Devices[DeviceId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex)
                {
                    ThrowGraphRequestFailed(ex, DeviceId);
