...
{
    [GraphRoute("PATCH", "/directory/customSecurityAttributeDefinitions/{customSecurityAttributeDefinition-id}")]
    [Cmdlet(VerbsData.Update, "MgDirectoryCustomSecurityAttributeDefinition", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    ...
    public class UpdateMgDirectoryCustomSecurityAttributeDefinitionCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(CustomSecurityAttributeDefinitionId, "Update"))
                return;
    ...
    if (this.IsParameterBound(nameof(UsePreDefinedValuesOnly)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Directory.CustomSecurityAttributeDefinitions[CustomSecurityAttributeDefinitionId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Directory.CustomSecurityAttributeDefinitions[CustomSecurityAttributeDefinitionId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException)
                {
                    ThrowGraphRequestFailed(ex, CustomSecurityAttributeDefinitionId);
