...
{
    [GraphRoute("POST", "/domains/{domain-id}/serviceConfigurationRecords")]
    [Cmdlet(VerbsCommon.New, "MgDomainServiceConfigurationRecord", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.DirectoryManagement.Client.Models.DomainDnsRecord))]
    public class NewMgDomainServiceConfigurationRecordCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Ttl)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Domains[DomainId].ServiceConfigurationRecords.PostAsync(body, requestConfiguration =>
                {
