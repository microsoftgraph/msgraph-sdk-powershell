...
{
    [GraphRoute("GET", "/devices/{device-id}/registeredOwners")]
    [Cmdlet(VerbsCommon.Get, "MgDeviceRegisteredOwner")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.DirectoryManagement.Client.Models.DirectoryObject))]
    public class GetMgDeviceRegisteredOwnerCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Devices[DeviceId].RegisteredOwners.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Devices[DeviceId].RegisteredOwners.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
