...
{
    [GraphRoute("GET", "/devices/{device-id}/registeredUsers")]
    [Cmdlet(VerbsCommon.Get, "MgDeviceRegisteredUser")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.DirectoryManagement.Client.Models.DirectoryObject))]
    public class GetMgDeviceRegisteredUserCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Devices[DeviceId].RegisteredUsers.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Devices[DeviceId].RegisteredUsers.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
