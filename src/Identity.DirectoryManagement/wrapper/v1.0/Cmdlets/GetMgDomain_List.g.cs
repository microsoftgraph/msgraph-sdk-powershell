...
{
    [GraphRoute("GET", "/domains")]
    [Cmdlet(VerbsCommon.Get, "MgDomain_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.DirectoryManagement.Client.Models.Domain))]
    public class GetMgDomain_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Identity.DirectoryManagement.Client.Models.DomainCollectionResponse? result;
            try
            {
                result = client.Domains.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Domains.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
