...
{
    [GraphRoute("GET", "/directory/administrativeUnits")]
    [Cmdlet(VerbsCommon.Get, "MgDirectoryAdministrativeUnit_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.DirectoryManagement.Client.Models.AdministrativeUnit))]
    public class GetMgDirectoryAdministrativeUnit_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Directory.AdministrativeUnits.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Directory.AdministrativeUnits.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
