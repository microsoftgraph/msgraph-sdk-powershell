...
{
    [GraphRoute("PATCH", "/organization/{organization-id}/branding/localizations/{organizationalBrandingLocalization-id}")]
    [Cmdlet(VerbsData.Update, "MgOrganizationBrandingLocalization", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    ...
    public class UpdateMgOrganizationBrandingLocalizationCommand : PSCmdlet
    {
        ...
        {
            if (!ShouldProcess(OrganizationalBrandingLocalizationId, "Update"))
                return;
    ...
    if (this.IsParameterBound(nameof(LoginPageTextVisibilitySettings)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Organization[OrganizationId].Branding.Localizations[OrganizationalBrandingLocalizationId].PatchAsync(body, requestConfiguration =>
                {
            ...
            {
                ThrowTerminatingError(new ErrorRecord(ex, "GraphRequestFailed", ErrorCategory.InvalidOperation, OrganizationalBrandingLocalizationId));
                return;
            }
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Organization[OrganizationId].Branding.Localizations[OrganizationalBrandingLocalizationId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex)
                {
                    ThrowTerminatingError(new ErrorRecord(ex, "GraphRequestFailed", ErrorCategory.InvalidOperation, OrganizationalBrandingLocalizationId));
