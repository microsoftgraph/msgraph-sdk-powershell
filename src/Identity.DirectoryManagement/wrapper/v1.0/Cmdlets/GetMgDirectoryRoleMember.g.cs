...
{
    [GraphRoute("GET", "/directoryRoles/{directoryRole-id}/members")]
    [Cmdlet(VerbsCommon.Get, "MgDirectoryRoleMember")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.DirectoryManagement.Client.Models.DirectoryObject))]
    public class GetMgDirectoryRoleMemberCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.DirectoryRoles[DirectoryRoleId].Members.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.DirectoryRoles[DirectoryRoleId].Members.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
