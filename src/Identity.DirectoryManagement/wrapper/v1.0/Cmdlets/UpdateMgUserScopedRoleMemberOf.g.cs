...
{
    [GraphRoute("PATCH", "/users/{user-id}/scopedRoleMemberOf/{scopedRoleMembership-id}")]
    [Cmdlet(VerbsData.Update, "MgUserScopedRoleMemberOf", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.DirectoryManagement.Client.Models.ScopedRoleMembership))]
    public class UpdateMgUserScopedRoleMemberOfCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(RoleMemberInfo)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].ScopedRoleMemberOf[ScopedRoleMembershipId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Users[UserId].ScopedRoleMemberOf[ScopedRoleMembershipId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, ScopedRoleMembershipId);
