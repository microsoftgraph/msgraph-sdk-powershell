...
{
    [GraphRoute("POST", "/directory/remoteTenantGroups")]
    [Cmdlet(VerbsCommon.New, "MgDirectoryRemoteTenantGroup", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.DirectoryManagement.Client.Models.RemoteTenantGroup))]
    public class NewMgDirectoryRemoteTenantGroupCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(RemoteTenantPrimaryDomain)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Directory.RemoteTenantGroups.PostAsync(body, requestConfiguration =>
                {
