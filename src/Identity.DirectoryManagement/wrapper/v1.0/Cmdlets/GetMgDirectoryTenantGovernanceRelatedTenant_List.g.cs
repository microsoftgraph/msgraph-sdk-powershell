...
{
    [GraphRoute("GET", "/directory/tenantGovernance/relatedTenants")]
    [Cmdlet(VerbsCommon.Get, "MgDirectoryTenantGovernanceRelatedTenant_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.DirectoryManagement.Client.Models.RelatedTenant))]
    public class GetMgDirectoryTenantGovernanceRelatedTenant_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Identity.DirectoryManagement.Client.Models.RelatedTenantCollectionResponse? result;
            try
            {
                result = client.Directory.TenantGovernance.RelatedTenants.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Directory.TenantGovernance.RelatedTenants.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
