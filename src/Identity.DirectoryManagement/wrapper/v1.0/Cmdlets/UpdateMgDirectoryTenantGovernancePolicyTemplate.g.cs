...
{
    [GraphRoute("PATCH", "/directory/tenantGovernance/governancePolicyTemplates/{tenantGovernancePolicyTemplate-id}")]
    [Cmdlet(VerbsData.Update, "MgDirectoryTenantGovernancePolicyTemplate", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.DirectoryManagement.Client.Models.TenantGovernancePolicyTemplate))]
    public class UpdateMgDirectoryTenantGovernancePolicyTemplateCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(TenantGovernancePolicyTemplateId, "Update"))
                return;
    ...
    if (this.IsParameterBound(nameof(MultiTenantApplicationsToProvision)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Directory.TenantGovernance.GovernancePolicyTemplates[TenantGovernancePolicyTemplateId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Directory.TenantGovernance.GovernancePolicyTemplates[TenantGovernancePolicyTemplateId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, TenantGovernancePolicyTemplateId);
