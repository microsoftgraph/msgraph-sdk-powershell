...
{
    [GraphRoute("POST", "/domains/{domain-id}/verificationDnsRecords")]
    [Cmdlet(VerbsCommon.New, "MgDomainVerificationDnsRecord", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.DirectoryManagement.Client.Models.DomainDnsRecord))]
    public class NewMgDomainVerificationDnsRecordCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Ttl)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Domains[DomainId].VerificationDnsRecords.PostAsync(body, requestConfiguration =>
                {
