...
{
    [GraphRoute("POST", "/directory/tenantGovernance/governanceInvitations")]
    [Cmdlet(VerbsCommon.New, "MgDirectoryTenantGovernanceInvitation", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.DirectoryManagement.Client.Models.GovernanceInvitation))]
    public class NewMgDirectoryTenantGovernanceInvitationCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(GoverningTenantName)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Directory.TenantGovernance.GovernanceInvitations.PostAsync(body, requestConfiguration =>
                {
