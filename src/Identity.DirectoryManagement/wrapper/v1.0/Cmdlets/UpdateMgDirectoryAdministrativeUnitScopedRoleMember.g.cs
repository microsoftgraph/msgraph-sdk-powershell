...
{
    [GraphRoute("PATCH", "/directory/administrativeUnits/{administrativeUnit-id}/scopedRoleMembers/{scopedRoleMembership-id}")]
    [Cmdlet(VerbsData.Update, "MgDirectoryAdministrativeUnitScopedRoleMember", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.DirectoryManagement.Client.Models.ScopedRoleMembership))]
    public class UpdateMgDirectoryAdministrativeUnitScopedRoleMemberCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(RoleMemberInfo)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Directory.AdministrativeUnits[AdministrativeUnitId].ScopedRoleMembers[ScopedRoleMembershipId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Directory.AdministrativeUnits[AdministrativeUnitId].ScopedRoleMembers[ScopedRoleMembershipId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, ScopedRoleMembershipId);
