...
{
    [GraphRoute("GET", "/directory/recovery/snapshots/{snapshot-id}/recoveryJobs")]
    [Cmdlet(VerbsCommon.Get, "MgDirectoryRecoverySnapshotRecoveryJob_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.DirectoryManagement.Client.Models.EntraRecoveryServices.RecoveryJob))]
    public class GetMgDirectoryRecoverySnapshotRecoveryJob_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Identity.DirectoryManagement.Client.Models.EntraRecoveryServices.RecoveryJobCollectionResponse? result;
            try
            {
                result = client.Directory.Recovery.Snapshots[SnapshotId].RecoveryJobs.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Directory.Recovery.Snapshots[SnapshotId].RecoveryJobs.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
