...
{
    [GraphRoute("PATCH", "/organization/{organization-id}/branding")]
    [Cmdlet(VerbsData.Update, "MgOrganizationBranding", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.DirectoryManagement.Client.Models.OrganizationalBranding))]
    public class UpdateMgOrganizationBrandingCommand : GraphClientCmdlet
    {
        ...
        protected override void ProcessRecord()
        {
            ...
            var body = new Microsoft.Graph.PowerShell.Identity.DirectoryManagement.Client.Models.OrganizationalBranding();
    ...
    if (this.IsParameterBound(nameof(LoginPageTextVisibilitySettings)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Organization[OrganizationId].Branding.PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Organization[OrganizationId].Branding.GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException)
                {
                    ThrowGraphRequestFailed(ex, OrganizationId);
