...
{
    [GraphRoute("GET", "/devices/{device-id}/registeredUsers/graph.appRoleAssignment")]
    [Cmdlet(VerbsCommon.Get, "MgDeviceRegisteredUserAsAppRoleAssignment_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.DirectoryManagement.Client.Models.AppRoleAssignment))]
    public class GetMgDeviceRegisteredUserAsAppRoleAssignment_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Identity.DirectoryManagement.Client.Models.AppRoleAssignmentCollectionResponse? result;
            try
            {
                result = client.Devices[DeviceId].RegisteredUsers.GraphAppRoleAssignment.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Devices[DeviceId].RegisteredUsers.GraphAppRoleAssignment.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
