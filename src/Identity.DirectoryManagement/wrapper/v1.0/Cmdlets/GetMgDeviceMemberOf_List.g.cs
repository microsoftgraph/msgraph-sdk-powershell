...
{
    [GraphRoute("GET", "/devices/{device-id}/memberOf")]
    [Cmdlet(VerbsCommon.Get, "MgDeviceMemberOf_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.DirectoryManagement.Client.Models.DirectoryObject))]
    public class GetMgDeviceMemberOf_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Devices[DeviceId].MemberOf.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Devices[DeviceId].MemberOf.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
