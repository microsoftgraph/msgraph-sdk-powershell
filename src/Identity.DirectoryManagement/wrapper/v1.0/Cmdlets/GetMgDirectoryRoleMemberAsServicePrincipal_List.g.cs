...
{
    [GraphRoute("GET", "/directoryRoles/{directoryRole-id}/members/graph.servicePrincipal")]
    [Cmdlet(VerbsCommon.Get, "MgDirectoryRoleMemberAsServicePrincipal_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.DirectoryManagement.Client.Models.ServicePrincipal))]
    public class GetMgDirectoryRoleMemberAsServicePrincipal_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Identity.DirectoryManagement.Client.Models.ServicePrincipalCollectionResponse? result;
            try
            {
                result = client.DirectoryRoles[DirectoryRoleId].Members.GraphServicePrincipal.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.DirectoryRoles[DirectoryRoleId].Members.GraphServicePrincipal.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
