...
{
    [GraphRoute("GET", "/devices/{device-id}/memberOf/graph.group")]
    [Cmdlet(VerbsCommon.Get, "MgDeviceMemberOfAsGroup_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Identity.DirectoryManagement.Client.Models.Group))]
    public class GetMgDeviceMemberOfAsGroup_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Identity.DirectoryManagement.Client.Models.GroupCollectionResponse? result;
            try
            {
                result = client.Devices[DeviceId].MemberOf.GraphGroup.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Devices[DeviceId].MemberOf.GraphGroup.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
