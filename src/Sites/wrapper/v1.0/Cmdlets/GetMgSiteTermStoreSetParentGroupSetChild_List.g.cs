...
{
    [GraphRoute("GET", "/sites/{site-id}/termStore/sets/{set-id}/parentGroup/sets/{set-id1}/children")]
    [Cmdlet(VerbsCommon.Get, "MgSiteTermStoreSetParentGroupSetChild_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.TermStore.Term))]
    public class GetMgSiteTermStoreSetParentGroupSetChild_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Sites[SiteId].TermStore.Sets[SetId].ParentGroup.Sets[SetId1].Children.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Sites[SiteId].TermStore.Sets[SetId].ParentGroup.Sets[SetId1].Children.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
