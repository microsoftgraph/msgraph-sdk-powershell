...
{
    [GraphRoute("POST", "/sites/{site-id}/termStore/sets/{set-id}/children/{term-id}/children/{term-id1}/relations")]
    [Cmdlet(VerbsCommon.New, "MgSiteTermStoreSetChildRelation", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.TermStore.Relation))]
    public class NewMgSiteTermStoreSetChildRelationCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Relationship)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Sites[SiteId].TermStore.Sets[SetId].Children[TermId].Children[TermId1].Relations.PostAsync(body, requestConfiguration =>
                {
