...
{
    [GraphRoute("PATCH", "/sites/{site-id}/lists/{list-id}/permissions/{permission-id}")]
    [Cmdlet(VerbsData.Update, "MgSiteListPermission", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.Permission))]
    public class UpdateMgSiteListPermissionCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Link)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Sites[SiteId].Lists[ListId].Permissions[PermissionId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Sites[SiteId].Lists[ListId].Permissions[PermissionId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException)
                {
                    ThrowGraphRequestFailed(ex, PermissionId);
