...
{
    [GraphRoute("PATCH", "/groups/{group-id}/sites/{site-id}/lastModifiedByUser/mailboxSettings")]
    [Cmdlet(VerbsData.Update, "MgGroupSiteLastModifiedByUserMailboxSetting", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.MailboxSettings))]
    public class UpdateMgGroupSiteLastModifiedByUserMailboxSettingCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(WorkingHours)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Sites[SiteId].LastModifiedByUser.MailboxSettings.PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Groups[GroupId].Sites[SiteId].LastModifiedByUser.MailboxSettings.GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException)
                {
                    ThrowGraphRequestFailed(ex, SiteId);
