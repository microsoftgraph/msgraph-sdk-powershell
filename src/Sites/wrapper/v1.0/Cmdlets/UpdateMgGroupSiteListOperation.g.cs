...
{
    [GraphRoute("PATCH", "/groups/{group-id}/sites/{site-id}/lists/{list-id}/operations/{richLongRunningOperation-id}")]
    [Cmdlet(VerbsData.Update, "MgGroupSiteListOperation", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.RichLongRunningOperation))]
    public class UpdateMgGroupSiteListOperationCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Error)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Sites[SiteId].Lists[ListId].Operations[RichLongRunningOperationId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Groups[GroupId].Sites[SiteId].Lists[ListId].Operations[RichLongRunningOperationId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException)
                {
                    ThrowGraphRequestFailed(ex, RichLongRunningOperationId);
