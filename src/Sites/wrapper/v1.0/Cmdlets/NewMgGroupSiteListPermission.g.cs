...
{
    [GraphRoute("POST", "/groups/{group-id}/sites/{site-id}/lists/{list-id}/permissions")]
    [Cmdlet(VerbsCommon.New, "MgGroupSiteListPermission", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.Permission))]
    public class NewMgGroupSiteListPermissionCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Link)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Sites[SiteId].Lists[ListId].Permissions.PostAsync(body, requestConfiguration =>
                {
