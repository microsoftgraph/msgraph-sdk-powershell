...
{
    [GraphRoute("PATCH", "/sites/{site-id}/analytics/itemActivityStats/{itemActivityStat-id}/activities/{itemActivity-id}")]
    [Cmdlet(VerbsData.Update, "MgSiteAnalyticItemActivityStatActivity", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.ItemActivity))]
    public class UpdateMgSiteAnalyticItemActivityStatActivityCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Actor)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Sites[SiteId].Analytics.ItemActivityStats[ItemActivityStatId].Activities[ItemActivityId].PatchAsync(body, requestConfiguration =>
                {
