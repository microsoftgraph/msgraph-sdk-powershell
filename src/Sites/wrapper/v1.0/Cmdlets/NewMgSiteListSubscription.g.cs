...
{
    [GraphRoute("POST", "/sites/{site-id}/lists/{list-id}/subscriptions")]
    [Cmdlet(VerbsCommon.New, "MgSiteListSubscription", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.Subscription))]
    public class NewMgSiteListSubscriptionCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Resource)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Sites[SiteId].Lists[ListId].Subscriptions.PostAsync(body, requestConfiguration =>
                {
