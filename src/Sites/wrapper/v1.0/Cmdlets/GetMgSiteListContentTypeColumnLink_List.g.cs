...
{
    [GraphRoute("GET", "/sites/{site-id}/lists/{list-id}/contentTypes/{contentType-id}/columnLinks")]
    [Cmdlet(VerbsCommon.Get, "MgSiteListContentTypeColumnLink_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.ColumnLink))]
    public class GetMgSiteListContentTypeColumnLink_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Sites.Client.Models.ColumnLinkCollectionResponse? result;
            try
            {
                result = client.Sites[SiteId].Lists[ListId].ContentTypes[ContentTypeId].ColumnLinks.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Sites[SiteId].Lists[ListId].ContentTypes[ContentTypeId].ColumnLinks.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
