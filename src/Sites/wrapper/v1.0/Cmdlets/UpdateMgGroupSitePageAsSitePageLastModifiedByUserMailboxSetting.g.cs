...
{
    [GraphRoute("PATCH", "/groups/{group-id}/sites/{site-id}/pages/{baseSitePage-id}/graph.sitePage/lastModifiedByUser/mailboxSettings")]
    [Cmdlet(VerbsData.Update, "MgGroupSitePageAsSitePageLastModifiedByUserMailboxSetting", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.MailboxSettings))]
    public class UpdateMgGroupSitePageAsSitePageLastModifiedByUserMailboxSettingCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(WorkingHours)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Sites[SiteId].Pages[BaseSitePageId].GraphSitePage.LastModifiedByUser.MailboxSettings.PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Groups[GroupId].Sites[SiteId].Pages[BaseSitePageId].GraphSitePage.LastModifiedByUser.MailboxSettings.GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex)
                {
                    ThrowGraphRequestFailed(ex, BaseSitePageId);
