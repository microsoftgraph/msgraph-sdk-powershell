...
{
    [GraphRoute("GET", "/sites/{site-id}/pages/{baseSitePage-id}/createdByUser/serviceProvisioningErrors")]
    [Cmdlet(VerbsCommon.Get, "MgSitePageCreatedByUserServiceProvisioningError")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.ServiceProvisioningError))]
    public class GetMgSitePageCreatedByUserServiceProvisioningErrorCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Sites[SiteId].Pages[BaseSitePageId].CreatedByUser.ServiceProvisioningErrors.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Sites[SiteId].Pages[BaseSitePageId].CreatedByUser.ServiceProvisioningErrors.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
