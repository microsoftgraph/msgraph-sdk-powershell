...
{
    [GraphRoute("POST", "/sites/{site-id}/termStore/sets/{set-id}/parentGroup/sets")]
    [Cmdlet(VerbsCommon.New, "MgSiteTermStoreSetParentGroupSet", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.TermStore.Set))]
    public class NewMgSiteTermStoreSetParentGroupSetCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(ParentGroup)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Sites[SiteId].TermStore.Sets[SetId].ParentGroup.Sets.PostAsync(body, requestConfiguration =>
                {
