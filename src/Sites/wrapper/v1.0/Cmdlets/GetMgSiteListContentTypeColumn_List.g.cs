...
{
    [GraphRoute("GET", "/sites/{site-id}/lists/{list-id}/contentTypes/{contentType-id}/columns")]
    [Cmdlet(VerbsCommon.Get, "MgSiteListContentTypeColumn_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.ColumnDefinition))]
    public class GetMgSiteListContentTypeColumn_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Sites.Client.Models.ColumnDefinitionCollectionResponse? result;
            try
            {
                result = client.Sites[SiteId].Lists[ListId].ContentTypes[ContentTypeId].Columns.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Sites[SiteId].Lists[ListId].ContentTypes[ContentTypeId].Columns.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
