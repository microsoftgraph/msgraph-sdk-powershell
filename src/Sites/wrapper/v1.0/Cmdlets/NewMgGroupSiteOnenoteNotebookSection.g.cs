...
{
    [GraphRoute("POST", "/groups/{group-id}/sites/{site-id}/onenote/notebooks/{notebook-id}/sections")]
    [Cmdlet(VerbsCommon.New, "MgGroupSiteOnenoteNotebookSection", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.OnenoteSection))]
    public class NewMgGroupSiteOnenoteNotebookSectionCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Links)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Sites[SiteId].Onenote.Notebooks[NotebookId].Sections.PostAsync(body, requestConfiguration =>
                {
