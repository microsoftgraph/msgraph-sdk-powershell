...
{
    [GraphRoute("GET", "/sites/{site-id}/lists")]
    [Cmdlet(VerbsCommon.Get, "MgSiteList_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.List))]
    public class GetMgSiteList_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Sites[SiteId].Lists.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Sites[SiteId].Lists.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
