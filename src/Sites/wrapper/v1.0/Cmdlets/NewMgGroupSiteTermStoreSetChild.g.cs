...
{
    [GraphRoute("POST", "/groups/{group-id}/sites/{site-id}/termStore/sets/{set-id}/children")]
    [Cmdlet(VerbsCommon.New, "MgGroupSiteTermStoreSetChild", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.TermStore.Term))]
    public class NewMgGroupSiteTermStoreSetChildCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Properties)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Sites[SiteId].TermStore.Sets[SetId].Children.PostAsync(body, requestConfiguration =>
                {
