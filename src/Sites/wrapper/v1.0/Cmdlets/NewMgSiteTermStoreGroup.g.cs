...
{
    [GraphRoute("POST", "/sites/{site-id}/termStore/groups")]
    [Cmdlet(VerbsCommon.New, "MgSiteTermStoreGroup", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.TermStore.Group))]
    public class NewMgSiteTermStoreGroupCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Scope)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Sites[SiteId].TermStore.Groups.PostAsync(body, requestConfiguration =>
                {
