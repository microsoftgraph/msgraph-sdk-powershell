...
{
    [GraphRoute("GET", "/groups/{group-id}/sites/{site-id}/termStore/sets/{set-id}/relations")]
    [Cmdlet(VerbsCommon.Get, "MgGroupSiteTermStoreSetRelation_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.TermStore.Relation))]
    public class GetMgGroupSiteTermStoreSetRelation_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Sites[SiteId].TermStore.Sets[SetId].Relations.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Groups[GroupId].Sites[SiteId].TermStore.Sets[SetId].Relations.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
