...
{
    [GraphRoute("GET", "/groups/{group-id}/sites/{site-id}/termStore/groups/{group-id1}/sets")]
    [Cmdlet(VerbsCommon.Get, "MgGroupSiteTermStoreGroupSet_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.TermStore.Set))]
    public class GetMgGroupSiteTermStoreGroupSet_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Sites[SiteId].TermStore.Groups[GroupId1].Sets.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Groups[GroupId].Sites[SiteId].TermStore.Groups[GroupId1].Sets.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
