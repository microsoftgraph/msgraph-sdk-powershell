...
{
    [GraphRoute("GET", "/groups/{group-id}/sites/{site-id}/termStore/sets")]
    [Cmdlet(VerbsCommon.Get, "MgGroupSiteTermStoreSet_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.TermStore.Set))]
    public class GetMgGroupSiteTermStoreSet_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Sites[SiteId].TermStore.Sets.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Groups[GroupId].Sites[SiteId].TermStore.Sets.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
