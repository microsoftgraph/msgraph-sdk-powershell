...
{
    [GraphRoute("POST", "/sites/{site-id}/lists/{list-id}/contentTypes/{contentType-id}/columns")]
    [Cmdlet(VerbsCommon.New, "MgSiteListContentTypeColumn", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.ColumnDefinition))]
    public class NewMgSiteListContentTypeColumnCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Validation)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Sites[SiteId].Lists[ListId].ContentTypes[ContentTypeId].Columns.PostAsync(body, requestConfiguration =>
                {
