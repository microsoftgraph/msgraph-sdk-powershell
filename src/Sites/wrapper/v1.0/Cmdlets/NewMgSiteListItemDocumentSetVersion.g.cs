...
{
    [GraphRoute("POST", "/sites/{site-id}/lists/{list-id}/items/{listItem-id}/documentSetVersions")]
    [Cmdlet(VerbsCommon.New, "MgSiteListItemDocumentSetVersion", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.DocumentSetVersion))]
    public class NewMgSiteListItemDocumentSetVersionCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Items)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Sites[SiteId].Lists[ListId].Items[ListItemId].DocumentSetVersions.PostAsync(body, requestConfiguration =>
                {
