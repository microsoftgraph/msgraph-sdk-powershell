...
{
    [GraphRoute("POST", "/sites/{site-id}/lists/{list-id}/items/{listItem-id}/permissions")]
    [Cmdlet(VerbsCommon.New, "MgSiteListItemPermission", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.Permission))]
    public class NewMgSiteListItemPermissionCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Link)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Sites[SiteId].Lists[ListId].Items[ListItemId].Permissions.PostAsync(body, requestConfiguration =>
                {
