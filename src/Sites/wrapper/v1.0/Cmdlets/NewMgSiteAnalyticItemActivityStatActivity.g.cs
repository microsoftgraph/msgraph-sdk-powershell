...
{
    [GraphRoute("POST", "/sites/{site-id}/analytics/itemActivityStats/{itemActivityStat-id}/activities")]
    [Cmdlet(VerbsCommon.New, "MgSiteAnalyticItemActivityStatActivity", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.ItemActivity))]
    public class NewMgSiteAnalyticItemActivityStatActivityCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Actor)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Sites[SiteId].Analytics.ItemActivityStats[ItemActivityStatId].Activities.PostAsync(body, requestConfiguration =>
                {
