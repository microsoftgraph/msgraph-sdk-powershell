...
{
    [GraphRoute("GET", "/groups/{group-id}/sites/{site-id}/termStore/sets/{set-id}/terms")]
    [Cmdlet(VerbsCommon.Get, "MgGroupSiteTermStoreSetTerm_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.TermStore.Term))]
    public class GetMgGroupSiteTermStoreSetTerm_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Sites[SiteId].TermStore.Sets[SetId].Terms.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Groups[GroupId].Sites[SiteId].TermStore.Sets[SetId].Terms.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
