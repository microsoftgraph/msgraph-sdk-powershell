...
{
    [GraphRoute("GET", "/groups/{group-id}/sites/{site-id}/termStores")]
    [Cmdlet(VerbsCommon.Get, "MgGroupSiteTermStore")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.TermStore.Store))]
    public class GetMgGroupSiteTermStoreCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Sites.Client.Models.TermStore.StoreCollectionResponse? result;
            try
            {
                result = client.Groups[GroupId].Sites[SiteId].TermStores.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Groups[GroupId].Sites[SiteId].TermStores.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
