...
{
    [GraphRoute("GET", "/groups/{group-id}/sites/{site-id}/permissions")]
    [Cmdlet(VerbsCommon.Get, "MgGroupSitePermission_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.Permission))]
    public class GetMgGroupSitePermission_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Sites[SiteId].Permissions.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Groups[GroupId].Sites[SiteId].Permissions.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
