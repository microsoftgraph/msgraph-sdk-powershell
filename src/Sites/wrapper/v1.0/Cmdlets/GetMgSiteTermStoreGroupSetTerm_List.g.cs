...
{
    [GraphRoute("GET", "/sites/{site-id}/termStore/groups/{group-id}/sets/{set-id}/terms")]
    [Cmdlet(VerbsCommon.Get, "MgSiteTermStoreGroupSetTerm_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.TermStore.Term))]
    public class GetMgSiteTermStoreGroupSetTerm_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Sites[SiteId].TermStore.Groups[GroupId].Sets[SetId].Terms.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Sites[SiteId].TermStore.Groups[GroupId].Sets[SetId].Terms.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
