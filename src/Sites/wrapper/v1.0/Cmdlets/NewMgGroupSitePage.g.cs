...
{
    [GraphRoute("POST", "/groups/{group-id}/sites/{site-id}/pages")]
    [Cmdlet(VerbsCommon.New, "MgGroupSitePage", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.BaseSitePage))]
    public class NewMgGroupSitePageCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(PublishingState)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Sites[SiteId].Pages.PostAsync(body, requestConfiguration =>
                {
