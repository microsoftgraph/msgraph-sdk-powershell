...
{
    [GraphRoute("POST", "/groups/{group-id}/sites/{site-id}/lists/{list-id}/contentTypes")]
    [Cmdlet(VerbsCommon.New, "MgGroupSiteListContentType", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.ContentType))]
    public class NewMgGroupSiteListContentTypeCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Order)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Sites[SiteId].Lists[ListId].ContentTypes.PostAsync(body, requestConfiguration =>
                {
