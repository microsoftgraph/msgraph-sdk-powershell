...
{
    [GraphRoute("POST", "/groups/{group-id}/sites/{site-id}/lists/{list-id}/columns")]
    [Cmdlet(VerbsCommon.New, "MgGroupSiteListColumn", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.ColumnDefinition))]
    public class NewMgGroupSiteListColumnCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Validation)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Sites[SiteId].Lists[ListId].Columns.PostAsync(body, requestConfiguration =>
                {
