...
{
    [GraphRoute("POST", "/sites/{site-id}/termStores")]
    [Cmdlet(VerbsCommon.New, "MgSiteTermStore", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.TermStore.Store))]
    public class NewMgSiteTermStoreCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(LanguageTags)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Sites[SiteId].TermStores.PostAsync(body, requestConfiguration =>
                {
