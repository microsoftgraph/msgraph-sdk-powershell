...
{
    [GraphRoute("POST", "/groups/{group-id}/sites/{site-id}/termStore/groups/{group-id1}/sets/{set-id}/relations")]
    [Cmdlet(VerbsCommon.New, "MgGroupSiteTermStoreGroupSetRelation", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.TermStore.Relation))]
    public class NewMgGroupSiteTermStoreGroupSetRelationCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Relationship)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Sites[SiteId].TermStore.Groups[GroupId1].Sets[SetId].Relations.PostAsync(body, requestConfiguration =>
                {
