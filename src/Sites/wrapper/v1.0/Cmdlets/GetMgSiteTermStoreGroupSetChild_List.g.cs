...
{
    [GraphRoute("GET", "/sites/{site-id}/termStore/groups/{group-id}/sets/{set-id}/children")]
    [Cmdlet(VerbsCommon.Get, "MgSiteTermStoreGroupSetChild_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.TermStore.Term))]
    public class GetMgSiteTermStoreGroupSetChild_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Sites[SiteId].TermStore.Groups[GroupId].Sets[SetId].Children.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Sites[SiteId].TermStore.Groups[GroupId].Sets[SetId].Children.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
