...
{
    [GraphRoute("POST", "/sites/{site-id}/termStore/sets")]
    [Cmdlet(VerbsCommon.New, "MgSiteTermStoreSet", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.TermStore.Set))]
    public class NewMgSiteTermStoreSetCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(ParentGroup)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Sites[SiteId].TermStore.Sets.PostAsync(body, requestConfiguration =>
                {
