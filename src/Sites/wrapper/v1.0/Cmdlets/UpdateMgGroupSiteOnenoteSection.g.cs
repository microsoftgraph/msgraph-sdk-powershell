...
{
    [GraphRoute("PATCH", "/groups/{group-id}/sites/{site-id}/onenote/sections/{onenoteSection-id}")]
    [Cmdlet(VerbsData.Update, "MgGroupSiteOnenoteSection", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.OnenoteSection))]
    public class UpdateMgGroupSiteOnenoteSectionCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Links)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Sites[SiteId].Onenote.Sections[OnenoteSectionId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Groups[GroupId].Sites[SiteId].Onenote.Sections[OnenoteSectionId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, OnenoteSectionId);
