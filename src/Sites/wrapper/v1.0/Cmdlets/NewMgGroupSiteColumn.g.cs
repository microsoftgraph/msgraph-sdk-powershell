...
{
    [GraphRoute("POST", "/groups/{group-id}/sites/{site-id}/columns")]
    [Cmdlet(VerbsCommon.New, "MgGroupSiteColumn", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.ColumnDefinition))]
    public class NewMgGroupSiteColumnCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Validation)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Sites[SiteId].Columns.PostAsync(body, requestConfiguration =>
                {
