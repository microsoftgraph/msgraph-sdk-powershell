...
{
    [GraphRoute("POST", "/sites/{site-id}/columns")]
    [Cmdlet(VerbsCommon.New, "MgSiteColumn", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.ColumnDefinition))]
    public class NewMgSiteColumnCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Validation)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Sites[SiteId].Columns.PostAsync(body, requestConfiguration =>
                {
