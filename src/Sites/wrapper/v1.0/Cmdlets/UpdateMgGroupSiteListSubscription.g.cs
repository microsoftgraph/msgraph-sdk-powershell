...
{
    [GraphRoute("PATCH", "/groups/{group-id}/sites/{site-id}/lists/{list-id}/subscriptions/{subscription-id}")]
    [Cmdlet(VerbsData.Update, "MgGroupSiteListSubscription", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.Subscription))]
    public class UpdateMgGroupSiteListSubscriptionCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Resource)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Sites[SiteId].Lists[ListId].Subscriptions[SubscriptionId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Groups[GroupId].Sites[SiteId].Lists[ListId].Subscriptions[SubscriptionId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException)
                {
                    ThrowGraphRequestFailed(ex, SubscriptionId);
