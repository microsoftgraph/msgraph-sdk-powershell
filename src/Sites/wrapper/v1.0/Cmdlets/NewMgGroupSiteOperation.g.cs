...
{
    [GraphRoute("POST", "/groups/{group-id}/sites/{site-id}/operations")]
    [Cmdlet(VerbsCommon.New, "MgGroupSiteOperation", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.RichLongRunningOperation))]
    public class NewMgGroupSiteOperationCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Error)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Sites[SiteId].Operations.PostAsync(body, requestConfiguration =>
                {
