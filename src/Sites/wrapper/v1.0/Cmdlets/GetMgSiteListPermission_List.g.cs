...
{
    [GraphRoute("GET", "/sites/{site-id}/lists/{list-id}/permissions")]
    [Cmdlet(VerbsCommon.Get, "MgSiteListPermission_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.Permission))]
    public class GetMgSiteListPermission_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Sites[SiteId].Lists[ListId].Permissions.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Sites[SiteId].Lists[ListId].Permissions.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
