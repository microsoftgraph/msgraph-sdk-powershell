...
{
    [GraphRoute("POST", "/groups/{group-id}/sites/{site-id}/pages/{baseSitePage-id}/graph.sitePage/canvasLayout/horizontalSections")]
    [Cmdlet(VerbsCommon.New, "MgGroupSitePageAsSitePageCanvaLayoutHorizontalSection", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.HorizontalSection))]
    public class NewMgGroupSitePageAsSitePageCanvaLayoutHorizontalSectionCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Layout)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Sites[SiteId].Pages[BaseSitePageId].GraphSitePage.CanvasLayout.HorizontalSections.PostAsync(body, requestConfiguration =>
                {
