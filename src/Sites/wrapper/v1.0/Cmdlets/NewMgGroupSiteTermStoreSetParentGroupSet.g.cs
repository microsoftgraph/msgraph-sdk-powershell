...
{
    [GraphRoute("POST", "/groups/{group-id}/sites/{site-id}/termStore/sets/{set-id}/parentGroup/sets")]
    [Cmdlet(VerbsCommon.New, "MgGroupSiteTermStoreSetParentGroupSet", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.TermStore.Set))]
    public class NewMgGroupSiteTermStoreSetParentGroupSetCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(ParentGroup)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Sites[SiteId].TermStore.Sets[SetId].ParentGroup.Sets.PostAsync(body, requestConfiguration =>
                {
