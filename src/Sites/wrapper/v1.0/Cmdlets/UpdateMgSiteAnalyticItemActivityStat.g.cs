...
{
    [GraphRoute("PATCH", "/sites/{site-id}/analytics/itemActivityStats/{itemActivityStat-id}")]
    [Cmdlet(VerbsData.Update, "MgSiteAnalyticItemActivityStat", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.ItemActivityStat))]
    public class UpdateMgSiteAnalyticItemActivityStatCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Move)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Sites[SiteId].Analytics.ItemActivityStats[ItemActivityStatId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Sites[SiteId].Analytics.ItemActivityStats[ItemActivityStatId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException)
                {
                    ThrowGraphRequestFailed(ex, ItemActivityStatId);
