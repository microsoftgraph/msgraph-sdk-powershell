...
{
    [GraphRoute("PATCH", "/sites/{site-id}/termStore/groups/{group-id}/sets/{set-id}/parentGroup")]
    [Cmdlet(VerbsData.Update, "MgSiteTermStoreGroupSetParentGroup", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.TermStore.Group))]
    public class UpdateMgSiteTermStoreGroupSetParentGroupCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Scope)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Sites[SiteId].TermStore.Groups[GroupId].Sets[SetId].ParentGroup.PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Sites[SiteId].TermStore.Groups[GroupId].Sets[SetId].ParentGroup.GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex)
                {
                    ThrowGraphRequestFailed(ex, SetId);
