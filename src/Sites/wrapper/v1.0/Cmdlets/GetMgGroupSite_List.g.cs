...
{
    [GraphRoute("GET", "/groups/{group-id}/sites")]
    [Cmdlet(VerbsCommon.Get, "MgGroupSite_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.Site))]
    public class GetMgGroupSite_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Sites.Client.Models.SiteCollectionResponse? result;
            try
            {
                result = client.Groups[GroupId].Sites.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Groups[GroupId].Sites.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
