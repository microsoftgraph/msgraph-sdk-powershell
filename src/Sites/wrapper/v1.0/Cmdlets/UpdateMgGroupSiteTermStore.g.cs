...
{
    [GraphRoute("PATCH", "/groups/{group-id}/sites/{site-id}/termStore")]
    [Cmdlet(VerbsData.Update, "MgGroupSiteTermStore", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.TermStore.Store))]
    public class UpdateMgGroupSiteTermStoreCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(LanguageTags)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Sites[SiteId].TermStore.PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Groups[GroupId].Sites[SiteId].TermStore.GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException)
                {
                    ThrowGraphRequestFailed(ex, SiteId);
