...
{
    [GraphRoute("PATCH", "/sites/{site-id}/lists/{list-id}/items/{listItem-id}")]
    [Cmdlet(VerbsData.Update, "MgSiteListItem", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.ListItem))]
    public class UpdateMgSiteListItemCommand : PSCmdlet
    {
        ...
        [Parameter(Mandatory = false)]
        public Microsoft.Graph.PowerShell.Sites.Client.Models.ContentTypeInfo? ContentType { get; set; }

        [Parameter(Mandatory = false)]
        public Microsoft.Graph.PowerShell.Sites.Client.Models.Deleted? Deleted { get; set; }
        ...
        protected override void ProcessRecord()
        {
            ...
            var body = new Microsoft.Graph.PowerShell.Sites.Client.Models.ListItem();
    ...
    if (this.IsParameterBound(nameof(SharepointIds)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Sites.Client.Models.ListItem? result;
            try
            {
                result = client.Sites[SiteId].Lists[ListId].Items[ListItemId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Sites[SiteId].Lists[ListId].Items[ListItemId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex)
                {
                    ThrowTerminatingError(new ErrorRecord(ex, "GraphRequestFailed", ErrorCategory.InvalidOperation, ListItemId));
