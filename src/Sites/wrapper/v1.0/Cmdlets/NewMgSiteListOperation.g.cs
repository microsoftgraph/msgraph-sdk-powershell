...
{
    [GraphRoute("POST", "/sites/{site-id}/lists/{list-id}/operations")]
    [Cmdlet(VerbsCommon.New, "MgSiteListOperation", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.RichLongRunningOperation))]
    public class NewMgSiteListOperationCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Error)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Sites[SiteId].Lists[ListId].Operations.PostAsync(body, requestConfiguration =>
                {
