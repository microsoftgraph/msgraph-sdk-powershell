...
{
    [GraphRoute("POST", "/sites/{site-id}/termStore/groups/{group-id}/sets/{set-id}/relations")]
    [Cmdlet(VerbsCommon.New, "MgSiteTermStoreGroupSetRelation", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.TermStore.Relation))]
    public class NewMgSiteTermStoreGroupSetRelationCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Relationship)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Sites[SiteId].TermStore.Groups[GroupId].Sets[SetId].Relations.PostAsync(body, requestConfiguration =>
                {
