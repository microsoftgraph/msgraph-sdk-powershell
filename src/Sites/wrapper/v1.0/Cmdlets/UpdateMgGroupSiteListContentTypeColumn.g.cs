...
{
    [GraphRoute("PATCH", "/groups/{group-id}/sites/{site-id}/lists/{list-id}/contentTypes/{contentType-id}/columns/{columnDefinition-id}")]
    [Cmdlet(VerbsData.Update, "MgGroupSiteListContentTypeColumn", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.ColumnDefinition))]
    public class UpdateMgGroupSiteListContentTypeColumnCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Validation)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Sites[SiteId].Lists[ListId].ContentTypes[ContentTypeId].Columns[ColumnDefinitionId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Groups[GroupId].Sites[SiteId].Lists[ListId].ContentTypes[ContentTypeId].Columns[ColumnDefinitionId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, ColumnDefinitionId);
