...
{
    [GraphRoute("PATCH", "/sites/{site-id}")]
    [Cmdlet(VerbsData.Update, "MgSite", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.Site))]
    public class UpdateMgSiteCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(SiteCollection)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Sites[SiteId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Sites[SiteId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex)
                {
                    ThrowGraphRequestFailed(ex, SiteId);
