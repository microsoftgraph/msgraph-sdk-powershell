...
{
    [GraphRoute("GET", "/sites/{site-id}/contentTypes/{contentType-id}/columns")]
    [Cmdlet(VerbsCommon.Get, "MgSiteContentTypeColumn_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.ColumnDefinition))]
    public class GetMgSiteContentTypeColumn_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Sites[SiteId].ContentTypes[ContentTypeId].Columns.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Sites[SiteId].ContentTypes[ContentTypeId].Columns.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
