...
{
    [GraphRoute("POST", "/sites/{site-id}/termStore/sets/{set-id}/terms")]
    [Cmdlet(VerbsCommon.New, "MgSiteTermStoreSetTerm", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.TermStore.Term))]
    public class NewMgSiteTermStoreSetTermCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Properties)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Sites[SiteId].TermStore.Sets[SetId].Terms.PostAsync(body, requestConfiguration =>
                {
