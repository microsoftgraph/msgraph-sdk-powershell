...
{
    [GraphRoute("POST", "/sites/{site-id}/termStore/groups/{group-id}/sets/{set-id}/children")]
    [Cmdlet(VerbsCommon.New, "MgSiteTermStoreGroupSetChild", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.TermStore.Term))]
    public class NewMgSiteTermStoreGroupSetChildCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Properties)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Sites[SiteId].TermStore.Groups[GroupId].Sets[SetId].Children.PostAsync(body, requestConfiguration =>
                {
