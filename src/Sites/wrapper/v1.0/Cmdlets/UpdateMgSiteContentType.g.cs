...
{
    [GraphRoute("PATCH", "/sites/{site-id}/contentTypes/{contentType-id}")]
    [Cmdlet(VerbsData.Update, "MgSiteContentType", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.ContentType))]
    public class UpdateMgSiteContentTypeCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Order)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Sites[SiteId].ContentTypes[ContentTypeId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Sites[SiteId].ContentTypes[ContentTypeId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException)
                {
                    ThrowGraphRequestFailed(ex, ContentTypeId);
