...
{
    [GraphRoute("POST", "/sites/{site-id}/termStore/groups/{group-id}/sets/{set-id}/terms")]
    [Cmdlet(VerbsCommon.New, "MgSiteTermStoreGroupSetTerm", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.TermStore.Term))]
    public class NewMgSiteTermStoreGroupSetTermCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Properties)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Sites[SiteId].TermStore.Groups[GroupId].Sets[SetId].Terms.PostAsync(body, requestConfiguration =>
                {
