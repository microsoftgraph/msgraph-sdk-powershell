...
{
    [GraphRoute("POST", "/groups/{group-id}/sites/{site-id}/termStore/sets/{set-id}/relations")]
    [Cmdlet(VerbsCommon.New, "MgGroupSiteTermStoreSetRelation", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.TermStore.Relation))]
    public class NewMgGroupSiteTermStoreSetRelationCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Relationship)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Sites[SiteId].TermStore.Sets[SetId].Relations.PostAsync(body, requestConfiguration =>
                {
