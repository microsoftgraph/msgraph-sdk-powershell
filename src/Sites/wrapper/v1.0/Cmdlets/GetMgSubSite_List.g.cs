...
{
    [GraphRoute("GET", "/sites/{site-id}/sites")]
    [Cmdlet(VerbsCommon.Get, "MgSubSite_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.Site))]
    public class GetMgSubSite_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Sites[SiteId].Sites.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Sites[SiteId].Sites.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
