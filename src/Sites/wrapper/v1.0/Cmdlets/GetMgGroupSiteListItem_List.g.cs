...
{
    [GraphRoute("GET", "/groups/{group-id}/sites/{site-id}/lists/{list-id}/items")]
    [Cmdlet(VerbsCommon.Get, "MgGroupSiteListItem_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.ListItem))]
    public class GetMgGroupSiteListItem_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Sites.Client.Models.ListItemCollectionResponse? result;
            try
            {
                result = client.Groups[GroupId].Sites[SiteId].Lists[ListId].Items.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Groups[GroupId].Sites[SiteId].Lists[ListId].Items.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
