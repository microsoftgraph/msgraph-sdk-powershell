...
{
    [GraphRoute("POST", "/groups/{group-id}/sites/{site-id}/permissions")]
    [Cmdlet(VerbsCommon.New, "MgGroupSitePermission", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.Permission))]
    public class NewMgGroupSitePermissionCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Link)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Sites[SiteId].Permissions.PostAsync(body, requestConfiguration =>
                {
