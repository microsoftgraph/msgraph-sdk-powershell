...
{
    [GraphRoute("POST", "/sites/{site-id}/termStore/sets/{set-id}/relations")]
    [Cmdlet(VerbsCommon.New, "MgSiteTermStoreSetRelation", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.TermStore.Relation))]
    public class NewMgSiteTermStoreSetRelationCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Relationship)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Sites[SiteId].TermStore.Sets[SetId].Relations.PostAsync(body, requestConfiguration =>
                {
