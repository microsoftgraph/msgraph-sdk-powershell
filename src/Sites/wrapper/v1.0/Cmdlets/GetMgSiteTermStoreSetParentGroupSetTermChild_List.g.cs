...
{
    [GraphRoute("GET", "/sites/{site-id}/termStore/sets/{set-id}/parentGroup/sets/{set-id1}/terms/{term-id}/children")]
    [Cmdlet(VerbsCommon.Get, "MgSiteTermStoreSetParentGroupSetTermChild_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.TermStore.Term))]
    public class GetMgSiteTermStoreSetParentGroupSetTermChild_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Sites[SiteId].TermStore.Sets[SetId].ParentGroup.Sets[SetId1].Terms[TermId].Children.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Sites[SiteId].TermStore.Sets[SetId].ParentGroup.Sets[SetId1].Terms[TermId].Children.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
