...
{
    [GraphRoute("POST", "/groups/{group-id}/sites/{site-id}/onenote/operations")]
    [Cmdlet(VerbsCommon.New, "MgGroupSiteOnenoteOperation", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.OnenoteOperation))]
    public class NewMgGroupSiteOnenoteOperationCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Error)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Sites[SiteId].Onenote.Operations.PostAsync(body, requestConfiguration =>
                {
