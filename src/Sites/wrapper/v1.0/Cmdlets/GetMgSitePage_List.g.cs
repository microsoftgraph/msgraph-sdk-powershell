...
{
    [GraphRoute("GET", "/sites/{site-id}/pages")]
    [Cmdlet(VerbsCommon.Get, "MgSitePage_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.BaseSitePage))]
    public class GetMgSitePage_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Sites.Client.Models.BaseSitePageCollectionResponse? result;
            try
            {
                result = client.Sites[SiteId].Pages.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Sites[SiteId].Pages.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
