...
{
    [GraphRoute("GET", "/groups/{group-id}/sites/{site-id}/onenote/notebooks")]
    [Cmdlet(VerbsCommon.Get, "MgGroupSiteOnenoteNotebook_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.Notebook))]
    public class GetMgGroupSiteOnenoteNotebook_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Sites.Client.Models.NotebookCollectionResponse? result;
            try
            {
                result = client.Groups[GroupId].Sites[SiteId].Onenote.Notebooks.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Groups[GroupId].Sites[SiteId].Onenote.Notebooks.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
