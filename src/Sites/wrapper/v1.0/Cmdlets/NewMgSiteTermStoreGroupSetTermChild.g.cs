...
{
    [GraphRoute("POST", "/sites/{site-id}/termStore/groups/{group-id}/sets/{set-id}/terms/{term-id}/children")]
    [Cmdlet(VerbsCommon.New, "MgSiteTermStoreGroupSetTermChild", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.TermStore.Term))]
    public class NewMgSiteTermStoreGroupSetTermChildCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Properties)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Sites[SiteId].TermStore.Groups[GroupId].Sets[SetId].Terms[TermId].Children.PostAsync(body, requestConfiguration =>
                {
