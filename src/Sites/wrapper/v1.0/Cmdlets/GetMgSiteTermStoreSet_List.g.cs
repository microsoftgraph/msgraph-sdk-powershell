...
{
    [GraphRoute("GET", "/sites/{site-id}/termStore/sets")]
    [Cmdlet(VerbsCommon.Get, "MgSiteTermStoreSet_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.TermStore.Set))]
    public class GetMgSiteTermStoreSet_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Sites[SiteId].TermStore.Sets.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Sites[SiteId].TermStore.Sets.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
