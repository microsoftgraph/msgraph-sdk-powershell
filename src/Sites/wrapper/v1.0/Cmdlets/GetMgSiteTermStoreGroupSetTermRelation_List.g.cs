...
{
    [GraphRoute("GET", "/sites/{site-id}/termStore/groups/{group-id}/sets/{set-id}/terms/{term-id}/relations")]
    [Cmdlet(VerbsCommon.Get, "MgSiteTermStoreGroupSetTermRelation_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.TermStore.Relation))]
    public class GetMgSiteTermStoreGroupSetTermRelation_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Sites[SiteId].TermStore.Groups[GroupId].Sets[SetId].Terms[TermId].Relations.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Sites[SiteId].TermStore.Groups[GroupId].Sets[SetId].Terms[TermId].Relations.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
