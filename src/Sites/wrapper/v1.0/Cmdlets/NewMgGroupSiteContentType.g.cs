...
{
    [GraphRoute("POST", "/groups/{group-id}/sites/{site-id}/contentTypes")]
    [Cmdlet(VerbsCommon.New, "MgGroupSiteContentType", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.ContentType))]
    public class NewMgGroupSiteContentTypeCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Order)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Sites[SiteId].ContentTypes.PostAsync(body, requestConfiguration =>
                {
