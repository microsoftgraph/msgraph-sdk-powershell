...
{
    [GraphRoute("POST", "/sites/{site-id}/lists/{list-id}/columns")]
    [Cmdlet(VerbsCommon.New, "MgSiteListColumn", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.ColumnDefinition))]
    public class NewMgSiteListColumnCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Validation)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Sites[SiteId].Lists[ListId].Columns.PostAsync(body, requestConfiguration =>
                {
