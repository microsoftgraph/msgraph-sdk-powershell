...
{
    [GraphRoute("GET", "/groups/{group-id}/sites/{site-id}/createdByUser/serviceProvisioningErrors")]
    [Cmdlet(VerbsCommon.Get, "MgGroupSiteCreatedByUserServiceProvisioningError")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.ServiceProvisioningError))]
    public class GetMgGroupSiteCreatedByUserServiceProvisioningErrorCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Sites[SiteId].CreatedByUser.ServiceProvisioningErrors.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Groups[GroupId].Sites[SiteId].CreatedByUser.ServiceProvisioningErrors.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
