...
{
    [GraphRoute("GET", "/sites/{site-id}/lists/{list-id}/columns")]
    [Cmdlet(VerbsCommon.Get, "MgSiteListColumn_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.ColumnDefinition))]
    public class GetMgSiteListColumn_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Sites[SiteId].Lists[ListId].Columns.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Sites[SiteId].Lists[ListId].Columns.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
