...
{
    [GraphRoute("PATCH", "/sites/{site-id}/termStore/groups/{group-id}/sets/{set-id}")]
    [Cmdlet(VerbsData.Update, "MgSiteTermStoreGroupSet", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.TermStore.Set))]
    public class UpdateMgSiteTermStoreGroupSetCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(ParentGroup)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Sites[SiteId].TermStore.Groups[GroupId].Sets[SetId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Sites[SiteId].TermStore.Groups[GroupId].Sets[SetId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex)
                {
                    ThrowGraphRequestFailed(ex, SetId);
