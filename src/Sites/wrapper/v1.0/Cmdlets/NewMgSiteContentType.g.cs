...
{
    [GraphRoute("POST", "/sites/{site-id}/contentTypes")]
    [Cmdlet(VerbsCommon.New, "MgSiteContentType", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.ContentType))]
    public class NewMgSiteContentTypeCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Order)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Sites[SiteId].ContentTypes.PostAsync(body, requestConfiguration =>
                {
