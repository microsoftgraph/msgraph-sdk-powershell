...
{
    [GraphRoute("POST", "/sites/{site-id}/termStore/groups/{group-id}/sets")]
    [Cmdlet(VerbsCommon.New, "MgSiteTermStoreGroupSet", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.TermStore.Set))]
    public class NewMgSiteTermStoreGroupSetCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(ParentGroup)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Sites[SiteId].TermStore.Groups[GroupId].Sets.PostAsync(body, requestConfiguration =>
                {
