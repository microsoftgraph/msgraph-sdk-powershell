...
{
    [GraphRoute("POST", "/sites/{site-id}/termStore/sets/{set-id}/parentGroup/sets/{set-id1}/children")]
    [Cmdlet(VerbsCommon.New, "MgSiteTermStoreSetParentGroupSetChild", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.TermStore.Term))]
    public class NewMgSiteTermStoreSetParentGroupSetChildCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Properties)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Sites[SiteId].TermStore.Sets[SetId].ParentGroup.Sets[SetId1].Children.PostAsync(body, requestConfiguration =>
                {
