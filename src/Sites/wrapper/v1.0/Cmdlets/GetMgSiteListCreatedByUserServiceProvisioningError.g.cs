...
{
    [GraphRoute("GET", "/sites/{site-id}/lists/{list-id}/createdByUser/serviceProvisioningErrors")]
    [Cmdlet(VerbsCommon.Get, "MgSiteListCreatedByUserServiceProvisioningError")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.ServiceProvisioningError))]
    public class GetMgSiteListCreatedByUserServiceProvisioningErrorCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Sites[SiteId].Lists[ListId].CreatedByUser.ServiceProvisioningErrors.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Sites[SiteId].Lists[ListId].CreatedByUser.ServiceProvisioningErrors.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
