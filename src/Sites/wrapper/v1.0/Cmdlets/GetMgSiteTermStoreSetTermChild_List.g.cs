...
{
    [GraphRoute("GET", "/sites/{site-id}/termStore/sets/{set-id}/terms/{term-id}/children")]
    [Cmdlet(VerbsCommon.Get, "MgSiteTermStoreSetTermChild_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.TermStore.Term))]
    public class GetMgSiteTermStoreSetTermChild_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Sites[SiteId].TermStore.Sets[SetId].Terms[TermId].Children.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Sites[SiteId].TermStore.Sets[SetId].Terms[TermId].Children.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
