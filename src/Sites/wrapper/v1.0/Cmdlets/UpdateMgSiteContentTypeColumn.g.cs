...
{
    [GraphRoute("PATCH", "/sites/{site-id}/contentTypes/{contentType-id}/columns/{columnDefinition-id}")]
    [Cmdlet(VerbsData.Update, "MgSiteContentTypeColumn", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.ColumnDefinition))]
    public class UpdateMgSiteContentTypeColumnCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Validation)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Sites[SiteId].ContentTypes[ContentTypeId].Columns[ColumnDefinitionId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Sites[SiteId].ContentTypes[ContentTypeId].Columns[ColumnDefinitionId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, ColumnDefinitionId);
