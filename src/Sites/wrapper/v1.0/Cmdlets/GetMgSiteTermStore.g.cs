...
{
    [GraphRoute("GET", "/sites/{site-id}/termStores")]
    [Cmdlet(VerbsCommon.Get, "MgSiteTermStore")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.TermStore.Store))]
    public class GetMgSiteTermStoreCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Sites[SiteId].TermStores.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Sites[SiteId].TermStores.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
