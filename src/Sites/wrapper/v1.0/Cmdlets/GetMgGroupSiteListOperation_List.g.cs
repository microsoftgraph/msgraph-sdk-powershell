...
{
    [GraphRoute("GET", "/groups/{group-id}/sites/{site-id}/lists/{list-id}/operations")]
    [Cmdlet(VerbsCommon.Get, "MgGroupSiteListOperation_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Sites.Client.Models.RichLongRunningOperation))]
    public class GetMgGroupSiteListOperation_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Sites.Client.Models.RichLongRunningOperationCollectionResponse? result;
            try
            {
                result = client.Groups[GroupId].Sites[SiteId].Lists[ListId].Operations.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Groups[GroupId].Sites[SiteId].Lists[ListId].Operations.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
