...
{
    [GraphRoute("PATCH", "/users/{user-id}/activities/{userActivity-id}")]
    [Cmdlet(VerbsData.Update, "MgUserActivity", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.CrossDeviceExperiences.Client.Models.UserActivity))]
    public class UpdateMgUserActivityCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(UserActivityId, "Update"))
                return;
    ...
    if (this.IsParameterBound(nameof(ContentInfo)))
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].Activities[UserActivityId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Users[UserId].Activities[UserActivityId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException)
                {
                    ThrowGraphRequestFailed(ex, UserActivityId);
