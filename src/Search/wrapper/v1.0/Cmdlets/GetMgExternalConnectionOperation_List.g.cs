...
{
    [GraphRoute("GET", "/external/connections/{externalConnection-id}/operations")]
    [Cmdlet(VerbsCommon.Get, "MgExternalConnectionOperation_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Search.Client.Models.ExternalConnectors.ConnectionOperation))]
    public class GetMgExternalConnectionOperation_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Search.Client.Models.ExternalConnectors.ConnectionOperationCollectionResponse? result;
            try
            {
                result = client.External.Connections[ExternalConnectionId].Operations.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.External.Connections[ExternalConnectionId].Operations.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
