...
namespace Microsoft.Graph.PowerShell.Search
{
    ...
    [Cmdlet(VerbsCommon.Get, "MgExternalConnectionGroup_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Search.Client.Models.ExternalConnectors.ExternalGroup))]
    public class GetMgExternalConnectionGroup_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Search.Client.Models.ExternalConnectors.ExternalGroupCollectionResponse? result;
            try
            {
                result = client.External.Connections[ExternalConnectionId].Groups.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.External.Connections[ExternalConnectionId].Groups.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
