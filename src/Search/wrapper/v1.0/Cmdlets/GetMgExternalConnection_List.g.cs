...
namespace Microsoft.Graph.PowerShell.Search
{
    ...
    [Cmdlet(VerbsCommon.Get, "MgExternalConnection_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Search.Client.Models.ExternalConnectors.ExternalConnection))]
    public class GetMgExternalConnection_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Search.Client.Models.ExternalConnectors.ExternalConnectionCollectionResponse? result;
            try
            {
                result = client.External.Connections.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.External.Connections.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
