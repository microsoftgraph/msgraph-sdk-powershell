...
{
    [GraphRoute("PATCH", "/search/bookmarks/{bookmark-id}")]
    [Cmdlet(VerbsData.Update, "MgSearchBookmark", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Search.Client.Models.Search.Bookmark))]
    public class UpdateMgSearchBookmarkCommand : PSCmdlet
    {
        ...
        {
            if (!ShouldProcess(BookmarkId, "Update"))
                return;
    ...
    if (this.IsParameterBound(nameof(TargetedVariations)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Search.Bookmarks[BookmarkId].PatchAsync(body, requestConfiguration =>
                {
            ...
            {
                ThrowTerminatingError(new ErrorRecord(ex, "GraphRequestFailed", ErrorCategory.InvalidOperation, BookmarkId));
                return;
            }
