...
{
    [GraphRoute("POST", "/external/connections")]
    [Cmdlet(VerbsCommon.New, "MgExternalConnection", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Search.Client.Models.ExternalConnectors.ExternalConnection))]
    public class NewMgExternalConnectionCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(SearchSettings)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.External.Connections.PostAsync(body, requestConfiguration =>
                {
