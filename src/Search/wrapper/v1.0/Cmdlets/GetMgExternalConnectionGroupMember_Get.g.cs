...
{
    [GraphRoute("GET", "/external/connections/{externalConnection-id}/groups/{externalGroup-id}/members/{identity-id}")]
    [Cmdlet(VerbsCommon.Get, "MgExternalConnectionGroupMember_Get")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Search.Client.Models.ExternalConnectors.Identity))]
    public class GetMgExternalConnectionGroupMember_GetCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.External.Connections[ExternalConnectionId].Groups[ExternalGroupId].Members[IdentityId].GetAsync(requestConfiguration =>
                {
