...
{
    [GraphRoute("POST", "/servicePrincipals/{servicePrincipal-id}/federatedIdentityCredentials")]
    [Cmdlet(VerbsCommon.New, "MgServicePrincipalFederatedIdentityCredential", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Applications.Client.Models.FederatedIdentityCredential))]
    public class NewMgServicePrincipalFederatedIdentityCredentialCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(ClaimsMatchingExpression)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.ServicePrincipals[ServicePrincipalId].FederatedIdentityCredentials.PostAsync(body, requestConfiguration =>
                {
