...
{
    [GraphRoute("PATCH", "/onPremisesPublishingProfiles/{onPremisesPublishingProfile-id}/agentGroups/{onPremisesAgentGroup-id}/agents/{onPremisesAgent-id}")]
    [Cmdlet(VerbsData.Update, "MgOnPremisePublishingProfileAgentGroupAgent", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Applications.Client.Models.OnPremisesAgent))]
    public class UpdateMgOnPremisePublishingProfileAgentGroupAgentCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(SupportedPublishingTypes)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.OnPremisesPublishingProfiles[OnPremisesPublishingProfileId].AgentGroups[OnPremisesAgentGroupId].Agents[OnPremisesAgentId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.OnPremisesPublishingProfiles[OnPremisesPublishingProfileId].AgentGroups[OnPremisesAgentGroupId].Agents[OnPremisesAgentId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, OnPremisesAgentId);
