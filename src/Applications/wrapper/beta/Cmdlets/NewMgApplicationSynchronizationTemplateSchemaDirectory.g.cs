...
{
    [GraphRoute("POST", "/applications/{application-id}/synchronization/templates/{synchronizationTemplate-id}/schema/directories")]
    [Cmdlet(VerbsCommon.New, "MgApplicationSynchronizationTemplateSchemaDirectory", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Applications.Client.Models.DirectoryDefinition))]
    public class NewMgApplicationSynchronizationTemplateSchemaDirectoryCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(SynchronizationTemplateId, "New"))
                return;
    ...
    if (this.IsParameterBound(nameof(Objects)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Applications[ApplicationId].Synchronization.Templates[SynchronizationTemplateId].Schema.Directories.PostAsync(body, requestConfiguration =>
                {
