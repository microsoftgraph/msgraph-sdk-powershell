...
{
    [GraphRoute("GET", "/servicePrincipals/{servicePrincipal-id}/oauth2PermissionGrants")]
    [Cmdlet(VerbsCommon.Get, "MgServicePrincipalOauth2PermissionGrant_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Applications.Client.Models.OAuth2PermissionGrant))]
    public class GetMgServicePrincipalOauth2PermissionGrant_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Applications.Client.Models.OAuth2PermissionGrantCollectionResponse? result;
            try
            {
                result = client.ServicePrincipals[ServicePrincipalId].Oauth2PermissionGrants.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.ServicePrincipals[ServicePrincipalId].Oauth2PermissionGrants.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
