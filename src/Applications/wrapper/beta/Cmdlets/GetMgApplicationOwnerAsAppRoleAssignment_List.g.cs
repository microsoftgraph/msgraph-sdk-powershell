...
{
    [GraphRoute("GET", "/applications/{application-id}/owners/graph.appRoleAssignment")]
    [Cmdlet(VerbsCommon.Get, "MgApplicationOwnerAsAppRoleAssignment_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Applications.Client.Models.AppRoleAssignment))]
    public class GetMgApplicationOwnerAsAppRoleAssignment_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Applications[ApplicationId].Owners.GraphAppRoleAssignment.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Applications[ApplicationId].Owners.GraphAppRoleAssignment.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
