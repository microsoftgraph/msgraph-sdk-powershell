...
{
    [GraphRoute("PATCH", "/users/{user-id}/appRoleAssignments/{appRoleAssignment-id}")]
    [Cmdlet(VerbsData.Update, "MgUserAppRoleAssignment", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Applications.Client.Models.AppRoleAssignment))]
    public class UpdateMgUserAppRoleAssignmentCommand : GraphClientCmdlet
    {
        ...
        [Parameter(Mandatory = false)]
        public global::System.DateTimeOffset? CreationTimestamp { get; set; }
    ...
    if (this.IsParameterBound(nameof(CreationTimestamp)))
        body.CreationTimestamp = CreationTimestamp;
    ...
    if (this.IsParameterBound(nameof(ResourceId)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].AppRoleAssignments[AppRoleAssignmentId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Users[UserId].AppRoleAssignments[AppRoleAssignmentId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, AppRoleAssignmentId);
