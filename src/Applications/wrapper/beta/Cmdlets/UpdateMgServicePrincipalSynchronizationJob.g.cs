...
{
    [GraphRoute("PATCH", "/servicePrincipals/{servicePrincipal-id}/synchronization/jobs/{synchronizationJob-id}")]
    [Cmdlet(VerbsData.Update, "MgServicePrincipalSynchronizationJob", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Applications.Client.Models.SynchronizationJob))]
    public class UpdateMgServicePrincipalSynchronizationJobCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(SynchronizationJobSettings)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.ServicePrincipals[ServicePrincipalId].Synchronization.Jobs[SynchronizationJobId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.ServicePrincipals[ServicePrincipalId].Synchronization.Jobs[SynchronizationJobId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, SynchronizationJobId);
