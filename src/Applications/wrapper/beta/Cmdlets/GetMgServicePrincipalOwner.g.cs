...
{
    [GraphRoute("GET", "/servicePrincipals/{servicePrincipal-id}/owners")]
    [Cmdlet(VerbsCommon.Get, "MgServicePrincipalOwner")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Applications.Client.Models.DirectoryObject))]
    public class GetMgServicePrincipalOwnerCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.ServicePrincipals[ServicePrincipalId].Owners.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.ServicePrincipals[ServicePrincipalId].Owners.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
