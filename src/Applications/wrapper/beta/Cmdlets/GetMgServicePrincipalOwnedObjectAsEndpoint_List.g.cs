...
{
    [GraphRoute("GET", "/servicePrincipals/{servicePrincipal-id}/ownedObjects/graph.endpoint")]
    [Cmdlet(VerbsCommon.Get, "MgServicePrincipalOwnedObjectAsEndpoint_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Applications.Client.Models.Endpoint))]
    public class GetMgServicePrincipalOwnedObjectAsEndpoint_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Applications.Client.Models.EndpointCollectionResponse? result;
            try
            {
                result = client.ServicePrincipals[ServicePrincipalId].OwnedObjects.GraphEndpoint.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.ServicePrincipals[ServicePrincipalId].OwnedObjects.GraphEndpoint.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
