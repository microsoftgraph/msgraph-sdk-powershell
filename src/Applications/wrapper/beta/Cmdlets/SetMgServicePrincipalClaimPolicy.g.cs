...
{
    [GraphRoute("PUT", "/servicePrincipals/{servicePrincipal-id}/claimsPolicy")]
    [Cmdlet(VerbsCommon.Set, "MgServicePrincipalClaimPolicy", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Applications.Client.Models.CustomClaimsPolicy))]
    public class SetMgServicePrincipalClaimPolicyCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Claims)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.ServicePrincipals[ServicePrincipalId].ClaimsPolicy.PutAsync(body, requestConfiguration =>
                {
