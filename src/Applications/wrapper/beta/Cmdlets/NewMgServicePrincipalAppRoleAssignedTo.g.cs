...
{
    [GraphRoute("POST", "/servicePrincipals/{servicePrincipal-id}/appRoleAssignedTo")]
    [Cmdlet(VerbsCommon.New, "MgServicePrincipalAppRoleAssignedTo", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Applications.Client.Models.AppRoleAssignment))]
    public class NewMgServicePrincipalAppRoleAssignedToCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(ResourceId)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.ServicePrincipals[ServicePrincipalId].AppRoleAssignedTo.PostAsync(body, requestConfiguration =>
                {
