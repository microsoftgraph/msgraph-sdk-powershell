...
{
    [GraphRoute("POST", "/groups/{group-id}/appRoleAssignments")]
    [Cmdlet(VerbsCommon.New, "MgGroupAppRoleAssignment", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Applications.Client.Models.AppRoleAssignment))]
    public class NewMgGroupAppRoleAssignmentCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(ResourceId)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].AppRoleAssignments.PostAsync(body, requestConfiguration =>
                {
