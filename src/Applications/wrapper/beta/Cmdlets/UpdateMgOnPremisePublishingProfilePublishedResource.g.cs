...
{
    [GraphRoute("PATCH", "/onPremisesPublishingProfiles/{onPremisesPublishingProfile-id}/publishedResources/{publishedResource-id}")]
    [Cmdlet(VerbsData.Update, "MgOnPremisePublishingProfilePublishedResource", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Applications.Client.Models.PublishedResource))]
    public class UpdateMgOnPremisePublishingProfilePublishedResourceCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(PublishingType)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.OnPremisesPublishingProfiles[OnPremisesPublishingProfileId].PublishedResources[PublishedResourceId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.OnPremisesPublishingProfiles[OnPremisesPublishingProfileId].PublishedResources[PublishedResourceId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, PublishedResourceId);
