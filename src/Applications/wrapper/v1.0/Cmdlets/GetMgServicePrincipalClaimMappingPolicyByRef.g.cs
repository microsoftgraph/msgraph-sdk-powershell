...
{
    [GraphRoute("GET", "/servicePrincipals/{servicePrincipal-id}/claimsMappingPolicies/$ref")]
    [Cmdlet(VerbsCommon.Get, "MgServicePrincipalClaimMappingPolicyByRef")]
    [OutputType(typeof(string))]
    public class GetMgServicePrincipalClaimMappingPolicyByRefCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.ServicePrincipals[ServicePrincipalId].ClaimsMappingPolicies.Ref.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.ServicePrincipals[ServicePrincipalId].ClaimsMappingPolicies.Ref.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
