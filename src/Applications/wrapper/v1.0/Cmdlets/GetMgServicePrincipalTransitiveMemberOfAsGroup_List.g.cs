...
{
    [GraphRoute("GET", "/servicePrincipals/{servicePrincipal-id}/transitiveMemberOf/graph.group")]
    [Cmdlet(VerbsCommon.Get, "MgServicePrincipalTransitiveMemberOfAsGroup_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Applications.Client.Models.Group))]
    public class GetMgServicePrincipalTransitiveMemberOfAsGroup_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Applications.Client.Models.GroupCollectionResponse? result;
            try
            {
                result = client.ServicePrincipals[ServicePrincipalId].TransitiveMemberOf.GraphGroup.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.ServicePrincipals[ServicePrincipalId].TransitiveMemberOf.GraphGroup.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
