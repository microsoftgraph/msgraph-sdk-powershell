...
{
    [GraphRoute("GET", "/applications/{application-id}/tokenLifetimePolicies/$ref")]
    [Cmdlet(VerbsCommon.Get, "MgApplicationTokenLifetimePolicyByRef")]
    [OutputType(typeof(string))]
    public class GetMgApplicationTokenLifetimePolicyByRefCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Applications[ApplicationId].TokenLifetimePolicies.Ref.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Applications[ApplicationId].TokenLifetimePolicies.Ref.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
