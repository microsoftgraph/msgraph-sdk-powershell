...
{
    [GraphRoute("GET", "/applications/{application-id}/tokenLifetimePolicies")]
    [Cmdlet(VerbsCommon.Get, "MgApplicationTokenLifetimePolicy")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Applications.Client.Models.TokenLifetimePolicy))]
    public class GetMgApplicationTokenLifetimePolicyCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Applications[ApplicationId].TokenLifetimePolicies.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Applications[ApplicationId].TokenLifetimePolicies.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
