...
{
    [GraphRoute("POST", "/servicePrincipals/{servicePrincipal-id}/endpoints")]
    [Cmdlet(VerbsCommon.New, "MgServicePrincipalEndpoint", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Applications.Client.Models.Endpoint))]
    public class NewMgServicePrincipalEndpointCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Uri)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.ServicePrincipals[ServicePrincipalId].Endpoints.PostAsync(body, requestConfiguration =>
                {
