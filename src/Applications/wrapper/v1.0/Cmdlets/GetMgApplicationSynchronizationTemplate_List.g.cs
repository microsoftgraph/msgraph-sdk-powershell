...
{
    [GraphRoute("GET", "/applications/{application-id}/synchronization/templates")]
    [Cmdlet(VerbsCommon.Get, "MgApplicationSynchronizationTemplate_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Applications.Client.Models.SynchronizationTemplate))]
    public class GetMgApplicationSynchronizationTemplate_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Applications[ApplicationId].Synchronization.Templates.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Applications[ApplicationId].Synchronization.Templates.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
