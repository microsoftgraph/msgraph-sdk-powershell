...
{
    [GraphRoute("GET", "/servicePrincipals/{servicePrincipal-id}/homeRealmDiscoveryPolicies/$ref")]
    [Cmdlet(VerbsCommon.Get, "MgServicePrincipalHomeRealmDiscoveryPolicyByRef")]
    [OutputType(typeof(string))]
    public class GetMgServicePrincipalHomeRealmDiscoveryPolicyByRefCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.ServicePrincipals[ServicePrincipalId].HomeRealmDiscoveryPolicies.Ref.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.ServicePrincipals[ServicePrincipalId].HomeRealmDiscoveryPolicies.Ref.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
