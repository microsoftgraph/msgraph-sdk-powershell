...
{
    [GraphRoute("GET", "/applications/{application-id}/appManagementPolicies")]
    [Cmdlet(VerbsCommon.Get, "MgApplicationAppManagementPolicy")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Applications.Client.Models.AppManagementPolicy))]
    public class GetMgApplicationAppManagementPolicyCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Applications[ApplicationId].AppManagementPolicies.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Applications[ApplicationId].AppManagementPolicies.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
