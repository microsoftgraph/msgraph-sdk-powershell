...
{
    [GraphRoute("GET", "/applications/{application-id}/federatedIdentityCredentials")]
    [Cmdlet(VerbsCommon.Get, "MgApplicationFederatedIdentityCredential_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Applications.Client.Models.FederatedIdentityCredential))]
    public class GetMgApplicationFederatedIdentityCredential_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Applications.Client.Models.FederatedIdentityCredentialCollectionResponse? result;
            try
            {
                result = client.Applications[ApplicationId].FederatedIdentityCredentials.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Applications[ApplicationId].FederatedIdentityCredentials.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
