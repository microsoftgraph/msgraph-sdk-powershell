...
{
    [GraphRoute("PATCH", "/applications/{application-id}/synchronization/jobs/{synchronizationJob-id}")]
    [Cmdlet(VerbsData.Update, "MgApplicationSynchronizationJob", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Applications.Client.Models.SynchronizationJob))]
    public class UpdateMgApplicationSynchronizationJobCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(SynchronizationJobSettings)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Applications[ApplicationId].Synchronization.Jobs[SynchronizationJobId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Applications[ApplicationId].Synchronization.Jobs[SynchronizationJobId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException)
                {
                    ThrowGraphRequestFailed(ex, SynchronizationJobId);
