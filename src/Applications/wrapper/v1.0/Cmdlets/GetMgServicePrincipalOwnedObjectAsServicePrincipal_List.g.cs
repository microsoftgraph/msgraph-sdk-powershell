...
{
    [GraphRoute("GET", "/servicePrincipals/{servicePrincipal-id}/ownedObjects/graph.servicePrincipal")]
    [Cmdlet(VerbsCommon.Get, "MgServicePrincipalOwnedObjectAsServicePrincipal_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Applications.Client.Models.ServicePrincipal))]
    public class GetMgServicePrincipalOwnedObjectAsServicePrincipal_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Applications.Client.Models.ServicePrincipalCollectionResponse? result;
            try
            {
                result = client.ServicePrincipals[ServicePrincipalId].OwnedObjects.GraphServicePrincipal.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.ServicePrincipals[ServicePrincipalId].OwnedObjects.GraphServicePrincipal.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
