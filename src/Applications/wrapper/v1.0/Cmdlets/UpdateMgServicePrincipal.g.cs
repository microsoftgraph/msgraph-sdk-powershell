...
{
    [GraphRoute("PATCH", "/servicePrincipals/{servicePrincipal-id}")]
    [Cmdlet(VerbsData.Update, "MgServicePrincipal", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Applications.Client.Models.ServicePrincipal))]
    public class UpdateMgServicePrincipalCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(VerifiedPublisher)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.ServicePrincipals[ServicePrincipalId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.ServicePrincipals[ServicePrincipalId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException)
                {
                    ThrowGraphRequestFailed(ex, ServicePrincipalId);
