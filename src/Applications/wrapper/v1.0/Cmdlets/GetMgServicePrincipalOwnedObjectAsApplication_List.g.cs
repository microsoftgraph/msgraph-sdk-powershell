...
{
    [GraphRoute("GET", "/servicePrincipals/{servicePrincipal-id}/ownedObjects/graph.application")]
    [Cmdlet(VerbsCommon.Get, "MgServicePrincipalOwnedObjectAsApplication_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Applications.Client.Models.Application))]
    public class GetMgServicePrincipalOwnedObjectAsApplication_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Applications.Client.Models.ApplicationCollectionResponse? result;
            try
            {
                result = client.ServicePrincipals[ServicePrincipalId].OwnedObjects.GraphApplication.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.ServicePrincipals[ServicePrincipalId].OwnedObjects.GraphApplication.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
