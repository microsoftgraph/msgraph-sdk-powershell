...
{
    [GraphRoute("POST", "/applications/{application-id}/synchronization/jobs")]
    [Cmdlet(VerbsCommon.New, "MgApplicationSynchronizationJob", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Applications.Client.Models.SynchronizationJob))]
    public class NewMgApplicationSynchronizationJobCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(SynchronizationJobSettings)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Applications[ApplicationId].Synchronization.Jobs.PostAsync(body, requestConfiguration =>
                {
