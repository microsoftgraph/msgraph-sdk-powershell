...
{
    [GraphRoute("GET", "/applications/{application-id}/owners")]
    [Cmdlet(VerbsCommon.Get, "MgApplicationOwner")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Applications.Client.Models.DirectoryObject))]
    public class GetMgApplicationOwnerCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Applications[ApplicationId].Owners.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Applications[ApplicationId].Owners.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
