...
{
    [GraphRoute("PATCH", "/applications/{application-id}/synchronization/jobs/{synchronizationJob-id}/schema/directories/{directoryDefinition-id}")]
    [Cmdlet(VerbsData.Update, "MgApplicationSynchronizationJobSchemaDirectory", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Applications.Client.Models.DirectoryDefinition))]
    public class UpdateMgApplicationSynchronizationJobSchemaDirectoryCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Objects)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Applications[ApplicationId].Synchronization.Jobs[SynchronizationJobId].Schema.Directories[DirectoryDefinitionId].PatchAsync(body, requestConfiguration =>
                {
