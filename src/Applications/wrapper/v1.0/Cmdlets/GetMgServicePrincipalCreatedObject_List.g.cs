...
{
    [GraphRoute("GET", "/servicePrincipals/{servicePrincipal-id}/createdObjects")]
    [Cmdlet(VerbsCommon.Get, "MgServicePrincipalCreatedObject_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Applications.Client.Models.DirectoryObject))]
    public class GetMgServicePrincipalCreatedObject_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Applications.Client.Models.DirectoryObjectCollectionResponse? result;
            try
            {
                result = client.ServicePrincipals[ServicePrincipalId].CreatedObjects.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.ServicePrincipals[ServicePrincipalId].CreatedObjects.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
