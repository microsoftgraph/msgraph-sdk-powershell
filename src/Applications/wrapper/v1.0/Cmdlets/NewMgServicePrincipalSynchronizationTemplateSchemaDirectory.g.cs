...
{
    [GraphRoute("POST", "/servicePrincipals/{servicePrincipal-id}/synchronization/templates/{synchronizationTemplate-id}/schema/directories")]
    [Cmdlet(VerbsCommon.New, "MgServicePrincipalSynchronizationTemplateSchemaDirectory", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Applications.Client.Models.DirectoryDefinition))]
    public class NewMgServicePrincipalSynchronizationTemplateSchemaDirectoryCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Objects)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.ServicePrincipals[ServicePrincipalId].Synchronization.Templates[SynchronizationTemplateId].Schema.Directories.PostAsync(body, requestConfiguration =>
                {
