...
{
    [GraphRoute("GET", "/servicePrincipals/{servicePrincipal-id}/appRoleAssignments")]
    [Cmdlet(VerbsCommon.Get, "MgServicePrincipalAppRoleAssignment_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Applications.Client.Models.AppRoleAssignment))]
    public class GetMgServicePrincipalAppRoleAssignment_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.ServicePrincipals[ServicePrincipalId].AppRoleAssignments.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.ServicePrincipals[ServicePrincipalId].AppRoleAssignments.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
