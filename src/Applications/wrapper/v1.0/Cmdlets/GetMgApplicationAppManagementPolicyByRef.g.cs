...
{
    [GraphRoute("GET", "/applications/{application-id}/appManagementPolicies/$ref")]
    [Cmdlet(VerbsCommon.Get, "MgApplicationAppManagementPolicyByRef")]
    [OutputType(typeof(string))]
    public class GetMgApplicationAppManagementPolicyByRefCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Applications[ApplicationId].AppManagementPolicies.Ref.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Applications[ApplicationId].AppManagementPolicies.Ref.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
