...
{
    [GraphRoute("GET", "/servicePrincipals/{servicePrincipal-id}/tokenLifetimePolicies/$ref")]
    [Cmdlet(VerbsCommon.Get, "MgServicePrincipalTokenLifetimePolicyByRef")]
    [OutputType(typeof(string))]
    public class GetMgServicePrincipalTokenLifetimePolicyByRefCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.ServicePrincipals[ServicePrincipalId].TokenLifetimePolicies.Ref.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.ServicePrincipals[ServicePrincipalId].TokenLifetimePolicies.Ref.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
