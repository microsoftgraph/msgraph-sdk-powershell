...
{
    [GraphRoute("GET", "/directoryObjects")]
    [Cmdlet(VerbsCommon.Get, "MgDirectoryObject_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.DirectoryObjects.Client.Models.DirectoryObject))]
    public class GetMgDirectoryObject_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.DirectoryObjects.Client.Models.DirectoryObjectCollectionResponse? result;
            try
            {
                result = client.DirectoryObjects.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.DirectoryObjects.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
