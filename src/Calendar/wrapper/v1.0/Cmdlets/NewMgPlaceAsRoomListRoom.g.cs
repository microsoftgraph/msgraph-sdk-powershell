...
{
    [GraphRoute("POST", "/places/{place-id}/graph.roomList/rooms")]
    [Cmdlet(VerbsCommon.New, "MgPlaceAsRoomListRoom", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Calendar.Client.Models.Room))]
    public class NewMgPlaceAsRoomListRoomCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(TeamsEnabledState)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Places[PlaceId].GraphRoomList.Rooms.PostAsync(body, requestConfiguration =>
                {
