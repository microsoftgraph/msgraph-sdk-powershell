...
{
    [GraphRoute("GET", "/groups/{group-id}/calendar/calendarPermissions")]
    [Cmdlet(VerbsCommon.Get, "MgGroupCalendarPermission_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Calendar.Client.Models.CalendarPermission))]
    public class GetMgGroupCalendarPermission_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Calendar.Client.Models.CalendarPermissionCollectionResponse? result;
            try
            {
                result = client.Groups[GroupId].Calendar.CalendarPermissions.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Groups[GroupId].Calendar.CalendarPermissions.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
