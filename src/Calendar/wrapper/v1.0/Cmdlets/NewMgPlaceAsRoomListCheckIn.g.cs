...
{
    [GraphRoute("POST", "/places/{place-id}/graph.roomList/checkIns")]
    [Cmdlet(VerbsCommon.New, "MgPlaceAsRoomListCheckIn", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Calendar.Client.Models.CheckInClaim))]
    public class NewMgPlaceAsRoomListCheckInCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(CheckInMethod)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Places[PlaceId].GraphRoomList.CheckIns.PostAsync(body, requestConfiguration =>
                {
