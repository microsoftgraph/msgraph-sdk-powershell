...
{
    [GraphRoute("POST", "/places/{place-id}/graph.workspace/checkIns")]
    [Cmdlet(VerbsCommon.New, "MgPlaceAsWorkspaceCheckIn", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Calendar.Client.Models.CheckInClaim))]
    public class NewMgPlaceAsWorkspaceCheckInCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(CheckInMethod)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Places[PlaceId].GraphWorkspace.CheckIns.PostAsync(body, requestConfiguration =>
                {
