...
{
    [GraphRoute("POST", "/users/{user-id}/events")]
    [Cmdlet(VerbsCommon.New, "MgUserEvent", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Calendar.Client.Models.Event))]
    public class NewMgUserEventCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Type)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].Events.PostAsync(body, requestConfiguration =>
                {
