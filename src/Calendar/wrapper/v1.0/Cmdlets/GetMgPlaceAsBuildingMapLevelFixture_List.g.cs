...
{
    [GraphRoute("GET", "/places/{place-id}/graph.building/map/levels/{levelMap-id}/fixtures")]
    [Cmdlet(VerbsCommon.Get, "MgPlaceAsBuildingMapLevelFixture_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Calendar.Client.Models.FixtureMap))]
    public class GetMgPlaceAsBuildingMapLevelFixture_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Calendar.Client.Models.FixtureMapCollectionResponse? result;
            try
            {
                result = client.Places[PlaceId].GraphBuilding.Map.Levels[LevelMapId].Fixtures.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Places[PlaceId].GraphBuilding.Map.Levels[LevelMapId].Fixtures.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
