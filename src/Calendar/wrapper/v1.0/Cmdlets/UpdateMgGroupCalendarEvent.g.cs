...
{
    [GraphRoute("PATCH", "/groups/{group-id}/calendar/events/{event-id}")]
    [Cmdlet(VerbsData.Update, "MgGroupCalendarEvent", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Calendar.Client.Models.Event))]
    public class UpdateMgGroupCalendarEventCommand : PSCmdlet
    {
        ...
        {
            if (!ShouldProcess(EventId, "Update"))
                return;
    ...
    if (this.IsParameterBound(nameof(Type)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Calendar.Events[EventId].PatchAsync(body, requestConfiguration =>
                {
            ...
            {
                ThrowTerminatingError(new ErrorRecord(ex, "GraphRequestFailed", ErrorCategory.InvalidOperation, EventId));
                return;
            }
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Groups[GroupId].Calendar.Events[EventId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex)
                {
                    ThrowTerminatingError(new ErrorRecord(ex, "GraphRequestFailed", ErrorCategory.InvalidOperation, EventId));
