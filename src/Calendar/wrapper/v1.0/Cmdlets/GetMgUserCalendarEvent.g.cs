...
{
    [GraphRoute("GET", "/users/{user-id}/calendars/{calendar-id}/events")]
    [Cmdlet(VerbsCommon.Get, "MgUserCalendarEvent")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Calendar.Client.Models.Event))]
    public class GetMgUserCalendarEventCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].Calendars[CalendarId].Events.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Users[UserId].Calendars[CalendarId].Events.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
