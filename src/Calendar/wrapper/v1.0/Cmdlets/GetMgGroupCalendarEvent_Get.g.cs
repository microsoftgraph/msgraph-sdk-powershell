...
{
    [GraphRoute("GET", "/groups/{group-id}/calendar/events/{event-id}")]
    [Cmdlet(VerbsCommon.Get, "MgGroupCalendarEvent_Get")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Calendar.Client.Models.Event))]
    public class GetMgGroupCalendarEvent_GetCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Calendar.Events[EventId].GetAsync(requestConfiguration =>
                {
