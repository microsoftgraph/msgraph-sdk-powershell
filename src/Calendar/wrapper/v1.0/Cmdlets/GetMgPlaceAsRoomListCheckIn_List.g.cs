...
{
    [GraphRoute("GET", "/places/{place-id}/graph.roomList/checkIns")]
    [Cmdlet(VerbsCommon.Get, "MgPlaceAsRoomListCheckIn_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Calendar.Client.Models.CheckInClaim))]
    public class GetMgPlaceAsRoomListCheckIn_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Places[PlaceId].GraphRoomList.CheckIns.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Places[PlaceId].GraphRoomList.CheckIns.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
