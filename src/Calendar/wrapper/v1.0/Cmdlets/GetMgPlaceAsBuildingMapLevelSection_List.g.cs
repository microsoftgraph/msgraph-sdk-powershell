...
{
    [GraphRoute("GET", "/places/{place-id}/graph.building/map/levels/{levelMap-id}/sections")]
    [Cmdlet(VerbsCommon.Get, "MgPlaceAsBuildingMapLevelSection_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Calendar.Client.Models.SectionMap))]
    public class GetMgPlaceAsBuildingMapLevelSection_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Calendar.Client.Models.SectionMapCollectionResponse? result;
            try
            {
                result = client.Places[PlaceId].GraphBuilding.Map.Levels[LevelMapId].Sections.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Places[PlaceId].GraphBuilding.Map.Levels[LevelMapId].Sections.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
