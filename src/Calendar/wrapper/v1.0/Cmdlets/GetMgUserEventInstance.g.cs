...
{
    [GraphRoute("GET", "/users/{user-id}/events/{event-id}/instances")]
    [Cmdlet(VerbsCommon.Get, "MgUserEventInstance")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Calendar.Client.Models.Event))]
    public class GetMgUserEventInstanceCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].Events[EventId].Instances.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Users[UserId].Events[EventId].Instances.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
