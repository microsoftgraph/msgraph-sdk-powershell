...
{
    [GraphRoute("POST", "/users/{user-id}/events/{event-id}/attachments")]
    [Cmdlet(VerbsCommon.New, "MgUserEventAttachment", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Calendar.Client.Models.Attachment))]
    public class NewMgUserEventAttachmentCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Size)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].Events[EventId].Attachments.PostAsync(body, requestConfiguration =>
                {
