...
{
    [GraphRoute("PATCH", "/places/{place-id}/graph.room/checkIns/{checkInClaim-calendarEventId}")]
    [Cmdlet(VerbsData.Update, "MgPlaceAsRoomCheckIn", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Calendar.Client.Models.CheckInClaim))]
    public class UpdateMgPlaceAsRoomCheckInCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(CheckInMethod)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Places[PlaceId].GraphRoom.CheckIns[CheckInClaimCalendarEventId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Places[PlaceId].GraphRoom.CheckIns[CheckInClaimCalendarEventId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex)
                {
                    ThrowGraphRequestFailed(ex, CheckInClaimCalendarEventId);
