...
{
    [GraphRoute("POST", "/places/{place-id}/graph.roomList/rooms/{room-id}/checkIns")]
    [Cmdlet(VerbsCommon.New, "MgPlaceAsRoomListRoomCheckIn", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Calendar.Client.Models.CheckInClaim))]
    public class NewMgPlaceAsRoomListRoomCheckInCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(CheckInMethod)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Places[PlaceId].GraphRoomList.Rooms[RoomId].CheckIns.PostAsync(body, requestConfiguration =>
                {
