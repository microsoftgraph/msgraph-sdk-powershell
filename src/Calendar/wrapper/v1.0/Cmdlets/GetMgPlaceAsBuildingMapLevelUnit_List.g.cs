...
{
    [GraphRoute("GET", "/places/{place-id}/graph.building/map/levels/{levelMap-id}/units")]
    [Cmdlet(VerbsCommon.Get, "MgPlaceAsBuildingMapLevelUnit_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Calendar.Client.Models.UnitMap))]
    public class GetMgPlaceAsBuildingMapLevelUnit_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Calendar.Client.Models.UnitMapCollectionResponse? result;
            try
            {
                result = client.Places[PlaceId].GraphBuilding.Map.Levels[LevelMapId].Units.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Places[PlaceId].GraphBuilding.Map.Levels[LevelMapId].Units.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
