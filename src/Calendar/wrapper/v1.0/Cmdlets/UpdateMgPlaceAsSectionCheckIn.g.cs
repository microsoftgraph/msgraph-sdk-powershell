...
{
    [GraphRoute("PATCH", "/places/{place-id}/graph.section/checkIns/{checkInClaim-calendarEventId}")]
    [Cmdlet(VerbsData.Update, "MgPlaceAsSectionCheckIn", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Calendar.Client.Models.CheckInClaim))]
    public class UpdateMgPlaceAsSectionCheckInCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(CheckInMethod)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Places[PlaceId].GraphSection.CheckIns[CheckInClaimCalendarEventId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Places[PlaceId].GraphSection.CheckIns[CheckInClaimCalendarEventId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, CheckInClaimCalendarEventId);
