...
{
    [GraphRoute("PATCH", "/places/{place-id}/graph.roomList/rooms/{room-id}/checkIns/{checkInClaim-calendarEventId}")]
    [Cmdlet(VerbsData.Update, "MgPlaceAsRoomListRoomCheckIn", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Calendar.Client.Models.CheckInClaim))]
    public class UpdateMgPlaceAsRoomListRoomCheckInCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(CheckInMethod)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Places[PlaceId].GraphRoomList.Rooms[RoomId].CheckIns[CheckInClaimCalendarEventId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Places[PlaceId].GraphRoomList.Rooms[RoomId].CheckIns[CheckInClaimCalendarEventId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException)
                {
                    ThrowGraphRequestFailed(ex, CheckInClaimCalendarEventId);
