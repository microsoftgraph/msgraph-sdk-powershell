...
{
    [GraphRoute("POST", "/groups/{group-id}/calendar/events")]
    [Cmdlet(VerbsCommon.New, "MgGroupCalendarEvent", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Calendar.Client.Models.Event))]
    public class NewMgGroupCalendarEventCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(Type)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Groups[GroupId].Calendar.Events.PostAsync(body, requestConfiguration =>
                {
