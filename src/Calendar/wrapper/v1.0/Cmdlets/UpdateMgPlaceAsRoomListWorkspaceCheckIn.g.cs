...
{
    [GraphRoute("PATCH", "/places/{place-id}/graph.roomList/workspaces/{workspace-id}/checkIns/{checkInClaim-calendarEventId}")]
    [Cmdlet(VerbsData.Update, "MgPlaceAsRoomListWorkspaceCheckIn", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Calendar.Client.Models.CheckInClaim))]
    public class UpdateMgPlaceAsRoomListWorkspaceCheckInCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(CheckInMethod)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Places[PlaceId].GraphRoomList.Workspaces[WorkspaceId].CheckIns[CheckInClaimCalendarEventId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.Places[PlaceId].GraphRoomList.Workspaces[WorkspaceId].CheckIns[CheckInClaimCalendarEventId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException)
                {
                    ThrowGraphRequestFailed(ex, CheckInClaimCalendarEventId);
