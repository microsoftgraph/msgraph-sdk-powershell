...
{
    [GraphRoute("GET", "/places/{place-id}/graph.roomList/rooms/{room-id}/checkIns")]
    [Cmdlet(VerbsCommon.Get, "MgPlaceAsRoomListRoomCheckIn_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Calendar.Client.Models.CheckInClaim))]
    public class GetMgPlaceAsRoomListRoomCheckIn_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Calendar.Client.Models.CheckInClaimCollectionResponse? result;
            try
            {
                result = client.Places[PlaceId].GraphRoomList.Rooms[RoomId].CheckIns.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Places[PlaceId].GraphRoomList.Rooms[RoomId].CheckIns.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
