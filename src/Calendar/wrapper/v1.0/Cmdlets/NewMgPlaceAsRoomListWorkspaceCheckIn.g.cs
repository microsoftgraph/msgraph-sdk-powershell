...
{
    [GraphRoute("POST", "/places/{place-id}/graph.roomList/workspaces/{workspace-id}/checkIns")]
    [Cmdlet(VerbsCommon.New, "MgPlaceAsRoomListWorkspaceCheckIn", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Calendar.Client.Models.CheckInClaim))]
    public class NewMgPlaceAsRoomListWorkspaceCheckInCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(WorkspaceId, "New"))
                return;
    ...
    if (this.IsParameterBound(nameof(CheckInMethod)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Places[PlaceId].GraphRoomList.Workspaces[WorkspaceId].CheckIns.PostAsync(body, requestConfiguration =>
                {
