...
{
    [GraphRoute("GET", "/places/{place-id}/graph.roomList/rooms/{room-id}")]
    [Cmdlet(VerbsCommon.Get, "MgPlaceAsRoomListRoom_Get")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Calendar.Client.Models.Room))]
    public class GetMgPlaceAsRoomListRoom_GetCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Places[PlaceId].GraphRoomList.Rooms[RoomId].GetAsync(requestConfiguration =>
                {
