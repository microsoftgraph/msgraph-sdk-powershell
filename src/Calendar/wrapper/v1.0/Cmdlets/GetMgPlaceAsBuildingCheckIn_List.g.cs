...
{
    [GraphRoute("GET", "/places/{place-id}/graph.building/checkIns")]
    [Cmdlet(VerbsCommon.Get, "MgPlaceAsBuildingCheckIn_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Calendar.Client.Models.CheckInClaim))]
    public class GetMgPlaceAsBuildingCheckIn_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Places[PlaceId].GraphBuilding.CheckIns.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Places[PlaceId].GraphBuilding.CheckIns.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
