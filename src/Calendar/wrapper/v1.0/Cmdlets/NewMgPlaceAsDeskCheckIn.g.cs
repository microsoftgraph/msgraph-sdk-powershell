...
{
    [GraphRoute("POST", "/places/{place-id}/graph.desk/checkIns")]
    [Cmdlet(VerbsCommon.New, "MgPlaceAsDeskCheckIn", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Calendar.Client.Models.CheckInClaim))]
    public class NewMgPlaceAsDeskCheckInCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(CheckInMethod)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Places[PlaceId].GraphDesk.CheckIns.PostAsync(body, requestConfiguration =>
                {
