...
{
    [GraphRoute("POST", "/places/{place-id}/graph.section/checkIns")]
    [Cmdlet(VerbsCommon.New, "MgPlaceAsSectionCheckIn", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Calendar.Client.Models.CheckInClaim))]
    public class NewMgPlaceAsSectionCheckInCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(CheckInMethod)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Places[PlaceId].GraphSection.CheckIns.PostAsync(body, requestConfiguration =>
                {
