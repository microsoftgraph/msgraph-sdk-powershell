...
{
    [GraphRoute("GET", "/places/{place-id}/graph.floor/checkIns")]
    [Cmdlet(VerbsCommon.Get, "MgPlaceAsFloorCheckIn_List")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Calendar.Client.Models.CheckInClaim))]
    public class GetMgPlaceAsFloorCheckIn_ListCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Places[PlaceId].GraphFloor.CheckIns.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Places[PlaceId].GraphFloor.CheckIns.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
