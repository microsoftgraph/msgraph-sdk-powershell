...
{
    [GraphRoute("POST", "/users/{user-id}/calendars/{calendar-id}/events")]
    [Cmdlet(VerbsCommon.New, "MgUserCalendarEvent", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Calendar.Client.Models.Event))]
    public class NewMgUserCalendarEventCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(CalendarId, "New"))
                return;
    ...
    if (this.IsParameterBound(nameof(Type)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Users[UserId].Calendars[CalendarId].Events.PostAsync(body, requestConfiguration =>
                {
