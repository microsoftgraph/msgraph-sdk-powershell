...
{
    [GraphRoute("POST", "/places/{place-id}/graph.building/checkIns")]
    [Cmdlet(VerbsCommon.New, "MgPlaceAsBuildingCheckIn", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Calendar.Client.Models.CheckInClaim))]
    public class NewMgPlaceAsBuildingCheckInCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(CheckInMethod)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Places[PlaceId].GraphBuilding.CheckIns.PostAsync(body, requestConfiguration =>
                {
