...
{
    [GraphRoute("GET", "/groups/{group-id}/events/{event-id}/instances")]
    [Cmdlet(VerbsCommon.Get, "MgGroupEventInstance")]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Calendar.Client.Models.Event))]
    public class GetMgGroupEventInstanceCommand : GraphClientCmdlet
    {
        ...
        var client = new ApiClient(requestAdapter);
            ...
            Microsoft.Graph.PowerShell.Calendar.Client.Models.EventCollectionResponse? result;
            try
            {
                result = client.Groups[GroupId].Events[EventId].Instances.GetAsync(requestConfiguration =>
                {
        ...
        AddRequestHeaders(requestConfiguration.Headers);
                ...
                if (All.IsPresent)
                {
                    ...
                    while (!string.IsNullOrEmpty(nextLink) && !Stopping && (!this.IsParameterBound(nameof(Top)) || fetched < Top))
                    {
                        ...
                        result = client.Groups[GroupId].Events[EventId].Instances.WithUrl(nextLink).GetAsync(requestConfiguration =>
                        {
