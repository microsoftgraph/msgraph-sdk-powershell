...
{
    [GraphRoute("POST", "/places/{place-id}/graph.building/map/levels/{levelMap-id}/sections")]
    [Cmdlet(VerbsCommon.New, "MgPlaceAsBuildingMapLevelSection", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.Calendar.Client.Models.SectionMap))]
    public class NewMgPlaceAsBuildingMapLevelSectionCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(PlaceId1)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.Places[PlaceId].GraphBuilding.Map.Levels[LevelMapId].Sections.PostAsync(body, requestConfiguration =>
                {
