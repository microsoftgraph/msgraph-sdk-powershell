...
{
    [GraphRoute("PATCH", "/roleManagement/exchange/roleDefinitions/{unifiedRoleDefinition-id}/inheritsPermissionsFrom/{unifiedRoleDefinition-id1}")]
    [Cmdlet(VerbsData.Update, "MgRoleManagementExchangeRoleDefinitionInheritPermissionFrom", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.DeviceManagement.Enrollment.Client.Models.UnifiedRoleDefinition))]
    public class UpdateMgRoleManagementExchangeRoleDefinitionInheritPermissionFromCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(RolePermissions)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.RoleManagement.Exchange.RoleDefinitions[UnifiedRoleDefinitionId].InheritsPermissionsFrom[UnifiedRoleDefinitionId1].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.RoleManagement.Exchange.RoleDefinitions[UnifiedRoleDefinitionId].InheritsPermissionsFrom[UnifiedRoleDefinitionId1].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, UnifiedRoleDefinitionId1);
