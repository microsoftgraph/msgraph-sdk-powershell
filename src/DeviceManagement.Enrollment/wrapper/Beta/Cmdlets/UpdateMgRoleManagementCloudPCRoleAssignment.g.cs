...
{
    [GraphRoute("PATCH", "/roleManagement/cloudPC/roleAssignments/{unifiedRoleAssignmentMultiple-id}")]
    [Cmdlet(VerbsData.Update, "MgRoleManagementCloudPCRoleAssignment", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.DeviceManagement.Enrollment.Client.Models.UnifiedRoleAssignmentMultiple))]
    public class UpdateMgRoleManagementCloudPCRoleAssignmentCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(RoleDefinitionId)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.RoleManagement.CloudPC.RoleAssignments[UnifiedRoleAssignmentMultipleId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.RoleManagement.CloudPC.RoleAssignments[UnifiedRoleAssignmentMultipleId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, UnifiedRoleAssignmentMultipleId);
