...
{
    [GraphRoute("PATCH", "/roleManagement/deviceManagement/roleDefinitions/{unifiedRoleDefinition-id}")]
    [Cmdlet(VerbsData.Update, "MgRoleManagementDeviceManagementRoleDefinition", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.DeviceManagement.Enrollment.Client.Models.UnifiedRoleDefinition))]
    public class UpdateMgRoleManagementDeviceManagementRoleDefinitionCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(RolePermissions)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.RoleManagement.DeviceManagement.RoleDefinitions[UnifiedRoleDefinitionId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.RoleManagement.DeviceManagement.RoleDefinitions[UnifiedRoleDefinitionId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, UnifiedRoleDefinitionId);
