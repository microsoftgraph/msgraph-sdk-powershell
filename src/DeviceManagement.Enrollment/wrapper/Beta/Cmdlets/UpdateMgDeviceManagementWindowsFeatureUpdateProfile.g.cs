...
{
    [GraphRoute("PATCH", "/deviceManagement/windowsFeatureUpdateProfiles/{windowsFeatureUpdateProfile-id}")]
    [Cmdlet(VerbsData.Update, "MgDeviceManagementWindowsFeatureUpdateProfile", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.DeviceManagement.Enrollment.Client.Models.WindowsFeatureUpdateProfile))]
    public class UpdateMgDeviceManagementWindowsFeatureUpdateProfileCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(WindowsFeatureUpdateProfileId, "Update"))
                return;
    ...
    if (this.IsParameterBound(nameof(RolloutSettings)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.DeviceManagement.WindowsFeatureUpdateProfiles[WindowsFeatureUpdateProfileId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.DeviceManagement.WindowsFeatureUpdateProfiles[WindowsFeatureUpdateProfileId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, WindowsFeatureUpdateProfileId);
