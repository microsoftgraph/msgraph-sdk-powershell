...
{
    [GraphRoute("PATCH", "/deviceManagement/appleUserInitiatedEnrollmentProfiles/{appleUserInitiatedEnrollmentProfile-id}")]
    [Cmdlet(VerbsData.Update, "MgDeviceManagementAppleUserInitiatedEnrollmentProfile", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    ...
    public class UpdateMgDeviceManagementAppleUserInitiatedEnrollmentProfileCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(AppleUserInitiatedEnrollmentProfileId, "Update"))
                return;
    ...
    if (this.IsParameterBound(nameof(Platform)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.DeviceManagement.AppleUserInitiatedEnrollmentProfiles[AppleUserInitiatedEnrollmentProfileId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.DeviceManagement.AppleUserInitiatedEnrollmentProfiles[AppleUserInitiatedEnrollmentProfileId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, AppleUserInitiatedEnrollmentProfileId);
