...
{
    [GraphRoute("PATCH", "/roleManagement/cloudPC/roleDefinitions/{unifiedRoleDefinition-id}")]
    [Cmdlet(VerbsData.Update, "MgRoleManagementCloudPCRoleDefinition", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.DeviceManagement.Enrollment.Client.Models.UnifiedRoleDefinition))]
    public class UpdateMgRoleManagementCloudPCRoleDefinitionCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(RolePermissions)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.RoleManagement.CloudPC.RoleDefinitions[UnifiedRoleDefinitionId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.RoleManagement.CloudPC.RoleDefinitions[UnifiedRoleDefinitionId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, UnifiedRoleDefinitionId);
