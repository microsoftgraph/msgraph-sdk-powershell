...
{
    [GraphRoute("PATCH", "/roleManagement/exchange/transitiveRoleAssignments/{unifiedRoleAssignment-id}")]
    [Cmdlet(VerbsData.Update, "MgRoleManagementExchangeTransitiveRoleAssignment", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.DeviceManagement.Enrollment.Client.Models.UnifiedRoleAssignment))]
    public class UpdateMgRoleManagementExchangeTransitiveRoleAssignmentCommand : GraphClientCmdlet
    {
    ...
    if (this.IsParameterBound(nameof(RoleDefinitionId)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.RoleManagement.Exchange.TransitiveRoleAssignments[UnifiedRoleAssignmentId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.RoleManagement.Exchange.TransitiveRoleAssignments[UnifiedRoleAssignmentId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, UnifiedRoleAssignmentId);
