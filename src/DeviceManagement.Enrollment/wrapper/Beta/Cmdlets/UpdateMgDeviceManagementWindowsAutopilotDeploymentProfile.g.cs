...
{
    [GraphRoute("PATCH", "/deviceManagement/windowsAutopilotDeploymentProfiles/{windowsAutopilotDeploymentProfile-id}")]
    [Cmdlet(VerbsData.Update, "MgDeviceManagementWindowsAutopilotDeploymentProfile", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(Microsoft.Graph.PowerShell.DeviceManagement.Enrollment.Client.Models.WindowsAutopilotDeploymentProfile))]
    public class UpdateMgDeviceManagementWindowsAutopilotDeploymentProfileCommand : GraphClientCmdlet
    {
        ...
        {
            if (!ShouldProcess(WindowsAutopilotDeploymentProfileId, "Update"))
                return;
    ...
    if (this.IsParameterBound(nameof(OutOfBoxExperienceSettings)))
        ...
        var client = new ApiClient(requestAdapter);
            ...
            try
            {
                result = client.DeviceManagement.WindowsAutopilotDeploymentProfiles[WindowsAutopilotDeploymentProfileId].PatchAsync(body, requestConfiguration =>
                {
            ...
            if (result is null)
            {
                ...
                try
                {
                    result = client.DeviceManagement.WindowsAutopilotDeploymentProfiles[WindowsAutopilotDeploymentProfileId].GetAsync().GetAwaiter().GetResult();
                }
                catch (Exception ex) when (ex is not PipelineStoppedException && ex is not OperationCanceledException)
                {
                    ThrowGraphRequestFailed(ex, WindowsAutopilotDeploymentProfileId);
