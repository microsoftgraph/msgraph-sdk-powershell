...
[HashTable] $ModuleMetadata = Get-Content $ModuleMetadataJson | ConvertFrom-Json -AsHashTable
$ModuleMetadata.Remove("assemblyOriginatorKeyFile")

# Import scripts
. $NuspecHelperPS1

if ($null -eq $ModuleMetadata["version"]) {
...
$ApiVersion | ForEach-Object {
    ...
    if ($null -ne $LoadedAuthModule) {
        # TODO: Handle pre release dependency.
    ...
    $ModuleMapping.Keys | ForEach-Object {
        ...
        if ($null -ne $LoadedModule) {
            # TODO: Handle pre release dependencies.
            # $LoadedModule.Prerelease
