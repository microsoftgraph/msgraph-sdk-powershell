...
function Get-FilesByProfile {
    ...
    Copy-Files -SourcePath $FromPath -DestPath $ToPath -GraphProfile $GraphProfile -Module $Module
}
function Copy-Files {
    param(
        [ValidateSet("beta", "v1.0")]
    ...
    if ((Test-Path $SourcePath)) {
        foreach ($File in Get-ChildItem $SourcePath) {
            ...
            $FileToCheck = Join-Path $DestPath "$FileName.md"
