...
$ExistingRollUpModule = Find-Module $RollUpModule -Repository $RepositoryName

if($ExistingRollUpModule -eq $null){
...
foreach($tag in $Tags)
{
    try {
        ...
        & autorest --title:$tag --docOutputFolder:$DocOutputFolder --rollUpModule:$RollUpModule .\config\AutoRestConfig.yml --verbose
